...
#include "TetherGeometryLibrary.generated.h"
 ...
 *    as long as the tether keeps them registered. Caller must Release()
 *    when done.
 *  - All ops run on GameThread (tether worker dispatches there); large
 *    sweeps (M5-6 voxel merge, M5-2 boolean on dense meshes) can block
 *    the GT for seconds. Split them across exec calls per
 *    feedback_tether_exec_holds_gamethread.
 ...
 *    bodies live in TetherGeometryLibrary_Stubs.cpp and log a
 ...
 */
UCLASS()
class TETHER_API UTetherGeometryLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Geometry")
	 ...
	 *
	 * @param Handle    Tether handle (must be live).
	 ...
	 * single tether exec. CreateNewStaticMeshAssetFromMesh triggers an asset-reference
	 * completing modal that deadlocks the GameThread when other ops are waiting in
	 * the queue. Pattern:
	 *   exec1: load + transform/edit; exec2: save_to_new
	 *
	 * @param Handle       Source tether handle.
	 * @param NewAssetPath Content path for the new asset, e.g. "/Game/_TetherTest/SM_Out".
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Geometry")
	static FTetherMeshInfo GetMeshInfo(int32 Handle);
	 ...
	 * into separate exec calls for chained ops (per feedback_tether_exec_holds_gamethread).
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Geometry")
	 ...
	 * feedback_tether_exec_holds_gamethread, isolate dense voxel calls in their
	 * own tether exec.
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Geometry")
	 ...
	 * keep this in its own tether exec — texture asset creation triggers a
