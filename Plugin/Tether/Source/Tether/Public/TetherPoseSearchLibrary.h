...
#include "TetherPoseSearchLibrary.generated.h"

// ─── Schema (PSS) info ─────────────────────────────────────

USTRUCT(BlueprintType)
struct FTetherPSSChannel
{
	GENERATED_BODY()

	/** Channel class short name, e.g. "PoseSearchFeatureChannel_Position". */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|PoseSearch")
	FString Kind;
...
struct FTetherPSSInfo
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|PoseSearch")
	TArray<FTetherPSSSkeleton> Skeletons;
};

/** Result of creating or updating a Pose Search Crashing Legs channel. */
USTRUCT(BlueprintType)
struct FTetherPSSCrashingLegsChannelResult
{
	GENERATED_BODY()
 ...
 * (UE_LOG output isn't captured by the tether — only Python stdout/stderr is).
 */
USTRUCT(BlueprintType)
struct FTetherPSDAddResult
{
	GENERATED_BODY()
...
UCLASS()
class TETHER_API UTetherPoseSearchLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|PoseSearch")
	static FTetherPSSInfo GetSchemaInfo(const FString& SchemaPath);

	/** List the top-level channels on a PoseSearchSchema (sub-channels not flattened — see SubChannelCount). */
	UFUNCTION(BlueprintCallable, Category = "Tether|PoseSearch")
	static TArray<FTetherPSSChannel> ListSchemaChannels(const FString& SchemaPath);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|PoseSearch")
	static FTetherPSSCrashingLegsChannelResult SetCrashingLegsChannel(
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|PoseSearch")
	static FTetherPSDInfo GetDatabaseInfo(const FString& DatabasePath);

	/** List every animation entry in the database (Sequence/BlendSpace/Composite/Montage). */
	UFUNCTION(BlueprintCallable, Category = "Tether|PoseSearch")
	static TArray<FTetherPSDAnimEntry> ListDatabaseAnimations(const FString& DatabasePath);
	 ...
	 *
	 * @return FTetherPSDAddResult — Index >= 0 on success, -1 on failure with
	 *         Error populated. Check Error to find out *why* (asset path typo,
	 *         unsupported anim type, bad mirror string, …).
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|PoseSearch")
	static FTetherPSDAddResult AddAnimationToDatabase(const FString& DatabasePath, const FString& AnimationAssetPath,
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|PoseSearch")
	static FTetherPSDAddResult AddBlendSpaceToDatabase(const FString& DatabasePath, const FString& BlendSpacePath,
