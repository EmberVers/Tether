...
#include "TetherAssetLibrary.generated.h"

/** Search scope for keyword-based asset search. */
UENUM(BlueprintType)
enum class ETetherAssetSearchScope : uint8
{
...
struct FTetherAssetTag
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset")
	FString Key;
...
struct FTetherAssetInfo
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset")
	TArray<FTetherAssetTag> Tags;
};

/** Result of a fail-closed CreateDataAsset call. */
USTRUCT(BlueprintType)
struct FTetherDataAssetCreateResult
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherMeshLODStats
{
	GENERATED_BODY()

	/** LOD index (0 = highest detail). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 LODIndex = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 VertexCount = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 TriangleCount = 0;

	/** Material slot indices referenced by this LOD's sections. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") TArray<int32> MaterialIndices;
};

/** Summary of a UStaticMesh asset. */
USTRUCT(BlueprintType)
struct FTetherStaticMeshInfo
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") bool bFound = false;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FString AssetPath;

	/** World-space bounds of the full mesh (LOD 0). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FVector BoundsOrigin = FVector::ZeroVector;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FVector BoundsExtent = FVector::ZeroVector;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") float BoundsSphereRadius = 0.0f;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumLODs = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") TArray<FTetherMeshLODStats> LODStats;

	/** Material slot names in declaration order. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") TArray<FString> MaterialSlotNames;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") TArray<FString> MaterialAssetPaths;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumSockets = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") TArray<FString> SocketNames;

	/** true = has a simple collision mesh body; false = render-only. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") bool bHasCollision = false;

	/** Number of UV channels on LOD 0. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumUVChannels = 0;

	/** Nanite-enabled flag (UE 5.x). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") bool bHasNaniteData = false;
};

/** Summary of a USkeletalMesh asset. */
USTRUCT(BlueprintType)
struct FTetherSkeletalMeshInfo
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FString SkeletonPath;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumBones = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumSockets = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") TArray<FString> SocketNames;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumMorphTargets = 0;

	/** Path to the bound UPhysicsAsset if any. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FString PhysicsAssetPath;
};

/** One declared material slot on a UStaticMesh or USkeletalMesh asset. */
USTRUCT(BlueprintType)
struct FTetherMeshMaterialSlot
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherMeshMaterialAssignment
{
	GENERATED_BODY()

	/** Zero-based slot index. Leave at -1 to resolve by SlotName instead. */
	UPROPERTY(BlueprintReadWrite, Category = "Tether|Asset")
	int32 MaterialIndex = INDEX_NONE;
...
USTRUCT(BlueprintType)
struct FTetherMeshMaterialEditResult
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset")
	TArray<FTetherMeshMaterialSlot> Slots;
...
struct FTetherTextureInfo
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 Width = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 Height = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumMips = 0;

	/** EPixelFormat name (e.g. "PF_DXT5", "PF_BC7", "PF_FloatRGBA"). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FString PixelFormat;

	/** TextureCompressionSettings enum name ("TC_Default", "TC_NormalMap", ...). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FString CompressionSettings;

	/** TextureGroup enum name ("TEXTUREGROUP_World", "TEXTUREGROUP_UI", ...). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FString LODGroup;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") bool bSRGB = false;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") bool bNeverStream = false;

	/** Bytes actually resident in memory (GetResourceSize). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int64 ResourceSizeBytes = 0;
};

/** Summary of a USoundWave / USoundCue asset. */
USTRUCT(BlueprintType)
struct FTetherSoundInfo
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") FString SoundKind;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") float DurationSeconds = 0.0f;

	/** Sample rate in Hz. 0 if not applicable (e.g. SoundCue). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 SampleRate = 0;

	/** 1 = mono, 2 = stereo, etc. 0 if not applicable. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int32 NumChannels = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") bool bLooping = false;

	/** Approx compressed data bytes (SoundWave); 0 otherwise. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Asset") int64 CompressedDataBytes = 0;
};
 ...
 */
USTRUCT(BlueprintType)
struct FTetherSearchableNameRef
{
	GENERATED_BODY()
...
/**
 * Asset query utilities exposed to Python/Blueprint via Tether.
 * Ported from UnrealClientProtocol (MIT License - Italink).
 */
UCLASS()
class TETHER_API UTetherAssetLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherDataAssetCreateResult CreateDataAsset(
		const FString& AssetPath,
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherAssetInfo GetAssetInfo(const FString& AssetPath);
	 ...
	 * `unreal.TetherEditorLibrary.fixup_redirectors` for batch cleanup.
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherStaticMeshInfo GetStaticMeshInfo(const FString& AssetPath);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherSkeletalMeshInfo GetSkeletalMeshInfo(const FString& AssetPath);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static TArray<FTetherMeshMaterialSlot> GetMeshMaterialSlots(const FString& MeshAssetPath);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherMeshMaterialEditResult SetMeshMaterial(
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherMeshMaterialEditResult SetMeshMaterialBySlotName(
		const FString& MeshAssetPath,
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherMeshMaterialEditResult SetMeshMaterials(
		const FString& MeshAssetPath,
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherTextureInfo GetTextureInfo(const FString& AssetPath);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static FTetherSoundInfo GetSoundInfo(const FString& AssetPath);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Asset")
	static TArray<FTetherSearchableNameRef> GetSearchableNamesUsedByAsset(
		const FString& AssetPath,
	 ...
	 * TetherGameplayTagLibrary.list_all_registered_tags instead.
	 *
