...
#include "TetherReactiveTypes.h"
#include "TetherReactiveAdapter.h"
#include "TetherReactiveSubsystem.generated.h"

/**
 * Full handler record stored in the registry. Not a USTRUCT — exposed via
 * FTetherHandlerSummary / FTetherHandlerDetail to Blueprints/Python.
 */
struct FTetherHandlerRecord
{
	FString HandlerId;
	...
	ETetherTrigger TriggerType = ETetherTrigger::None;
	...
	ETetherHandlerLifetime Lifetime = ETetherHandlerLifetime::Permanent;
	int32 RemainingCalls = -1;
	ETetherErrorPolicy ErrorPolicy = ETetherErrorPolicy::LogContinue;
	int32 ThrottleMs = 0;
	bool bPaused = false;
 ...
 * transitions.
 *
 * Threading: all mutation + dispatch happens on the GameThread. The tether
 ...
 */
UCLASS()
class TETHER_API UTetherReactiveSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()
...
public:
	...
	static UTetherReactiveSubsystem* Get();

	// ─── Registration (called by TetherReactiveLibrary entry points) ──
	 ...
	 */
	FString RegisterHandler(FTetherHandlerRecord&& Record);
	...
	TArray<FTetherHandlerSummary> ListAllHandlers(
		const FString& FilterScope,
		const FString& FilterTriggerTypeName,
		const FString& FilterTag) const;

	bool GetHandler(const FString& HandlerId, FTetherHandlerDetail& OutDetail) const;
	bool GetStats(const FString& HandlerId, FTetherHandlerStats& OutStats) const;
	 ...
	 * by internal tether systems that need self-driving periodic work
	...
	// project's Saved/Tether/ folder. Save is debounced 100 ms after
	...
	/** Absolute path used for save/load. Default: <ProjectSaved>/Tether/reactive-handlers.json */
	...
	FString RestoreSingleRecord(FTetherHandlerRecord&& Record);
...
private:
	void RegisterAdapter(TUniquePtr<ITetherReactiveAdapter> Adapter);
	...
	TMap<FString, TSharedRef<FTetherHandlerRecord>> Handlers;
	TArray<TUniquePtr<ITetherReactiveAdapter>> Adapters;
	...
	TArray<FTetherHandlerRecord> DeferredHandlers;
