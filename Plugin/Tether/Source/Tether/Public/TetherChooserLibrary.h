...
#include "TetherChooserLibrary.generated.h"

USTRUCT(BlueprintType)
struct FTetherCHTColumn
{
	GENERATED_BODY()

	/** Column struct short name, e.g. "FloatRangeColumn", "EnumColumn". */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Chooser")
	FString Kind;
...
struct FTetherCHTRow
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Chooser")
	FTetherCHTRowResult Result;
};

USTRUCT(BlueprintType)
struct FTetherCHTInfo
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Chooser")
	FTetherCHTRowResult Fallback;
};

USTRUCT(BlueprintType)
struct FTetherCHTEvaluation
{
	GENERATED_BODY()
...
UCLASS()
class TETHER_API UTetherChooserLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	static FTetherCHTInfo GetChooserInfo(const FString& ChooserTablePath);

	/** List columns (kind, binding, disabled flag) — top-to-bottom in editor order. */
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	static TArray<FTetherCHTColumn> ListChooserColumns(const FString& ChooserTablePath);

	/** List rows including disabled flag and resolved result info. */
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	static TArray<FTetherCHTRow> ListChooserRows(const FString& ChooserTablePath);

	/** Fetch a single row's result (asset / class / nested chooser path). */
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	static FTetherCHTRowResult GetChooserRowResult(const FString& ChooserTablePath, int32 RowIndex);
	 ...
	 * read+modify ContextData via raw set_editor_property until a future tether call
	 * lands.
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	 ...
	 * context class, etc.) only goes to UE_LOG, which the tether doesn't capture.
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	static FTetherCHTEvaluation EvaluateChooserWithContextObject(const FString& ChooserTablePath, const FString& ContextObjectPath);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	static TArray<FTetherCHTRowResult> EvaluateChooserMultiWithContextObject(const FString& ChooserTablePath, const FString& ContextObjectPath);

	/** List every potential row result + fallback as paths — useful for "what could this CHT possibly return?" without filter inputs. */
	UFUNCTION(BlueprintCallable, Category = "Tether|Chooser")
	static TArray<FTetherCHTRowResult> ListPossibleResults(const FString& ChooserTablePath);
};
