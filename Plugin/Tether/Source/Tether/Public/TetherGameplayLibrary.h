...
#include "TetherGameplayLibrary.generated.h"
...
struct FAgentVisibleActor
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Agent")
	FName ActorName;
...
USTRUCT(BlueprintType)
struct FTetherInputComponentInstance
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherMappingContextEntry
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherInputActionState
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherLegacyAxisMapping
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherLegacyActionMapping
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherInputBindingIssue
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class UTetherGameplayLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	 ...
	 * lasts one tick, and the Python tether can't reliably call every
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherInputComponentInstance> GetInputActionTriggersFull(const FString& InputActionPath);

	/** Same for IA->Modifiers. */
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherInputComponentInstance> GetInputActionModifiersFull(const FString& InputActionPath);
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherInputReference> FindInputActionReferences(const FString& InputActionPath,
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherInputReference> FindInputMappingContextReferences(const FString& MappingContextPath,
		const FString& BlueprintPackagePathFilter = TEXT(""));
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherMappingContextEntry> GetActiveMappingContextStack();

	/** 读取已就绪本地玩家的 IA 运行时状态；客户端未就绪或纯服务端 PIE 返回 bHasState=false。 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static FTetherInputActionState GetCurrentInputActionState(const FString& InputActionPath);

	/** Dump the tether's currently pending one-shot inject queue (E4 part 2 — sticky already exposed via GetStickyInputs). */
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherLegacyAxisMapping> ListLegacyAxisMappings();

	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherLegacyActionMapping> ListLegacyActionMappings();
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherInputBindingIssue> ValidateInputBindings(const FString& BlueprintPackagePathFilter = TEXT(""));

	/** Find keys bound to multiple IAs within a single IMC, or across the supplied set of IMCs. */
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherKeyConflict> DetectKeyConflicts(const TArray<FString>& MappingContextPaths);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Agent")
	static TArray<FTetherIMCMapping> GetInputMappingContextMappings(const FString& MappingContextPath);
	...
	// have physics simulation enabled — see `tether-level-api.set_actor_
