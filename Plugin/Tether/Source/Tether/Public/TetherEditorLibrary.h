...
#include "TetherEditorLibrary.generated.h"

// ─── Editor state ───────────────────────────────────────────
USTRUCT(BlueprintType)
struct FTetherEditorState
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor")
...
// ─── Viewport screenshot ────────────────────────────────────
USTRUCT(BlueprintType)
struct FTetherScreenshotResult
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FTetherChannelCaptureResult
{
	GENERATED_BODY()
...
// ─── Live Coding compile ───────────────────────────────────
USTRUCT(BlueprintType)
struct FTetherLiveCodingResult
{
	GENERATED_BODY()
...
/** Captured tether TCP request record. */
USTRUCT(BlueprintType)
struct FTetherCallLogEntry
{
	GENERATED_BODY()

	/** Request id from the client (uuid4 when produced by tether.py). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") FString RequestId;

	/** "exec" | "ping" | "debug_resume" | "gamethread_ping" | other. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") FString Command;

	/** First ~80 chars of the Python script for `exec` calls; empty otherwise. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") FString ScriptPreview;

	/** UTC fractional seconds since the Unix epoch (FDateTime::UtcNow() projected). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") double UnixSecondsUtc = 0;

	/** Wall-clock time for the whole HandleClient run (recv → send), ms. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") float TotalDurationMs = 0;

	/** Time spent inside Python exec (queue wait + run). 0 for non-exec commands. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") float ExecDurationMs = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") bool bSuccess = false;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") int32 OutputBytes = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") int32 ErrorBytes = 0;

	/** Client endpoint, e.g. "127.0.0.1:51234". */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") FString Endpoint;

	/** First ~200 chars of the error message (populated when bSuccess=false). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") FString ErrorPreview;
};

/** Aggregated tether-call statistics across the current ring buffer. */
USTRUCT(BlueprintType)
struct FTetherCallStats
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") int32 TotalCalls = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") int32 SuccessCount = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") int32 FailureCount = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") float AvgDurationMs = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") float MinDurationMs = 0;
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") float MaxDurationMs = 0;
	/** 95th-percentile total duration across the buffered entries. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") float P95DurationMs = 0;

	/** Max entries the ring buffer holds (configurable via SetTetherCallLogCapacity). */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") int32 Capacity = 0;

	/** Number of entries evicted because the ring filled up — across the whole session. */
	UPROPERTY(BlueprintReadOnly, Category = "Tether|Editor") int32 TotalDropped = 0;
};

/**
 * Editor session / asset control via Tether.
 */
UCLASS()
class TETHER_API UTetherEditorLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherEditorState GetEditorState();

	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static TArray<FTetherOpenedAsset> GetOpenedAssets();
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherViewportCamera GetEditorViewportCamera();
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static TArray<FTetherCompileResult> CompileBlueprints(const TArray<FString>& BlueprintPaths);
	 ...
	 *                         bIncludeBase64 = true).
	 * @param bIncludeBase64   Fill FTetherScreenshotResult::Base64 with the
	 *                         compressed PNG bytes. Base64 inflates ~33%.
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherScreenshotResult CaptureActiveViewport(const FString& OutFilePath, bool bIncludeBase64);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherScreenshotResult CaptureActiveViewportAsDisplayed(const FString& OutFilePath, bool bIncludeBase64);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherChannelCaptureResult CaptureViewportChannel(
	 ...
	 * TetherGameplayLibrary to grab the player pawn's camera and
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherChannelCaptureResult CaptureChannelFromPose(
		const FString& Channel,
		FVector Location,
		FRotator Rotation,
		float FOV,
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherScreenshotResult CaptureViewportHitProxyMap(
		const FString& OutFilePath,
		bool bIncludeBase64);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherLiveCodingResult TriggerLiveCodingCompile(bool bWaitForCompletion);
	...
	/**
	 * Emit a message to GLog under a dedicated "TetherPy" category.
	...
	/**
	 * Return up to `MaxEntries` most recent tether TCP calls (newest last).
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static TArray<FTetherCallLogEntry> GetTetherCallLog(int32 MaxEntries = 100);

	/** Aggregate stats across all currently-buffered tether calls. */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FTetherCallStats GetTetherCallStats();

	/** Drop every buffered tether-call record. Returns the number of dropped entries. */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static int32 ClearTetherCallLog();

	/** Current ring-buffer capacity (default 500). */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static int32 GetTetherCallLogCapacity();
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static int32 SetTetherCallLogCapacity(int32 Capacity);

	// ─── Signature registry ─────────────────────────────────

	/**
	 * Enumerate every BlueprintCallable UFUNCTION on classes inside the
	 * `/Script/Tether` package and return a JSON summary: class,
	 ...
	 * as a one-call alternative to paging through `tether-*-api.md` docs
	 * when an agent wants to know the complete API surface.
	 *
	 * The JSON shape is:
	 *   { "generated_utc": "...", "engine_version": "...",
	 *     "libraries": [ { "class_name": "UTetherXxxLibrary",
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Editor")
	static FString DumpTetherSignatureRegistry();
};
