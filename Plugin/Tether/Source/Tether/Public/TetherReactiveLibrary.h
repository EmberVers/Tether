...
#include "TetherReactiveLibrary.generated.h"
 ...
 * event payload injected as globals (see tether-reactive.md for the
...
public:
	 ...
	 * @param Script       Python source. See tether-reactive.md for available globals (ctx,
	 *                     event_instigator, event_target, event_magnitude, tether_state, state,
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Reactive")
	 ...
	 * "AttrSet.Field" (e.g. "TetherTestAttrSet.Health") or bare "Field".
	 ...
	 * @param FilterTriggerType "" matches all, otherwise an ETetherTrigger name like "GameplayEvent".
	 * @param FilterTag         "" matches all, otherwise a tag string that must be in a handler's Tags.
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Reactive")
	static TArray<FTetherHandlerSummary> ListAllHandlers(
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Reactive")
	static FTetherHandlerDetail GetHandler(const FString& HandlerId);

	/** Current stats (calls, avg µs, max µs, last error). */
	UFUNCTION(BlueprintCallable, Category = "Tether|Reactive")
	static FTetherHandlerStats GetHandlerStats(const FString& HandlerId);
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Reactive|Persistence")
	 ...
	 *  <ProjectSaved>/Tether/reactive-handlers.json). */
	UFUNCTION(BlueprintCallable, Category = "Tether|Reactive|Persistence")
	...
	static bool ResolveForRestore(FTetherHandlerRecord& Record);
};
