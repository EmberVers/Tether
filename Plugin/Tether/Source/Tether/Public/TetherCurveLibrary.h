...
#include "TetherCurveLibrary.generated.h"

/**
 * One key of a rich curve, wire-format for the tether.
 *
...
struct FTetherRichCurveKey
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadWrite, Category = "Tether|Curve")
	float Time = 0.f;
...
USTRUCT(BlueprintType)
struct FTetherCurveInfo
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Tether|Curve")
	FString Name;
...
/**
 * Curve-asset introspection and editing via Tether.
 *
 ...
 */
UCLASS()
class TETHER_API UTetherCurveLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|Curve")
	static FTetherCurveInfo GetCurveInfo(const FString& CurvePath);

	/**
	 * Read all keys from one channel as FTetherRichCurveKey structs.
	 * ChannelIndex: 0 for UCurveFloat, 0..2 for UCurveVector (X/Y/Z), 0..3 for UCurveLinearColor (R/G/B/A).
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|Curve")
	static TArray<FTetherRichCurveKey> GetCurveKeys(const FString& CurvePath, int32 ChannelIndex);
	...
	UFUNCTION(BlueprintCallable, Category = "Tether|CurveTable")
	static FTetherCurveTableInfo GetCurveTableInfo(const FString& CurveTablePath);

	/**
	 * Read one row's keys as FTetherRichCurveKey. For SimpleCurves tables, the simple-curve
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Tether|CurveTable")
	static TArray<FTetherRichCurveKey> GetCurveTableRowKeys(
