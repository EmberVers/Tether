#include "TetherDataTableLibrary.h"
...
#define LOCTEXT_NAMESPACE "TetherDataTable"

// ─── Helpers ────────────────────────────────────────────────

namespace TetherDataTableImpl
{
	UDataTable* LoadDT(const FString& Path)
	{
		UDataTable* DT = LoadObject<UDataTable>(nullptr, *Path);
		if (!DT)
			UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load DataTable '%s'"), *Path);
		return DT;
	}
	...
	bool BuildDataTableJSONString(const UDataTable* DT, FString& OutJson)
	{
		...
		if (!RowStruct)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: DataTable '%s' has no row struct"), *DT->GetPathName());
			return false;
		}
		...
		for (TFieldIterator<FProperty> It(RowStruct); It; ++It)
		{
			...
			if (Prop && DataTableUtils::GetPropertyExportName(Prop) == RowNameField)
			{
				UE_LOG(LogTemp, Warning,
					TEXT("Tether: DataTable '%s' JSON row-key field '%s' conflicts with row property '%s'; preserving row keys and omitting that property value. Set ImportKeyField to a non-conflicting name to export both."),
		...
		for (const TPair<FName, uint8*>& Pair : RowMap)
		{
			...
			if (!RowData)
			{
				UE_LOG(LogTemp, Warning, TEXT("Tether: DataTable '%s' row '%s' has null row data"), *DT->GetPathName(), *Pair.Key.ToString());
				continue;
			}
...
FTetherDataTableInfo UTetherDataTableLibrary::GetDataTableRows(const FString& DataTablePath)
{
	FTetherDataTableInfo Result;
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return Result;
	...
	for (const auto& Pair : RowMap)
		Result.Rows.Add(TetherDataTableImpl::BuildRow(Pair.Key, Pair.Value, RowStruct, nullptr));
...
FTetherDataTableInfo UTetherDataTableLibrary::GetDataTableSummary(const FString& DataTablePath)
{
	FTetherDataTableInfo Result;
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return Result;
...
TArray<FString> UTetherDataTableLibrary::GetDataTableRowNames(const FString& DataTablePath)
{
...
FTetherDataTableRow UTetherDataTableLibrary::GetDataTableRow(const FString& DataTablePath, const FString& RowName)
{
	FTetherDataTableRow Result;
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return Result;
	...
	return TetherDataTableImpl::BuildRow(FName(*RowName), *RowPtr, RowStruct, nullptr);
}

// ─── GetDataTableRowField ───────────────────────────────────

FString UTetherDataTableLibrary::GetDataTableRowField(
	const FString& DataTablePath, const FString& RowName, const FString& FieldName)
{
	...
	FProperty* Prop = TetherDataTableImpl::FindProperty(RowStruct, FieldName);
	if (!Prop) return FString();

	return TetherDataTableImpl::ExportFieldValue(Prop, *RowPtr);
}

// ─── GetDataTableColumn ─────────────────────────────────────

TArray<FString> UTetherDataTableLibrary::GetDataTableColumn(const FString& DataTablePath, const FString& FieldName)
{
	TArray<FString> Result;
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return Result;
	...
	if (!Prop)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Field '%s' not found on DataTable '%s'"), *FieldName, *DataTablePath);
		return Result;
	}

	for (const auto& Pair : DT->GetRowMap())
	{
		Result.Add(FString::Printf(TEXT("%s = %s"), *Pair.Key.ToString(), *TetherDataTableImpl::ExportFieldValue(Prop, Pair.Value)));
	}
	return Result;
}
...
FTetherDataTableInfo UTetherDataTableLibrary::GetDataTableRowsFiltered(
	...
	const TArray<FString>& ColumnFilter)
{
	...
	for (const auto& Pair : DT->GetRowMap())
	{
		...
		Result.Rows.Add(TetherDataTableImpl::BuildRow(Pair.Key, Pair.Value, RowStruct, ColumnAllow.Num() > 0 ? &ColumnAllow : nullptr));
	}
...
TArray<FString> UTetherDataTableLibrary::SearchDataTableRows(
	const FString& DataTablePath,
	...
	for (const auto& Pair : DT->GetRowMap())
	{
		...
		if (!bMatched)
		{
			for (TFieldIterator<FProperty> It(RowStruct); It; ++It)
			{
				...
				FString V = TetherDataTableImpl::ExportFieldValue(*It, Pair.Value);
...
TArray<FString> UTetherDataTableLibrary::GetDataTablesUsingStruct(const FString& RowStructName)
{
	TArray<FString> Result;
...
bool UTetherDataTableLibrary::SetDataTableRowField(
	const FString& DataTablePath,
	...
	const FString& ExportedValue)
{
	...
	if (!RowPtr)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Row '%s' not found"), *RowName);
		return false;
	}
	...
	if (!Prop)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Field '%s' not found"), *FieldName);
		return false;
	}
	...
	const bool bOk = TetherDataTableImpl::ImportFieldText(Prop, *RowPtr, ExportedValue);
...
bool UTetherDataTableLibrary::AddDataTableRow(
	const FString& DataTablePath,
	const FString& RowName,
	const TMap<FString, FString>& FieldValues)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Row '%s' already exists"), *RowName);
		return false;
	}
	...
	{
		if (FProperty* Prop = TetherDataTableImpl::FindProperty(RowStruct, KV.Key))
			TetherDataTableImpl::ImportFieldText(Prop, NewRow, KV.Value);
	}
...
bool UTetherDataTableLibrary::RemoveDataTableRow(const FString& DataTablePath, const FString& RowName)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return false;
...
bool UTetherDataTableLibrary::DuplicateDataTableRow(
	const FString& DataTablePath,
...
bool UTetherDataTableLibrary::RenameDataTableRow(
	const FString& DataTablePath,
...
bool UTetherDataTableLibrary::ReorderDataTableRows(
	const FString& DataTablePath,
...
bool UTetherDataTableLibrary::ExportDataTableToCSV(const FString& DataTablePath, const FString& OutCsvFilePath)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return false;
...
bool UTetherDataTableLibrary::ImportDataTableFromCSV(const FString& DataTablePath, const FString& CsvFilePath)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return false;
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not read CSV file '%s'"), *CsvFilePath);
		return false;
	}
	...
	for (const FString& E : Errors)
		UE_LOG(LogTemp, Warning, TEXT("Tether CSV import: %s"), *E);
	return Errors.Num() == 0;
}

// ─── DoesDataTableRowExist ──────────────────────────────────

bool UTetherDataTableLibrary::DoesDataTableRowExist(
...
bool UTetherDataTableLibrary::SetDataTableRowFields(
	const FString& DataTablePath,
	const FString& RowName,
	const TMap<FString, FString>& FieldValues)
{
	...
	{
		FProperty* Prop = TetherDataTableImpl::FindProperty(RowStruct, Entry.Key);
		if (!Prop)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: Field '%s' not found"), *Entry.Key);
			continue;
		}
		if (TetherDataTableImpl::ImportFieldText(Prop, *RowPtr, Entry.Value))
		{
			++NumApplied;
		}
	}
...
FString UTetherDataTableLibrary::GetDataTableAsJSONString(const FString& DataTablePath)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return FString();

	FString Contents;
	if (!TetherDataTableImpl::BuildDataTableJSONString(DT, Contents))
	{
		return FString();
	}
	return Contents;
...
bool UTetherDataTableLibrary::ExportDataTableToJSON(
	const FString& DataTablePath, const FString& OutJsonFilePath)
{
	...
	FString Contents;
	if (!TetherDataTableImpl::BuildDataTableJSONString(DT, Contents) || Contents.IsEmpty())
	{
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not write JSON file '%s'"), *OutJsonFilePath);
		return false;
	}
	return true;
}
...
bool UTetherDataTableLibrary::ImportDataTableFromJSON(
	const FString& DataTablePath, const FString& JsonFilePath)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not read JSON file '%s'"), *JsonFilePath);
		return false;
	}
	...
	for (const FString& E : Errors)
		UE_LOG(LogTemp, Warning, TEXT("Tether JSON import: %s"), *E);
	return Errors.Num() == 0;
}

// ─── GetDataTableColumnTypes ────────────────────────────────

TArray<FTetherDataTableColumn> UTetherDataTableLibrary::GetDataTableColumnTypes(const FString& DataTablePath)
{
	TArray<FTetherDataTableColumn> Result;
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return Result;
...
TMap<FString, FString> UTetherDataTableLibrary::GetDataTableRowAsMap(
	const FString& DataTablePath, const FString& RowName)
{
	...
	{
		FProperty* Prop = *It;
		Result.Add(Prop->GetName(), TetherDataTableImpl::ExportFieldValue(Prop, *RowPtr));
	}
	return Result;
}
...
FString UTetherDataTableLibrary::GetDataTableRowAsJSONString(
	const FString& DataTablePath, const FString& RowName)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return FString();
	...
	{
		FProperty* Prop = *It;
		FString Value = TetherDataTableImpl::ExportFieldValue(Prop, *RowPtr);
...
int32 UTetherDataTableLibrary::ClearDataTable(const FString& DataTablePath)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return 0;
...
int32 UTetherDataTableLibrary::CopyDataTableRows(
	const FString& SourceDataTablePath,
	const FString& DestDataTablePath,
	const TArray<FString>& RowNames,
	bool bOverwrite)
{
	UDataTable* Src = TetherDataTableImpl::LoadDT(SourceDataTablePath);
	UDataTable* Dst = TetherDataTableImpl::LoadDT(DestDataTablePath);
	if (!Src || !Dst) return 0;

	const UScriptStruct* RowStruct = Src->GetRowStruct();
	if (!RowStruct || Dst->GetRowStruct() != RowStruct)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: CopyDataTableRows requires matching row structs"));
		return 0;
	}
...
FString UTetherDataTableLibrary::GetDataTableRowStructPath(const FString& DataTablePath)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return FString();
	const UScriptStruct* RS = DT->GetRowStruct();
...
TArray<FString> UTetherDataTableLibrary::FindDataTableRowsByFieldValue(
	const FString& DataTablePath,
	const FString& FieldName,
	const FString& Value,
	bool bCaseSensitive)
{
	...
	if (!Prop)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Field '%s' not found on '%s'"), *FieldName, *DataTablePath);
		return Result;
	}
	...
	for (const auto& Pair : DT->GetRowMap())
	{
		const FString V = TetherDataTableImpl::ExportFieldValue(Prop, Pair.Value);
...
FString UTetherDataTableLibrary::GetDataTableRowsAsJSONArray(
	const FString& DataTablePath,
	const TArray<FString>& RowFilter,
	const TArray<FString>& ColumnFilter)
{
...
TArray<FString> UTetherDataTableLibrary::DiffDataTableRows(
...
{
	TArray<FString> Result;
	UDataTable* A = TetherDataTableImpl::LoadDT(DataTablePathA);
	UDataTable* B = TetherDataTableImpl::LoadDT(DataTablePathB);
	...
	if (!SA || SA != SB)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: DiffDataTableRows requires matching row structs"));
		return Result;
	}
	...
	{
		FProperty* Prop = *It;
		const FString VA = TetherDataTableImpl::ExportFieldValue(Prop, *PA);
		const FString VB = TetherDataTableImpl::ExportFieldValue(Prop, *PB);
...
TMap<FString, FString> UTetherDataTableLibrary::GetDataTableRowDefaults(const FString& DataTablePath)
{
	TMap<FString, FString> Result;
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	if (!DT) return Result;
	...
	{
		FProperty* Prop = *It;
		Result.Add(Prop->GetName(), TetherDataTableImpl::ExportFieldValue(Prop, Buffer.GetData()));
	}
...
namespace TetherDataTableImpl
{
	...
	FTetherDataTableImportResult RunImport(UDataTable* DT, const FString& AssetPath, TImporter Importer)
	{
		FTetherDataTableImportResult Out;
...
FTetherDataTableImportResult UTetherDataTableLibrary::ImportDataTableFromCSVText(
	const FString& DataTablePath, const FString& CsvContent)
{
	UDataTable* DT = TetherDataTableImpl::LoadDT(DataTablePath);
	return TetherDataTableImpl::RunImport(DT, DataTablePath,
		[&](UDataTable* T) { return T->CreateTableFromCSVString(CsvContent); });
}

FTetherDataTableImportResult UTetherDataTableLibrary::ImportDataTableFromJSONText(
...
FTetherDataTableImportResult UTetherDataTableLibrary::CreateDataTableFromCSV(
	const FString& AssetPath, const FString& RowStructPath, const FString& CsvContent)
{
	FTetherDataTableImportResult Out;
	Out.AssetPath = AssetPath;

	UScriptStruct* RowStruct = TetherDataTableImpl::ResolveRowStruct(RowStructPath);
	FString CreateError;
	UDataTable* DT = TetherDataTableImpl::CreateDataTableAsset(AssetPath, RowStruct, CreateError);
	if (!DT)
	{
...
FTetherDataTableImportResult UTetherDataTableLibrary::CreateDataTableFromJSON(
