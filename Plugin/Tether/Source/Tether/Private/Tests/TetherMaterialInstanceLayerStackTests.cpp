...
namespace TetherMaterialInstanceLayerStackTests
{
	...
	bool EntriesEqual(const FTetherMaterialLayerEntry& A, const FTetherMaterialLayerEntry& B)
	{
	...
	bool SnapshotsEqual(const FTetherMaterialLayerStack& A, const FTetherMaterialLayerStack& B)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialInstanceLayerStackTransientTest,
	"Tether.Material.InstanceLayerStack.Transient",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialInstanceLayerStackTransientTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialInstanceLayerStackTests;
	(void)Parameters;
	...
	GEditor->ResetTransaction(NSLOCTEXT(
		"TetherTests", "MaterialLayerStackResetBefore",
	...
	ON_SCOPE_EXIT
	{
		...
		{
			GEditor->ResetTransaction(NSLOCTEXT(
				"TetherTests", "MaterialLayerStackResetAfter",
				"清理材质图层栈测试事务 / Clear material-layer-stack test transactions"));
		}
	};

	const FString InstancePath = Fixture.Instance->GetPathName();
	const FTetherMaterialLayerStack Initial =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(InstancePath);
	...
	TArray<FTetherMaterialLayerEntry> InvalidLinked = Initial.Layers;
	InvalidLinked[1].bRestrictToLayerRelatives = !InvalidLinked[1].bRestrictToLayerRelatives;
	const FTetherMaterialLayerStackOpResult InvalidLinkedResult =
		UTetherMaterialLibrary::SetMaterialInstanceLayerStack(InstancePath, InvalidLinked);
	...
	FTetherMaterialLayerEntry FunctionInstanceEntry;
	...
	FTetherMaterialLayerEntry DuplicatePointerEntry;
	...
	FTetherMaterialLayerEntry NullFunctionEntry;
	...
	const FTetherMaterialLayerStackOpResult SetResult =
		UTetherMaterialLibrary::SetMaterialInstanceLayerStack(InstancePath, Replacement);
	...
	const FTetherMaterialLayerStack RoundTrip =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(InstancePath);
	...
	TArray<FTetherMaterialLayerEntry> Invalid = Replacement;
	Invalid[2].Guid = Invalid[1].Guid;
	const FTetherMaterialLayerStack BeforeInvalid =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(InstancePath);
	const FTetherMaterialLayerStackOpResult InvalidResult =
		UTetherMaterialLibrary::SetMaterialInstanceLayerStack(InstancePath, Invalid);
	...
	const FTetherMaterialLayerStack AfterInvalid =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(InstancePath);
	...
	const FTetherMaterialLayerStackOpResult NoOpResult =
		UTetherMaterialLibrary::SetMaterialInstanceLayerStack(InstancePath, RoundTrip.Layers);
	...
	if (GEditor)
	{
		...
		const FTetherMaterialLayerStack AfterUndo =
			UTetherMaterialLibrary::GetMaterialInstanceLayerStack(InstancePath);
		...
		const FTetherMaterialLayerStack AfterRedo =
			UTetherMaterialLibrary::GetMaterialInstanceLayerStack(InstancePath);
	...
	const FTetherMaterialLayerStackOpResult SameRootCopyResult =
		UTetherMaterialLibrary::CopyMaterialInstanceLayerStack(InstancePath, CopyDestinationPath);
	...
	const FTetherMaterialLayerStack Copied =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(CopyDestinationPath);
	TestTrue(TEXT("Same-root copy preserves the complete reflected snapshot"),
		SnapshotsEqual(RoundTrip, Copied));

	const FTetherMaterialLayerStackOpResult EmptyResult =
		UTetherMaterialLibrary::SetMaterialInstanceLayerStack(InstancePath, {});
	...
	const FTetherMaterialLayerStack EmptySnapshot =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(InstancePath);
	...
	TransientPackage->SetDirtyFlag(false);
	const FTetherMaterialLayerStackOpResult EmptyNoOpResult =
		UTetherMaterialLibrary::SetMaterialInstanceLayerStack(InstancePath, {});
	...
	const FTetherMaterialLayerStack IncompatibleBefore =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(IncompatiblePath);
	const FTetherMaterialLayerStackOpResult CopyResult =
		UTetherMaterialLibrary::CopyMaterialInstanceLayerStack(InstancePath, IncompatiblePath);
	...
	const FTetherMaterialLayerStack IncompatibleAfter =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(IncompatiblePath);
	...
	const FTetherMaterialLayerStack NoStack =
		UTetherMaterialLibrary::GetMaterialInstanceLayerStack(PlainInstance->GetPathName());
