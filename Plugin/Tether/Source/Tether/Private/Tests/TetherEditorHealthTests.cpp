...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherEditorHealthInitialStateTest,
	"Tether.Server.EditorHealth.InitialStateIsStale",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherEditorHealthInitialStateTest::RunTest(const FString& Parameters)
{
	double Now = 10.0;
	FTetherEditorHealthCache Cache([&Now]() { return Now; });

	const FTetherEditorHealthReadout Status = Cache.Read(
		TetherEditorHealthTests::StaleAfterSeconds);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherEditorHealthInjectedClockTest,
	"Tether.Server.EditorHealth.InjectedClockControlsAges",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherEditorHealthInjectedClockTest::RunTest(const FString& Parameters)
{
	double Now = 100.0;
	FString NowUtc = TEXT("2026-08-08T00:00:00Z");
	FTetherEditorHealthCache Cache(
		[&Now]() { return Now; },
		[&NowUtc]() { return NowUtc; });
	Cache.SetReady(true);
	Cache.RecordEngineTick();
	NowUtc = TEXT("2026-08-08T00:00:00.010Z");
	Cache.RecordSlateTick(FTetherModalAttention());

	Now = 100.5;
	FTetherEditorHealthReadout Status = Cache.Read(
		TetherEditorHealthTests::StaleAfterSeconds);
	...
	Now = 102.001;
	Status = Cache.Read(TetherEditorHealthTests::StaleAfterSeconds);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherEditorHealthModalCopyTest,
	"Tether.Server.EditorHealth.ModalSummaryIsCopied",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherEditorHealthModalCopyTest::RunTest(const FString& Parameters)
{
	double Now = 20.0;
	...
	Cache.RecordSlateTick(Modal);
	FTetherEditorHealthReadout Updated = Cache.Read(
		TetherEditorHealthTests::StaleAfterSeconds);
	...
	Cache.RecordSlateTick(FTetherModalAttention());
	Updated = Cache.Read(TetherEditorHealthTests::StaleAfterSeconds);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherEditorHealthClockRollbackTest,
	"Tether.Server.EditorHealth.ClockRollbackClampsAge",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherEditorHealthClockRollbackTest::RunTest(const FString& Parameters)
{
	double Now = 50.0;
	FTetherEditorHealthCache Cache([&Now]() { return Now; });
	Cache.RecordEngineTick();
