...
namespace TetherWorldSelectionTests
{
	/**
	...
	public:
		explicit FScopedPIEWorld(bool bHasBegunPlay, bool bAddPlayerController = false, bool bAttachLocalPlayer = false)
		{
			...
			const FName WorldName(*FString::Printf(TEXT("TetherWorldSelection_%d"), NextWorldIndex++));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherGeneralWorldKeepsServerOrderTest,
	"Tether.Gameplay.WorldSelection.ProductionWiring.GeneralKeepsFirstValid",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherGeneralWorldKeepsServerOrderTest::RunTest(const FString& Parameters)
{
	using namespace TetherWorldSelectionTests;
	(void)Parameters;
	...
	TestEqual(TEXT("general and authority-sensitive calls preserve first-valid/server ordering"),
		TetherAgentImpl::SelectFirstValidPIEWorld(Contexts), ServerWorld.GetWorld());
	TestEqual(TEXT("local player calls select the real local-client context"),
		TetherAgentImpl::SelectFirstLocalPlayerPIEWorld(Contexts), LocalClientWorld.GetWorld());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherLocalWorldStartupFailsClosedTest,
	"Tether.Gameplay.WorldSelection.ProductionWiring.ClientStartupFailsClosed",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherLocalWorldStartupFailsClosedTest::RunTest(const FString& Parameters)
{
	using namespace TetherWorldSelectionTests;
	(void)Parameters;
	...
	TestNull(TEXT("local-only calls do not fall back to the server while the client is unready"),
		TetherAgentImpl::SelectFirstLocalPlayerPIEWorld(Contexts));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherLocalWorldRequiresBegunPlayTest,
	"Tether.Gameplay.WorldSelection.ProductionWiring.RequiresBegunPlay",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherLocalWorldRequiresBegunPlayTest::RunTest(const FString& Parameters)
{
	using namespace TetherWorldSelectionTests;
	(void)Parameters;
	...
	TestNull(TEXT("general selector rejects a PIE world before BeginPlay"),
		TetherAgentImpl::SelectFirstValidPIEWorld(Contexts));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherStandaloneLocalWorldTest,
	"Tether.Gameplay.WorldSelection.ProductionWiring.StandaloneLocal",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherStandaloneLocalWorldTest::RunTest(const FString& Parameters)
{
	using namespace TetherWorldSelectionTests;
	(void)Parameters;
	...
	TestEqual(TEXT("standalone with a real LocalPlayer is a valid local-player world"),
		TetherAgentImpl::SelectFirstLocalPlayerPIEWorld(Contexts), StandaloneWorld.GetWorld());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherServerOnlyHasNoLocalWorldTest,
	"Tether.Gameplay.WorldSelection.ProductionWiring.ServerOnlyHasNoLocalWorld",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherServerOnlyHasNoLocalWorldTest::RunTest(const FString& Parameters)
{
	using namespace TetherWorldSelectionTests;
	(void)Parameters;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherStickyInputReplacementFailsClosedTest,
	"Tether.Gameplay.StickyInput.ProductionWiring.ReplacementFailsClosed",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherStickyInputReplacementFailsClosedTest::RunTest(const FString& Parameters)
{
	using namespace TetherWorldSelectionTests;
	(void)Parameters;
	...
	TestTrue(TEXT("captured local-player identity starts valid"),
		TetherAgentImpl::IsSameLocalPlayerPIEIdentity(FirstWorld, FirstController, FirstLocalPlayer));
	...
	TestTrue(TEXT("second captured local-player identity starts valid"),
		TetherAgentImpl::IsSameLocalPlayerPIEIdentity(SecondWorld, SecondController, SecondLocalPlayer));
