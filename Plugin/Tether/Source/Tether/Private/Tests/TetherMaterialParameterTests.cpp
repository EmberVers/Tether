...
namespace TetherMaterialParameterTests
{
	static FTetherMIParamSet MakeRequest(
		const TCHAR* Name,
		const TCHAR* Type,
		const TCHAR* Value,
	...
	struct FTransientMaterialFixture
	{
		...
		FTransientMaterialFixture()
		{
			...
			Package = CreatePackage(*FString::Printf(TEXT("/Temp/TetherMaterialParameterTest_%s"), *Suffix));
	...
	static const FTetherMaterialParam* FindReadParameter(
		const FTetherMaterialInstanceInfo& Info,
		const TCHAR* Type,
		const FMaterialParameterInfo& Identity)
	{
		return Info.Parameters.FindByPredicate([Type, &Identity](const FTetherMaterialParam& Parameter)
		{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialParameterIdentityTest,
	"Tether.Material.Parameters.ExactAssociationIdentity",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialParameterIdentityTest::RunTest(const FString& Parameters)
{
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialParameterDiffAndPreviewFailureTest,
	"Tether.Material.Parameters.DiffIdentityAndPreviewFailure",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialParameterDiffAndPreviewFailureTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialParameterTests;
	...
	const FString Diff = UTetherMaterialLibrary::DiffMIParams(DiffA->GetPathName(), DiffB->GetPathName());
	...
	FTetherMIParamResult FailedSet;
	...
	FTetherMIParamResult SuccessfulSet;
	...
	const FString PreviewPath = FPaths::Combine(
		...
		FString::Printf(TEXT("TetherRejectedPreview_%s.png"), *FGuid::NewGuid().ToString(EGuidFormats::Digits)));
	IFileManager::Get().Delete(*PreviewPath, false, true, true);
	Fixture.Package->SetDirtyFlag(false);
	const bool bPreviewResult = UTetherMaterialLibrary::SetMIAndPreview(
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialParameterProductionTest,
	"Tether.Material.Parameters.ProductionSetReadAtomicityAndTransactions",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialParameterProductionTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialParameterTests;
	...
	GEditor->ResetTransaction(NSLOCTEXT(
		"TetherTests", "MaterialParameterResetBefore",
	...
	ON_SCOPE_EXIT
	{
		...
		{
			GEditor->ResetTransaction(NSLOCTEXT(
				"TetherTests", "MaterialParameterResetAfter",
	...
	FTetherMaterialInstanceInfo Read = UTetherMaterialLibrary::GetMaterialInstanceParameters(Fixture.InstancePath());
	...
	FTetherMIParamResult Initial = UTetherMaterialLibrary::SetMIParams(Fixture.InstancePath(), {
	...
	Read = UTetherMaterialLibrary::GetMaterialInstanceParameters(Fixture.InstancePath());
	const FTetherMaterialParam* ScalarRead = FindReadParameter(Read, TEXT("Scalar"), ScalarInfo);
	...
	FTetherMIParamResult Mixed = UTetherMaterialLibrary::SetMIParams(Fixture.InstancePath(), {
	...
	Fixture.Package->SetDirtyFlag(false);
	FTetherMIParamResult Duplicate = UTetherMaterialLibrary::SetMIParams(Fixture.InstancePath(), {
	...
	AddExpectedError(
		...
		EAutomationExpectedErrorFlags::Contains,
		1);
	FTetherMIParamResult LateFailure = UTetherMaterialLibrary::SetMIParams(Fixture.InstancePath(), {
	...
	FTetherMIParamResult StaticResult = UTetherMaterialLibrary::SetMIParams(Fixture.InstancePath(), {
	...
	FTetherMIParamResult TransactionalChange = UTetherMaterialLibrary::SetMIParams(Fixture.InstancePath(), {
		MakeRequest(TEXT("ScalarParam"), TEXT("Scalar"), TEXT("0.9"))});
	TestTrue(TEXT("Transactional scalar change succeeds"), TransactionalChange.bSuccess);
	Fixture.Package->SetDirtyFlag(false);
	FTetherMIParamResult NoOp = UTetherMaterialLibrary::SetMIParams(Fixture.InstancePath(), {
		MakeRequest(TEXT("ScalarParam"), TEXT("Scalar"), TEXT("0.9"))});
