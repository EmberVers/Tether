...
namespace TetherDataTableJsonTests
{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherDataTableJsonRoundTripTest,
	"Tether.DataTable.JSON.AuthoredNamesAndRowKeyRoundTrip",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherDataTableJsonRoundTripTest::RunTest(const FString& Parameters)
{
	using namespace TetherDataTableJsonTests;
	(void)Parameters;
	...
	const FString Exported = UTetherDataTableLibrary::GetDataTableAsJSONString(Source->GetPathName());
	...
	const FString CollisionJson = UTetherDataTableLibrary::GetDataTableAsJSONString(Source->GetPathName());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherDataTableJsonUtf8Test,
	"Tether.DataTable.JSON.FileExportUsesUtf8WithoutBom",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherDataTableJsonUtf8Test::RunTest(const FString& Parameters)
{
	using namespace TetherDataTableJsonTests;
	(void)Parameters;
	...
	const FString JsonPath = FPaths::CreateTempFilename(
		*FPaths::ProjectIntermediateDir(), TEXT("TetherDataTableJson_"), TEXT(".json"));
	ON_SCOPE_EXIT
	{
	...
	TestTrue(TEXT("Production file export succeeds"),
		UTetherDataTableLibrary::ExportDataTableToJSON(Table->GetPathName(), JsonPath));
	TArray<uint8> Bytes;
