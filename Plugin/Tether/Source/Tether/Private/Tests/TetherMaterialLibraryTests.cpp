...
namespace TetherTextureRefreshTests
{
	/**
	 ...
	 */
	class FRecordingOperations final : public ITetherTextureRefreshOperations
	{
	public:
...
namespace TetherMaterialGraphSafetyTests
{
	...
	static FTetherMaterialGraphOp MakeOp(const TCHAR* Name)
	{
		FTetherMaterialGraphOp Op;
		Op.Op = Name;
		return Op;
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherTextureRefreshInvalidPathTest,
	"Tether.Material.TextureRefresh.InvalidAndNonTexturePaths",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherTextureRefreshInvalidPathTest::RunTest(const FString& Parameters)
{
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(
	...
	const FTetherTextureRefreshResult Missing = UTetherMaterialLibrary::RefreshTextureResource(
		MissingPath, false);
	...
	const bool bInitialDirty = Package->IsDirty();
	const FName ObjectName = MakeUniqueObjectName(Package.Get(), UMaterial::StaticClass(), TEXT("TetherTextureRefreshNonTexture"));
	TStrongObjectPtr<UMaterial> NonTexture(NewObject<UMaterial>(Package.Get(), ObjectName, RF_Transient));
	Package->SetDirtyFlag(bInitialDirty);
	const FTetherTextureRefreshResult WrongType = UTetherMaterialLibrary::RefreshTextureResource(
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherTextureRefreshOrdinarySubmissionTest,
	"Tether.Material.TextureRefresh.OrdinarySubmissionOperations",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherTextureRefreshOrdinarySubmissionTest::RunTest(const FString& Parameters)
{
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(TEXT("OrdinarySubmission"), true);
	if (!TestTrue(TEXT("Independent test package was created"), Package.IsValid()))
	{
		return false;
	}
	TStrongObjectPtr<UTexture2D> Texture = TetherTextureRefreshTests::MakeTexture(
		*Package, TEXT("TetherTextureRefreshOrdinary"));
	...
	const bool bInitialDirty = Package->IsDirty();
	TetherTextureRefreshTests::FRecordingOperations Operations(true);
	FTetherTextureRefreshResult Result;
	Result.bFound = true;
	TetherTextureRefresh::Submit(*Texture, false, Operations, Result);
	...
	TetherTextureRefreshTests::TestCall(*this, Operations.Calls, 0, TetherTextureRefreshTests::ERecordedCall::IsCompiling,
		TEXT("Ordinary refresh checks compilation first"));
	TetherTextureRefreshTests::TestCall(*this, Operations.Calls, 1, TetherTextureRefreshTests::ERecordedCall::UpdateResource,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherTextureRefreshForcedSubmissionTest,
	"Tether.Material.TextureRefresh.ForcedSubmissionOperations",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherTextureRefreshForcedSubmissionTest::RunTest(const FString& Parameters)
{
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(TEXT("ForcedSubmission"));
	if (!TestTrue(TEXT("Independent test package was created"), Package.IsValid()))
	{
		return false;
	}
	TStrongObjectPtr<UTexture2D> Texture = TetherTextureRefreshTests::MakeTexture(
		*Package, TEXT("TetherTextureRefreshForced"));
	if (!TestTrue(TEXT("Temporary texture was created"), Texture.IsValid()))
	{
		return false;
	}
	...
	const bool bInitialDirty = Package->IsDirty();
	TetherTextureRefreshTests::FRecordingOperations Operations(false);
	FTetherTextureRefreshResult Result;
	Result.bFound = true;
	TetherTextureRefresh::Submit(*Texture, true, Operations, Result);
	...
	TetherTextureRefreshTests::TestCall(*this, Operations.Calls, 1, TetherTextureRefreshTests::ERecordedCall::BlockOnAnyAsyncBuild,
		TEXT("Forced refresh blocks its texture's async build second"));
	TetherTextureRefreshTests::TestCall(*this, Operations.Calls, 2, TetherTextureRefreshTests::ERecordedCall::UpdateResourceWithParams,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialGraphPreflightAtomicityTest,
	"Tether.Material.GraphSafety.PreflightLeavesLiveMaterialUnchanged",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialGraphPreflightAtomicityTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialGraphSafetyTests;
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(TEXT("GraphPreflight"));
	...
	FTetherMaterialGraphOp Set = MakeOp(TEXT("set_prop"));
	Set.DstRef = Constant->MaterialExpressionGuid.ToString();
	Set.Property = TEXT("R");
	Set.Value = TEXT("2.5");
	FTetherMaterialGraphOp InvalidConnect = MakeOp(TEXT("connect"));
	...
	const TArray<FTetherMaterialGraphOp> Ops{Set, InvalidConnect};

	const FTetherMaterialGraphOpResult Validation =
		UTetherMaterialLibrary::ValidateMaterialGraphOps(Material->GetPathName(), Ops);
	...
	const FTetherMaterialGraphOpResult Apply =
		UTetherMaterialLibrary::ApplyMaterialGraphOps(Material->GetPathName(), Ops, false);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialGraphConnectedDeleteTest,
	"Tether.Material.GraphSafety.DeleteRequiresOrderedDisconnect",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialGraphConnectedDeleteTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialGraphSafetyTests;
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(TEXT("GraphDelete"));
	...
	FTetherMaterialGraphOp Delete = MakeOp(TEXT("delete"));
	Delete.DstRef = ConstantGuid.ToString();
	const FTetherMaterialGraphOpResult Refused =
		UTetherMaterialLibrary::ApplyMaterialGraphOps(
			Material->GetPathName(), TArray<FTetherMaterialGraphOp>{Delete}, false);
	...
	FTetherMaterialGraphOp Disconnect = MakeOp(TEXT("disconnect_in"));
	Disconnect.DstRef = Add->MaterialExpressionGuid.ToString();
	Disconnect.DstInput = TEXT("A");
	const FTetherMaterialGraphOpResult Removed =
		UTetherMaterialLibrary::ApplyMaterialGraphOps(
			Material->GetPathName(), TArray<FTetherMaterialGraphOp>{Disconnect, Delete}, false);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialGraphNamedRerouteTest,
	"Tether.Material.GraphSafety.NamedRerouteDependencyIsExplicit",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialGraphNamedRerouteTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialGraphSafetyTests;
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(TEXT("GraphNamedReroute"));
	...
	Declaration->Name = TEXT("TetherValue");
	...
	const FTetherMaterialGraph Graph =
		UTetherMaterialLibrary::GetMaterialGraph(Material->GetPathName());
	...
	const bool bHasLogicalEdge = Graph.Connections.ContainsByPredicate(
		[&](const FTetherMaterialGraphConnection& Connection)
		{
	...
	const FTetherMaterialAnalysis Analysis =
		UTetherMaterialLibrary::AnalyzeMaterial(Material->GetPathName(), 0, 0);
	const bool bFalseUnusedFinding = Analysis.Findings.ContainsByPredicate(
		[&](const FTetherMaterialFinding& Finding)
		{
	...
	TestFalse(
		...
		UTetherMaterialLibrary::DeleteMaterialExpression(
			Material->GetPathName(), Declaration->MaterialExpressionGuid));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialGraphIncompleteDependencyTest,
	"Tether.Material.GraphSafety.IncompleteDependencyFailsClosed",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialGraphIncompleteDependencyTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialGraphSafetyTests;
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(TEXT("GraphIncomplete"));
	...
	FTetherMaterialGraphOp Add = MakeOp(TEXT("add"));
	Add.ClassName = TEXT("Constant");
	const FTetherMaterialGraphOpResult Refused =
		UTetherMaterialLibrary::ApplyMaterialGraphOps(
			Material->GetPathName(), TArray<FTetherMaterialGraphOp>{Add}, false);
	...
	const FTetherMaterialAutoFixResult AutoFix =
		UTetherMaterialLibrary::AutoFixMaterial(
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTetherMaterialGraphCompositeExpansionTest,
	"Tether.Material.GraphSafety.CompositeGatewaysAndMembershipAreExplicit",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FTetherMaterialGraphCompositeExpansionTest::RunTest(const FString& Parameters)
{
	using namespace TetherMaterialGraphSafetyTests;
	TStrongObjectPtr<UPackage> Package = TetherTextureRefreshTests::MakePackage(TEXT("GraphComposite"));
	...
	const FTetherMaterialGraphNode* MemberNode = Graph.Nodes.FindByPredicate(
		[&](const FTetherMaterialGraphNode& Node)
		{
