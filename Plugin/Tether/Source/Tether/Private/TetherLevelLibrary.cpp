#include "TetherLevelLibrary.h"
#include "TetherCompat.h"
...
#define LOCTEXT_NAMESPACE "TetherLevel"

// ─── Helpers ────────────────────────────────────────────────

namespace TetherLevelImpl
{
	...
	FTetherTransform ToTetherTransform(const FTransform& T)
	{
		FTetherTransform Out;
	...
	FTetherActorBrief MakeBrief(AActor* Actor)
	{
		FTetherActorBrief B;
	...
	FTetherLevelComponentInfo MakeComponentInfo(UActorComponent* C)
	{
		FTetherLevelComponentInfo CI;
		CI.Name = C->GetName();
...
FTetherLevelSummary UTetherLevelLibrary::GetLevelSummary()
{
	FTetherLevelSummary S;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
		return S;
	}
...
int32 UTetherLevelLibrary::GetActorCount(const FString& ClassFilter)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		if (TetherLevelImpl::MatchesClassFilter(It->GetClass(), ClassFilter))
		{
			++N;
		}
	}
...
TArray<FString> UTetherLevelLibrary::GetActorNames(const FString& ClassFilter, const FString& TagFilter, const FString& NameFilter)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (!TetherLevelImpl::MatchesClassFilter(A->GetClass(), ClassFilter))
		{
			continue;
		}
...
TArray<FTetherActorBrief> UTetherLevelLibrary::ListActors(
	...
	int32 MaxResults)
{
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (bSelectedOnly && !TetherLevelImpl::ActorIsSelected(A))
		{
			continue;
		}
		Out.Add(TetherLevelImpl::MakeBrief(A));
...
FTetherActorInfo UTetherLevelLibrary::GetActorInfo(const FString& ActorName)
{
	FTetherActorInfo Info;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AActor* Actor = TetherLevelImpl::FindActor(World, ActorName);
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Actor '%s' not found"), *ActorName);
		return Info;
	}
	...
	Info.Transform = TetherLevelImpl::ToTetherTransform(Actor->GetActorTransform());
	for (const FName& Tag : Actor->Tags)
	{
	...
	for (UActorComponent* C : Comps)
	{
		if (C)
		{
			Info.Components.Add(TetherLevelImpl::MakeComponentInfo(C));
		}
	}
	return Info;
}

FTetherTransform UTetherLevelLibrary::GetActorTransform(const FString& ActorName)
{
	FTetherTransform T;
	if (AActor* A = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName))
	{
		T = TetherLevelImpl::ToTetherTransform(A->GetActorTransform());
	}
	return T;
}

TArray<FTetherLevelComponentInfo> UTetherLevelLibrary::GetActorComponents(const FString& ActorName)
{
	TArray<FTetherLevelComponentInfo> Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Actor '%s' not found"), *ActorName);
		return Out;
	...
	for (UActorComponent* C : Comps)
	{
		if (C)
		{
			Out.Add(TetherLevelImpl::MakeComponentInfo(C));
		}
	}
	return Out;
}

TArray<FString> UTetherLevelLibrary::FindActorsByClass(const FString& ClassPath, int32 MaxResults)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (TetherLevelImpl::MatchesClassFilter(A->GetClass(), ClassPath))
		{
...
TArray<FString> UTetherLevelLibrary::FindActorsByTag(const FString& Tag)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
...
TArray<FString> UTetherLevelLibrary::GetSelectedActors()
{
	TArray<FString> Out;
	if (!GEditor)
	{
...
TArray<FTetherStreamingLevel> UTetherLevelLibrary::GetStreamingLevels()
{
	TArray<FTetherStreamingLevel> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
		return Out;
...
FString UTetherLevelLibrary::GetCurrentLevelPath()
{
	if (UWorld* World = TetherLevelImpl::GetEditorWorld())
	{
		if (UPackage* Pkg = World->GetOutermost())
		{
			return Pkg->GetName();
...
FString UTetherLevelLibrary::SpawnActor(const FString& ClassPath, FVector Location, FRotator Rotation)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
	...
	UClass* Cls = TetherLevelImpl::ResolveActorClass(ClassPath);
	if (!Cls)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not resolve actor class '%s'"), *ClassPath);
		return FString();
	}
	...
	if (!New)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SpawnActor failed for class '%s'"), *ClassPath);
		return FString();
	}
	return New->GetActorLabel();
}

bool UTetherLevelLibrary::DestroyActor(const FString& ActorName)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AActor* Actor = TetherLevelImpl::FindActor(World, ActorName);
	if (!Actor)
...
int32 UTetherLevelLibrary::DestroyActors(const TArray<FString>& ActorNames)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
	...
	{
		if (AActor* A = TetherLevelImpl::FindActor(World, Name))
		{
			A->Modify();
...
bool UTetherLevelLibrary::SetActorTransform(const FString& ActorName, FVector Location, FRotator Rotation, FVector Scale)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::MoveActor(const FString& ActorName, FVector DeltaLocation, FRotator DeltaRotation)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::AttachActor(const FString& ChildName, const FString& ParentName, const FString& SocketName)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AActor* Child = TetherLevelImpl::FindActor(World, ChildName);
	AActor* Parent = TetherLevelImpl::FindActor(World, ParentName);
	if (!Child || !Parent)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AttachActor — child or parent not found"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AttachActor — missing root component"));
		return false;
	}
...
bool UTetherLevelLibrary::DetachActor(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::SelectActors(const TArray<FString>& ActorNames, bool bAddToSelection)
{
	if (!GEditor)
	{
		return false;
...
bool UTetherLevelLibrary::DeselectAllActors()
{
	if (!GEditor)
	{
		return false;
...
bool UTetherLevelLibrary::SetActorLabel(const FString& ActorName, const FString& NewLabel)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::SetActorHiddenInGame(const FString& ActorName, bool bHidden)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
namespace TetherLevelImpl
{
	...
	void CollectAttachmentTree(AActor* Actor, int32 Depth, TArray<FString>& Out)
	{
		...
		for (AActor* C : Children)
		{
			TetherLevelImpl::CollectAttachmentTree(C, Depth + 1, Out);
		}
	}
}

FString UTetherLevelLibrary::GetActorProperty(const FString& ActorName, const FString& PropertyPath)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
	...
	if (!TetherLevelImpl::ResolvePropertyPath(Actor, PropertyPath, Prop, Container, Owner))
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Property path '%s' not found on '%s'"), *PropertyPath, *ActorName);
		return FString();
	}
	FString Out;
...
bool UTetherLevelLibrary::SetActorProperty(const FString& ActorName, const FString& PropertyPath, const FString& ExportedValue)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
	...
	if (!ImportResult)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: ImportText failed for path '%s' = '%s'"), *PropertyPath, *ExportedValue);
		return false;
	}
...
namespace TetherLevelImpl
{
	...
	void FillPropertyInfos(UClass* Cls, TArray<FTetherPropertyInfo>& Out)
	{
		if (!Cls)
		{
			return;
...
TArray<FTetherPropertyInfo> UTetherLevelLibrary::ListActorProperties(const FString& ActorName)
{
	TArray<FTetherPropertyInfo> Result;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
		return Result;
	}
	TetherLevelImpl::FillPropertyInfos(Actor->GetClass(), Result);
	return Result;
}

TArray<FTetherPropertyInfo> UTetherLevelLibrary::ListClassProperties(const FString& ClassPath)
{
	TArray<FTetherPropertyInfo> Result;
	UClass* Cls = TetherLevelImpl::ResolveClass(ClassPath);
	if (!Cls)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: ListClassProperties — class '%s' not found"), *ClassPath);
		return Result;
	}
	TetherLevelImpl::FillPropertyInfos(Cls, Result);
	return Result;
}

// ─── Actor-targeted function invocation (functional-test driver) ──

bool UTetherLevelLibrary::InvokeFunctionOnActor(
...
TArray<FString> UTetherLevelLibrary::GetAttachmentTree(const FString& ActorName)
{
	TArray<FString> Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	...
	TetherLevelImpl::CollectAttachmentTree(Actor, 0, Out);
	return Out;
}

TArray<FTetherActorRadiusHit> UTetherLevelLibrary::FindActorsInRadius(FVector Location, float Radius, const FString& ClassFilter)
{
	TArray<FTetherActorRadiusHit> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World || Radius <= 0.f)
	{
		return Out;
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		FTetherActorRadiusHit H;
		H.Name = A->GetActorLabel();
		H.ClassName = A->GetClass()->GetName();
		H.Distance = FMath::Sqrt(DistSq);
		Out.Add(H);
	}
	Out.Sort([](const FTetherActorRadiusHit& A, const FTetherActorRadiusHit& B) { return A.Distance < B.Distance; });
	return Out;
}

TArray<FString> UTetherLevelLibrary::DuplicateActors(const TArray<FString>& ActorNames)
{
	TArray<FString> Out;
	if (!GEditor)
	{
	...
	for (const FString& Name : ActorNames)
	{
		AActor* Src = TetherLevelImpl::FindActor(World, Name);
		if (!Src)
		{
			continue;
		}
...
FTetherActorBounds UTetherLevelLibrary::GetActorBounds(const FString& ActorName)
{
	FTetherActorBounds Out;
	AActor* A = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!A)
	{
		return Out;
	}
...
TArray<FString> UTetherLevelLibrary::GetActorsInBox(FVector Min, FVector Max, const FString& ClassFilter)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	...
	{
		AActor* A = *It;
		if (!A || !TetherLevelImpl::MatchesClassFilter(A->GetClass(), ClassFilter))
		{
			continue;
		}
...
FString UTetherLevelLibrary::FindNearestActor(FVector Location, const FString& ClassFilter)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
...
float UTetherLevelLibrary::GetActorDistance(const FString& ActorA, const FString& ActorB)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AActor* A = TetherLevelImpl::FindActor(World, ActorA);
	AActor* B = TetherLevelImpl::FindActor(World, ActorB);
	if (!A || !B)
	{
		return -1.f;
	}
...
bool UTetherLevelLibrary::IsActorSelected(const FString& ActorName)
{
	if (!GEditor)
	{
		return false;
...
bool UTetherLevelLibrary::SetActorHiddenInEditor(const FString& ActorName, bool bHidden)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::AddActorTag(const FString& ActorName, const FName Tag)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor || Tag.IsNone())
	{
...
bool UTetherLevelLibrary::RemoveActorTag(const FString& ActorName, const FName Tag)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor || Tag.IsNone())
	{
...
TArray<FString> UTetherLevelLibrary::GetActorClassHistogram()
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
...
TArray<FString> UTetherLevelLibrary::GetActorMaterials(const FString& ActorName)
{
	TArray<FString> Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
...
FString UTetherLevelLibrary::GetActorFolder(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::SetActorFolder(const FString& ActorName, const FString& FolderPath)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
TArray<FString> UTetherLevelLibrary::GetActorFolders()
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
...
TArray<FString> UTetherLevelLibrary::GetActorsInFolder(const FString& FolderPath, bool bRecursive)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
...
FString UTetherLevelLibrary::LineTraceFirstActor(FVector Start, FVector End)
{
	UWorld* World = TetherLevelImpl::GetRuntimeWorld();
	if (!World)
	{
		return FString();
	}
	FHitResult Hit;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherLineTrace), /*bTraceComplex*/ true);
...
bool UTetherLevelLibrary::GetHeightAt(
	...
	FString& OutActorLabel, float& OutGroundZ)
{
	...
	FHitResult Hit;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherGetHeightAt), /*bTraceComplex*/ true);
...
int32 UTetherLevelLibrary::GetHeightProfileAlong(
	...
	TArray<float>& OutHeights, TArray<FString>& OutActorLabels)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherHeightProfile), /*bTraceComplex*/ true);
...
float UTetherLevelLibrary::MeasureCeilingHeight(const FVector& Origin, float MaxUp)
{
	...
	FHitResult Hit;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherMeasureCeiling), /*bTraceComplex*/ true);
...
int32 UTetherLevelLibrary::ProbeFanXY(const FVector& Origin,
	...
	TArray<float>& OutDistances, TArray<FString>& OutActorLabels)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherProbeFan), /*bTraceComplex*/ true);
...
namespace TetherLevelImpl
{
	...
	// Process-global so the graph survives across tether exec calls.
...
void UTetherLevelLibrary::NavGraphClear()
{
	TetherLevelImpl::GNavGraph.Reset();
}

bool UTetherLevelLibrary::NavGraphAddNode(const FString& Name, const FVector& Location)
{
	if (Name.IsEmpty()) return false;
	const bool bIsNew = !TetherLevelImpl::GNavGraph.Contains(Name);
	TetherLevelImpl::FNavNode& Node = TetherLevelImpl::GNavGraph.FindOrAdd(Name);
	Node.Location = Location;
	return bIsNew;
}

bool UTetherLevelLibrary::NavGraphAddEdge(const FString& From, const FString& To, float Cost)
{
	if (From.IsEmpty() || To.IsEmpty()) return false;
	TetherLevelImpl::FNavNode* FromN = TetherLevelImpl::GNavGraph.Find(From);
	TetherLevelImpl::FNavNode* ToN = TetherLevelImpl::GNavGraph.Find(To);
	if (!FromN || !ToN) return false;
	FromN->OutEdges.Add(To, FMath::Max(Cost, 0.0f));
	return true;
}

TArray<FString> UTetherLevelLibrary::NavGraphListNodes()
{
	TArray<FString> Names;
	TetherLevelImpl::GNavGraph.GetKeys(Names);
	return Names;
}

bool UTetherLevelLibrary::NavGraphGetNodeLocation(const FString& Name, FVector& OutLocation)
{
	OutLocation = FVector::ZeroVector;
	if (const TetherLevelImpl::FNavNode* Node = TetherLevelImpl::GNavGraph.Find(Name))
	{
...
TArray<FString> UTetherLevelLibrary::NavGraphShortestPath(const FString& From, const FString& To, float& OutTotalCost)
{
	return TetherLevelImpl::DijkstraPath(From, To, OutTotalCost);
}

bool UTetherLevelLibrary::NavGraphSaveJson(const FString& FilePath)
{
	...
	for (const auto& Pair : TetherLevelImpl::GNavGraph)
	{
...
bool UTetherLevelLibrary::NavGraphLoadJson(const FString& FilePath)
{
	if (FilePath.IsEmpty()) return false;
	FString Raw;
	...
	if (Root->TryGetArrayField(TEXT("nodes"), NodesArr))
	{
		for (const TSharedPtr<FJsonValue>& Val : *NodesArr)
		{
			...
			TetherLevelImpl::FNavNode& N = TetherLevelImpl::GNavGraph.FindOrAdd(Name);
			N.Location = Loc;
		}
	}
	...
	if (Root->TryGetArrayField(TEXT("edges"), EdgesArr))
	{
		for (const TSharedPtr<FJsonValue>& Val : *EdgesArr)
		{
			...
			TetherLevelImpl::FNavNode* FromN = TetherLevelImpl::GNavGraph.Find(From);
			if (FromN && TetherLevelImpl::GNavGraph.Contains(To))
			{
...
namespace TetherLevelImpl
{
		...
		const FString& OutPngPath)
	{
		...
		SpawnInfo.Name = MakeUniqueObjectName(World, ASceneCapture2D::StaticClass(), TEXT("TetherCaptureCam"));
		...
		UTextureRenderTarget2D* RT = NewObject<UTextureRenderTarget2D>(
			GetTransientPackage(),
			MakeUniqueObjectName(GetTransientPackage(), UTextureRenderTarget2D::StaticClass(), TEXT("TetherCaptureRT")));
...
bool UTetherLevelLibrary::CaptureOrthoTopDown(
	...
	float CameraHeight)
{
	...
	return TetherLevelImpl::CaptureSceneToPng(
		World, CamLoc, CamRot,
		/*bOrthographic=*/ true,
...
bool UTetherLevelLibrary::CaptureFromPose(
...
namespace TetherLevelImpl
{
		...
		TArray<FColor>& OutPixels)
	{
		FActorSpawnParameters SpawnInfo;
		SpawnInfo.ObjectFlags |= RF_Transient;
		SpawnInfo.Name = MakeUniqueObjectName(World, ASceneCapture2D::StaticClass(), TEXT("TetherAnimPoseCam"));
		ASceneCapture2D* CaptureActor = World->SpawnActor<ASceneCapture2D>(
			CameraLocation, CameraRotation, SpawnInfo);
		...
		UTextureRenderTarget2D* RT = NewObject<UTextureRenderTarget2D>(GetTransientPackage(),
			MakeUniqueObjectName(GetTransientPackage(), UTextureRenderTarget2D::StaticClass(), TEXT("TetherAnimPoseRT")));
		...
		TArray<FBoneChainDef>& OutChains)
	{
		...
		#define TETHER_FIND(arr) FindFirstBone(RefSkel, arr, UE_ARRAY_COUNT(arr), ValidBoneCount)
		...
		// Spine — cyan
		{
			...
			AppendBone(C, TETHER_FIND(SpineSeg1));
			AppendBone(C, TETHER_FIND(SpineSeg2));
			AppendBone(C, TETHER_FIND(SpineSeg3));
			AppendBone(C, TETHER_FIND(SpineSeg4));
			AppendBone(C, TETHER_FIND(SpineSeg5));
			Finalise(C);
		}
		// Left arm — yellow
		{
			FBoneChainDef C; C.Color = FColor(255, 220, 0, 255);
			AppendBone(C, TETHER_FIND(LClav));
			AppendBone(C, TETHER_FIND(LUpper));
			AppendBone(C, TETHER_FIND(LLower));
			AppendBone(C, TETHER_FIND(LHand));
			Finalise(C);
		}
		// Right arm — orange (distinguishable from left under vision analysis)
		{
			FBoneChainDef C; C.Color = FColor(255, 140, 0, 255);
			AppendBone(C, TETHER_FIND(RClav));
			AppendBone(C, TETHER_FIND(RUpper));
			AppendBone(C, TETHER_FIND(RLower));
			AppendBone(C, TETHER_FIND(RHand));
			Finalise(C);
		}
		// Left leg — magenta
		{
			FBoneChainDef C; C.Color = FColor(255, 60, 220, 255);
			AppendBone(C, TETHER_FIND(PelvisAlias));
			AppendBone(C, TETHER_FIND(LThigh));
			AppendBone(C, TETHER_FIND(LCalf));
			AppendBone(C, TETHER_FIND(LFoot));
			Finalise(C);
		}
		// Right leg — purple
		{
			...
			AppendBone(C, TETHER_FIND(PelvisAlias));
			AppendBone(C, TETHER_FIND(RThigh));
			AppendBone(C, TETHER_FIND(RCalf));
			AppendBone(C, TETHER_FIND(RFoot));
			Finalise(C);
		}
...
bool UTetherLevelLibrary::CaptureAnimPoseGrid(
	...
	const FString& FilePath)
{
	...
	TetherLevelImpl::AttachPreviewAssets(SkelComp, Anim->GetSkeleton(), PreviewScene);

	// 4c. Enhance lighting for proper shading.
	TetherLevelImpl::EnhancePreviewLighting(PreviewScene);
	...
	TetherLevelImpl::ApplyPoseAtTime(SkelComp, Anim, 0.0f);
	const int32 PelvisIdx = TetherLevelImpl::FindPelvisBoneIndex(
	...
	if (bRootTrajectory && PelvisIdx != INDEX_NONE)
	{
		...
		TetherLevelImpl::SampleRootTrajectory(
	...
	TetherLevelImpl::ApplyPoseAtTime(SkelComp, Anim, Time);
	TetherLevelImpl::FlushPoseToRenderProxy(SkelComp);
	...
	float LegacyRadius;
	TetherLevelImpl::ComputePoseFraming(Mesh, SkelComp, LegacyCentre, LegacyRadius);
	...
	for (int32 i = 0; i < NumViews; ++i)
	{
		ViewOk[i] = TetherLevelImpl::ResolveViewDirection(Views[i], ViewDirs[i]);
		if (!ViewOk[i])
		{
			UE_LOG(LogTemp, Warning,
	...
	if (bPerViewFraming)
	{
		...
		TetherLevelImpl::CollectBoneWorldPositions(SkelComp, BonePositions);
		...
		TetherLevelImpl::ComputePerViewFraming(
	...
	TArray<TetherLevelImpl::FBoneChainDef> BoneChains;
	if (bBoneOverlay)
	{
		TetherLevelImpl::BuildBoneChains(
			RefSkel, SkelComp->GetComponentSpaceTransforms().Num(),
			BoneChains);
	}
	...
	if (bGroundGrid)
	{
		...
		TetherLevelImpl::SpawnGroundGridGeometry(
			PreviewScene,
	...
	for (int32 ViewIdx = 0; ViewIdx < NumViews; ++ViewIdx)
	{
		...
		TArray<FColor> Pixels;
		if (!TetherLevelImpl::CaptureViewToPixels(
		...
		if (bRootTrajectory && TrajGround.Num() >= 2)
		{
			...
			Highlights.Add(Time);
			TetherLevelImpl::DrawTrajectoryOnCell(
		...
		if (bBoneOverlay)
		{
			TetherLevelImpl::DrawBoneOverlayOnCell(
				Pixels.GetData(), CellWidth, CellHeight,
				SkelComp, BoneChains,
				CamLocs[ViewIdx], CamRots[ViewIdx], FOVDeg);
		}
...
bool UTetherLevelLibrary::CaptureAnimMontageTimeline(
	const FString& AnimPath,
	const FString& SkeletalMeshPath,
	...
	const FString& FilePath)
{
	...
	{
		TetherLevelImpl::ApplyPoseAtTime(SkelComp, Anim, T);
		const TArray<FTransform>& CSBones = SkelComp->GetComponentSpaceTransforms();
		const FTransform CompToWorld = SkelComp->GetComponentTransform();
	...
	TetherLevelImpl::ApplyPoseAtTime(SkelComp, Anim, SampleTimes[MidIdx]);
	...
	{
		ViewOk[c] = TetherLevelImpl::ResolveViewDirection(Views[c], ViewDirs[c]);
		if (!ViewOk[c])
		{
			UE_LOG(LogTemp, Warning,
	...
	{
		TetherLevelImpl::ApplyPoseAtTime(SkelComp, Anim, SampleTimes[Row]);
		TetherLevelImpl::FlushPoseToRenderProxy(SkelComp);
...
bool UTetherLevelLibrary::LineTraceHitInfo(
	...
	FString& OutActorLabel, float& OutDistance, FVector& OutImpactLocation)
{
	...
	FHitResult Hit;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherLineTraceHitInfo), /*bTraceComplex*/ true);
	const bool bHit = World->LineTraceSingleByChannel(Hit, Start, End, ECC_Visibility, Params);
	if (!bHit)
	{
		return false;
...
TArray<FString> UTetherLevelLibrary::MultiLineTraceActors(FVector Start, FVector End)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetRuntimeWorld();
	if (!World)
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherMultiLineTrace), /*bTraceComplex*/ true);
...
FString UTetherLevelLibrary::SphereTraceFirstActor(FVector Start, FVector End, float Radius)
{
	UWorld* World = TetherLevelImpl::GetRuntimeWorld();
	if (!World)
	{
	...
	FHitResult Hit;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherSphereTrace), /*bTraceComplex*/ false);
...
TArray<FString> UTetherLevelLibrary::MultiSphereTraceActors(FVector Start, FVector End, float Radius)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetRuntimeWorld();
	if (!World)
	...
	TArray<FHitResult> Hits;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherMultiSphereTrace), /*bTraceComplex*/ false);
...
FString UTetherLevelLibrary::BoxTraceFirstActor(FVector Start, FVector End, FVector BoxHalfExtent)
{
	UWorld* World = TetherLevelImpl::GetRuntimeWorld();
	if (!World)
	{
	...
	FHitResult Hit;
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherBoxTrace), /*bTraceComplex*/ false);
...
TArray<FString> UTetherLevelLibrary::OverlapSphereActors(FVector Center, float Radius, const FString& ClassFilter)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetRuntimeWorld();
	if (!World)
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherOverlapSphere), /*bTraceComplex*/ false);
...
TArray<FString> UTetherLevelLibrary::GetActorSockets(const FString& ActorName)
{
	TArray<FString> Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
...
FTetherTransform UTetherLevelLibrary::GetSocketWorldTransform(const FString& ActorName, const FString& ComponentName, const FName SocketName)
{
	FTetherTransform Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	...
	USceneComponent* SC = Cast<USceneComponent>(TetherLevelImpl::FindComponent(Actor, ComponentName));
	if (!SC)
	{
		return Out;
	}
	...
	return TetherLevelImpl::ToTetherTransform(T);
}

FTetherTransform UTetherLevelLibrary::GetComponentWorldTransform(const FString& ActorName, const FString& ComponentName)
{
	FTetherTransform Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	...
	return TetherLevelImpl::ToTetherTransform(SC->GetComponentTransform());
}

bool UTetherLevelLibrary::SetComponentVisibility(const FString& ActorName, const FString& ComponentName, bool bVisible, bool bPropagateToChildren)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::SetComponentMobility(const FString& ActorName, const FString& ComponentName, const FString& Mobility)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Invalid Mobility '%s' (expected Static/Stationary/Movable)"), *Mobility);
		return false;
	}
...
bool UTetherLevelLibrary::SnapActorToFloor(const FString& ActorName, float MaxDistance)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AActor* Actor = TetherLevelImpl::FindActor(World, ActorName);
	if (!Actor)
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherSnapToFloor), /*bTraceComplex=*/ false, Actor);
	FHitResult Hit;
	if (!World->LineTraceSingleByChannel(Hit, Start, End, ECC_Visibility, Params))
	{
		return false;
...
int32 UTetherLevelLibrary::SnapActorsToGrid(const TArray<FString>& ActorNames, float GridSize)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	...
	for (const FString& Name : ActorNames)
	{
		AActor* A = TetherLevelImpl::FindActor(World, Name);
		if (!A) continue;
		A->Modify();
...
int32 UTetherLevelLibrary::OffsetActors(const TArray<FString>& ActorNames, FVector DeltaLocation)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
...
FTetherActorBounds UTetherLevelLibrary::GetLevelBounds()
{
	FTetherActorBounds Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
...
int32 UTetherLevelLibrary::IsolateActors(const TArray<FString>& KeepVisible)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
...
int32 UTetherLevelLibrary::ShowAllActors()
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
...
TArray<FString> UTetherLevelLibrary::GetHiddenActorNames()
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
...
int32 UTetherLevelLibrary::ToggleActorsHidden(const TArray<FString>& ActorNames)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
...
FString UTetherLevelLibrary::GetActorMesh(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::SetActorMesh(const FString& ActorName, const FString& MeshAssetPath)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::SetActorMaterial(const FString& ActorName, int32 MaterialIndex, const FString& MaterialAssetPath)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
FString UTetherLevelLibrary::GetActorCollisionProfile(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	UPrimitiveComponent* Prim = TetherLevelImpl::GetPrimaryPrimitive(Actor);
	if (!Prim)
	{
		return FString();
	}
...
bool UTetherLevelLibrary::SetActorCollisionProfile(const FString& ActorName, const FString& ProfileName)
{
...
bool UTetherLevelLibrary::SetActorSimulatePhysics(const FString& ActorName, bool bSimulate)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	UPrimitiveComponent* Prim = TetherLevelImpl::GetPrimaryPrimitive(Actor);
	if (!Prim)
...
bool UTetherLevelLibrary::SetActorEnableCollision(const FString& ActorName, bool bEnabled)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
FString UTetherLevelLibrary::GetActorRootComponentName(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
...
FString UTetherLevelLibrary::AddComponentOfClass(const FString& ActorName, const FString& ComponentClassPath)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
...
bool UTetherLevelLibrary::RemoveComponent(const FString& ActorName, const FString& ComponentName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
	...
	UActorComponent* Comp = TetherLevelImpl::FindComponent(Actor, ComponentName);
	if (!Comp)
	{
		return false;
	}
...
bool UTetherLevelLibrary::SetStreamingLevelLoaded(const FString& PackageName, bool bLoaded)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	ULevelStreaming* SL = TetherLevelImpl::FindStreamingLevel(World, PackageName);
	if (!SL)
	{
		return false;
	}
...
bool UTetherLevelLibrary::SetStreamingLevelVisible(const FString& PackageName, bool bVisible)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	ULevelStreaming* SL = TetherLevelImpl::FindStreamingLevel(World, PackageName);
	if (!SL)
...
bool UTetherLevelLibrary::IsStreamingLevelLoaded(const FString& PackageName)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	ULevelStreaming* SL = TetherLevelImpl::FindStreamingLevel(World, PackageName);
...
bool UTetherLevelLibrary::FlushLevelStreaming()
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World)
	{
...
float UTetherLevelLibrary::GetWorldGravity()
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
...
bool UTetherLevelLibrary::SetWorldGravity(float Gravity, bool bOverride)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AWorldSettings* WS = World ? World->GetWorldSettings() : nullptr;
	if (!WS)
...
float UTetherLevelLibrary::GetKillZ()
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AWorldSettings* WS = World ? World->GetWorldSettings() : nullptr;
...
bool UTetherLevelLibrary::SetKillZ(float NewKillZ)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AWorldSettings* WS = World ? World->GetWorldSettings() : nullptr;
	if (!WS)
...
namespace TetherLevelImpl
{
	static bool DownwardTrace(UWorld* World, float X, float Y, float StartHeight, FHitResult& OutHit, AActor* IgnoreActor = nullptr)
	{
		...
		FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherGroundTrace), /*bTraceComplex=*/ false);
...
float UTetherLevelLibrary::GetGroundHeightAt(float X, float Y, float StartHeight)
{
	FHitResult Hit;
	if (!TetherLevelImpl::DownwardTrace(TetherLevelImpl::GetEditorWorld(), X, Y, StartHeight, Hit))
	{
		return -1.0e6f;
	}
...
bool UTetherLevelLibrary::GetGroundNormalAt(float X, float Y, FVector& OutNormal, float StartHeight)
{
...
FString UTetherLevelLibrary::GetGroundHitActor(float X, float Y, float StartHeight)
{
	FHitResult Hit;
	if (!TetherLevelImpl::DownwardTrace(TetherLevelImpl::GetRuntimeWorld(), X, Y, StartHeight, Hit))
	{
		return FString();
	}
	AActor* A = Hit.GetActor();
...
float UTetherLevelLibrary::GetActorGroundClearance(const FString& ActorName, float MaxDistance)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	AActor* Actor = TetherLevelImpl::FindActor(World, ActorName);
	if (!Actor)
	...
	const FVector Start = Actor->GetActorLocation();
	const FVector End = Start - FVector(0, 0, FMath::Max(MaxDistance, 0.0f));
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherActorGround), /*bTraceComplex=*/ false, Actor);
	FHitResult Hit;
	if (!World->LineTraceSingleByChannel(Hit, Start, End, ECC_Visibility, Params))
	{
		return -1.0f;
...
TArray<FString> UTetherLevelLibrary::GetAllDescendants(const FString& ActorName)
{
	TArray<FString> Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	...
	TetherLevelImpl::CollectDescendantsRecursive(Actor, Out);
	return Out;
}

TArray<FString> UTetherLevelLibrary::GetActorSiblings(const FString& ActorName)
{
	TArray<FString> Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return Out;
...
FString UTetherLevelLibrary::GetRootAttachParent(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
...
int32 UTetherLevelLibrary::GetAttachmentDepth(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return -1;
	int32 Depth = 0;
...
int32 UTetherLevelLibrary::GetActorVertexCount(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return -1;
...
int32 UTetherLevelLibrary::GetActorTriangleCount(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return -1;
	TArray<UStaticMeshComponent*> SMCs;
...
int32 UTetherLevelLibrary::GetActorMaterialSlotCount(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return -1;
...
int32 UTetherLevelLibrary::GetActorLODCount(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return -1;
	TArray<UStaticMeshComponent*> SMCs;
...
TArray<FString> UTetherLevelLibrary::GetAllActorTagsInLevel()
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return Out;
...
int32 UTetherLevelLibrary::CountActorsByTag(const FName Tag)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
...
int32 UTetherLevelLibrary::SelectActorsByTag(const FName Tag, bool bAddToSelection)
{
...
int32 UTetherLevelLibrary::RemoveTagFromAllActors(const FName Tag)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World || Tag.IsNone()) return 0;
...
bool UTetherLevelLibrary::IsActorOfClass(const FString& ActorName, const FString& ClassPath)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return false;
	return TetherLevelImpl::MatchesClassFilter(Actor->GetClass(), ClassPath);
}

FString UTetherLevelLibrary::GetActorParentClass(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
...
TArray<FString> UTetherLevelLibrary::GetActorClassHierarchy(const FString& ActorName)
{
	TArray<FString> Out;
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return Out;
...
TArray<FString> UTetherLevelLibrary::FindActorsByClassAndTag(const FString& ClassFilter, const FName Tag)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		AActor* A = *It;
		if (!A) continue;
		if (!TetherLevelImpl::MatchesClassFilter(A->GetClass(), ClassFilter)) continue;
...
int32 UTetherLevelLibrary::RotateActors(const TArray<FString>& ActorNames, FRotator DeltaRotation)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::ScaleActors(const TArray<FString>& ActorNames, FVector ScaleMultiplier)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::SetActorsUniformScale(const TArray<FString>& ActorNames, float UniformScale)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::MoveActorsToFolder(const TArray<FString>& ActorNames, const FString& FolderPath)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::RenameFolder(const FString& OldFolder, const FString& NewFolder)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::DissolveFolder(const FString& FolderPath)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
bool UTetherLevelLibrary::IsFolderEmpty(const FString& FolderPath)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
...
TArray<FString> UTetherLevelLibrary::GetActorsInSublevel(const FString& PackageName)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	ULevel* Level = TetherLevelImpl::FindLevelByPackage(World, PackageName);
	if (!Level) return Out;
...
int32 UTetherLevelLibrary::CountActorsInSublevel(const FString& PackageName)
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	ULevel* Level = TetherLevelImpl::FindLevelByPackage(World, PackageName);
	if (!Level) return 0;
...
FString UTetherLevelLibrary::GetActorLevelPackageName(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor) return FString();
...
int32 UTetherLevelLibrary::GetPersistentLevelActorCount()
{
	UWorld* World = TetherLevelImpl::GetEditorWorld();
...
int32 UTetherLevelLibrary::InvertSelection()
{
	if (!GEditor) return 0;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::SelectAllActors()
{
	if (!GEditor) return 0;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::SelectActorsInBox(FVector Min, FVector Max, bool bAddToSelection)
{
	if (!GEditor) return 0;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::GetSelectionCount()
{
	if (!GEditor) return 0;
	USelection* Sel = GEditor->GetSelectedActors();
...
FTetherActorBounds UTetherLevelLibrary::GetSelectionBounds()
{
	FTetherActorBounds Out;
...
FVector UTetherLevelLibrary::GetSelectionCentroid()
{
...
TArray<FString> UTetherLevelLibrary::GetSelectionClassSet()
{
	TArray<FString> Out;
	if (!GEditor) return Out;
	USelection* Sel = GEditor->GetSelectedActors();
...
TArray<FString> UTetherLevelLibrary::FindActorsInCone(FVector Origin, FVector Direction, float HalfAngleDeg, float MaxDistance, const FString& ClassFilter)
{
	TArray<FString> Out;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return Out;
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (!A) continue;
		if (!TetherLevelImpl::MatchesClassFilter(A->GetClass(), ClassFilter)) continue;
		if (TetherLevelImpl::IsPointInsideCone(A->GetActorLocation(), Origin, Direction, HalfAngleDeg, MaxDistance))
		{
			Out.Add(A->GetActorLabel());
		}
	}
...
bool UTetherLevelLibrary::IsActorInCone(const FString& ActorName, FVector Origin, FVector Direction, float HalfAngleDeg, float MaxDistance)
{
	AActor* A = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!A) return false;
	return TetherLevelImpl::IsPointInsideCone(A->GetActorLocation(), Origin, Direction, HalfAngleDeg, MaxDistance);
}

FVector UTetherLevelLibrary::ClosestPointOnSegment(FVector Point, FVector SegmentStart, FVector SegmentEnd)
{
	return FMath::ClosestPointOnSegment(Point, SegmentStart, SegmentEnd);
}

float UTetherLevelLibrary::DistanceFromPointToSegment(FVector Point, FVector SegmentStart, FVector SegmentEnd)
{
	return FMath::PointDistToSegment(Point, SegmentStart, SegmentEnd);
}

int32 UTetherLevelLibrary::SelectActorsInSphere(FVector Center, float Radius, bool bAddToSelection)
{
	if (!GEditor) return 0;
	UWorld* World = TetherLevelImpl::GetEditorWorld();
	if (!World) return 0;
...
int32 UTetherLevelLibrary::MirrorActors(const TArray<FString>& ActorNames, const FString& Axis)
{
...
bool UTetherLevelLibrary::SetComponentRelativeTransform(
	const FString& ActorName,
...
int32 UTetherLevelLibrary::ResetActorMaterials(const FString& ActorName)
{
	AActor* Actor = TetherLevelImpl::FindActor(TetherLevelImpl::GetEditorWorld(), ActorName);
	if (!Actor)
	{
