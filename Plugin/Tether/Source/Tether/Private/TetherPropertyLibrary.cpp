#include "TetherPropertyLibrary.h"
...
#define LOCTEXT_NAMESPACE "TetherPropertyLibrary"

namespace TetherPropertyOps
{
	// ── Object resolution ─────────────────────────────────────────
		...
		for (int32 i = 0; i < Path.Num(); ++i)
		{
			...
			if (!Prop)
			{
				UE_LOG(LogTemp, Warning,
					TEXT("TetherProperty: segment '%s' not found in %s"),
			...
			if (Seg.Index != INDEX_NONE)
			{
				...
				{
					UE_LOG(LogTemp, Warning,
						TEXT("TetherProperty: '[%d]' on non-array property '%s'"),
						Seg.Index, *Seg.Name);
					return FResolvedProp();
				}
				...
				{
					UE_LOG(LogTemp, Warning,
						TEXT("TetherProperty: index %d out of range [0,%d) for '%s'"),
			...
			if (i < Path.Num() - 1)
			{
				...
				{
					UE_LOG(LogTemp, Warning,
						TEXT("TetherProperty: cannot recurse past non-struct property '%s'"),
						*Seg.Name);
					return FResolvedProp();
				}
...
TArray<FTetherUPropertyInfo> UTetherPropertyLibrary::ListUProperties(
	const FString& ObjectOrClassPath, bool bIncludeInherited)
{
	TArray<FTetherUPropertyInfo> Result;
	UObject* Obj = TetherPropertyOps::ResolveObjectOrClass(ObjectOrClassPath);
		...
		It; ++It)
	{
		...
		Info.CppAccess = TetherPropertyOps::DecodeAccess(PropFlags);
		Info.Flags = TetherPropertyOps::DecodeFlags(PropFlags);
...
FString UTetherPropertyLibrary::GetUPropertyAsExportText(
	const FString& ObjectOrClassPath, const FString& PropertyPath, bool& OutSuccess)
{
	...
	const TArray<UObject*> Candidates = TetherPropertyOps::ResolveCandidates(ObjectOrClassPath);
	...
	const FString& ValueExportText, bool bFireChangeNotify)
{
	...
	for (UObject* Obj : TetherPropertyOps::ResolveCandidates(ObjectOrClassPath))
	{
		...
		if (!AfterParse || AfterParse == Start)
		{
			...
			UE_LOG(LogTemp, Warning,
				TEXT("TetherProperty: ImportText failed (or consumed nothing) for '%s' = '%s'"),
				*PropertyPath, *ValueExportText);
			return false;
		}

		// Commit
		FScopedTransaction Transaction(LOCTEXT("TetherSetUProperty", "Tether: Set UProperty"));
		Obj->Modify();
		...
		{
			TetherPropertyOps::FireChangeNotify(Obj, Resolved.PropertyChain);
		}
		return true;
	}
	return false;
	...
	const FString& ElementExportText, bool bFireChangeNotify)
{
	...
	FScopedTransaction Transaction(LOCTEXT("TetherArrayAppend", "Tether: Array Append"));
	Obj->Modify();
	...
	if (StructProp && StructProp->Struct == FGameplayTagContainer::StaticStruct())
	{
		...
		{
			UE_LOG(LogTemp, Warning,
				TEXT("TetherProperty: ArrayAppend on FGameplayTagContainer — tag '%s' not registered"),
				*TagName);
			return false;
		}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherProperty: ArrayAppend target '%s' is neither FArrayProperty nor FGameplayTagContainer"),
			*PropertyPath);
		return false;
	}
	...
	if (!AfterParse || AfterParse == Start)
	{
		...
		UE_LOG(LogTemp, Warning,
			TEXT("TetherProperty: ArrayAppend ImportText failed (or consumed nothing) for '%s'"), *ElementExportText);
		return false;
	}
	...
	int32 Index, bool bFireChangeNotify)
{
	...
	FScopedTransaction Transaction(LOCTEXT("TetherArrayRemove", "Tether: Array Remove"));
	Obj->Modify();
	...
	if (StructProp && StructProp->Struct == FGameplayTagContainer::StaticStruct())
	{
		...
		if (bFireChangeNotify) TetherPropertyOps::FireChangeNotify(Obj, Resolved.PropertyChain);
		return true;
	}
	...
	bool bFireChangeNotify)
{
	...
	FScopedTransaction Transaction(LOCTEXT("TetherArrayClear", "Tether: Array Clear"));
	Obj->Modify();
...
FString UTetherPropertyLibrary::GetAssetCDOPath(const FString& AssetPath)
{
