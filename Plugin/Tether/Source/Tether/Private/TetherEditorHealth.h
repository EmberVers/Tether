 ...
 */
struct FTetherEditorHealthReadout
{
	bool bReady = false;
	bool bStale = true;
...
public:
	...
	explicit FTetherEditorHealthCache(FClock InClock);

	/** 注入全部时钟以确定性测试 wire 时间字段。 / Inject both clocks to test wire timestamps deterministically. */
	FTetherEditorHealthCache(FClock InClock, FUtcClock InUtcClock);
	...
	void RecordSlateTick(const FTetherModalAttention& InModal);

	/** 按指定阈值读取年龄和陈旧性。 / Read ages and staleness using the supplied threshold. */
	FTetherEditorHealthReadout Read(double StaleAfterSeconds);
