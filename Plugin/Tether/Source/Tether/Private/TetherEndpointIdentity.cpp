...
FTetherEndpointIdentity FTetherEndpointIdentity::Create()
{
...
FString FTetherEndpointIdentity::NormalizeProjectPath(FString Path)
{
...
void FTetherEndpointIdentity::AppendToResponse(const TSharedRef<FJsonObject>& Response) const
{
	Response->SetNumberField(TetherProtocol::ProtocolVersionField, ProtocolVersion);
	Response->SetStringField(TetherProtocol::InstanceIdField, InstanceId);
	Response->SetNumberField(TetherProtocol::ProcessIdField, ProcessId);
	Response->SetStringField(TetherProtocol::ProjectPathField, ProjectPath);
}

bool FTetherEndpointIdentity::ValidateRequest(
	...
	FString& OutError) const
{
	...
	if (!Request.IsValid()
		|| !Request->TryGetObjectField(TetherProtocol::ExpectedField, ExpectedPtr)
		|| ExpectedPtr == nullptr
		|| !ExpectedPtr->IsValid())
	{
	...
	const TSharedPtr<FJsonValue>* ProtocolValue = Expected->Values.Find(TetherProtocol::ProtocolVersionField);
	...
	const TSharedPtr<FJsonValue>* InstanceValue = Expected->Values.Find(TetherProtocol::InstanceIdField);
	...
	const TSharedPtr<FJsonValue>* PidValue = Expected->Values.Find(TetherProtocol::ProcessIdField);
	if (PidValue == nullptr
		|| !PidValue->IsValid()
	...
	const TSharedPtr<FJsonValue>* ProjectPathValue = Expected->Values.Find(TetherProtocol::ProjectPathField);
