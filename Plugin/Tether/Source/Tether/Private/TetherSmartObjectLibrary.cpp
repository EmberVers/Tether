...
#define LOCTEXT_NAMESPACE "TetherSmartObject"

namespace TetherSmartObjectImpl
{
...
{
	LastError = Message;
	UE_LOG(LogTemp, Warning, TEXT("Tether SmartObject: %s"), *Message);
	return false;
}
...
static bool JsonToTagQuery(const FString& JsonText, FGameplayTagQuery& OutQuery)
{
	...
	OutQuery.Build(Expression, TEXT("Authored by Tether"));
	return true;
}
...
static TArray<FTetherSmartObjectPropertyInfo> ListProperties(FPropertyBindingDataView View, bool bIncludeInherited)
{
	TArray<FTetherSmartObjectPropertyInfo> Result;
	...
	for (TFieldIterator<FProperty> It(Struct, Flags); It; ++It)
	{
		...
		if (!Address) continue;
		FTetherSmartObjectPropertyInfo Info;
...
static FTetherSmartObjectPropertyResult ReadProperty(
	FPropertyBindingDataView View, const FString& PropertyPath, UObject* Owner)
{
	FTetherSmartObjectPropertyResult Result;
...
{
	return TetherSmartObjectImpl::LastError;
}

FTetherSmartObjectCreateResult UTetherSmartObjectLibrary::CreateSmartObjectDefinition(const FString& AssetPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	FTetherSmartObjectCreateResult Result;
	if (IsPieRunning())
	{
...
FTetherSmartObjectDefinitionInfo UTetherSmartObjectLibrary::GetSmartObjectDefinitionInfo(const FString& AssetPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	FTetherSmartObjectDefinitionInfo Info;
...
FTetherSmartObjectValidationResult UTetherSmartObjectLibrary::ValidateSmartObjectDefinition(const FString& AssetPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	FTetherSmartObjectValidationResult Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	if (!Definition)
	{
		Result.Error = LastError;
	...
	{
		FTetherSmartObjectValidationMessage Out;
...
TArray<FTetherSmartObjectPropertyInfo> UTetherSmartObjectLibrary::ListSmartObjectDefinitionProperties(
	const FString& AssetPath, bool bIncludeInherited)
{
	...
	return Definition ? ListProperties(FPropertyBindingDataView(Definition), bIncludeInherited)
		: TArray<FTetherSmartObjectPropertyInfo>();
}

FTetherSmartObjectPropertyResult UTetherSmartObjectLibrary::GetSmartObjectDefinitionProperty(
...
bool UTetherSmartObjectLibrary::SetSmartObjectDefinitionProperty(
	const FString& AssetPath, const FString& PropertyPath, const FString& Value)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectDefinitionProperty", "Tether: Set Smart Object Definition Property"));
	Definition->Modify();
...
FString UTetherSmartObjectLibrary::GetSmartObjectTagQueryJson(
...
bool UTetherSmartObjectLibrary::SetSmartObjectTagQueryJson(
	const FString& AssetPath, const FString& QueryJson, const FString& SlotId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectTagQuery", "Tether: Set Smart Object Tag Query"));
	Definition->Modify();
	if (SlotId.IsEmpty())
	{
...
bool UTetherSmartObjectLibrary::SetSmartObjectTags(const FString& AssetPath,
	const TArray<FString>& Tags, const FString& SlotId, const FString& TagSet)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectTags", "Tether: Set Smart Object Tags"));
	Definition->Modify();
	if (SlotId.IsEmpty())
	{
...
bool UTetherSmartObjectLibrary::SetSmartObjectTagPolicies(const FString& AssetPath,
	const FString& UserTagsFilteringPolicy, const FString& ActivityTagsMergingPolicy)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectTagPolicies", "Tether: Set Smart Object Tag Policies"));
	Definition->Modify();
...
FString UTetherSmartObjectLibrary::AddSmartObjectSlot(const FString& AssetPath, const FString& Name,
	const FVector& Offset, const FRotator& Rotation, bool bEnabled, int32 InsertIndex)
{
	...
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	if (!Definition) return FString();
	FScopedTransaction Transaction(LOCTEXT("AddSmartObjectSlot", "Tether: Add Smart Object Slot"));
	Definition->Modify();
...
FString UTetherSmartObjectLibrary::DuplicateSmartObjectSlot(const FString& AssetPath,
	const FString& SourceSlotId, const FString& NewName, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("DuplicateSmartObjectSlot", "Tether: Duplicate Smart Object Slot"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::RemoveSmartObjectSlot(const FString& AssetPath, const FString& SlotId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveSmartObjectSlot", "Tether: Remove Smart Object Slot"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::MoveSmartObjectSlot(const FString& AssetPath,
	const FString& SlotId, int32 NewIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("MoveSmartObjectSlot", "Tether: Move Smart Object Slot"));
	Definition->Modify();
...
TArray<FTetherSmartObjectPropertyInfo> UTetherSmartObjectLibrary::ListSmartObjectSlotProperties(
	const FString& AssetPath, const FString& SlotId)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
...
FTetherSmartObjectPropertyResult UTetherSmartObjectLibrary::GetSmartObjectSlotProperty(
...
bool UTetherSmartObjectLibrary::SetSmartObjectSlotProperty(const FString& AssetPath,
	const FString& SlotId, const FString& PropertyPath, const FString& Value)
{
	...
	FSmartObjectSlotDefinition* Slot = FindSlot(Definition, SlotId);
	if (!Slot) return false;
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectSlotProperty", "Tether: Set Smart Object Slot Property"));
	Definition->Modify();
...
TArray<FTetherSmartObjectTypeInfo> UTetherSmartObjectLibrary::ListSmartObjectBehaviorTypes()
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectTypeInfo> Result;
	for (TObjectIterator<UClass> It; It; ++It)
	{
		...
		FTetherSmartObjectTypeInfo Info;
	...
	Result.Sort([](const FTetherSmartObjectTypeInfo& A, const FTetherSmartObjectTypeInfo& B)
	{
...
TArray<FTetherSmartObjectBehaviorInfo> UTetherSmartObjectLibrary::ListSmartObjectBehaviorDefinitions(
	const FString& AssetPath, const FString& SlotId)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectBehaviorInfo> Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	...
	for (int32 Index = 0; Index < Behaviors->Num(); ++Index)
	{
		...
		FTetherSmartObjectBehaviorInfo Info;
		Info.SlotId = SlotId;
		Info.Index = Index;
...
FString UTetherSmartObjectLibrary::AddSmartObjectBehaviorDefinition(const FString& AssetPath,
	const FString& BehaviorClassPath, const FString& SlotId, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddSmartObjectBehavior", "Tether: Add Smart Object Behavior"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::RemoveSmartObjectBehaviorDefinition(
	const FString& AssetPath, const FString& BehaviorObjectPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveSmartObjectBehavior", "Tether: Remove Smart Object Behavior"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::MoveSmartObjectBehaviorDefinition(const FString& AssetPath,
	const FString& BehaviorObjectPath, int32 NewIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("MoveSmartObjectBehavior", "Tether: Move Smart Object Behavior"));
	Definition->Modify();
...
TArray<FTetherSmartObjectPropertyInfo> UTetherSmartObjectLibrary::ListSmartObjectBehaviorProperties(
...
FTetherSmartObjectPropertyResult UTetherSmartObjectLibrary::GetSmartObjectBehaviorProperty(
...
bool UTetherSmartObjectLibrary::SetSmartObjectBehaviorProperty(const FString& AssetPath,
	const FString& BehaviorObjectPath, const FString& PropertyPath, const FString& Value)
{
	...
	FBehaviorLocation Location = FindBehavior(Definition, BehaviorObjectPath);
	if (!Location.Behavior) return false;
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectBehaviorProperty", "Tether: Set Smart Object Behavior Property"));
	Definition->Modify();
	Location.Behavior->Modify();
...
TArray<FTetherSmartObjectTypeInfo> UTetherSmartObjectLibrary::ListSmartObjectDefinitionDataTypes(
	const FString& AssetPath, const FString& SlotId)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
...
TArray<FTetherSmartObjectDefinitionDataInfo> UTetherSmartObjectLibrary::ListSmartObjectDefinitionData(
	const FString& AssetPath, const FString& SlotId)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectDefinitionDataInfo> Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	...
	for (int32 Index = 0; Index < Data->Num(); ++Index)
	{
		...
		if (!Struct) continue;
		FTetherSmartObjectDefinitionDataInfo Info;
		Info.SlotId = SlotId;
...
FString UTetherSmartObjectLibrary::AddSmartObjectDefinitionData(const FString& AssetPath,
	const FString& StructTypePath, const FString& SlotId, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddSmartObjectDefinitionData", "Tether: Add Smart Object Definition Data"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::RemoveSmartObjectDefinitionData(
	const FString& AssetPath, const FString& DataId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveSmartObjectDefinitionData", "Tether: Remove Smart Object Definition Data"));
	Definition->Modify();
	Location.Array->RemoveAt(Location.Index);
	NotifyDefinitionChanged(Definition, Location.RootProperty);
	return true;
...
bool UTetherSmartObjectLibrary::MoveSmartObjectDefinitionData(
	const FString& AssetPath, const FString& DataId, int32 NewIndex)
{
	...
	if (Location.Index == NewIndex) return true;
	FScopedTransaction Transaction(LOCTEXT("MoveSmartObjectDefinitionData", "Tether: Move Smart Object Definition Data"));
	Definition->Modify();
...
TArray<FTetherSmartObjectPropertyInfo> UTetherSmartObjectLibrary::ListSmartObjectDefinitionDataProperties(
...
FTetherSmartObjectPropertyResult UTetherSmartObjectLibrary::GetSmartObjectDefinitionDataProperty(
...
bool UTetherSmartObjectLibrary::SetSmartObjectDefinitionDataProperty(const FString& AssetPath,
	const FString& DataId, const FString& PropertyPath, const FString& Value)
{
	...
	FDefinitionDataLocation Location = FindDefinitionData(Definition, DataId);
	if (!Location.Proxy) return false;
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectDefinitionDataProperty", "Tether: Set Smart Object Definition Data Property"));
	Definition->Modify();
...
TArray<FTetherSmartObjectTypeInfo> UTetherSmartObjectLibrary::ListSmartObjectWorldConditionTypes(
	const FString& AssetPath, const FString& SlotId)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
...
TArray<FTetherSmartObjectWorldConditionInfo> UTetherSmartObjectLibrary::ListSmartObjectWorldConditions(
	const FString& AssetPath, const FString& SlotId)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectWorldConditionInfo> Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	FWorldConditionQueryDefinition* Query = nullptr;
	FName Root;
	if (!Definition || !GetConditionQuery(Definition, SlotId, Query, Root)) return Result;
	...
	for (int32 Index = 0; Index < Conditions->Num(); ++Index)
	{
		...
		FTetherSmartObjectWorldConditionInfo Info;
		Info.SlotId = SlotId;
		Info.Index = Index;
...
int32 UTetherSmartObjectLibrary::AddSmartObjectWorldCondition(const FString& AssetPath,
	...
	int32 ExpressionDepth, bool bInvert, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddSmartObjectWorldCondition", "Tether: Add Smart Object World Condition"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::RemoveSmartObjectWorldCondition(const FString& AssetPath,
	const FString& SlotId, int32 ConditionIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveSmartObjectWorldCondition", "Tether: Remove Smart Object World Condition"));
	Definition->Modify();
	const FWorldConditionQueryDefinition Backup = *Query;
...
bool UTetherSmartObjectLibrary::MoveSmartObjectWorldCondition(const FString& AssetPath,
	const FString& SlotId, int32 ConditionIndex, int32 NewIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("MoveSmartObjectWorldCondition", "Tether: Move Smart Object World Condition"));
	Definition->Modify();
	const FWorldConditionQueryDefinition Backup = *Query;
...
bool UTetherSmartObjectLibrary::SetSmartObjectWorldConditionExpression(const FString& AssetPath,
	const FString& SlotId, int32 ConditionIndex, const FString& Operator, int32 ExpressionDepth, bool bInvert)
{
	...
	TArray<FWorldConditionEditable>* Conditions = GetEditableConditions(*Query);
	if (!Conditions || !Conditions->IsValidIndex(ConditionIndex)) return SetError(TEXT("World Condition index is out of range"));
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectWorldConditionExpression", "Tether: Set Smart Object World Condition Expression"));
	Definition->Modify();
	const FWorldConditionQueryDefinition Backup = *Query;
...
TArray<FTetherSmartObjectPropertyInfo> UTetherSmartObjectLibrary::ListSmartObjectWorldConditionProperties(
...
FTetherSmartObjectPropertyResult UTetherSmartObjectLibrary::GetSmartObjectWorldConditionProperty(
...
bool UTetherSmartObjectLibrary::SetSmartObjectWorldConditionProperty(const FString& AssetPath,
	const FString& SlotId, int32 ConditionIndex, const FString& PropertyPath, const FString& Value)
{
	...
	TArray<FWorldConditionEditable>* Conditions = GetEditableConditions(*Query);
	if (!Conditions || !Conditions->IsValidIndex(ConditionIndex)) return SetError(TEXT("World Condition index is out of range"));
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectWorldConditionProperty", "Tether: Set Smart Object World Condition Property"));
	Definition->Modify();
	const FWorldConditionQueryDefinition Backup = *Query;
...
TArray<FTetherSmartObjectParameterInfo> UTetherSmartObjectLibrary::ListSmartObjectParameters(
	const FString& AssetPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectParameterInfo> Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	...
	{
		FTetherSmartObjectParameterInfo Info;
...
FString UTetherSmartObjectLibrary::AddSmartObjectParameter(const FString& AssetPath,
	const FString& Name, const FString& Type, const FString& DefaultValue)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddSmartObjectParameter", "Tether: Add Smart Object Parameter"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::RemoveSmartObjectParameter(const FString& AssetPath, const FString& Name)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveSmartObjectParameter", "Tether: Remove Smart Object Parameter"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::RenameSmartObjectParameter(const FString& AssetPath,
	const FString& OldName, const FString& NewName)
{
	...
	FInstancedPropertyBag* Parameters = Definition ? GetParameters(Definition) : nullptr;
	if (!Definition || !Parameters) return false;
	FScopedTransaction Transaction(LOCTEXT("RenameSmartObjectParameter", "Tether: Rename Smart Object Parameter"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::SetSmartObjectParameterValue(const FString& AssetPath,
	const FString& Name, const FString& Value)
{
	...
	if (!Parameters->FindPropertyDescByName(FName(*Name)))
		return SetError(FString::Printf(TEXT("parameter '%s' was not found"), *Name));
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectParameter", "Tether: Set Smart Object Parameter"));
	Definition->Modify();
...
TArray<FTetherSmartObjectBindableStructInfo> UTetherSmartObjectLibrary::ListSmartObjectBindableStructs(
	const FString& AssetPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectBindableStructInfo> Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	if (!Definition) return Result;
	auto Add = [&](const FString& Kind, const FGuid& Id, const FString& Name, const UStruct* Struct)
	{
		FTetherSmartObjectBindableStructInfo Info;
		Info.Kind = Kind;
...
TArray<FTetherSmartObjectBindingInfo> UTetherSmartObjectLibrary::ListSmartObjectBindings(
	const FString& AssetPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectBindingInfo> Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	...
	{
		FTetherSmartObjectBindingInfo Info;
...
bool UTetherSmartObjectLibrary::AddSmartObjectBinding(const FString& AssetPath,
	const FString& SourceId, const FString& SourcePath, const FString& TargetId, const FString& TargetPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddSmartObjectBinding", "Tether: Add Smart Object Binding"));
	Definition->Modify();
...
bool UTetherSmartObjectLibrary::RemoveSmartObjectBinding(const FString& AssetPath,
	const FString& TargetId, const FString& TargetPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveSmartObjectBinding", "Tether: Remove Smart Object Binding"));
	Definition->Modify();
...
static FTetherSmartObjectComponentInfo MakeComponentInfo(USmartObjectComponent* Component)
{
	FTetherSmartObjectComponentInfo Info;
...
static FTetherSmartObjectCollectionInfo MakeCollectionInfo(ASmartObjectPersistentCollection* Collection)
{
	FTetherSmartObjectCollectionInfo Info;
...
static FTetherSmartObjectRuntimeSlotInfo MakeRuntimeSlotInfo(USmartObjectSubsystem* Subsystem,
	const FSmartObjectSlotHandle& SlotHandle, ESmartObjectClaimPriority Priority)
{
	FTetherSmartObjectRuntimeSlotInfo Info;
...
static FTetherSmartObjectClaimResult MakeClaimResult(const FString& Token, const FStoredClaim& Stored,
	USmartObjectSubsystem* Subsystem, bool bSuccess, const FString& Error = FString())
{
	FTetherSmartObjectClaimResult Result;
...
static bool BuildEntranceRequest(const FTetherSmartObjectEntranceRequest& In,
...
static FTetherSmartObjectEntranceResult MakeEntranceResult(const FSmartObjectSlotEntranceLocationResult& In)
{
	FTetherSmartObjectEntranceResult Out;
	Out.bFound = true;
...
TArray<FTetherSmartObjectComponentInfo> UTetherSmartObjectLibrary::ListSmartObjectComponents(bool bPIEOnly)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectComponentInfo> Result;
	for (TObjectIterator<USmartObjectComponent> It; It; ++It)
	{
	...
	Result.Sort([](const FTetherSmartObjectComponentInfo& A, const FTetherSmartObjectComponentInfo& B)
	{
...
FTetherSmartObjectComponentInfo UTetherSmartObjectLibrary::GetSmartObjectComponentInfo(
...
FString UTetherSmartObjectLibrary::AddSmartObjectComponent(const FString& ActorPath,
	...
	bool bRegisterWithSubsystem)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddSmartObjectComponent", "Tether: Add Smart Object Component"));
	Actor->Modify();
...
bool UTetherSmartObjectLibrary::RemoveSmartObjectComponent(const FString& ComponentPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveSmartObjectComponent", "Tether: Remove Smart Object Component"));
...
bool UTetherSmartObjectLibrary::SetSmartObjectComponentDefinition(const FString& ComponentPath,
	const FString& DefinitionAssetPath, bool bRegisterWithSubsystem)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetSmartObjectComponentDefinition", "Tether: Set Smart Object Component Definition"));
	Component->Modify();
...
bool UTetherSmartObjectLibrary::ControlSmartObjectComponent(
...
TArray<FTetherSmartObjectCollectionInfo> UTetherSmartObjectLibrary::ListPersistentSmartObjectCollections(
	bool bPIEOnly)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectCollectionInfo> Result;
	for (TObjectIterator<ASmartObjectPersistentCollection> It; It; ++It)
	{
	...
	Result.Sort([](const FTetherSmartObjectCollectionInfo& A, const FTetherSmartObjectCollectionInfo& B)
	{
...
FString UTetherSmartObjectLibrary::CreatePersistentSmartObjectCollection(const FString& ActorLabel)
{
	...
	FScopedTransaction Transaction(LOCTEXT("CreateSmartObjectCollection", "Tether: Create Smart Object Collection"));
...
bool UTetherSmartObjectLibrary::DestroyPersistentSmartObjectCollection(const FString& CollectionActorPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("DestroySmartObjectCollection", "Tether: Destroy Smart Object Collection"));
...
bool UTetherSmartObjectLibrary::ControlPersistentSmartObjectCollection(
	const FString& CollectionActorPath, const FString& Action)
{
	...
	if (!Collection) return false;
	const FString Key = NormalizeToken(Action);
	FScopedTransaction Transaction(LOCTEXT("ControlSmartObjectCollection", "Tether: Control Smart Object Collection"));
	Collection->Modify();
	if (Key == TEXT("rebuild"))
	{
...
TArray<FTetherSmartObjectCollectionEntryInfo> UTetherSmartObjectLibrary::ListPersistentSmartObjectCollectionEntries(
	const FString& CollectionActorPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectCollectionEntryInfo> Result;
	ASmartObjectPersistentCollection* Collection = FindCollection(CollectionActorPath);
	...
	for (int32 Index = 0; Index < Collection->GetEntries().Num(); ++Index)
	{
		...
		FTetherSmartObjectCollectionEntryInfo Info;
		Info.Index = Index;
...
TArray<FTetherSmartObjectQueryResult> UTetherSmartObjectLibrary::QuerySmartObjects(
	...
	bool bSortByDistance, int32 MaxResults)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectQueryResult> Result;
	UWorld* World = GetCurrentWorld();
	...
	{
		const FTetherSmartObjectRuntimeSlotInfo Slot = MakeRuntimeSlotInfo(Subsystem, Raw.SlotHandle, Priority);
		FTetherSmartObjectQueryResult Info;
	...
	if (bSortByDistance)
	{
		Result.StableSort([](const FTetherSmartObjectQueryResult& A, const FTetherSmartObjectQueryResult& B)
		{
...
TArray<FTetherSmartObjectRuntimeSlotInfo> UTetherSmartObjectLibrary::ListSmartObjectRuntimeSlots(
	const FString& SmartObjectHandle, const FString& ComponentPath, const FString& ClaimPriority)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectRuntimeSlotInfo> Result;
	ESmartObjectClaimPriority Priority;
	if (!ParseEnumToken(ClaimPriority, Priority, TEXT("claim priority"))) return Result;
	...
	Result.Sort([](const FTetherSmartObjectRuntimeSlotInfo& A, const FTetherSmartObjectRuntimeSlotInfo& B)
	{
...
FString UTetherSmartObjectLibrary::CreateRuntimeSmartObject(const FString& DefinitionAssetPath,
...
FTetherSmartObjectClaimResult UTetherSmartObjectLibrary::ClaimSmartObjectSlot(
...
FTetherSmartObjectClaimResult UTetherSmartObjectLibrary::OccupySmartObjectClaim(
...
TArray<FTetherSmartObjectClaimResult> UTetherSmartObjectLibrary::ListSmartObjectClaims()
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectClaimResult> Result;
...
bool UTetherSmartObjectLibrary::SetSmartObjectRuntimeTags(const FString& Handle,
...
bool UTetherSmartObjectLibrary::SetSmartObjectRuntimeEnabled(const FString& SmartObjectHandle,
...
bool UTetherSmartObjectLibrary::SetSmartObjectRuntimeSlotEnabled(const FString& SlotHandle,
	bool bEnabled)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
...
bool UTetherSmartObjectLibrary::SendSmartObjectSlotEvent(const FString& SlotHandle,
...
FTetherSmartObjectEntranceResult UTetherSmartObjectLibrary::FindSmartObjectEntrance(
	const FString& SlotHandle, const FTetherSmartObjectEntranceRequest& Request)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	FTetherSmartObjectEntranceResult Result;
	USmartObjectSubsystem* Subsystem = GetSubsystem(GetCurrentWorld());
	FSmartObjectSlotHandle Slot;
...
TArray<FTetherSmartObjectEntranceResult> UTetherSmartObjectLibrary::ValidateSmartObjectDefinitionEntrances(
	const FString& AssetPath, const FTransform& OwnerTransform,
	const FTetherSmartObjectEntranceRequest& Request, const FString& SkipActorPath)
{
	using namespace TetherSmartObjectImpl;
	ClearError();
	TArray<FTetherSmartObjectEntranceResult> Result;
	USmartObjectDefinition* Definition = LoadDefinition(AssetPath);
	UWorld* World = GetCurrentWorld();
