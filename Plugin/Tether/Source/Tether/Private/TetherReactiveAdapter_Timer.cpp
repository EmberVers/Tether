...
DEFINE_LOG_CATEGORY_STATIC(LogTetherReactiveTimer, Log, All);
 ...
 * UTetherReactiveSubsystem::DispatchOne (bypasses Subject/Selector filter).
 *
 ...
 */
class FTetherTimerAdapter : public ITetherReactiveAdapter
{
public:
	...
	virtual void OnHandlerAdded(const FTetherHandlerRecord& Record) override
	{
		...
		if (Interval <= 0.0)
		{
			UE_LOG(LogTetherReactiveTimer, Warning,
...
namespace TetherReactiveAdapters
{
	...
	{
		return MakeUnique<FTetherTimerAdapter>();
	}
}
