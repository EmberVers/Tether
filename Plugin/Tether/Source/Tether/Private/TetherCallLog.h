...
/** Single captured tether TCP request (internal — not a USTRUCT). */
struct FTetherCallRecord
{
	FString RequestId;
...
/**
 * Thread-safe ring buffer for tether TCP call records. Populated from
 ...
 */
class FTetherCallLog
{
public:
	static FTetherCallLog& Get();

	void Append(FTetherCallRecord&& Record);
	...
	/**
	 * Resize the ring. Clamped to [10, 5000] by the caller (SetTetherCallLogCapacity).
	 ...
	 */
	TArray<FTetherCallRecord> Snapshot(int32 MaxEntries) const;

private:
	FTetherCallLog() = default;

	mutable FCriticalSection Lock;
	TArray<FTetherCallRecord> Entries;
	int32 Capacity = 500;
