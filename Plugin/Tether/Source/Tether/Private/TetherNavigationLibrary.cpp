#include "TetherNavigationLibrary.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogTetherNav, Log, All);

namespace TetherNavImpl
{
...
bool UTetherNavigationLibrary::ExportNavMeshToObj(const FString& OutFilePath, FString& OutSummary)
{
	UWorld* World = TetherNavImpl::GetEditorWorld();
	if (!World)
	{
		OutSummary = TEXT("no editor world");
		UE_LOG(LogTetherNav, Warning, TEXT("ExportNavMeshToObj: %s"), *OutSummary);
		return false;
	}

	ARecastNavMesh* NavMesh = TetherNavImpl::FindRecastNavMesh(World);
	if (!NavMesh)
	{
		OutSummary = TEXT("no ARecastNavMesh in current level");
		UE_LOG(LogTetherNav, Warning, TEXT("ExportNavMeshToObj: %s (world=%s)"),
			*OutSummary, *World->GetName());
		return false;
	}
	UE_LOG(LogTetherNav, Log, TEXT("ExportNavMeshToObj: found navmesh=%s tiles=%d"),
	...
	ObjText += FString::Printf(TEXT("# Tether NavMesh export\n"));
	...
	UE_LOG(LogTetherNav, Log, TEXT("ExportNavMeshToObj: %s"), *OutSummary);
	return true;
}
