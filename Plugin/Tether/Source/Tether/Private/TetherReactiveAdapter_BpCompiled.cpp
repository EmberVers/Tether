...
DEFINE_LOG_CATEGORY_STATIC(LogTetherReactiveBp, Log, All);
 ...
 * (ctx['blueprint_path']).
 */
class FTetherBpCompiledAdapter : public ITetherReactiveAdapter
{
public:
	...
	virtual void OnHandlerAdded(const FTetherHandlerRecord& Record) override
	{
		...
		{
			UE_LOG(LogTetherReactiveBp, Warning,
...
private:
	...
	void FirePerSubject(UBlueprint* BP)
	{
		...
		using namespace TetherReactiveAdapterImpl_Bp;
...
namespace TetherReactiveAdapters
{
	...
	{
		return MakeUnique<FTetherBpCompiledAdapter>();
	}
}
