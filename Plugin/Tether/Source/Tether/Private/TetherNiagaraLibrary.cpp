...
#define LOCTEXT_NAMESPACE "TetherNiagaraLibrary"

namespace TetherNiagaraImpl
{
...
{
	LastError = Error;
	UE_LOG(LogTemp, Warning, TEXT("Tether|Niagara: %s"), *Error);
	return false;
}
...
static FTetherNiagaraPreviewInfo BuildPreviewInfo(const FString& Handle, UNiagaraComponent* Component)
{
	FTetherNiagaraPreviewInfo Info;
	Info.Handle = Handle;
	...
	if (Instance)
	{
		for (const FNiagaraEmitterInstanceRef& EmitterRef : Instance->GetEmitters())
		{
			...
			if (!Emitter) continue;
			FTetherNiagaraEmitterRuntimeInfo Runtime;
...
static void AddAuditIssue(FTetherNiagaraAuditResult& Result, const FString& Severity,
	const FString& Message, const FString& EmitterId = FString())
{
	FTetherNiagaraCompileMessage Issue;
...
{
	return TetherNiagaraImpl::LastError;
}

TArray<FTetherNiagaraTemplateInfo> UTetherNiagaraLibrary::ListNiagaraTemplates(
	const FString& AssetType, const FString& Query, int32 MaxResults)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraTemplateInfo> Result;
	...
	for (const FAssetData& Data : Assets)
	{
		...
		FTetherNiagaraTemplateInfo Info;
		Info.AssetPath = Path;
	...
	Result.Sort([](const FTetherNiagaraTemplateInfo& A, const FTetherNiagaraTemplateInfo& B)
	{
...
static FTetherNiagaraScriptInfo BuildNiagaraScriptInfo(UNiagaraScript* Script)
{
	FTetherNiagaraScriptInfo Info;
	if (!Script) return Info;
	Info.AssetPath = Script->GetPathName();
	Info.Name = Script->GetName();
	Info.Usage = TetherNiagaraImpl::UsageToString(Script->GetUsage());
...
TArray<FTetherNiagaraScriptInfo> UTetherNiagaraLibrary::ListNiagaraScripts(
	const FString& Usage, const FString& Query, int32 MaxResults)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraScriptInfo> Result;
	...
	for (const FAssetData& Data : Assets)
	{
		...
		if (!Script) continue;
		FTetherNiagaraScriptInfo Info = BuildNiagaraScriptInfo(Script);
	...
	Result.Sort([](const FTetherNiagaraScriptInfo& A, const FTetherNiagaraScriptInfo& B)
	{
		return A.AssetPath < B.AssetPath;
	});
	return Result;
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::CreateNiagaraSystem(
	const FString& AssetPath, const FString& TemplateSystemPath, bool bSave)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	FTetherNiagaraOperationResult Result;
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::CreateNiagaraEmitter(
...
TArray<FTetherNiagaraEmitterInfo> UTetherNiagaraLibrary::ListNiagaraEmitters(const FString& SystemPath)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraEmitterInfo> Result;
	UNiagaraSystem* System = LoadNiagaraAsset<UNiagaraSystem>(SystemPath, TEXT("Niagara System"));
	if (!System) return Result;
	for (FNiagaraEmitterHandle& Handle : System->GetEmitterHandles())
	{
		FTetherNiagaraEmitterInfo Info;
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::AddNiagaraEmitter(
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::DuplicateNiagaraEmitter(
...
bool UTetherNiagaraLibrary::RemoveNiagaraEmitter(
...
bool UTetherNiagaraLibrary::RenameNiagaraEmitter(
	const FString& SystemPath, const FString& EmitterIdOrName, const FString& NewName, bool bSave)
{
	using namespace TetherNiagaraImpl;
	ClearError();
...
bool UTetherNiagaraLibrary::SetNiagaraEmitterEnabled(
...
bool UTetherNiagaraLibrary::SetNiagaraEmitterProperties(const FString& SystemPath,
...
static FTetherNiagaraModuleInfo BuildModuleInfo(const TetherNiagaraImpl::FModuleContext& Context)
{
	using namespace TetherNiagaraImpl;
	FTetherNiagaraModuleInfo Info;
...
TArray<FTetherNiagaraModuleInfo> UTetherNiagaraLibrary::ListNiagaraModules(
	const FString& SystemPath, const FString& EmitterIdOrName, const FString& Usage)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraModuleInfo> Result;
	UNiagaraSystem* System = LoadNiagaraAsset<UNiagaraSystem>(SystemPath, TEXT("Niagara System"));
	if (!System) return Result;
	const FString UsageToken = NormalizeToken(Usage);
...
static UObject* GetLinkedInputObject(const TetherNiagaraImpl::FModuleContext& Context,
...
TArray<FTetherNiagaraModuleInputInfo> UTetherNiagaraLibrary::ListNiagaraModuleInputs(
	const FString& SystemPath, const FString& ModuleId, bool bIncludeHidden)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraModuleInputInfo> Result;
	UNiagaraSystem* System = LoadNiagaraAsset<UNiagaraSystem>(SystemPath, TEXT("Niagara System"));
	FModuleContext Context;
	...
	for (const FNiagaraVariable& Input : Inputs)
	{
		...
		FTetherNiagaraModuleInputInfo Info;
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::AddNiagaraModule(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::RemoveNiagaraModule(
...
bool UTetherNiagaraLibrary::SetNiagaraModuleEnabled(
...
bool UTetherNiagaraLibrary::SetNiagaraModuleInput(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::LinkNiagaraModuleInput(const FString& SystemPath,
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::SetNiagaraModuleDynamicInput(
...
bool UTetherNiagaraLibrary::SetNiagaraModuleObjectInput(const FString& SystemPath,
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::SetNiagaraModuleDataInterfaceInput(
	const FString& SystemPath, const FString& ModuleId, const FString& InputName,
	const FString& DataInterfaceClassPath, const TArray<FTetherNiagaraPropertyValue>& Properties, bool bSave)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	FTetherNiagaraOperationResult Result;
	...
	for (const FTetherNiagaraPropertyValue& Entry : Properties)
	{
...
TArray<FTetherNiagaraPropertyInfo> UTetherNiagaraLibrary::ListNiagaraModuleInputObjectProperties(
	const FString& SystemPath, const FString& ModuleId, const FString& InputName, bool bIncludeAdvanced)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraPropertyInfo> Result;
	UNiagaraSystem* System = LoadNiagaraAsset<UNiagaraSystem>(SystemPath, TEXT("Niagara System"));
	FModuleContext Context;
	FNiagaraVariable Input;
	FGuid VariableId;
	...
	for (TFieldIterator<FProperty> It(Object->GetClass(), EFieldIterationFlags::IncludeSuper); It; ++It)
	{
		...
		FTetherNiagaraPropertyInfo Info;
...
bool UTetherNiagaraLibrary::SetNiagaraModuleInputObjectProperty(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::ResetNiagaraModuleInput(const FString& SystemPath,
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::AddNiagaraParameterAssignment(
	const FString& SystemPath, const FString& EmitterIdOrName, const FString& Usage,
...
TArray<FTetherNiagaraParameterInfo> UTetherNiagaraLibrary::ListNiagaraUserParameters(const FString& SystemPath)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraParameterInfo> Result;
	UNiagaraSystem* System = LoadNiagaraAsset<UNiagaraSystem>(SystemPath, TEXT("Niagara System"));
	if (!System) return Result;
	FNiagaraUserRedirectionParameterStore& Store = System->GetExposedParameters();
	TArray<FNiagaraVariable> Parameters;
	...
	{
		FTetherNiagaraParameterInfo Info;
	...
	Result.Sort([](const FTetherNiagaraParameterInfo& A, const FTetherNiagaraParameterInfo& B)
	{
...
bool UTetherNiagaraLibrary::AddNiagaraUserParameter(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::SetNiagaraUserParameterDefault(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::RenameNiagaraUserParameter(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::RemoveNiagaraUserParameter(
...
TArray<FTetherNiagaraRendererInfo> UTetherNiagaraLibrary::ListNiagaraRenderers(
	const FString& SystemPath, const FString& EmitterIdOrName)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraRendererInfo> Result;
	UNiagaraSystem* System = LoadNiagaraAsset<UNiagaraSystem>(SystemPath, TEXT("Niagara System"));
	if (!System) return Result;
	...
	for (FNiagaraEmitterHandle* Handle : Handles)
	{
		...
		for (UNiagaraRendererProperties* Renderer : Data->GetRenderers())
		{
			...
			FTetherNiagaraRendererInfo Info;
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::AddNiagaraRenderer(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::RemoveNiagaraRenderer(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::SetNiagaraRendererEnabled(const FString& SystemPath,
...
TArray<FTetherNiagaraPropertyInfo> UTetherNiagaraLibrary::ListNiagaraRendererProperties(
...
FString UTetherNiagaraLibrary::GetNiagaraRendererProperty(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::SetNiagaraRendererProperty(const FString& SystemPath,
	const FString& EmitterIdOrName, const FString& RendererId, const FString& PropertyName,
	const FString& Value, bool bSave)
{
	using namespace TetherNiagaraImpl;
...
bool UTetherNiagaraLibrary::SetNiagaraRendererMaterial(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::SetNiagaraRendererBinding(const FString& SystemPath,
...
bool UTetherNiagaraLibrary::SetNiagaraSystemWarmup(
...
bool UTetherNiagaraLibrary::SetNiagaraSystemFixedBounds(
...
bool UTetherNiagaraLibrary::SetNiagaraSystemEffectType(
...
static void AppendScriptDiagnostics(FTetherNiagaraCompileResult& Result, UNiagaraScript* Script,
	const FString& EmitterId)
{
	...
	{
		FTetherNiagaraCompileMessage Message;
		...
		Message.ScriptUsage = TetherNiagaraImpl::UsageToString(Script->GetUsage());
...
static FTetherNiagaraCompileResult BuildCompileDiagnostics(UNiagaraSystem* System)
{
	FTetherNiagaraCompileResult Result;
	if (!System)
	{
		Result.Error = TetherNiagaraImpl::LastError;
		return Result;
	}
...
FTetherNiagaraCompileResult UTetherNiagaraLibrary::CompileNiagaraSystem(
	const FString& SystemPath, bool bForce, bool bWaitForGpuShaders, bool bSave)
{
	...
	FTetherNiagaraCompileResult Result = BuildCompileDiagnostics(System);
	if (bSave && !SaveAsset(System))
	{
...
FTetherNiagaraAuditResult UTetherNiagaraLibrary::ValidateNiagaraSystem(const FString& SystemPath,
	bool bCheckMaterials, bool bCheckBounds, int32 MaxEmitters, int32 MaxRenderersPerEmitter,
	int32 MaxModulesPerEmitter)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	FTetherNiagaraAuditResult Result;
	UNiagaraSystem* System = LoadNiagaraAsset<UNiagaraSystem>(SystemPath, TEXT("Niagara System"));
	if (!System)
	{
	...
	const FTetherNiagaraCompileResult Compile = BuildCompileDiagnostics(System);
	for (const FTetherNiagaraCompileMessage& Message : Compile.Messages)
	{
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::CreateNiagaraSystemFromRecipe(
	const FString& AssetPath, const FTetherNiagaraSystemRecipe& Recipe, bool bCompile, bool bSave)
{
	using namespace TetherNiagaraImpl;
	ClearError();
	FTetherNiagaraOperationResult Result = CreateNiagaraSystem(AssetPath, Recipe.TemplatePath, false);
	if (!Result.bSuccess) return Result;
	const FString SystemPath = Result.AssetPath;
	auto Fail = [&](const FString& Step) -> FTetherNiagaraOperationResult
	{
		Result.bSuccess = false;
	...
	for (const FTetherNiagaraParameterSpec& Parameter : Recipe.UserParameters)
	{
	...
	for (const FTetherNiagaraEmitterSpec& EmitterSpec : Recipe.Emitters)
	{
		FTetherNiagaraOperationResult AddedEmitter = AddNiagaraEmitter(
		...
		for (const FTetherNiagaraModuleSpec& ModuleSpec : EmitterSpec.Modules)
		{
			FTetherNiagaraOperationResult AddedModule = AddNiagaraModule(SystemPath, EmitterId,
			...
			for (const FTetherNiagaraInputValue& Input : ModuleSpec.Inputs)
			{
				...
				{
					FTetherNiagaraOperationResult Dynamic = SetNiagaraModuleDynamicInput(SystemPath,
				...
				{
					FTetherNiagaraOperationResult DataInterface = SetNiagaraModuleDataInterfaceInput(SystemPath,
		...
		for (const FTetherNiagaraRendererSpec& RendererSpec : EmitterSpec.Renderers)
		{
			FTetherNiagaraOperationResult AddedRenderer = AddNiagaraRenderer(SystemPath, EmitterId,
			...
			for (const FTetherNiagaraPropertyValue& Entry : RendererSpec.Properties)
			...
			for (const FTetherNiagaraPropertyValue& Entry : RendererSpec.Bindings)
	...
	if (bCompile)
	{
		FTetherNiagaraCompileResult Compile = CompileNiagaraSystem(SystemPath, true, true, bSave);
		if (!Compile.bSuccess)
		{
			Result.bSuccess = false;
...
{
	FNiagaraVariable Existing;
	UNiagaraSystem* System = LoadObject<UNiagaraSystem>(nullptr, *TetherNiagaraImpl::NormalizeObjectPath(SystemPath));
	if (FindUserParameter(System, Name, Existing))
	{
		if (Existing.GetType() != TetherNiagaraImpl::ResolveType(Type))
		{
			Warnings.Add(FString::Printf(TEXT("existing user parameter '%s' has incompatible type"), *Name));
			return false;
		}
		return UTetherNiagaraLibrary::SetNiagaraUserParameterDefault(SystemPath, Name, Value, false);
	}
	return UTetherNiagaraLibrary::AddNiagaraUserParameter(SystemPath, Name, Type, Value, false);
}
...
{
	int32 BoundCount = 0;
	const FNiagaraTypeDefinition RequiredTypeDefinition = TetherNiagaraImpl::ResolveType(RequiredType);
	const TArray<FTetherNiagaraModuleInfo> Modules = UTetherNiagaraLibrary::ListNiagaraModules(SystemPath, FString(), TEXT("All"));
	for (const FTetherNiagaraModuleInfo& Module : Modules)
	{
		bool bModuleMatches = ModuleTerms.IsEmpty();
		const FString ModuleToken = TetherNiagaraImpl::NormalizeToken(Module.Name);
		for (const FString& Term : ModuleTerms)
			if (ModuleToken.Contains(TetherNiagaraImpl::NormalizeToken(Term))) { bModuleMatches = true; break; }
		if (!bModuleMatches) continue;
		const TArray<FTetherNiagaraModuleInputInfo> Inputs = UTetherNiagaraLibrary::ListNiagaraModuleInputs(
			SystemPath, Module.Id, true);
		const FTetherNiagaraModuleInputInfo* BestInput = nullptr;
		int32 BestScore = MIN_int32;
		for (const FTetherNiagaraModuleInputInfo& Input : Inputs)
		{
			const FNiagaraTypeDefinition ActualTypeDefinition = TetherNiagaraImpl::ResolveType(Input.Type);
			if (RequiredTypeDefinition.IsValid()
				&& (!ActualTypeDefinition.IsValid() || ActualTypeDefinition != RequiredTypeDefinition)) continue;
			const FString InputToken = TetherNiagaraImpl::NormalizeToken(Input.Name);
			int32 Score = MIN_int32;
			for (int32 TermIndex = 0; TermIndex < InputTerms.Num(); ++TermIndex)
			{
				const FString TermToken = TetherNiagaraImpl::NormalizeToken(InputTerms[TermIndex]);
		...
		if (BestInput && UTetherNiagaraLibrary::LinkNiagaraModuleInput(SystemPath, Module.Id, BestInput->Name,
...
static void ApplyPresetMaterial(const FString& SystemPath, const FString& MaterialPath, TArray<FString>& Warnings)
{
	...
	for (const FTetherNiagaraRendererInfo& Renderer : UTetherNiagaraLibrary::ListNiagaraRenderers(SystemPath, FString()))
	{
		if (Renderer.Type == TEXT("Sprite") || Renderer.Type == TEXT("Ribbon")
			|| Renderer.Type == TEXT("Mesh") || Renderer.Type == TEXT("Decal"))
		{
			if (!UTetherNiagaraLibrary::SetNiagaraRendererMaterial(SystemPath, Renderer.EmitterId,
...
static FTetherNiagaraOperationResult FinalizePreset(FTetherNiagaraOperationResult Result, bool bSave)
{
	if (!Result.bSuccess) return Result;
	FTetherNiagaraCompileResult Compile = UTetherNiagaraLibrary::CompileNiagaraSystem(
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::CreateWeaponTrailEffect(const FString& AssetPath,
	...
	float Lifetime, float SpawnRate, bool bLocalSpace, bool bSave)
{
	...
	{
		FTetherNiagaraOperationResult Invalid;
		Invalid.Message = FString::Printf(TEXT("unknown weapon-trail style '%s'; use Ribbon, Beam, or StaticBeam"), *Style);
		SetError(Invalid.Message);
		return Invalid;
	}
	FTetherNiagaraOperationResult Result = CreateNiagaraSystem(AssetPath, FString(), false);
	if (!Result.bSuccess) return Result;
	FTetherNiagaraOperationResult Emitter = AddNiagaraEmitter(Result.AssetPath, TEXT("WeaponTrail"), TemplatePath, false);
	...
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherColor"), TEXT("Color"), ColorValue(Color), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherWidth"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.01f, Width)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherLifetime"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.01f, Lifetime)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherSpawnRate"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.0f, SpawnRate)), Result.Warnings);
	if (bRibbonStyle)
	{
		for (const FTetherNiagaraModuleInfo& Module : ListNiagaraModules(Result.AssetPath, Emitter.Id, TEXT("All")))
		{
		...
		for (const FTetherNiagaraRendererInfo& Renderer : ListNiagaraRenderers(Result.AssetPath, Emitter.Id))
		...
		FTetherNiagaraOperationResult RibbonRenderer = AddNiagaraRenderer(Result.AssetPath, Emitter.Id,
			TEXT("Ribbon"), TEXT("TetherWeaponTrailRibbon"), RibbonMaterialPath, FString(), false);
		...
		BindPresetParameter(Result.AssetPath, TEXT("TetherColor"),
			{TEXT("Initialize")}, {TEXT("Color")}, TEXT("Color"), Result.Warnings);
		BindPresetParameter(Result.AssetPath, TEXT("TetherWidth"),
			{TEXT("Initialize")}, {TEXT("Ribbon Width"), TEXT("Width")}, TEXT("Float"), Result.Warnings);
		BindPresetParameter(Result.AssetPath, TEXT("TetherLifetime"),
			{TEXT("Initialize")}, {TEXT("Lifetime")},
			TEXT("Float"), Result.Warnings);
		BindPresetParameter(Result.AssetPath, TEXT("TetherSpawnRate"),
	...
	else
	{
		...
		Result.Warnings.Add(TEXT("beam styles spawn one beam per activation; TetherSpawnRate remains exposed for optional custom spawning"));
	}
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::CreateSparkEffect(const FString& AssetPath,
	...
	float Speed, float Lifetime, float Gravity, bool bCollision, bool bSave)
{
	...
	FTetherNiagaraOperationResult Result = CreateNiagaraSystem(AssetPath, TemplatePath, false);
	if (!Result.bSuccess) return Result;
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherColor"), TEXT("Color"), ColorValue(Color), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherCount"), TEXT("Int"), FString::FromInt(FMath::Max(1, Count)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherSpeed"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.0f, Speed)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherLifetime"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.01f, Lifetime)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherGravity"), TEXT("Vec3"), VectorValue(FVector(0.0, 0.0, Gravity)), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherColor"), {TEXT("Initialize"), TEXT("Color")}, {TEXT("Color")}, TEXT("Color"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherCount"), {TEXT("SpawnBurst")}, {TEXT("SpawnCount"), TEXT("Count")}, TEXT("Int"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherSpeed"), {TEXT("AddVelocity")},
		{TEXT("Velocity Strength"), TEXT("Velocity Speed"), TEXT("Speed")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherLifetime"), {TEXT("Initialize")}, {TEXT("Lifetime")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherGravity"), {TEXT("Gravity")}, {TEXT("Gravity"), TEXT("Acceleration")}, TEXT("Vec3"), Result.Warnings);
	if (bCollision)
	{
		bool bHasCollision = false;
		TArray<FTetherNiagaraModuleInfo> Modules = ListNiagaraModules(Result.AssetPath, FString(), TEXT("All"));
		for (const FTetherNiagaraModuleInfo& Module : Modules)
			if (NormalizeToken(Module.Name).Contains(TEXT("collision"))) { bHasCollision = true; break; }
		if (!bHasCollision)
		{
			const TArray<FTetherNiagaraEmitterInfo> Emitters = ListNiagaraEmitters(Result.AssetPath);
			if (!Emitters.IsEmpty())
			{
				FTetherNiagaraOperationResult Collision = AddNiagaraModule(Result.AssetPath, Emitters[0].Id,
					TEXT("ParticleUpdate"), TEXT("/Niagara/Modules/Collision/Collision.Collision"),
					TEXT("TetherCollision"), -1, true, false);
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::CreateExplosionEffect(const FString& AssetPath,
	...
	float Duration, int32 DebrisCount, bool bShockwave, bool bLight, bool bSave)
{
	...
	FTetherNiagaraOperationResult Result = CreateNiagaraSystem(AssetPath,
		TEXT("/Niagara/DefaultAssets/Templates/Systems/SimpleExplosion.SimpleExplosion"), false);
	if (!Result.bSuccess) return Result;
	TArray<FTetherNiagaraEmitterInfo> Emitters = ListNiagaraEmitters(Result.AssetPath);
	bool bFoundShockwave = false;
	for (const FTetherNiagaraEmitterInfo& Emitter : Emitters)
	{
	...
	{
		FTetherNiagaraOperationResult Shockwave = AddNiagaraEmitter(Result.AssetPath, TEXT("ShockwaveRibbon"),
	...
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherCoreColor"), TEXT("Color"), ColorValue(CoreColor), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherScale"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.01f, Scale)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherShockwaveRadius"), TEXT("Float"),
		FString::SanitizeFloat(250.0f * FMath::Max(0.01f, Scale)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherDuration"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.05f, Duration)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherDebrisCount"), TEXT("Int"), FString::FromInt(FMath::Max(0, DebrisCount)), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherCoreColor"), {TEXT("Initialize"), TEXT("Color")}, {TEXT("Color")}, TEXT("Color"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherScale"), {TEXT("Initialize")},
		{TEXT("Mesh Uniform Scale"), TEXT("Scale")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherShockwaveRadius"), {TEXT("ShapeLocation")},
		{TEXT("Sphere Radius"), TEXT("Ring Radius"), TEXT("Radius")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherDuration"), {TEXT("Initialize")}, {TEXT("Lifetime"), TEXT("Duration")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherDebrisCount"), {TEXT("SpawnBurst")}, {TEXT("SpawnCount"), TEXT("Count")}, TEXT("Int"), Result.Warnings);
	...
	{
		FTetherNiagaraOperationResult Light = AddNiagaraRenderer(Result.AssetPath, Emitters[0].Id,
			TEXT("Light"), TEXT("TetherExplosionLight"), FString(), FString(), false);
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::CreateDissolveEffect(const FString& AssetPath,
	const FString& Style, const FString& MaterialPath, const FLinearColor& Color, int32 Count,
	float Duration, float Radius, const FVector& Direction, bool bSave)
{
	...
	FTetherNiagaraOperationResult Result = CreateNiagaraSystem(AssetPath, FString(), false);
	if (!Result.bSuccess) return Result;
	FTetherNiagaraOperationResult Emitter = AddNiagaraEmitter(Result.AssetPath, TEXT("DissolveParticles"),
	...
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherColor"), TEXT("Color"), ColorValue(Color), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherCount"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(1, Count)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherDuration"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.05f, Duration)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherRadius"), TEXT("Float"), FString::SanitizeFloat(FMath::Max(0.0f, Radius)), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("TetherDirection"), TEXT("Vec3"), VectorValue(Direction.GetSafeNormal()), Result.Warnings);
	EnsurePresetParameter(Result.AssetPath, TEXT("DissolveAmount"), TEXT("Float"), TEXT("0.0"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherColor"), {TEXT("Initialize"), TEXT("Color")}, {TEXT("Color")}, TEXT("Color"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherCount"), {TEXT("SpawnRate")}, {TEXT("SpawnRate")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherDuration"), {TEXT("Initialize")}, {TEXT("Lifetime"), TEXT("Duration")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherRadius"), {TEXT("ShapeLocation"), TEXT("Sphere")}, {TEXT("Sphere Radius"), TEXT("Radius")}, TEXT("Float"), Result.Warnings);
	BindPresetParameter(Result.AssetPath, TEXT("TetherDirection"), {TEXT("Wind")},
...
FTetherNiagaraOperationResult UTetherNiagaraLibrary::SpawnNiagaraPreview(
	const FString& SystemPath, const FTransform& Transform, bool bAutoActivate, bool bResetOnChange)
{
	...
	{
		const FTetherNiagaraCompileResult Compile = CompileNiagaraSystem(SystemPath, true, true, false);
		if (!Compile.bSuccess)
		{
...
TArray<FTetherNiagaraPreviewInfo> UTetherNiagaraLibrary::ListNiagaraPreviews()
{
	using namespace TetherNiagaraImpl;
	ClearError();
	TArray<FTetherNiagaraPreviewInfo> Result;
	for (auto It = PreviewComponents.CreateIterator(); It; ++It)
	{
		if (!It.Value().IsValid())
		{
...
bool UTetherNiagaraLibrary::AdvanceNiagaraPreview(
...
bool UTetherNiagaraLibrary::SetNiagaraPreviewTransform(
...
bool UTetherNiagaraLibrary::SetNiagaraPreviewVariable(const FString& Handle,
