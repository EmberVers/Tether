#include "TetherStructLibrary.h"

#include "Misc/EngineVersionComparison.h"
#include "TetherTypeParse.h"
 ...
 *  to a raw pointer tethers the two safely (resolves the soft ptr if loaded). */
...
FTetherStructCreateResult UTetherStructLibrary::CreateUserDefinedStruct(const FString& AssetPath)
{
	using namespace TetherStructImpl;

	FTetherStructCreateResult Result;
...
bool UTetherStructLibrary::AddStructVariable(const FString& StructPath,
...
bool UTetherStructLibrary::RemoveStructVariable(const FString& StructPath, const FString& Name)
{
	using namespace TetherStructImpl;
...
bool UTetherStructLibrary::RenameStructVariable(const FString& StructPath,
...
bool UTetherStructLibrary::ChangeStructVariableType(const FString& StructPath,
...
bool UTetherStructLibrary::SetStructVariableDefault(const FString& StructPath,
...
bool UTetherStructLibrary::MoveStructVariable(const FString& StructPath,
...
TArray<FTetherStructVariableInfo>
UTetherStructLibrary::GetStructVariables(const FString& StructPath)
{
	using namespace TetherStructImpl;

	TArray<FTetherStructVariableInfo> Out;
	UUserDefinedStruct* S = LoadStruct(StructPath);
	if (!S) return Out;
	...
	for (const FStructVariableDescription& V : *Descs)
	{
		FTetherStructVariableInfo Info;
		Info.Name = V.FriendlyName.IsEmpty() ? V.VarName.ToString() : V.FriendlyName;
		Info.TypeString = TetherTypeParseImpl::PinTypeToString(DescToPinType(V));
...
FTetherStructInfo UTetherStructLibrary::GetStructInfo(const FString& StructPath)
{
	using namespace TetherStructImpl;

	FTetherStructInfo Info;
...
bool UTetherStructLibrary::SetStructVariableTooltip(const FString& StructPath,
...
bool UTetherStructLibrary::SetStructVariableEditOnInstance(const FString& StructPath,
...
bool UTetherStructLibrary::SetStructTooltip(const FString& StructPath, const FString& Tooltip)
{
	using namespace TetherStructImpl;
