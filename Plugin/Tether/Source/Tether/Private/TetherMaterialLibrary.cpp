...
#include "TetherTextureRefreshOperations.h"
...
namespace TetherMaterialImpl
{
	...
	static void AppendStaticSwitchOverrides(
		...
		TArray<FTetherMaterialParam>& OutParameters)
	{
	 ...
	 */
	class FTextureRefreshOperations final : public ITetherTextureRefreshOperations
	{
	public:
	...
	static TObjectType* LoadTetherObject(const FString& ObjectPath)
	{
	...
	static FTetherMaterialLayerEntry MakeLayerEntry(const FMaterialLayersFunctions& Stack, int32 Index)
	{
		FTetherMaterialLayerEntry Entry;
		Entry.Index = Index;
...
FTetherMaterialInstanceInfo UTetherMaterialLibrary::GetMaterialInstanceParameters(
	const FString& MaterialPath)
{
	using namespace TetherMaterialImpl;

	FTetherMaterialInstanceInfo Result;

	UMaterialInstance* MI = LoadObject<UMaterialInstance>(nullptr, *MaterialPath);
	if (!MI)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load MaterialInstance '%s'"), *MaterialPath);
		return Result;
	}
...
FTetherMaterialLayerStack UTetherMaterialLibrary::GetMaterialInstanceLayerStack(
	const FString& MaterialInstancePath)
{
	using namespace TetherMaterialImpl;

	FTetherMaterialLayerStack Out;
	UMaterialInstanceConstant* MIC = LoadTetherObject<UMaterialInstanceConstant>(MaterialInstancePath);
	if (!MIC)
	{
...
FTetherMaterialLayerStackOpResult UTetherMaterialLibrary::SetMaterialInstanceLayerStack(
	const FString& MaterialInstancePath,
	const TArray<FTetherMaterialLayerEntry>& Layers)
{
	using namespace TetherMaterialImpl;

	FTetherMaterialLayerStackOpResult Out;
	UMaterialInstanceConstant* MIC = LoadTetherObject<UMaterialInstanceConstant>(MaterialInstancePath);
	if (!MIC)
	{
		Out.Error = FString::Printf(
	...
	{
		const FTetherMaterialLayerEntry& Entry = Layers[Index];
		...
		{
			LayerFunction = LoadTetherObject<UMaterialFunctionInterface>(Entry.LayerAssetPath);
			if (!LayerFunction
		...
		if (!Entry.BlendAssetPath.IsEmpty() && Entry.BlendAssetPath != TEXT("None"))
		{
			...
			BlendFunction = LoadTetherObject<UMaterialFunctionInterface>(Entry.BlendAssetPath);
			if (!BlendFunction
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"Tether", "SetMaterialInstanceLayerStack",
...
FTetherMaterialLayerStackOpResult UTetherMaterialLibrary::CopyMaterialInstanceLayerStack(
	...
	const FString& DestinationMaterialInstancePath)
{
	...
	FTetherMaterialLayerStackOpResult Out;
	UMaterialInstanceConstant* Source = LoadTetherObject<UMaterialInstanceConstant>(SourceMaterialInstancePath);
	UMaterialInstanceConstant* Destination = LoadTetherObject<UMaterialInstanceConstant>(DestinationMaterialInstancePath);
	...
	const FTetherMaterialLayerStack Snapshot = GetMaterialInstanceLayerStack(SourceMaterialInstancePath);
...
FTetherMaterialInfo UTetherMaterialLibrary::GetMaterialInfo(const FString& MaterialPath)
{
	...
	FTetherMaterialInfo Info;

	UMaterialInterface* MatInterface = LoadObject<UMaterialInterface>(nullptr, *MaterialPath);
	if (!MatInterface)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetMaterialInfo could not load '%s'"), *MaterialPath);
		return Info;
	}
...
TArray<FTetherMaterialFunctionSummary> UTetherMaterialLibrary::ListMaterialFunctions(
	const FString& PathPrefix,
	int32 MaxResults)
{
	TArray<FTetherMaterialFunctionSummary> Result;
	...
	for (const FAssetData& AssetData : Assets)
	{
		...
		FTetherMaterialFunctionSummary Summary;
...
FTetherMaterialFunctionInfo UTetherMaterialLibrary::GetMaterialFunction(const FString& FunctionPath)
{
	...
	FTetherMaterialFunctionInfo Info;

	UMaterialFunction* MF = LoadObject<UMaterialFunction>(nullptr, *FunctionPath);
	if (!MF)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetMaterialFunction could not load '%s'"), *FunctionPath);
		return Info;
	}
	...
	for (const TObjectPtr<UMaterialExpression>& Expr : Expressions)
	{
		...
		{
			FTetherMaterialFunctionPort Port;
	...
	Info.Inputs.Sort([](const FTetherMaterialFunctionPort& A, const FTetherMaterialFunctionPort& B)
	{
		return A.SortPriority < B.SortPriority;
	});
	Info.Outputs.Sort([](const FTetherMaterialFunctionPort& A, const FTetherMaterialFunctionPort& B)
	{
		return A.SortPriority < B.SortPriority;
	});
...
namespace TetherMaterialImpl
{
	static void AppendMILayerOverrides(const UMaterialInstance* MI, FTetherMaterialInstanceLayer& Layer)
	{
		for (const FScalarParameterValue& P : MI->ScalarParameterValues)
		{
			FTetherMaterialParam Param;
...
FTetherMaterialInstanceChain UTetherMaterialLibrary::ListMaterialInstanceChain(const FString& MaterialPath)
{
	...
	FTetherMaterialInstanceChain Chain;

	UMaterialInterface* Current = LoadObject<UMaterialInterface>(nullptr, *MaterialPath);
	if (!Current)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: ListMaterialInstanceChain could not load '%s'"), *MaterialPath);
		return Chain;
	}
	...
	{
		FTetherMaterialInstanceLayer Layer;
...
namespace TetherMaterialImpl
{
	...
	static FTetherMaterialGraphNode BuildGraphNode(UMaterialExpression* Expr)
	{
		FTetherMaterialGraphNode Node;
	...
	static void AddOpaqueDependency(FTetherMaterialGraph& Graph, const FString& Diagnostic)
	{
		Graph.OpaqueDependencies.AddUnique(Diagnostic);
	}

	static void MarkCompositeUnexpanded(
		FTetherMaterialGraph& Graph,
		...
		FTetherMaterialGraph& Graph)
	{
		...
		for (const TObjectPtr<UMaterialExpression>& ExprPtr : Expressions)
		{
			...
			for (FExpressionInputIterator It{Expr}; It; ++It)
			{
				...
				FTetherMaterialGraphConnection Conn;
				...
				Cast<UMaterialExpressionNamedRerouteUsage>(Expr))
			{
				...
				{
					FTetherMaterialGraphConnection Logical;
			...
			if (UMaterialExpressionComposite* Composite = Cast<UMaterialExpressionComposite>(Expr))
			{
				auto CollectGateway = [&](UMaterialExpressionPinBase* PinBase, bool bInput)
				{
					...
					for (int32 PinIndex = 0; PinIndex < PinBase->ReroutePins.Num(); ++PinIndex)
					{
						...
						FTetherMaterialGraphConnection Mapping;
		...
		TArray<FString>* OutDiagnostics = nullptr)
	{
		...
		FTetherMaterialGraph DependencyGraph;
...
{
	FTetherMaterialParameterCollectionInfo Info;

	UMaterialParameterCollection* MPC = LoadObject<UMaterialParameterCollection>(nullptr, *CollectionPath);
	if (!MPC)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetMaterialParameterCollection could not load '%s'"), *CollectionPath);
		return Info;
	}
...
namespace TetherMaterialImpl
{
	static ERHIFeatureLevel::Type ParseFeatureLevel(const FString& Str)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Tether: unknown FeatureLevel '%s' — falling back to GMaxRHIFeatureLevel"), *Str);
		return GMaxRHIFeatureLevel;
	}
	...
	static EMaterialQualityLevel::Type ParseQuality(const FString& Str)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Tether: unknown Quality '%s' — falling back to High"), *Str);
		return EMaterialQualityLevel::High;
	}
	...
	static void CollectShaderInstructionStats(
		...
		TArray<FTetherMaterialShaderStat>& OutStats)
	{
		if (!Resource)
		{
			return;
		...
		if (Resource->IsUIMaterial())
		{
			for (const TPair<FName, TArray<FMaterialStatsUtils::FRepresentativeShaderInfo>>& Pair : ShaderTypeNamesAndDescriptions)
			{
				for (const FMaterialStatsUtils::FRepresentativeShaderInfo& ShaderInfo : Pair.Value)
				{
					...
					FTetherMaterialShaderStat Stat;
...
FTetherMaterialGraph UTetherMaterialLibrary::GetMaterialGraph(const FString& MaterialPath)
{
	...
	if (!LoadedObj)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetMaterialGraph could not load '%s'"), *MaterialPath);
		return Graph;
	}
	...
	else
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: GetMaterialGraph: '%s' is neither UMaterial nor UMaterialFunction (%s)"),
...
FTetherMaterialStats UTetherMaterialLibrary::GetMaterialStats(
	const FString& MaterialPath,
	...
	const FString& QualityStr)
{
	...
	FTetherMaterialStats Stats;

	UMaterialInterface* MatInterface = LoadObject<UMaterialInterface>(nullptr, *MaterialPath);
	if (!MatInterface)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetMaterialStats could not load '%s'"), *MaterialPath);
		return Stats;
	}
...
namespace TetherMaterialImpl
{
	static UStaticMesh* LoadPreviewMesh(const FString& Preset)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: unknown preview mesh '%s' — falling back to shaderball"), *Preset);
			Path = TEXT("/Engine/EngineMeshes/SM_MatPreviewMesh_01.SM_MatPreviewMesh_01");
		}
	...
	static void SetupPreviewLighting(FPreviewScene& Scene, const FString& Preset)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: unknown lighting preset '%s' — falling back to studio"), *Preset);
		...
		const FString& OutPath)
	{
		...
		// the editor-world capture in TetherLevelLibrary where ReadPixels lands
		...
		bool bShaderComplexity)
	{
		...
		if (!MatInterface)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: PreviewMaterial could not load '%s'"), *MaterialPath);
			return false;
		}
		...
		if (!PreviewMesh)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: PreviewMaterial failed to load preview mesh for preset '%s'"), *MeshPreset);
			return false;
		}
		...
		SpawnInfo.Name = MakeUniqueObjectName(PreviewWorld, ASceneCapture2D::StaticClass(), TEXT("TetherMatPreviewCam"));
		...
		UTextureRenderTarget2D* RT = NewObject<UTextureRenderTarget2D>(
			GetTransientPackage(),
			MakeUniqueObjectName(GetTransientPackage(), UTextureRenderTarget2D::StaticClass(), TEXT("TetherMatPreviewRT")));
...
namespace TetherMaterialImpl
{
	 ...
	 * UMaterialGraphNode::GetShortenPinName so the tether accepts both the
	 ...
	 * so without this normalization the tether would silently reject the raw
...
bool UTetherMaterialLibrary::PreviewMaterial(
	const FString& MaterialPath,
	const FString& Mesh,
...
FTetherCreateAssetResult UTetherMaterialLibrary::CreateMaterial(
	const FString& Path,
	...
	bool bUseMaterialAttributes)
{
	...
	FTetherCreateAssetResult Result;
...
FTetherCreateAssetResult UTetherMaterialLibrary::CreateMaterialInstance(
...
FTetherAddExpressionResult UTetherMaterialLibrary::AddMaterialExpression(
	const FString& MaterialPath,
	...
	int32 Y)
{
	...
	FTetherAddExpressionResult Result;
...
bool UTetherMaterialLibrary::ConnectMaterialExpressions(
	const FString& MaterialPath,
	...
	FGuid DstGuid, const FString& DstInputName)
{
	...
	if (!Src || !Dst)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: ConnectMaterialExpressions: guid not found (src=%s dst=%s)"),
...
bool UTetherMaterialLibrary::DisconnectMaterialInput(
	const FString& MaterialPath,
	FGuid DstGuid, const FString& DstInputName)
{
	using namespace TetherMaterialImpl;
...
bool UTetherMaterialLibrary::ConnectMaterialOutput(
	const FString& MaterialPath,
	FGuid SrcGuid, const FString& SrcOutputName,
	const FString& PropertyName)
{
	...
	if (!Src)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: ConnectMaterialOutput: src guid not found (%s)"),
			*SrcGuid.ToString());
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: ConnectMaterialOutput: unknown property '%s'"), *PropertyName);
		return false;
	}
...
bool UTetherMaterialLibrary::DisconnectMaterialOutput(
	const FString& MaterialPath,
	const FString& PropertyName)
{
	using namespace TetherMaterialImpl;
...
bool UTetherMaterialLibrary::DeleteMaterialExpression(
	const FString& MaterialPath,
	FGuid Guid)
{
	using namespace TetherMaterialImpl;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: DeleteMaterialExpression refused because dependency data is incomplete: %s"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: DeleteMaterialExpression refused; %s is still consumed by %s. "
	...
	const FScopedTransaction Transaction(NSLOCTEXT(
		"Tether", "DeleteMaterialExpression", "Delete Material Expression"));
	Material->Modify();
	Expr->Modify();
...
namespace TetherMaterialImpl
{
	...
	static bool ApplyPropertyString(UMaterialExpression* Expr, const FString& PropertyName, const FString& Value)
	{
		...
		if (!Prop)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: property '%s' not found on %s"),
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: ImportText failed for %s::%s = '%s'"),
...
bool UTetherMaterialLibrary::SetMaterialExpressionProperty(
	const FString& MaterialPath,
	FGuid Guid,
	const FString& PropertyName,
	const FString& Value)
...
int32 UTetherMaterialLibrary::SetMaterialExpressionProperties(
	const FString& MaterialPath,
	FGuid Guid,
	const TArray<FTetherExpressionPropSet>& Properties)
{
	using namespace TetherMaterialImpl;
	...
	Expr->PreEditChange(nullptr);
	int32 Applied = 0;
	for (const FTetherExpressionPropSet& P : Properties)
	{
...
FGuid UTetherMaterialLibrary::AddMaterialComment(
	const FString& MaterialPath,
	int32 X, int32 Y,
...
FGuid UTetherMaterialLibrary::AddMaterialReroute(
	const FString& MaterialPath,
	int32 X, int32 Y)
{
	UMaterial* Material = LoadObject<UMaterial>(nullptr, *MaterialPath);
...
FTetherCreateAssetResult UTetherMaterialLibrary::CreateMaterialFunction(
	const FString& Path,
...
namespace TetherMaterialImpl
{
	...
	static FTetherMaterialGraphOpResult ExecuteMaterialGraphOps(
		UMaterial* Material,
		const TArray<FTetherMaterialGraphOp>& Ops)
	{
		FTetherMaterialGraphOpResult Result;
		Result.Guids.SetNum(Ops.Num());

		auto Fail = [&](int32 Index, const FString& Message) -> FTetherMaterialGraphOpResult&
		{
		...
		{
			const FTetherMaterialGraphOp& O = Ops[i];
	...
	static FTetherMaterialGraphOpResult PreflightMaterialGraphOps(
		UMaterial* Material,
		const TArray<FTetherMaterialGraphOp>& Ops)
	{
		FTetherMaterialGraphOpResult Result;
		...
		const FName SandboxName = MakeUniqueObjectName(
			GetTransientPackage(), UMaterial::StaticClass(), TEXT("TetherMaterialGraphPreflight"));
...
FTetherMaterialGraphOpResult UTetherMaterialLibrary::ValidateMaterialGraphOps(
	const FString& MaterialPath,
	const TArray<FTetherMaterialGraphOp>& Ops)
{
	using namespace TetherMaterialImpl;
...
FTetherMaterialGraphOpResult UTetherMaterialLibrary::ApplyMaterialGraphOps(
	const FString& MaterialPath,
	const TArray<FTetherMaterialGraphOp>& Ops,
	bool bCompile)
{
	...
	FTetherMaterialGraphOpResult Preflight = PreflightMaterialGraphOps(Material, Ops);
	...
	FScopedTransaction Transaction(NSLOCTEXT(
		"Tether", "ApplyMaterialGraphOps", "Apply Material Graph Operations"));
	Material->Modify();
	FTetherMaterialGraphOpResult Result = ExecuteMaterialGraphOps(Material, Ops);
...
int32 UTetherMaterialLibrary::AutoLayoutMaterialGraph(
	const FString& MaterialPath,
	int32 ColumnSpacing,
	int32 RowSpacing)
{
...
FString UTetherMaterialLibrary::DiffMaterialGraphSnapshots(
...
namespace TetherMaterialImpl
{
	static ECustomMaterialOutputType ParseCustomOutputType(const FString& Str)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Tether: unknown custom output type '%s' — falling back to Float1"), *Str);
		return CMOT_Float1;
	}

	static FString FindTetherSnippetsPath()
	{
		TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("Tether"));
		if (!Plugin.IsValid()) return FString();
		// Must match the mapping in TetherModule.cpp.
		return FPaths::Combine(Plugin->GetBaseDir(), TEXT("Shaders"), TEXT("Private"), TEXT("TetherSnippets.ush"));
	}
}

FTetherAddExpressionResult UTetherMaterialLibrary::AddCustomExpression(
	const FString& MaterialPath,
	int32 X, int32 Y,
...
namespace TetherMaterialImpl
{
	...
	static TArray<FParsedSnippet> ParseTetherSnippets(const TArray<FString>& Lines)
	{
...
TArray<FTetherShaderSnippet> UTetherMaterialLibrary::ListSharedSnippets()
{
	...
	TArray<FTetherShaderSnippet> Out;

	const FString SnippetsPath = FindTetherSnippetsPath();
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: could not read TetherSnippets.ush at '%s'"), *SnippetsPath);
		return Out;
	}

	for (const FParsedSnippet& P : ParseTetherSnippets(Lines))
	{
		FTetherShaderSnippet S;
		S.Name = P.Name;
...
FTetherMIParamResult UTetherMaterialLibrary::SetMIParams(
	const FString& MaterialInstancePath,
	const TArray<FTetherMIParamSet>& Params)
{
	using namespace TetherMaterialImpl;
	using namespace TetherMaterialParameterHelpers;

	FTetherMIParamResult Result;
	Result.Outcomes.Reserve(Params.Num());
	for (const FTetherMIParamSet& P : Params)
	{
		FTetherMIParamOutcome& Outcome = Result.Outcomes.AddDefaulted_GetRef();
		Outcome.Name = P.Name;
		Outcome.Type = P.Type;
	...
	if (!MIC)
	{
		...
		for (FTetherMIParamOutcome& Outcome : Result.Outcomes)
		{
	...
	{
		const FTetherMIParamSet& P = Params[ParamIndex];
		FTetherMIParamOutcome& Outcome = Result.Outcomes[ParamIndex];
	...
	const FScopedTransaction Transaction(NSLOCTEXT("Tether", "SetMIParams", "Set material instance parameters"));
	MIC->Modify();
	for (int32 ParamIndex = 0; ParamIndex < Validated.Num(); ++ParamIndex)
	{
...
void TetherTextureRefresh::Submit(
	UTexture& Texture,
	bool bForceDerivedDataRebuild,
	const ITetherTextureRefreshOperations& Operations,
	FTetherTextureRefreshResult& OutResult)
{
...
FTetherTextureRefreshResult UTetherMaterialLibrary::RefreshTextureResource(
...
#if WITH_EDITOR
	const TetherMaterialImpl::FTextureRefreshOperations Operations;
	TetherTextureRefresh::Submit(*Texture, bForceDerivedDataRebuild, Operations, Result);
#else
...
bool UTetherMaterialLibrary::SetMIAndPreview(
	const FString& MaterialInstancePath,
	const TArray<FTetherMIParamSet>& Params,
	const FString& Mesh,
	const FString& Lighting,
	...
	const FString& OutPngPath)
{
	return TetherMaterialParameterHelpers::SetMIAndPreviewAfterSuccessfulSet(
		[&]()
		{
...
namespace TetherMaterialImpl
{
		...
		TArray<FColor>& OutPixels)
	{
		...
		const FString ScratchPath = FPaths::Combine(
			FPaths::ProjectSavedDir(), TEXT("Tether"),
...
TArray<FString> UTetherMaterialLibrary::SweepMIParams(
	const FString& MaterialInstancePath,
	...
	const FString& OutGridPath)
{
	...
	if (!MIC)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SweepMIParams could not load '%s'"), *MaterialInstancePath);
		return Out;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SweepMIParams: param '%s' is not scalar or vector on '%s'"),
	...
	for (int32 i = 0; i < N; ++i)
	{
		...
		else
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Tether: SweepMIParams: value '%s' at index %d not parseable"), *V, i);
				continue;
			}
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: SweepMIParams: render failed at index %d"), i);
			continue;
		}
...
FTetherMIParamResult UTetherMaterialLibrary::SetMaterialParameterCollection(
...
FString UTetherMaterialLibrary::DiffMIParams(
	const FString& PathA,
	const FString& PathB)
{
	using namespace TetherMaterialImpl;
...
bool UTetherMaterialLibrary::CompileMaterial(
	const FString& MaterialPath,
	bool bSaveAfter)
{
	UMaterial* Material = LoadObject<UMaterial>(nullptr, *MaterialPath);
...
TArray<FString> UTetherMaterialLibrary::GetMaterialCompileErrors(
	const FString& MaterialPath,
	const FString& FeatureLevelStr,
	const FString& QualityStr)
{
	...
	if (!MatInterface)
	{
		Errors.Add(FString::Printf(TEXT("Tether: could not load material '%s'"), *MaterialPath));
		return Errors;
	}
...
FTetherShaderCompileStatus UTetherMaterialLibrary::GetMaterialShaderCompileStatus(
	const FString& MaterialPath,
	const FString& FeatureLevelStr,
	const FString& QualityStr)
{
	...
	FTetherShaderCompileStatus Out;
	...
	static APostProcessVolume* ResolveVolume(const FString& Label, bool bCreateIfMissing)
	{
		...
		if (bCreateIfMissing && Label.IsEmpty())
		{
			...
			if (NewVol)
			{
				...
				NewVol->SetActorLabel(TEXT("PPV_Tether"));
...
FTetherCreateAssetResult UTetherMaterialLibrary::CreatePostProcessMaterial(
	const FString& Path,
...
TArray<FTetherPostProcessVolumeInfo> UTetherMaterialLibrary::GetPostProcessState()
{
	...
	TArray<FTetherPostProcessVolumeInfo> Out;
	UWorld* World = GetEditorWorld();
	if (!World) return Out;
	...
	for (TActorIterator<APostProcessVolume> It(World); It; ++It)
	{
		...
		FTetherPostProcessVolumeInfo Info;
...
bool UTetherMaterialLibrary::ApplyPostProcessMaterial(
...
bool UTetherMaterialLibrary::RemovePostProcessMaterial(
	const FString& VolumeActor,
	const FString& MaterialPath)
{
	using namespace TetherMaterialImpl;
...
namespace TetherMaterialImpl
{
	...
	static void EmitFinding(TArray<FTetherMaterialFinding>& Out,
		...
		FString* OutError = nullptr)
	{
		...
		for (const FTetherMaterialGraphConnection& Connection : DependencyGraph.Connections)
		{
...
FTetherMaterialAnalysis UTetherMaterialLibrary::AnalyzeMaterial(
	const FString& MaterialPath,
	...
	int32 SamplerBudget)
{
	...
	// Collect stats (best effort — may report 0 if shader map still compiling).
	{
		...
		if (Resource)
		{
			...
			TArray<FTetherMaterialShaderStat> Stats;
			CollectShaderInstructionStats(Resource, Stats);
			Out.bShaderStatsReady = Stats.Num() > 0;
			for (const FTetherMaterialShaderStat& Stat : Stats)
			{
	...
	// Exception: a one-liner that's just `return TetherXxx(...);` through an
	...
	Out.Findings.Sort([&SevRank](const FTetherMaterialFinding& A, const FTetherMaterialFinding& B)
	{
...
FTetherMaterialAutoFixResult UTetherMaterialLibrary::AutoFixMaterial(
	const FString& MaterialPath,
	...
	bool bSaveAfter)
{
	...
	FTetherMaterialAutoFixResult Out;
	...
	FTetherMaterialAnalysis Analysis =
		UTetherMaterialLibrary::AnalyzeMaterial(MaterialPath, 0, 0);
	if (!Analysis.bFound)
	{
	...
	const FScopedTransaction Transaction(NSLOCTEXT("Tether",
	...
	if (RequestedFixes.Contains(TEXT("drop_unused")))
	{
		...
		for (const FTetherMaterialFinding& F : Analysis.Findings)
		{
...
FTetherMaterialGraphOpResult UTetherMaterialLibrary::SetMaterialAttributeLayers(
	const FString& MaterialPath,
	...
	const TArray<FString>& LayerNames)
{
	...
	FTetherMaterialGraphOpResult Out;
	Out.bSuccess = false;
	Out.FailedAtIndex = 0;
	...
	FScopedTransaction Tx(NSLOCTEXT("Tether", "SetMAL",
