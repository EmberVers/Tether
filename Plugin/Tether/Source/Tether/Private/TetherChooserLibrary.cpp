...
#define LOCTEXT_NAMESPACE "TetherChooser"

namespace TetherChooserImpl
{
	UChooserTable* LoadCHT(const FString& Path)
	{
		UChooserTable* CHT = LoadObject<UChooserTable>(nullptr, *Path);
		if (!CHT)
			UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load ChooserTable '%s'"), *Path);
		return CHT;
	}
	...
	FString DescribeRowResult(const FInstancedStruct& Result, FTetherCHTRowResult& Out)
	{
	...
	FTetherCHTRowResult MakeRowResult(const FInstancedStruct& Result)
	{
		FTetherCHTRowResult Out;
	...
	// ── Last-error capture ──
	// tether.exec runs sequentially on GameThread, so a static FString here is
	...
	void SetChooserError(const FString& Msg)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("Tether[chooser]: %s"), *Msg);
	}
...
{
	return TetherChooserImpl::GLastChooserError;
}
...
{
	using namespace TetherChooserImpl;
	FTetherCHTInfo Out;
...
bool UTetherChooserLibrary::SetChooserRowResultAsset(const FString& ChooserTablePath, int32 RowIndex, const FString& AssetPath)
{
	...
	if (!Asset)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SetChooserRowResultAsset cannot load '%s'"), *AssetPath);
		return false;
	}
...
bool UTetherChooserLibrary::SetChooserContextObjectClass(const FString& ChooserTablePath, const FString& ContextClassPath, const FString& Direction)
{
	...
	if (!ContextClass)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SetChooserContextObjectClass cannot load class '%s'"), *ContextClassPath);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SetChooserContextObjectClass invalid Direction '%s' — expected Read / Write / ReadWrite"), *Direction);
		return false;
	}
...
int32 UTetherChooserLibrary::AddChooserColumnByStructPath(const FString& ChooserTablePath, const FString& ColumnStructPath,
	const FString& BindingPropertyChain, int32 ContextIndex)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AddChooserColumnByStructPath '%s' is not a ChooserColumn"), *ColumnStructPath);
		return INDEX_NONE;
	}
...
int32 UTetherChooserLibrary::AddChooserColumnEnum(const FString& ChooserTablePath, const FString& BindingPropertyChain, const FString& EnumPath, int32 ContextIndex)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AddChooserColumnEnum cannot load enum '%s'"), *EnumPath);
		return INDEX_NONE;
	}
		...
		if (!bIsValidBoolText)
		{
			SetChooserError(FString::Printf(
				TEXT("set_chooser_cell_raw col=%d row=%d: BoolColumn cells use bare enum text "
				     ...
				     "See tether-chooser-api.md cell-format table."),
...
{
	using namespace TetherChooserImpl;
	TArray<FTetherCHTRowResult> Out;
...
TArray<FTetherCHTRowResult> UTetherChooserLibrary::ListPossibleResults(const FString& ChooserTablePath)
{
	...
	{
		FTetherCHTRowResult Fallback = MakeRowResult(CHT->FallbackResult);
