...
#define LOCTEXT_NAMESPACE "TetherAnim"
...
namespace TetherAnimNodeAccess
{
...
namespace TetherAnimImpl
{
	...
	UAnimBlueprint* LoadABP(const FString& Path)
	{
		...
		if (!ABP)
			UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load Anim Blueprint '%s'"), *Path);
		return ABP;
	}
	...
	void GatherNotifies(const UAnimSequenceBase* Seq, TArray<FTetherAnimNotifyInfo>& Out)
	{
		for (const FAnimNotifyEvent& N : Seq->Notifies)
		{
			FTetherAnimNotifyInfo Info;
			if (N.Notify)
...
TArray<FTetherStateMachineInfo> UTetherAnimLibrary::GetAnimGraphInfo(const FString& AnimBlueprintPath)
{
	TArray<FTetherStateMachineInfo> Result;

	UAnimBlueprint* ABP = TetherAnimImpl::LoadABP(AnimBlueprintPath);
	...
	for (UEdGraph* Graph : ABP->FunctionGraphs)
	{
		...
		for (UEdGraphNode* Node : Graph->Nodes)
		{
			...
			FTetherStateMachineInfo SMInfo;
			SMInfo.Name = TetherAnimImpl::GetNodeTitleSource(SMNode);
...
TArray<FTetherAnimGraphNodeInfo> UTetherAnimLibrary::GetAnimGraphNodes(const FString& AnimBlueprintPath)
{
	TArray<FTetherAnimGraphNodeInfo> Result;

	UAnimBlueprint* ABP = TetherAnimImpl::LoadABP(AnimBlueprintPath);
	if (!ABP) return Result;

	UEdGraph* AnimGraph = TetherAnimImpl::FindAnimGraph(ABP);
	...
	for (int32 i = 0; i < AnimGraph->Nodes.Num(); ++i)
	{
		...
		FTetherAnimGraphNodeInfo Info;
		Info.NodeIndex = i;
		Info.NodeGuid = Node->NodeGuid.ToString(EGuidFormats::Digits);
		Info.NodeTitle = TetherAnimImpl::GetNodeTitleSource(Node);
		...
		if (UAnimGraphNode_Base* AnimNode = Cast<UAnimGraphNode_Base>(Node))
		{
			...
			if (FAnimNode_Base* FNode = TetherAnimNodeAccess::GetFNode(AnimNode))
			{
				// Use the node's description text for detail (untranslated)
				Info.Detail = TetherAnimImpl::GetNodeTitleSource(AnimNode, ENodeTitleType::ListView);
			}
		}
		...
		for (UEdGraphPin* Pin : Node->Pins)
		{
			...
			for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
			{
				...
				FTetherAnimNodeConnection Conn;
...
TArray<FTetherAnimLayerInfo> UTetherAnimLibrary::GetAnimLinkedLayers(const FString& AnimBlueprintPath)
{
	TArray<FTetherAnimLayerInfo> Result;
	...
	for (UEdGraph* Graph : ABP->FunctionGraphs)
	{
		...
		for (UEdGraphNode* Node : Graph->Nodes)
		{
			...
			FTetherAnimLayerInfo Info;

			// Parse from node title (format: "InterfaceName - LayerName"), use source text
			FString FullTitle = TetherAnimImpl::GetNodeTitleSource(LayerNode);
			FString Left, Right;
...
TArray<FTetherAnimSlotInfo> UTetherAnimLibrary::GetAnimSlots(const FString& AnimBlueprintPath)
{
	TArray<FTetherAnimSlotInfo> Result;
	...
	for (UEdGraph* Graph : ABP->FunctionGraphs)
	{
		...
		for (UEdGraphNode* Node : Graph->Nodes)
		{
			...
			FTetherAnimSlotInfo Info;
			Info.GraphName = Graph->GetName();
			Info.NodeTitle = TetherAnimImpl::GetNodeTitleSource(SlotNode);

			// Access the internal FAnimNode_Slot to get SlotName
			FStructProperty* NodeProp = TetherAnimNodeAccess::GetFNodeProperty(SlotNode);
			if (NodeProp)
			{
...
// Forward decls — actual definitions live later in the TetherAnimWriteImpl namespace block.
namespace TetherAnimWriteImpl
{
...
namespace TetherAnimImpl
{
	...
	{
		FAnimNode_Base* FNode = TetherAnimNodeAccess::GetFNode(AnimNode);
		UScriptStruct* NodeStruct = TetherAnimNodeAccess::GetFNodeType(AnimNode);
...
TArray<FString> UTetherAnimLibrary::GetAnimNodeDetails(
	const FString& AnimBlueprintPath, int32 NodeIndex)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: NodeIndex %d out of range [0, %d)"),
	...
	if (!AnimNode)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Node at index %d is not an AnimGraphNode"), NodeIndex);
		return Result;
	}

	TetherAnimImpl::DumpAnimNodeProperties(AnimNode, Result);
	return Result;
}

TArray<FString> UTetherAnimLibrary::GetAnimNodeDetailsByGuid(
	const FString& AnimBlueprintPath, const FString& GraphName, const FString& NodeGuid)
{
	...
	UEdGraph* Graph = TetherAnimWriteImpl::FindAnyGraphByName(ABP, GraphName);
	if (!Graph)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetAnimNodeDetailsByGuid graph '%s' not found in '%s'"),
			*GraphName, *AnimBlueprintPath);
		return Result;
	}

	UEdGraphNode* Node = TetherAnimWriteImpl::FindNodeByGuid(Graph, NodeGuid);
	if (!Node)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetAnimNodeDetailsByGuid node guid '%s' not found in graph '%s'"),
			*NodeGuid, *GraphName);
		return Result;
	}
	...
	if (!AnimNode)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GetAnimNodeDetailsByGuid node '%s' is %s, not an AnimGraphNode"),
...
TArray<FString> UTetherAnimLibrary::GetAnimCurves(const FString& AnimBlueprintPath)
{
	TArray<FString> Result;
...
FTetherAnimSequenceInfo UTetherAnimLibrary::GetAnimSequenceInfo(const FString& SequencePath)
{
	FTetherAnimSequenceInfo Result;

	UAnimSequence* Seq = LoadObject<UAnimSequence>(nullptr, *SequencePath);
	if (!Seq)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load AnimSequence '%s'"), *SequencePath);
		return Result;
	}
...
FTetherMontageInfo UTetherAnimLibrary::GetMontageInfo(const FString& MontagePath)
{
	FTetherMontageInfo Result;

	UAnimMontage* Montage = LoadObject<UAnimMontage>(nullptr, *MontagePath);
	if (!Montage)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load AnimMontage '%s'"), *MontagePath);
		return Result;
	}
	...
	// Notifies
	TetherAnimImpl::GatherNotifies(Montage, Result.Notifies);
...
FTetherBlendSpaceInfo UTetherAnimLibrary::GetBlendSpaceInfo(const FString& BlendSpacePath)
{
	FTetherBlendSpaceInfo Result;

	UBlendSpace* BS = LoadObject<UBlendSpace>(nullptr, *BlendSpacePath);
	if (!BS)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load BlendSpace '%s'"), *BlendSpacePath);
		return Result;
	}
...
TArray<FTetherBoneInfo> UTetherAnimLibrary::GetSkeletonBoneTree(const FString& SkeletonPath)
{
	TArray<FTetherBoneInfo> Result;

	USkeleton* Skel = LoadObject<USkeleton>(nullptr, *SkeletonPath);
	if (!Skel)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load Skeleton '%s'"), *SkeletonPath);
		return Result;
	}
...
TArray<FTetherSocketInfo> UTetherAnimLibrary::GetSkeletonSockets(const FString& SkeletonPath)
{
	TArray<FTetherSocketInfo> Result;
...
bool UTetherAnimLibrary::AddAnimNotify(
	const FString& SequencePath, const FString& NotifyName, float TriggerTime, float Duration)
{
	...
	if (!Seq)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AddAnimNotify failed to load '%s'"), *SequencePath);
		return false;
	}
	if (TriggerTime < 0.f || TriggerTime > Seq->GetPlayLength())
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AddAnimNotify TriggerTime %f out of range [0, %f]"),
...
bool UTetherAnimLibrary::AddAnimNotifyState(
	...
	const float EndTime)
{
		...
		|| !NotifyStateClass->IsChildOf(UAnimNotifyState::StaticClass()))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAnimNotifyState invalid animation/class: Animation='%s' Class='%s'"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAnimNotifyState invalid window for '%s': Track=%s Window=[%.6f, %.6f] Length=%.6f"),
	...
	if (!AddedState)
	{
		...
		UE_LOG(LogTemp, Error,
			TEXT("Tether: failed to add notify state '%s' to '%s'"),
...
int32 UTetherAnimLibrary::RemoveAnimNotifyStatesByClass(
	const FString& AnimationPath,
	const FString& NotifyStateClassPath)
{
	UAnimSequenceBase* Animation = LoadObject<UAnimSequenceBase>(nullptr, *AnimationPath);
...
int32 UTetherAnimLibrary::RemoveAnimNotifiesByName(
...
TArray<FTetherMotionWarpingNotifyInfo> UTetherAnimLibrary::GetMotionWarpingNotifies(
	const FString& AnimationPath)
{
	TArray<FTetherMotionWarpingNotifyInfo> Result;
	const UAnimSequenceBase* Animation = LoadObject<UAnimSequenceBase>(nullptr, *AnimationPath);
	if (!Animation)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: GetMotionWarpingNotifies failed to load '%s'"), *AnimationPath);
		return Result;
	}
	...
	for (const FAnimNotifyEvent& NotifyEvent : Animation->Notifies)
	{
		...
		FTetherMotionWarpingNotifyInfo& Info = Result.AddDefaulted_GetRef();
...
bool UTetherAnimLibrary::SetMotionWarpingNotify(
	...
	const float StartTime, const float EndTime)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SetMotionWarpingNotify failed to load '%s'"), *AnimationPath);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: invalid Motion Warping notify for '%s': Target=%s Window=[%.6f, %.6f] Length=%.6f"),
	...
	if (bCreateMotionWarpingTrack)
	{
		...
		if (!UpdatedTrackNames.Contains(MotionWarpingTrackName))
		{
			...
			Transaction.Cancel();
			UE_LOG(LogTemp, Error,
				TEXT("Tether: failed to create MotionWarping notify track for '%s'"), *AnimationPath);
			return false;
		}
	}
	...
	if (!SkewWarp)
	{
		...
		Transaction.Cancel();
		UE_LOG(LogTemp, Error,
			TEXT("Tether: failed to construct Skew Warp notify for '%s'"), *AnimationPath);
		return false;
	}
...
bool UTetherAnimLibrary::SetAnimSequenceRateScale(
...
int32 UTetherAnimLibrary::CopyAndApplyAnimationModifiers(const FString& SourceSequencePath,
	const TArray<FString>& TargetSequencePaths)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: CopyAndApplyAnimationModifiers source '%s' has no configured modifiers"),
			*SourceSequencePath);
		return 0;
	}
	...
	for (const FString& TargetPath : TargetSequencePaths)
	{
		...
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: CopyAndApplyAnimationModifiers skipped invalid target '%s'"), *TargetPath);
			continue;
		}
		...
		for (const UAnimationModifier* SourceModifier : SourceUserData->GetAnimationModifierInstances())
		{
			...
			if (!TargetModifier)
			{
				...
				UE_LOG(LogTemp, Warning,
					TEXT("Tether: CopyAndApplyAnimationModifiers cannot add missing modifier %s to %s on Unreal Engine 5.5 or older because AddAnimationModifierOfClass is unavailable; modifier skipped."),
				...
				{
					UE_LOG(LogTemp, Warning, TEXT("Tether: failed to add modifier %s to %s"),
						*GetNameSafe(SourceModifier->GetClass()), *TargetPath);
					continue;
				}
...
bool UTetherAnimLibrary::AddMontageSection(
...
bool UTetherAnimLibrary::SetMontageSectionNext(
...
TArray<FTetherMontageSlotSegment> UTetherAnimLibrary::GetMontageSlotSegments(const FString& MontagePath)
{
	TArray<FTetherMontageSlotSegment> Result;
...
bool UTetherAnimLibrary::RemoveMontageSection(const FString& MontagePath, const FString& SectionName)
{
	UAnimMontage* Montage = LoadObject<UAnimMontage>(nullptr, *MontagePath);
...
bool UTetherAnimLibrary::SetMontageBlendTimes(const FString& MontagePath,
...
TArray<FTetherVirtualBoneInfo> UTetherAnimLibrary::GetSkeletonVirtualBones(const FString& SkeletonPath)
{
	TArray<FTetherVirtualBoneInfo> Result;
...
bool UTetherAnimLibrary::AddSkeletonSocket(const FString& SkeletonPath,
	...
	FVector RelativeLocation, FRotator RelativeRotation, FVector RelativeScale)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AddSkeletonSocket '%s' already exists"), *SocketName);
		return false;
	}
	// Verify bone exists.
	if (Skel->GetReferenceSkeleton().FindBoneIndex(BoneFName) == INDEX_NONE)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AddSkeletonSocket bone '%s' not found"), *ParentBoneName);
		return false;
	}
...
TArray<FString> UTetherAnimLibrary::ListAssetsForSkeleton(
...
bool UTetherAnimLibrary::RemoveSkeletonSocket(const FString& SkeletonPath, const FString& SocketName)
{
	USkeleton* Skel = LoadObject<USkeleton>(nullptr, *SkeletonPath);
	if (!Skel) return false;
...
TArray<FTetherAnimSyncMarker> UTetherAnimLibrary::GetAnimSyncMarkers(const FString& SequencePath)
{
	TArray<FTetherAnimSyncMarker> Result;
	...
	Result.Sort([](const FTetherAnimSyncMarker& A, const FTetherAnimSyncMarker& B) {
...
TArray<FTetherBlendProfileInfo> UTetherAnimLibrary::GetSkeletonBlendProfiles(const FString& SkeletonPath)
{
	TArray<FTetherBlendProfileInfo> Result;
	...
	{
		if (!BP) continue;
		FTetherBlendProfileInfo Info;
		Info.Name = BP->GetName();
		Info.Mode = TetherAnimImpl::BlendProfileModeToString(BP->GetMode());
...
TArray<FTetherBlendProfileEntry> UTetherAnimLibrary::GetBlendProfileEntries(
	const FString& SkeletonPath, const FString& ProfileName)
{
	TArray<FTetherBlendProfileEntry> Result;
	...
	{
		FTetherBlendProfileEntry Entry;
...
bool UTetherAnimLibrary::SetMontageSectionStartTime(
	const FString& MontagePath, const FString& SectionName, float StartTime)
{
	UAnimMontage* Montage = LoadObject<UAnimMontage>(nullptr, *MontagePath);
	if (!Montage) return false;
...
bool UTetherAnimLibrary::AddAnimSyncMarker(const FString& SequencePath, const FString& MarkerName, float Time)
{
...
int32 UTetherAnimLibrary::RemoveAnimSyncMarkersByName(const FString& SequencePath, const FString& MarkerName)
{
	UAnimSequence* Seq = LoadObject<UAnimSequence>(nullptr, *SequencePath);
	if (!Seq) return 0;
...
bool UTetherAnimLibrary::SetSkeletonSocketTransform(const FString& SkeletonPath, const FString& SocketName,
	FVector RelativeLocation, FRotator RelativeRotation, FVector RelativeScale)
{
	USkeleton* Skel = LoadObject<USkeleton>(nullptr, *SkeletonPath);
	if (!Skel) return false;
...
bool UTetherAnimLibrary::RenameSkeletonSocket(const FString& SkeletonPath, const FString& OldName, const FString& NewName)
{
...
FTetherAnimBlueprintInfo UTetherAnimLibrary::GetAnimBlueprintInfo(const FString& AnimBlueprintPath)
{
	FTetherAnimBlueprintInfo Info;
...
namespace TetherAnimWriteImpl
{
	 ...
	 * PostPlacedNewNode + AllocateDefaultPins. Matches the tether BP pattern
...
TArray<FTetherAnimGraphSummary> UTetherAnimLibrary::ListAnimGraphs(const FString& AnimBlueprintPath)
{
	TArray<FTetherAnimGraphSummary> Result;
	UAnimBlueprint* ABP = TetherAnimImpl::LoadABP(AnimBlueprintPath);
	if (!ABP) return Result;
...
TArray<FString> UTetherAnimLibrary::ListAnimGraphNodes(const FString& AnimBlueprintPath,
	const FString& GraphName)
{
	using namespace TetherAnimWriteImpl;
	...
	for (UEdGraphNode* N : Graph->Nodes)
	{
		...
		const FString Title = TetherAnimImpl::GetNodeTitleSource(N, ENodeTitleType::ListView);
		Rows.Add(FString::Printf(TEXT("%s\t%s\t%s"), *Guid, *ClassName, *Title));
	}
	return Rows;
}

FString UTetherAnimLibrary::FindAnimGraphNodeByClass(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeSequencePlayer(const FString& AnimBlueprintPath,
	const FString& GraphName, const FString& SequencePath, int32 PosX, int32 PosY)
{
	...
	if (!ABP) return FString();
	UEdGraph* Graph = FindAnyGraphByName(ABP, GraphName);
	if (!Graph) { UE_LOG(LogTemp, Warning, TEXT("Tether: graph '%s' not found in '%s'"), *GraphName, *AnimBlueprintPath); return FString(); }
...
FString UTetherAnimLibrary::AddAnimGraphNodeBlendSpacePlayer(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeSlot(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeStateMachine(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeLayeredBoneBlend(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeBlendListByBool(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeBlendListByInt(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeTwoWayBlend(const FString& AnimBlueprintPath,
	const FString& GraphName, int32 PosX, int32 PosY)
{
	using namespace TetherAnimWriteImpl;
...
FString UTetherAnimLibrary::AddAnimGraphNodeLinkedAnimLayer(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimGraphNodeByClassName(const FString& AnimBlueprintPath,
	const FString& GraphName, const FString& ShortClassName, int32 PosX, int32 PosY)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: no concrete UAnimGraphNode_Base subclass named '%s'"),
			*ShortClassName);
		return FString();
	}
...
bool UTetherAnimLibrary::ConnectAnimGraphPins(const FString& AnimBlueprintPath,
	const FString& GraphName,
...
bool UTetherAnimLibrary::DisconnectAnimGraphPin(const FString& AnimBlueprintPath,
...
bool UTetherAnimLibrary::RemoveAnimGraphNode(const FString& AnimBlueprintPath,
...
bool UTetherAnimLibrary::SetAnimGraphNodePosition(const FString& AnimBlueprintPath,
...
bool UTetherAnimLibrary::EnsurePoseHistoryCollectedBones(const FString& AnimBlueprintPath,
	const FString& GraphName, const FString& NodeGuid, const TArray<FName>& BoneNames)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: EnsurePoseHistoryCollectedBones requires an ABP with a target skeleton"));
		return false;
	}
	...
	if (!AnimNode)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Node '%s' is not an AnimGraph node"), *NodeGuid);
		return false;
	}

	FAnimNode_Base* RuntimeNode = TetherAnimNodeAccess::GetFNode(AnimNode);
	UScriptStruct* RuntimeNodeType = TetherAnimNodeAccess::GetFNodeType(AnimNode);
	if (!RuntimeNode || !RuntimeNodeType ||
		!RuntimeNodeType->IsChildOf(FAnimNode_PoseSearchHistoryCollector_Base::StaticStruct()))
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: Node '%s' is not a Pose Search History Collector"), *NodeGuid);
		return false;
	}
	...
	for (const FName BoneName : BoneNames)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: Bone '%s' does not exist on skeleton '%s'"),
...
bool UTetherAnimLibrary::SetAnimSequencePlayerSequence(const FString& AnimBlueprintPath,
...
bool UTetherAnimLibrary::SetAnimSlotName(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimState(const FString& AnimBlueprintPath,
	...
	int32 PosX, int32 PosY)
{
	...
	if (!SMGraph) { UE_LOG(LogTemp, Warning, TEXT("Tether: state machine '%s' not found"), *StateMachineGraphName); return FString(); }
...
FString UTetherAnimLibrary::AddAnimConduit(const FString& AnimBlueprintPath,
...
FString UTetherAnimLibrary::AddAnimTransition(const FString& AnimBlueprintPath,
...
bool UTetherAnimLibrary::RemoveAnimState(const FString& AnimBlueprintPath,
	const FString& StateMachineGraphName, const FString& StateName)
{
	using namespace TetherAnimWriteImpl;
...
FString UTetherAnimLibrary::GetAnimTransitionRuleGraphName(const FString& AnimBlueprintPath,
	const FString& StateMachineGraphName,
	const FString& FromStateName, const FString& ToStateName)
{
	using namespace TetherAnimWriteImpl;
...
bool UTetherAnimLibrary::RemoveAnimTransition(const FString& AnimBlueprintPath,
	const FString& StateMachineGraphName,
	const FString& FromStateName, const FString& ToStateName)
{
	using namespace TetherAnimWriteImpl;
...
bool UTetherAnimLibrary::SetAnimStateDefault(const FString& AnimBlueprintPath,
	const FString& StateMachineGraphName, const FString& StateName)
{
	using namespace TetherAnimWriteImpl;
...
bool UTetherAnimLibrary::RenameAnimState(const FString& AnimBlueprintPath,
	const FString& StateMachineGraphName,
...
bool UTetherAnimLibrary::SetAnimTransitionProperties(const FString& AnimBlueprintPath,
	const FString& StateMachineGraphName,
	const FString& FromStateName, const FString& ToStateName,
...
bool UTetherAnimLibrary::SetAnimTransitionConstRule(const FString& AnimBlueprintPath,
	const FString& StateMachineGraphName,
...
namespace TetherAnimLayoutImpl
{
	/**
...
FTetherAnimLayoutResult UTetherAnimLibrary::AutoLayoutAnimGraph(
	const FString& AnimBlueprintPath, const FString& GraphName,
	int32 HorizontalSpacing, int32 VerticalSpacing)
{
	using namespace TetherAnimWriteImpl;
	using namespace TetherAnimLayoutImpl;

	FTetherAnimLayoutResult Result;
	UAnimBlueprint* ABP = TetherAnimImpl::LoadABP(AnimBlueprintPath);
	...
	TetherAnimWriteImpl::FinalizeEdit(ABP, Graph);
...
FTetherAnimLayoutResult UTetherAnimLibrary::AutoLayoutStateMachine(
	...
	int32 HorizontalSpacing, int32 VerticalSpacing)
{
	...
	TetherAnimWriteImpl::FinalizeEdit(ABP, SMGraph);
