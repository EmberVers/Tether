#include "TetherGameplayAbilityLibrary.h"
...
namespace TetherGameplayAbilityImpl
{
...
FTetherGameplayAbilityInfo UTetherGameplayAbilityLibrary::GetGameplayAbilityBlueprintInfo(
	const FString& AbilityBlueprintPath)
{
	FTetherGameplayAbilityInfo Result;

	UClass* AbilityClass = TetherGameplayAbilityImpl::LoadGeneratedClassFromBlueprint(AbilityBlueprintPath);
	if (!AbilityClass || !AbilityClass->IsChildOf(UGameplayAbility::StaticClass()))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: '%s' is not a UGameplayAbility Blueprint"), *AbilityBlueprintPath);
		return Result;
	}
	...
	TetherGameplayAbilityImpl::TagContainerToStrings(CDO->GetAssetTags(), Result.AbilityTags);
#else
	// 5.4: GetAssetTags() not yet exposed; AbilityTags is the legacy field.
	TetherGameplayAbilityImpl::TagContainerToStrings(CDO->AbilityTags, Result.AbilityTags);
#endif
...
FTetherGameplayEffectInfo UTetherGameplayAbilityLibrary::GetGameplayEffectBlueprintInfo(
	const FString& EffectBlueprintPath)
{
	FTetherGameplayEffectInfo Result;

	UClass* EffectClass = TetherGameplayAbilityImpl::LoadGeneratedClassFromBlueprint(EffectBlueprintPath);
	if (!EffectClass || !EffectClass->IsChildOf(UGameplayEffect::StaticClass()))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: '%s' is not a UGameplayEffect Blueprint"), *EffectBlueprintPath);
		return Result;
	}
	...
	Result.DurationPolicy = TetherGameplayAbilityImpl::DurationPolicyToString(CDO->DurationPolicy);

	// Duration magnitude — best-effort constant.
	{
		float Mag = 0.f;
		if (TetherGameplayAbilityImpl::TryGetStaticMagnitude(CDO->DurationMagnitude, Mag))
		{
	...
	Result.PeriodSeconds = TetherGameplayAbilityImpl::GetScalableFloatConstant(CDO->Period);

	// Modifiers.
	for (const FGameplayModifierInfo& ModInfo : CDO->Modifiers)
	{
		FTetherGEModifierInfo Out;
		Out.Attribute = ModInfo.Attribute.GetName();
		Out.ModOp = TetherGameplayAbilityImpl::ModOpToString(static_cast<int32>(ModInfo.ModifierOp));

		float Mag = 0.f;
		if (TetherGameplayAbilityImpl::TryGetStaticMagnitude(ModInfo.ModifierMagnitude, Mag))
		{
			Out.Magnitude = Mag;
	...
	Result.StackingType = TetherGameplayAbilityImpl::StackingTypeToString(CDO->StackingType);
	...
	for (UGameplayEffectComponent* Comp :
		TetherGameplayAbilityImpl::GetGameplayEffectComponents(CDO))
	{
		if (!Comp)
		{
			continue;
		}
		FTetherGEComponentInfo CompInfo;
		CompInfo.ClassName = Comp->GetClass()->GetName();
		TetherGameplayAbilityImpl::CollectTagsFromObject(Comp, CompInfo.Tags);
...
FTetherAttributeSetInfo UTetherGameplayAbilityLibrary::GetAttributeSetInfo(
	const FString& AttributeSetClassPath)
{
	FTetherAttributeSetInfo Result;

	UClass* AS = TetherGameplayAbilityImpl::ResolveClassByPath(AttributeSetClassPath);
	if (!AS || !AS->IsChildOf(UAttributeSet::StaticClass()))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: '%s' is not a UAttributeSet class"), *AttributeSetClassPath);
		return Result;
	}
...
TArray<FString> UTetherGameplayAbilityLibrary::ListAbilitiesByTag(
	const FString& TagQuery, int32 MaxResults)
{
	...
	if (!Query.IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: '%s' is not a registered gameplay tag"), *TagQuery);
		return Results;
	}
...
TArray<FString> UTetherGameplayAbilityLibrary::ListGameplayEffectsByTag(
	const FString& TagQuery, int32 MaxResults)
{
	TArray<FString> Results;
	...
	for (const FAssetData& AD : Assets)
	{
		...
		if (TetherGameplayAbilityImpl::GameplayEffectHasTag(CDO, Query))
		{
			Results.Add(AD.GetSoftObjectPath().ToString());
			if (MaxResults > 0 && Results.Num() >= MaxResults)
			{
...
FTetherActorAbilitySystemInfo UTetherGameplayAbilityLibrary::GetActorAbilitySystemInfo(
	const FString& ActorName)
{
	FTetherActorAbilitySystemInfo Result;
	Result.ActorName = ActorName;

	AActor* Actor = TetherGameplayAbilityImpl::FindEditorActor(ActorName);
	if (!Actor)
	{
		return Result;
	}
	...
	{
		FTetherGrantedAbilityInfo Info;
		if (Spec.Ability)
		{
	...
	TetherGameplayAbilityImpl::TagContainerToStrings(OwnedTags, Result.OwnedTags);
...
TArray<FString> UTetherGameplayAbilityLibrary::ListGameplayTags(
	const FString& Filter, int32 MaxResults)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ListGameplayTags refuses empty filter with MaxResults=0 (would dump all tags)."));
		return Results;
	}
...
TArray<FString> UTetherGameplayAbilityLibrary::ListAttributeSets(
	const FString& Filter, int32 MaxResults)
{
	TArray<FString> Results;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ListAttributeSets refuses empty filter with MaxResults=0."));
		return Results;
	}
...
FTetherAttributeValue UTetherGameplayAbilityLibrary::GetAttributeValue(
	const FString& ActorName, const FString& AttributeName)
{
	FTetherAttributeValue Result;
...
TArray<FTetherActiveEffectInfo> UTetherGameplayAbilityLibrary::GetActorActiveEffects(
	const FString& ActorName, int32 MaxResults)
{
	TArray<FTetherActiveEffectInfo> Results;
...
TArray<FString> UTetherGameplayAbilityLibrary::GetTagParents(const FString& TagString)
{
	TArray<FString> Results;
...
bool UTetherGameplayAbilityLibrary::ActorHasGameplayTag(
	const FString& ActorName,
...
FTetherAbilityCooldownInfo UTetherGameplayAbilityLibrary::GetAbilityCooldownInfo(
	const FString& ActorName,
	const FString& AbilityBlueprintPath)
{
	FTetherAbilityCooldownInfo Result;

	UClass* AbilityClass =
		TetherGameplayAbilityImpl::LoadGeneratedClassFromBlueprint(AbilityBlueprintPath);
	if (!AbilityClass || !AbilityClass->IsChildOf(UGameplayAbility::StaticClass()))
	{
		return Result;
	}
	...
	if (UGameplayAbility* CDO = AbilityClass->GetDefaultObject<UGameplayAbility>())
	{
		...
		{
			TetherGameplayAbilityImpl::TagContainerToStrings(*CT, Result.CooldownTags);
		}
	}
...
TArray<FTetherActiveEffectInfo> UTetherGameplayAbilityLibrary::FindActiveEffectsByTag(
	const FString& ActorName,
...
TArray<FString> UTetherGameplayAbilityLibrary::ListAbilityBlueprints(
	const FString& Filter, int32 MaxResults)
{
	TArray<FString> Results;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ListAbilityBlueprints refuses empty filter with MaxResults=0."));
		return Results;
	}
...
TArray<FString> UTetherGameplayAbilityLibrary::ListGameplayEffectBlueprints(
	const FString& Filter, int32 MaxResults)
{
	TArray<FString> Results;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ListGameplayEffectBlueprints refuses empty filter with MaxResults=0."));
		return Results;
	}
...
TArray<FString> UTetherGameplayAbilityLibrary::ListAttributeSetBlueprints(
	const FString& Filter, int32 MaxResults)
{
	TArray<FString> Results;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ListAttributeSetBlueprints refuses empty filter with MaxResults=0."));
		return Results;
	}
...
bool UTetherGameplayAbilityLibrary::IsValidGameplayTag(const FString& TagString)
{
...
bool UTetherGameplayAbilityLibrary::TagMatches(
...
TArray<FTetherAttributeValue> UTetherGameplayAbilityLibrary::GetActorAttributes(
	const FString& ActorName)
{
	TArray<FTetherAttributeValue> Results;
...
TArray<FString> UTetherGameplayAbilityLibrary::FindChildTags(
...
FTetherAbilityTagRequirements UTetherGameplayAbilityLibrary::GetAbilityTagRequirements(
	const FString& AbilityBlueprintPath)
{
	FTetherAbilityTagRequirements Result;
	...
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("CancelAbilitiesWithTag"),  Result.CancelAbilitiesWithTag);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("BlockAbilitiesWithTag"),   Result.BlockAbilitiesWithTag);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("ActivationOwnedTags"),     Result.ActivationOwnedTags);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("ActivationRequiredTags"),  Result.ActivationRequiredTags);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("ActivationBlockedTags"),   Result.ActivationBlockedTags);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("SourceRequiredTags"),      Result.SourceRequiredTags);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("SourceBlockedTags"),       Result.SourceBlockedTags);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("TargetRequiredTags"),      Result.TargetRequiredTags);
	TetherGameplayAbilityImpl::ReadTagContainerProperty(CDO, TEXT("TargetBlockedTags"),       Result.TargetBlockedTags);

	return Result;
}

TArray<FTetherAbilityTriggerInfo> UTetherGameplayAbilityLibrary::GetAbilityTriggers(
	const FString& AbilityBlueprintPath)
{
	TArray<FTetherAbilityTriggerInfo> Results;
	...
	for (int32 i = 0; i < Helper.Num(); ++i)
	{
		...
		Out.TriggerSource = TetherGameplayAbilityImpl::TriggerSourceToString(SourceVal);
...
TArray<FString> UTetherGameplayAbilityLibrary::GetActorBlockedAbilityTags(const FString& ActorName)
{
	TArray<FString> Results;
	AActor* Actor = nullptr;
	UAbilitySystemComponent* ASC = TetherGameplayAbilityImpl::ResolveActorASC(ActorName, Actor);
	if (!ASC)
	{
		return Results;
	}
	...
	TetherGameplayAbilityImpl::TagContainerToStrings(Blocked, Results);
	return Results;
}

bool UTetherGameplayAbilityLibrary::ActorAbilityMeetsTagRequirements(
	const FString& ActorName,
	const FString& AbilityBlueprintPath,
	TArray<FString>& OutBlockingTags)
{
	...
	TetherGameplayAbilityImpl::TagContainerToStrings(Relevant, OutBlockingTags);
	return bOk;
}

TArray<FTetherActiveAbilityInfo> UTetherGameplayAbilityLibrary::GetActorActiveAbilities(
	const FString& ActorName)
{
	TArray<FTetherActiveAbilityInfo> Results;
	AActor* Actor = nullptr;
	UAbilitySystemComponent* ASC = TetherGameplayAbilityImpl::ResolveActorASC(ActorName, Actor);
	if (!ASC)
	{
...
int32 UTetherGameplayAbilityLibrary::SendGameplayEventByName(
	const FString& ActorName,
	...
	float EventMagnitude)
{
	...
	// Controller (and its PlayerState). Mirrors TetherReactiveLibrary's
...
bool UTetherGameplayAbilityLibrary::EnsureAbilitySystemComponent(const FString& ActorName, const FString& Location)
{
	AActor* Actor = TetherGameplayAbilityImpl::FindActorAnyWorld(ActorName);
	...
	UAbilitySystemComponent* NewASC = NewObject<UAbilitySystemComponent>(HostActor, TEXT("TetherTestASC"));
...
bool UTetherGameplayAbilityLibrary::EnsureTetherTestAttributeSet(const FString& ActorName)
{
	AActor* Actor = TetherGameplayAbilityImpl::FindActorAnyWorld(ActorName);
	if (!Actor) return false;

	UAbilitySystemComponent* ASC = TetherGameplayAbilityImpl::WalkForASC(Actor);
	if (!ASC)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("EnsureTetherTestAttributeSet: no ASC reachable on '%s' (call EnsureAbilitySystemComponent first)"),
			*ActorName);
		return false;
	}
	...
	for (const UAttributeSet* AS : ASC->GetSpawnedAttributes())
	{
		if (AS && AS->IsA(UTetherTestAttributeSet::StaticClass()))
		{
			return true;
		}
	}

	UTetherTestAttributeSet* NewAS = NewObject<UTetherTestAttributeSet>(ASC->GetOwner());
...
bool UTetherGameplayAbilityLibrary::SetActorAttributeValue(const FString& ActorName, const FString& AttributeName, float Value)
{
	AActor* Actor = TetherGameplayAbilityImpl::FindActorAnyWorld(ActorName);
	if (!Actor) return false;
...
namespace TetherGAWriteImpl
{
	...
	static UBlueprint* LoadAbilityBP(const FString& Path, UClass** OutGenClass = nullptr)
	{
		...
		if (!BP || !BP->GeneratedClass)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: could not load ability BP '%s'"), *Path);
			return nullptr;
		}
		if (!BP->GeneratedClass->IsChildOf(UGameplayAbility::StaticClass()))
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: '%s' is not a UGameplayAbility subclass"), *Path);
			return nullptr;
		}
	...
	static EGameplayAbilityInstancingPolicy::Type ParseInstancingPolicy(const FString& S, bool& bOk)
	{
		...
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: NonInstanced is deprecated since UE 5.5 — prefer InstancedPerActor."));
...
int32 UTetherGameplayAbilityLibrary::SetAbilityTagContainer(
	const FString& AbilityBlueprintPath,
	const FString& ContainerName,
	const TArray<FString>& Tags)
{
	using namespace TetherGAWriteImpl;

	if (!KnownTagContainers().Contains(ContainerName))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SetAbilityTagContainer unknown container '%s'"), *ContainerName);
		return -1;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: container '%s' not found on '%s'"), *ContainerName, *GenClass->GetName());
		return -1;
	}
	...
	for (const FString& TagStr : Tags)
	{
		...
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: skipping unregistered tag '%s' for %s.%s"),
...
bool UTetherGameplayAbilityLibrary::SetAbilityInstancingPolicy(
	const FString& AbilityBlueprintPath,
	const FString& Policy)
{
	...
	if (!bParseOk)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: unknown InstancingPolicy '%s'"), *Policy);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: InstancingPolicy not a TEnumAsByte on '%s'"), *GenClass->GetName());
		return false;
	}
...
bool UTetherGameplayAbilityLibrary::SetAbilityNetExecutionPolicy(
	const FString& AbilityBlueprintPath,
	const FString& Policy)
{
	using namespace TetherGAWriteImpl;
	...
	if (!bParseOk)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: unknown NetExecutionPolicy '%s'"), *Policy);
		return false;
	}
...
namespace TetherGAWriteImpl
{
		...
		const FString& GameplayEffectClassPath)
	{
		...
		{
			GEClass = TetherGameplayAbilityImpl::ResolveClassByPath(GameplayEffectClassPath);
			if (!GEClass)
			{
				UE_LOG(LogTemp, Warning,
					TEXT("Tether: could not resolve GE class '%s'"), *GameplayEffectClassPath);
				return false;
			}
			if (!GEClass->IsChildOf(UGameplayEffect::StaticClass()))
			{
				UE_LOG(LogTemp, Warning,
					TEXT("Tether: '%s' is not a UGameplayEffect subclass"), *GameplayEffectClassPath);
				return false;
			}
		}
...
bool UTetherGameplayAbilityLibrary::SetAbilityCost(
	const FString& AbilityBlueprintPath,
	const FString& CostGameplayEffectClassPath)
{
	return TetherGAWriteImpl::SetGEClassRef(
		AbilityBlueprintPath, TEXT("CostGameplayEffectClass"), CostGameplayEffectClassPath);
}

bool UTetherGameplayAbilityLibrary::SetAbilityCooldown(
	const FString& AbilityBlueprintPath,
...
int32 UTetherGameplayAbilityLibrary::AddAbilityTrigger(
	const FString& AbilityBlueprintPath,
	...
	const FString& TriggerSource)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAbilityTrigger '%s' is not a registered tag"), *TriggerTag);
		return -1;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAbilityTrigger unknown source '%s' (expect GameplayEvent/OwnedTagAdded/OwnedTagPresent)"),
			*TriggerSource);
		return -1;
	}
...
int32 UTetherGameplayAbilityLibrary::RemoveAbilityTriggerByTag(
	const FString& AbilityBlueprintPath,
	const FString& TriggerTag)
{
	using namespace TetherGAWriteImpl;
...
int32 UTetherGameplayAbilityLibrary::ClearAbilityTriggers(const FString& AbilityBlueprintPath)
{
	using namespace TetherGAWriteImpl;
...
namespace TetherGAGraphWriteImpl
{
	using namespace TetherGameplayAbilityImpl;
	using namespace TetherGAWriteImpl;
...
TArray<FString> UTetherGameplayAbilityLibrary::ListAbilityTaskClasses(const FString& Filter, int32 MaxResults)
{
	TArray<FString> Result;
	if (Filter.IsEmpty() && MaxResults <= 0)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ListAbilityTaskClasses refused empty filter + unlimited — pass a filter or MaxResults>0"));
		return Result;
	}
...
TArray<FString> UTetherGameplayAbilityLibrary::ListAbilityTaskFactories(const FString& TaskClassPath)
{
	using namespace TetherGAGraphWriteImpl;

	TArray<FString> Result;
	UClass* TaskClass = TetherGameplayAbilityImpl::ResolveClassByPath(TaskClassPath);
	if (!TaskClass || !TaskClass->IsChildOf(UAbilityTask::StaticClass()))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ListAbilityTaskFactories '%s' is not a UAbilityTask subclass"),
			*TaskClassPath);
		return Result;
	}
...
#if !UE_VERSION_OLDER_THAN(5, 7, 0)
FString UTetherGameplayAbilityLibrary::AddAbilityTaskNode(
	const FString& AbilityBlueprintPath,
	...
	int32 NodePosY)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAbilityTaskNode graph '%s' not found on '%s'"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAbilityTaskNode '%s' is not a UAbilityTask subclass"),
			*TaskClassPath);
		return FString();
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAbilityTaskNode '%s' has no BlueprintCallable static factory '%s' returning a UAbilityTask*"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAbilityTaskNode reflection set of ProxyFactory* failed on '%s'"),
...
FString UTetherGameplayAbilityLibrary::CreateGameplayAbilityBlueprint(
	...
	const FString& ParentClassPath)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: CreateGameplayAbilityBlueprint requires both DestContentPath and AssetName"));
		return FString();
	}
	if (!DestContentPath.StartsWith(TEXT("/")))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: DestContentPath must start with '/' (e.g. '/Game/GA')"));
		return FString();
	}
	...
	{
		ParentClass = TetherGameplayAbilityImpl::ResolveClassByPath(ParentClassPath);
		if (!ParentClass)
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: CreateGameplayAbilityBlueprint could not resolve parent '%s'"),
				*ParentClassPath);
			return FString();
		}
		...
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: '%s' is not a UGameplayAbility subclass"),
				*ParentClassPath);
			return FString();
		}
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: asset already exists at '%s'"), *PackageName);
		return FString();
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: CreatePackage failed for '%s'"), *PackageName);
		return FString();
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FKismetEditorUtilities::CreateBlueprint returned null for '%s'"),
			*PackageName);
		return FString();
	}
	...
	// Dirty menu uses, the existing TetherEditorLibrary save UFUNCTIONs
...
FString UTetherGameplayAbilityLibrary::AddAbilityCallFunctionNode(
	const FString& AbilityBlueprintPath,
	const FString& GraphName,
	...
	int32 NodePosY)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddAbilityCallFunctionNode '%s' not found on '%s' (tried internal name + DisplayName + ScriptName)"),
...
namespace TetherGEWriteImpl
{
	static UBlueprint* LoadEffectBP(const FString& Path, UClass** OutGenClass = nullptr)
	{
		...
		if (!BP || !BP->GeneratedClass)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether: could not load GE BP '%s'"), *Path);
			return nullptr;
		}
		if (!BP->GeneratedClass->IsChildOf(UGameplayEffect::StaticClass()))
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: '%s' is not a UGameplayEffect subclass"), *Path);
			return nullptr;
		}
		if (OutGenClass) *OutGenClass = BP->GeneratedClass;
		return BP;
...
bool UTetherGameplayAbilityLibrary::SetGEScalableFloatField(
	const FString& GameplayEffectBlueprintPath,
	const FString& FieldName,
	float FlatValue)
{
	using namespace TetherGEWriteImpl;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SetGEScalableFloatField unknown field '%s' (expect DurationMagnitude / MaxDurationMagnitude / Period)"),
			*FieldName);
		return false;
	}
	...
	if (!OuterSP)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: '%s' missing on %s"), *RealField, *GenClass->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SetGEScalableFloatField reflection walk failed on %s.%s"),
...
int32 UTetherGameplayAbilityLibrary::AddGEModifierScalable(
	const FString& GameplayEffectBlueprintPath,
	const FString& AttributeSetClassPath,
	...
	float FlatMagnitude)
{
	...
	UClass* AttrSetClass = TetherGameplayAbilityImpl::ResolveClassByPath(AttributeSetClassPath);
	if (!AttrSetClass || !AttrSetClass->IsChildOf(UAttributeSet::StaticClass()))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddGEModifierScalable '%s' is not a UAttributeSet subclass"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: attribute '%s' not found on '%s'"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: unknown mod op '%s' (Additive/Multiplicitive/Division/Override)"),
			*ModOp);
		return -1;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: AddGEModifierScalable failed to write magnitude"));
...
bool UTetherGameplayAbilityLibrary::RemoveGEModifier(
	const FString& GameplayEffectBlueprintPath,
	int32 Index)
{
	using namespace TetherGEWriteImpl;
...
int32 UTetherGameplayAbilityLibrary::ClearGEModifiers(
	const FString& GameplayEffectBlueprintPath)
{
	using namespace TetherGEWriteImpl;
...
int32 UTetherGameplayAbilityLibrary::AddGEComponent(
	const FString& GameplayEffectBlueprintPath,
	const FString& ComponentClassPath)
{
	using namespace TetherGEWriteImpl;

	UClass* CompClass = TetherGameplayAbilityImpl::ResolveClassByPath(ComponentClassPath);
	if (!CompClass || !CompClass->IsChildOf(UGameplayEffectComponent::StaticClass()))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: '%s' is not a UGameplayEffectComponent subclass"),
			*ComponentClassPath);
		return -1;
	}
	if (CompClass->HasAnyClassFlags(CLASS_Abstract))
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: '%s' is abstract"), *ComponentClassPath);
		return -1;
	}
	...
	if (!ArrP)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: 'GEComponents' array not found on %s"), *GenClass->GetName());
		return -1;
	}
...
int32 UTetherGameplayAbilityLibrary::RemoveGEComponentsByClass(
	const FString& GameplayEffectBlueprintPath,
	const FString& ComponentClassPath)
{
	using namespace TetherGEWriteImpl;
...
int32 UTetherGameplayAbilityLibrary::SetGEComponentInheritedTags(
	const FString& GameplayEffectBlueprintPath,
	...
	const TArray<FString>& Tags)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SetGEComponentInheritedTags index %d out of range (count=%d)"),
	...
	if (!Comp)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: GE component at index %d is null"), ComponentIndex);
		return -1;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: field '%s' not found on '%s'"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: field '%s' on '%s' is not an FInheritedTagContainer (got %s)"),
			*FieldName, *Comp->GetClass()->GetName(),
	...
	for (const FString& TagStr : Tags)
	{
		...
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: skipping unregistered tag '%s' on %s.%s"),
...
bool UTetherGameplayAbilityLibrary::SetGameplayCueTag(
	...
	const FString& TagString)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SetGameplayCueTag could not load '%s'"), *CueNotifyBlueprintPath);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: '%s' is not a AGameplayCueNotify_Actor/_Static subclass"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SetGameplayCueTag '%s' is not a registered tag"), *TagString);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: GameplayCueTag FGameplayTag UPROPERTY not found on %s"),
			*GenClass->GetName());
		return false;
	}
...
namespace TetherTagScanImpl
{
	...
	struct FScanState
	{
		...
		TArray<FTetherTagReference> Refs;
...
FTetherTagReferenceReport UTetherGameplayAbilityLibrary::FindGameplayTagReferences(
	const FString& TagQuery,
	const FString& PackagePath,
	bool bMatchExact,
	int32 MaxResults)
{
	using namespace TetherTagScanImpl;

	FTetherTagReferenceReport Report;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FindGameplayTagReferences '%s' is not a registered tag"), *TagQuery);
