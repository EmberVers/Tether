...
DEFINE_LOG_CATEGORY_STATIC(LogTetherReactiveMove, Log, All);
 ...
 */
class FTetherMovementModeAdapter : public ITetherReactiveAdapter
{
public:
	...
	virtual void OnHandlerAdded(const FTetherHandlerRecord& Record) override
	{
		...
		if (!Char)
		{
			UE_LOG(LogTetherReactiveMove, Warning,
	...
	void HandleChange(ACharacter* Char, EMovementMode PrevMode, uint8 PrevCustom)
	{
		...
		Ctx.Add(TEXT("current_mode_name"),   FString::Printf(TEXT("'%s'"),
			*TetherReactiveAdapterImpl_Move::MovementModeName(CurrMode)));
		Ctx.Add(TEXT("current_custom_mode"), FString::Printf(TEXT("%d"), CurrCustom));

		Sub->Dispatch(ETetherTrigger::MovementModeChanged,
...
private:
	struct FBinding
	{
		...
		TStrongObjectPtr<UTetherMovementModeListener> Listener;
...
void UTetherMovementModeListener::OnMovementModeChanged(ACharacter* Char, EMovementMode PrevMode, uint8 PrevCustom)
{
...
namespace TetherReactiveAdapters
{
	...
	{
		return MakeUnique<FTetherMovementModeAdapter>();
	}
}
