 ...
 */
enum class ETetherWorkState : uint8
{
	Queued,
	Running,
	Cancelled,
 ...
 */
class FTetherWorkAdmissionGate final
{
public:
...
class TTetherCancellableWork final
{
public:
	explicit TTetherCancellableWork(TFunction<TResult()>&& InBody)
	...
	TTetherCancellableWork(const TTetherCancellableWork&) = delete;
	TTetherCancellableWork& operator=(const TTetherCancellableWork&) = delete;
	...
	{
		{
			...
			if (State != ETetherWorkState::Queued)
			{
				return false;
			}
			State = ETetherWorkState::Running;
		}
		...
		{
			FScopeLock Lock(&StateLock);
			check(State == ETetherWorkState::Running);
			Result.Emplace(MoveTemp(WorkResult));
			State = ETetherWorkState::Completed;
		}
	 ...
	 */
	bool TryCancel(TResult&& CancellationResult, ETetherWorkState& OutObservedState)
	{
		{
			FScopeLock Lock(&StateLock);
			if (State != ETetherWorkState::Queued)
			...
			State = ETetherWorkState::Cancelled;
			OutObservedState = State;
		}
		CompletionEvent->Trigger();
		return true;
	...
	ETetherWorkState GetState() const
	{
		FScopeLock Lock(&StateLock);
		return State;
	}
...
private:
	...
	ETetherWorkState State = ETetherWorkState::Queued;
