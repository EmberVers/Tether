...
#define LOCTEXT_NAMESPACE "TetherRigLibrary"

namespace TetherRigImpl
{
...
{
	LastError = Error;
	UE_LOG(LogTemp, Warning, TEXT("Tether|Rig: %s"), *Error);
	return false;
}
...
static TArray<FTetherRigPropertyInfo> ListProperties(FPropertyBindingDataView View, UObject* Owner)
{
	TArray<FTetherRigPropertyInfo> Result;
	...
	Result.Sort([](const FTetherRigPropertyInfo& A, const FTetherRigPropertyInfo& B)
	{
...
static FTetherRigPropertyResult ReadProperty(FPropertyBindingDataView View, const FString& PropertyPath, UObject* Owner)
{
	FTetherRigPropertyResult Result;
...
static void AddIssue(FTetherRigValidationReport& Report, const FString& Severity, const FString& Code,
	const FString& Subject, const FString& Message)
{
	FTetherRigValidationIssue Issue;
...
static void AddQualityIssue(FTetherAnimationQualityReport& Report, const FString& Severity, const FString& Code,
	const FString& Subject, const FString& Message)
{
	FTetherRigValidationIssue Issue;
	Issue.Severity = Severity;
...
{
	TetherRigImpl::ClearError();
	return true;
}

FString UTetherRigLibrary::GetLastRigError()
{
	return TetherRigImpl::LastError;
}

TArray<FTetherRigTypeInfo> UTetherRigLibrary::ListRigTypes(
	const FString& Kind, const FString& Query, int32 MaxResults)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherRigTypeInfo> Result;
	...
	auto Matches = [&QueryToken](const FTetherRigTypeInfo& Info)
	{
	...
	Result.Sort([](const FTetherRigTypeInfo& A, const FTetherRigTypeInfo& B)
	{
	...
	const FString& AssetPath, const FString& SourceSkeletalAssetPath, bool bModularRig, bool bImportCurves)
{
	...
	FScopedTransaction Transaction(LOCTEXT("CreateControlRig", "Tether: Create Control Rig"));
...
TArray<FTetherRigElementInfo> UTetherRigLibrary::ListControlRigElements(
	const FString& AssetPath, const FString& ElementType)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherRigElementInfo> Result;
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
	if (!Blueprint) return Result;
...
bool UTetherRigLibrary::ImportControlRigHierarchy(
	const FString& AssetPath, const FString& SourceSkeletalAssetPath, bool bReplaceExisting, bool bImportCurves)
{
	...
	FScopedTransaction Transaction(LOCTEXT("ImportControlRigHierarchy", "Tether: Import Control Rig Hierarchy"));
...
FString UTetherRigLibrary::AddControlRigBone(const FString& AssetPath, const FString& Name,
	...
	bool bGlobalTransform, bool bImportedBone)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigBone", "Tether: Add Control Rig Bone"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigNull(const FString& AssetPath, const FString& Name,
	const FString& ParentName, const FString& ParentType, const FTransform& Transform, bool bGlobalTransform)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigNull", "Tether: Add Control Rig Null"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigControl(const FString& AssetPath, const FString& Name,
	...
	const FString& ShapeName, const FLinearColor& ShapeColor, bool bAnimatable)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigControl", "Tether: Add Control Rig Control"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigCurve(const FString& AssetPath, const FString& Name, float Value)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigCurve", "Tether: Add Control Rig Curve"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigConnector(const FString& AssetPath, const FString& Name,
	const FString& ConnectorType, const FString& Description, bool bOptional, bool bArray)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigConnector", "Tether: Add Control Rig Connector"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::RemoveControlRigElement(
	const FString& AssetPath, const FString& Name, const FString& ElementType)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveControlRigElement", "Tether: Remove Control Rig Element"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::RenameControlRigElement(const FString& AssetPath, const FString& Name,
	const FString& ElementType, const FString& NewName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RenameControlRigElement", "Tether: Rename Control Rig Element"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::ReparentControlRigElement(const FString& AssetPath, const FString& Name,
	const FString& ElementType, const FString& ParentName, const FString& ParentType, bool bMaintainGlobalTransform)
{
	...
	FScopedTransaction Transaction(LOCTEXT("ReparentControlRigElement", "Tether: Reparent Control Rig Element"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::SetControlRigElementTransform(const FString& AssetPath, const FString& Name,
	...
	bool bInitial, bool bAffectChildren)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetControlRigElementTransform", "Tether: Set Control Rig Element Transform"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::SetControlRigControlShape(const FString& AssetPath, const FString& ControlName,
	const FString& ShapeName, const FLinearColor& ShapeColor, bool bVisible)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetControlRigControlShape", "Tether: Set Control Rig Control Shape"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::AddControlRigElementTag(const FString& AssetPath, const FString& Name,
	const FString& ElementType, const FString& Tag)
{
	...
	if (!Key.IsValid() || !Blueprint->GetHierarchy()->Contains(Key))
		return SetError(FString::Printf(TEXT("hierarchy element '%s' was not found"), *Name));
	FScopedTransaction Transaction(LOCTEXT("AddControlRigElementTag", "Tether: Add Control Rig Element Tag"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::RemoveControlRigElementTag(const FString& AssetPath, const FString& Name,
	const FString& ElementType, const FString& Tag)
{
	using namespace TetherRigImpl;
	ClearError();
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveControlRigElementTag", "Tether: Remove Control Rig Element Tag"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
TArray<FTetherRigPropertyInfo> UTetherRigLibrary::ListControlRigControlProperties(
...
FTetherRigPropertyResult UTetherRigLibrary::GetControlRigControlProperty(
...
bool UTetherRigLibrary::SetControlRigControlProperty(const FString& AssetPath,
	const FString& ControlName, const FString& PropertyPath, const FString& Value)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetControlRigControlProperty", "Tether: Set Control Rig Control Property"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
	if (!Controller->SetControlSettings(Key, Settings, true))
...
TArray<FTetherRigVMGraphInfo> UTetherRigLibrary::ListControlRigGraphs(const FString& AssetPath)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherRigVMGraphInfo> Result;
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
	if (!Blueprint) return Result;
	for (URigVMGraph* Graph : Blueprint->GetAllModels())
	{
		if (!Graph) continue;
		FTetherRigVMGraphInfo Info;
...
TArray<FTetherRigVMNodeInfo> UTetherRigLibrary::ListControlRigNodes(
	const FString& AssetPath, const FString& GraphName)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherRigVMNodeInfo> Result;
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
...
TArray<FTetherRigVMLinkInfo> UTetherRigLibrary::ListControlRigLinks(
	const FString& AssetPath, const FString& GraphName)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherRigVMLinkInfo> Result;
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
	URigVMGraph* Graph = Blueprint ? FindGraph(Blueprint, GraphName) : nullptr;
	if (!Graph) return Result;
...
FString UTetherRigLibrary::AddControlRigMemberVariable(const FString& AssetPath, const FString& Name,
	const FString& CPPType, const FString& DefaultValue, bool bPublic, bool bReadOnly)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigMemberVariable", "Tether: Add Control Rig Member Variable"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::RemoveControlRigMemberVariable(const FString& AssetPath, const FString& Name)
{
	...
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
	if (!Blueprint) return false;
	FScopedTransaction Transaction(LOCTEXT("RemoveControlRigMemberVariable", "Tether: Remove Control Rig Member Variable"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigUnitNode(const FString& AssetPath, const FString& GraphName,
	const FString& UnitStructPath, const FString& MethodName, const FVector2D& Position, const FString& NodeName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigUnitNode", "Tether: Add Control Rig Unit Node"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigTemplateNode(const FString& AssetPath, const FString& GraphName,
	const FString& Notation, const FVector2D& Position, const FString& NodeName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigTemplateNode", "Tether: Add Control Rig Template Node"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigVariableNode(const FString& AssetPath, const FString& GraphName,
	...
	const FString& DefaultValue, const FVector2D& Position, const FString& NodeName, bool bCreateMemberVariable)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigVariableNode", "Tether: Add Control Rig Variable Node"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigCommentNode(const FString& AssetPath, const FString& GraphName,
	...
	const FLinearColor& Color, const FString& NodeName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddControlRigCommentNode", "Tether: Add Control Rig Comment Node"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FString UTetherRigLibrary::AddControlRigBranchNode(const FString& AssetPath, const FString& GraphName,
	const FVector2D& Position, const FString& NodeName)
{
	...
	URigVMController* Controller = Blueprint ? FindGraphController(Blueprint, GraphName) : nullptr;
	if (!Controller) return FString();
	FScopedTransaction Transaction(LOCTEXT("AddControlRigBranchNode", "Tether: Add Control Rig Branch Node"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::RemoveControlRigNode(
	const FString& AssetPath, const FString& GraphName, const FString& NodeName)
{
	...
	URigVMController* Controller = Blueprint ? FindGraphController(Blueprint, GraphName) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("RemoveControlRigNode", "Tether: Remove Control Rig Node"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::SetControlRigNodePosition(const FString& AssetPath, const FString& GraphName,
	const FString& NodeName, const FVector2D& Position)
{
	...
	URigVMController* Controller = Blueprint ? FindGraphController(Blueprint, GraphName) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetControlRigNodePosition", "Tether: Move Control Rig Node"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::SetControlRigPinDefaultValue(const FString& AssetPath, const FString& GraphName,
	const FString& PinPath, const FString& DefaultValue, bool bResizeArrays)
{
	...
	URigVMController* Controller = Blueprint ? FindGraphController(Blueprint, GraphName) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetControlRigPinDefaultValue", "Tether: Set Control Rig Pin Default"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::ConnectControlRigPins(const FString& AssetPath, const FString& GraphName,
	const FString& OutputPinPath, const FString& InputPinPath, bool bCreateCastNode)
{
	...
	URigVMController* Controller = Blueprint ? FindGraphController(Blueprint, GraphName) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("ConnectControlRigPins", "Tether: Connect Control Rig Pins"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
bool UTetherRigLibrary::DisconnectControlRigPins(const FString& AssetPath, const FString& GraphName,
	const FString& OutputPinPath, const FString& InputPinPath)
{
	...
	URigVMController* Controller = Blueprint ? FindGraphController(Blueprint, GraphName) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("DisconnectControlRigPins", "Tether: Disconnect Control Rig Pins"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FTetherRigLayoutResult UTetherRigLibrary::AutoLayoutControlRigGraph(const FString& AssetPath,
	const FString& GraphName, float HorizontalSpacing, float VerticalSpacing)
{
	using namespace TetherRigImpl;
	ClearError();
	FTetherRigLayoutResult Result;
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
	...
	FScopedTransaction Transaction(LOCTEXT("AutoLayoutControlRigGraph", "Tether: Auto Layout Control Rig Graph"));
	static_cast<UBlueprint*>(Blueprint)->Modify();
...
FTetherRigValidationReport UTetherRigLibrary::CompileControlRig(const FString& AssetPath, bool bSave)
{
	using namespace TetherRigImpl;
	ClearError();
	FTetherRigValidationReport Report;
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
	if (!Blueprint)
	{
...
{
	using namespace TetherRigImpl;
	FTetherRigValidationReport Report = CompileControlRig(AssetPath, false);
...
FTetherControlRigEvaluationResult UTetherRigLibrary::EvaluateControlRig(const FString& AssetPath,
	const FString& EventName, const TArray<FTetherRigNamedTransform>& InputControls)
{
	using namespace TetherRigImpl;
	ClearError();
	FTetherControlRigEvaluationResult Result;
	UControlRigBlueprint* Blueprint = LoadControlRig(AssetPath);
	if (!Blueprint)
	{
		Result.Error = LastError;
	...
	for (const FTetherRigNamedTransform& Input : InputControls)
	{
...
FTetherRigOperationResult UTetherRigLibrary::CreateIKRig(
	const FString& AssetPath, const FString& SkeletalMeshPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("CreateIKRig", "Tether: Create IK Rig"));
...
TArray<FTetherIKSolverInfo> UTetherRigLibrary::ListIKRigSolvers(const FString& AssetPath)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherIKSolverInfo> Result;
	UIKRigDefinition* Rig = LoadIKRig(AssetPath);
...
TArray<FTetherIKGoalInfo> UTetherRigLibrary::ListIKRigGoals(const FString& AssetPath)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherIKGoalInfo> Result;
	UIKRigDefinition* Rig = LoadIKRig(AssetPath);
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return Result;
...
TArray<FTetherIKChainInfo> UTetherRigLibrary::ListIKRigRetargetChains(const FString& AssetPath)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherIKChainInfo> Result;
	UIKRigDefinition* Rig = LoadIKRig(AssetPath);
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return Result;
	for (const FBoneChain& Chain : Controller->GetRetargetChains())
	{
		FTetherIKChainInfo Info;
...
int32 UTetherRigLibrary::AddIKRigSolver(const FString& AssetPath, const FString& SolverTypePath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddIKRigSolver", "Tether: Add IK Rig Solver"));
	Rig->Modify();
...
bool UTetherRigLibrary::RemoveIKRigSolver(const FString& AssetPath, int32 SolverIndex)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("RemoveIKRigSolver", "Tether: Remove IK Rig Solver"));
	Rig->Modify();
...
bool UTetherRigLibrary::MoveIKRigSolver(const FString& AssetPath, int32 SolverIndex, int32 TargetIndex)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("MoveIKRigSolver", "Tether: Move IK Rig Solver"));
	Rig->Modify();
...
bool UTetherRigLibrary::SetIKRigSolverEnabled(const FString& AssetPath, int32 SolverIndex, bool bEnabled)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRigSolverEnabled", "Tether: Set IK Rig Solver Enabled"));
	Rig->Modify();
...
bool UTetherRigLibrary::SetIKRigSolverBones(const FString& AssetPath, int32 SolverIndex,
	const FString& StartBone, const FString& EndBone)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetIKRigSolverBones", "Tether: Set IK Rig Solver Bones"));
	Rig->Modify();
...
FString UTetherRigLibrary::AddIKRigGoal(
	const FString& AssetPath, const FString& GoalName, const FString& BoneName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddIKRigGoal", "Tether: Add IK Rig Goal"));
	Rig->Modify();
...
bool UTetherRigLibrary::RemoveIKRigGoal(const FString& AssetPath, const FString& GoalName)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("RemoveIKRigGoal", "Tether: Remove IK Rig Goal"));
	Rig->Modify();
...
bool UTetherRigLibrary::ConnectIKRigGoalToSolver(
	const FString& AssetPath, const FString& GoalName, int32 SolverIndex)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("ConnectIKRigGoalToSolver", "Tether: Connect IK Goal To Solver"));
	Rig->Modify();
...
bool UTetherRigLibrary::DisconnectIKRigGoalFromSolver(
	const FString& AssetPath, const FString& GoalName, int32 SolverIndex)
{
	using namespace TetherRigImpl;
	ClearError();
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("DisconnectIKRigGoalFromSolver", "Tether: Disconnect IK Goal From Solver"));
	Rig->Modify();
...
FString UTetherRigLibrary::AddIKRigRetargetChain(const FString& AssetPath, const FString& ChainName,
	const FString& StartBone, const FString& EndBone, const FString& GoalName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddIKRigRetargetChain", "Tether: Add IK Rig Retarget Chain"));
	Rig->Modify();
...
bool UTetherRigLibrary::RemoveIKRigRetargetChain(const FString& AssetPath, const FString& ChainName)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("RemoveIKRigRetargetChain", "Tether: Remove IK Rig Retarget Chain"));
	Rig->Modify();
...
FString UTetherRigLibrary::RenameIKRigRetargetChain(
	const FString& AssetPath, const FString& ChainName, const FString& NewName)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return FString();
	FScopedTransaction Transaction(LOCTEXT("RenameIKRigRetargetChain", "Tether: Rename IK Rig Retarget Chain"));
	Rig->Modify();
...
bool UTetherRigLibrary::SetIKRigRetargetRoot(const FString& AssetPath, const FString& RootBoneName)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRigRetargetRoot", "Tether: Set IK Rig Retarget Root"));
	Rig->Modify();
...
bool UTetherRigLibrary::SetIKRigBoneExcluded(
	const FString& AssetPath, const FString& BoneName, bool bExcluded)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRigBoneExcluded", "Tether: Set IK Rig Bone Excluded"));
	Rig->Modify();
...
bool UTetherRigLibrary::ApplyIKRigAutoSetup(
	const FString& AssetPath, bool bRetargetDefinition, bool bFullBodyIK)
{
	...
	UIKRigController* Controller = Rig ? UIKRigController::GetController(Rig) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("ApplyIKRigAutoSetup", "Tether: Apply IK Rig Auto Setup"));
	Rig->Modify();
...
TArray<FTetherRigPropertyInfo> UTetherRigLibrary::ListIKRigProperties(const FString& AssetPath,
...
FTetherRigPropertyResult UTetherRigLibrary::GetIKRigProperty(const FString& AssetPath,
...
bool UTetherRigLibrary::SetIKRigProperty(const FString& AssetPath, const FString& TargetKind,
	int32 SolverIndex, const FString& TargetName, const FString& PropertyPath, const FString& Value)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetIKRigProperty", "Tether: Set IK Rig Property"));
	Rig->Modify();
...
FTetherRigOperationResult UTetherRigLibrary::CreateIKRetargeter(const FString& AssetPath,
	...
	const FString& TargetPreviewMeshPath, bool bAddDefaultOps)
{
	...
	FScopedTransaction Transaction(LOCTEXT("CreateIKRetargeter", "Tether: Create IK Retargeter"));
...
FTetherIKRetargeterInfo UTetherRigLibrary::GetIKRetargeterInfo(const FString& AssetPath)
{
	using namespace TetherRigImpl;
	ClearError();
	FTetherIKRetargeterInfo Result;
...
bool UTetherRigLibrary::ConfigureIKRetargeterAssets(const FString& AssetPath,
	...
	const FString& SourcePreviewMeshPath, const FString& TargetPreviewMeshPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("ConfigureIKRetargeterAssets", "Tether: Configure IK Retargeter Assets"));
	Retargeter->Modify();
	Controller->SetIKRig(ERetargetSourceOrTarget::Source, SourceRig);
	Controller->SetIKRig(ERetargetSourceOrTarget::Target, TargetRig);
...
TArray<FTetherIKRetargetOpInfo> UTetherRigLibrary::ListIKRetargetOps(const FString& AssetPath)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherIKRetargetOpInfo> Result;
	UIKRetargeter* Retargeter = LoadRetargeter(AssetPath);
...
int32 UTetherRigLibrary::AddIKRetargetOp(
	const FString& AssetPath, const FString& OpTypePath, const FString& OpName)
{
	...
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	if (!Controller) return INDEX_NONE;
	FScopedTransaction Transaction(LOCTEXT("AddIKRetargetOp", "Tether: Add IK Retarget Op"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::AddDefaultIKRetargetOps(const FString& AssetPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddDefaultIKRetargetOps", "Tether: Add Default IK Retarget Ops"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::RemoveIKRetargetOp(const FString& AssetPath, int32 OpIndex)
{
	...
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("RemoveIKRetargetOp", "Tether: Remove IK Retarget Op"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::MoveIKRetargetOp(const FString& AssetPath, int32 OpIndex, int32 TargetIndex)
{
	...
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("MoveIKRetargetOp", "Tether: Move IK Retarget Op"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::SetIKRetargetOpEnabled(const FString& AssetPath, int32 OpIndex, bool bEnabled)
{
	...
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRetargetOpEnabled", "Tether: Set IK Retarget Op Enabled"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::SetIKRetargetOpParent(const FString& AssetPath,
	const FString& ChildOpName, const FString& ParentOpName)
{
	...
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRetargetOpParent", "Tether: Set IK Retarget Op Parent"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::AutoMapIKRetargetChains(const FString& AssetPath,
	const FString& MappingType, bool bForceRemap, const FString& OpName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AutoMapIKRetargetChains", "Tether: Auto Map IK Retarget Chains"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::SetIKRetargetChainMapping(const FString& AssetPath,
	const FString& TargetChainName, const FString& SourceChainName, const FString& OpName)
{
	...
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	if (!Controller) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRetargetChainMapping", "Tether: Set IK Retarget Chain Mapping"));
	Retargeter->Modify();
...
TArray<FTetherIKChainMappingInfo> UTetherRigLibrary::ListIKRetargetChainMappings(
	const FString& AssetPath, const FString& OpName)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherIKChainMappingInfo> Result;
	UIKRetargeter* Retargeter = LoadRetargeter(AssetPath);
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	if (!Controller) return Result;
	for (int32 Index = 0; Index < Controller->GetNumRetargetOps(); ++Index)
	{
		...
		for (const FRetargetChainPair& Pair : Mapping->GetChainPairs())
		{
			FTetherIKChainMappingInfo Info;
...
TArray<FTetherRigPropertyInfo> UTetherRigLibrary::ListIKRetargetOpProperties(
...
FTetherRigPropertyResult UTetherRigLibrary::GetIKRetargetOpProperty(
...
bool UTetherRigLibrary::SetIKRetargetOpProperty(const FString& AssetPath, int32 OpIndex,
	const FString& PropertyPath, const FString& Value)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetIKRetargetOpProperty", "Tether: Set IK Retarget Op Property"));
	Retargeter->Modify();
...
TArray<FTetherIKRetargetPoseInfo> UTetherRigLibrary::ListIKRetargetPoses(
	const FString& AssetPath, const FString& Side)
{
	using namespace TetherRigImpl;
	ClearError();
	TArray<FTetherIKRetargetPoseInfo> Result;
	UIKRetargeter* Retargeter = LoadRetargeter(AssetPath);
	UIKRetargeterController* Controller = Retargeter ? UIKRetargeterController::GetController(Retargeter) : nullptr;
	bool bValid = false;
	...
	{
		FTetherIKRetargetPoseInfo Info;
	...
	Result.Sort([](const FTetherIKRetargetPoseInfo& A, const FTetherIKRetargetPoseInfo& B) { return A.Name < B.Name; });
	return Result;
}

FString UTetherRigLibrary::CreateIKRetargetPose(
	const FString& AssetPath, const FString& PoseName, const FString& Side)
{
	...
	FScopedTransaction Transaction(LOCTEXT("CreateIKRetargetPose", "Tether: Create IK Retarget Pose"));
	Retargeter->Modify();
...
FString UTetherRigLibrary::DuplicateIKRetargetPose(const FString& AssetPath,
	const FString& PoseName, const FString& NewName, const FString& Side)
{
	...
	const ERetargetSourceOrTarget ParsedSide = ParseSide(Side, bValid);
	if (!Controller || !bValid) return FString();
	FScopedTransaction Transaction(LOCTEXT("DuplicateIKRetargetPose", "Tether: Duplicate IK Retarget Pose"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::RenameIKRetargetPose(const FString& AssetPath,
	const FString& PoseName, const FString& NewName, const FString& Side)
{
	using namespace TetherRigImpl;
	ClearError();
	...
	FScopedTransaction Transaction(LOCTEXT("RenameIKRetargetPose", "Tether: Rename IK Retarget Pose"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::RemoveIKRetargetPose(
	const FString& AssetPath, const FString& PoseName, const FString& Side)
{
	using namespace TetherRigImpl;
	ClearError();
	...
	const ERetargetSourceOrTarget ParsedSide = ParseSide(Side, bValid);
	if (!Controller || !bValid) return false;
	FScopedTransaction Transaction(LOCTEXT("RemoveIKRetargetPose", "Tether: Remove IK Retarget Pose"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::SetCurrentIKRetargetPose(
	const FString& AssetPath, const FString& PoseName, const FString& Side)
{
	using namespace TetherRigImpl;
	ClearError();
	...
	const ERetargetSourceOrTarget ParsedSide = ParseSide(Side, bValid);
	if (!Controller || !bValid) return false;
	FScopedTransaction Transaction(LOCTEXT("SetCurrentIKRetargetPose", "Tether: Set Current IK Retarget Pose"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::SetIKRetargetPoseBoneRotation(const FString& AssetPath,
	const FString& Side, const FString& BoneName, const FQuat& RotationOffset)
{
	...
	const ERetargetSourceOrTarget ParsedSide = ParseSide(Side, bValid);
	if (!Controller || !bValid) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRetargetPoseBoneRotation", "Tether: Set IK Retarget Pose Bone Rotation"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::SetIKRetargetPoseRootOffset(
	const FString& AssetPath, const FString& Side, const FVector& TranslationOffset)
{
	...
	const ERetargetSourceOrTarget ParsedSide = ParseSide(Side, bValid);
	if (!Controller || !bValid) return false;
	FScopedTransaction Transaction(LOCTEXT("SetIKRetargetPoseRootOffset", "Tether: Set IK Retarget Pose Root Offset"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::ResetIKRetargetPose(const FString& AssetPath, const FString& Side,
	const FString& PoseName, const TArray<FString>& BoneNames)
{
	...
	FScopedTransaction Transaction(LOCTEXT("ResetIKRetargetPose", "Tether: Reset IK Retarget Pose"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::AutoAlignIKRetargetPose(const FString& AssetPath, const FString& Side,
	const TArray<FString>& BoneNames, const FString& Method)
{
	...
	TArray<FName> Bones;
	for (const FString& Bone : BoneNames) Bones.Add(FName(*Bone));
	FScopedTransaction Transaction(LOCTEXT("AutoAlignIKRetargetPose", "Tether: Auto Align IK Retarget Pose"));
	Retargeter->Modify();
...
bool UTetherRigLibrary::SaveCurrentIKRetargetProfile(const FString& AssetPath,
	const FString& ProfileName, bool bApplySourcePose, bool bApplyTargetPose, bool bForceAllIKOff)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SaveCurrentIKRetargetProfile", "Tether: Save IK Retarget Profile"));
	Retargeter->Modify();
	FScriptMapHelper Helper(ProfilesProperty, ProfilesProperty->ContainerPtrToValuePtr<void>(Retargeter));
...
bool UTetherRigLibrary::RemoveIKRetargetProfile(const FString& AssetPath, const FString& ProfileName)
{
	...
	if (!Retargeter || !ProfilesProperty)
		return Retargeter ? SetError(TEXT("IK Retargeter profile storage is unavailable on this engine build")) : false;
	FScopedTransaction Transaction(LOCTEXT("RemoveIKRetargetProfile", "Tether: Remove IK Retarget Profile"));
	Retargeter->Modify();
	FScriptMapHelper Helper(ProfilesProperty, ProfilesProperty->ContainerPtrToValuePtr<void>(Retargeter));
	const FName KeyName(*ProfileName);
...
bool UTetherRigLibrary::SetCurrentIKRetargetProfile(const FString& AssetPath, const FString& ProfileName)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetCurrentIKRetargetProfile", "Tether: Set Current IK Retarget Profile"));
	Retargeter->Modify();
...
FTetherRetargetBatchResult UTetherRigLibrary::BatchRetargetAnimations(const FString& RetargeterPath,
	...
	const FString& Suffix, bool bIncludeReferencedAssets, bool bOverwriteExisting, bool bSave)
{
	using namespace TetherRigImpl;
	ClearError();
	FTetherRetargetBatchResult Result;
...
FTetherRigValidationReport UTetherRigLibrary::ValidateIKRetargeter(
...
FTetherAnimationQualityReport UTetherRigLibrary::AnalyzeAnimationQuality(const FString& AnimationPath,
	...
	float FootSlideSpeedTolerance, float JointAngularDeltaToleranceDegrees, int32 MaxReportedBones)
{
	using namespace TetherRigImpl;
	ClearError();
	FTetherAnimationQualityReport Report;
	...
	for (int32 BoneIndex = 0; BoneIndex < BoneCount; ++BoneIndex)
	{
		FTetherAnimationBoneMetric Metric;
	...
	Report.BoneMetrics.Sort([](const FTetherAnimationBoneMetric& A, const FTetherAnimationBoneMetric& B)
	{
