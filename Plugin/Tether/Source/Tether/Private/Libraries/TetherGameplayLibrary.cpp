#include "TetherGameplayLibrary.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogTetherAgent, Log, All);

namespace TetherAgentImpl
{
	// ─── Sticky EnhancedInput state ────────────────────────────────────
	// Persists across tether exec calls. The GameThread ticker re-injects
...
bool UTetherGameplayLibrary::GetAgentObservation(
	...
	const FString& ClassFilter)
{
	...
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	if (!World)
	{
		UE_LOG(LogTetherAgent, Verbose, TEXT("GetAgentObservation: no ready local-player PIE world"));
		return false;
	}
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	if (!Pawn)
	{
		UE_LOG(LogTetherAgent, Verbose, TEXT("GetAgentObservation: no player pawn"));
		return false;
	}
	...
	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(TetherAgentLOS), /*bTraceComplex=*/ false, Pawn);
...
bool UTetherGameplayLibrary::FindNavPath(
	...
	float& OutPathLength)
{
	...
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
	...
	if (!World)
	{
		UE_LOG(LogTetherAgent, Warning, TEXT("FindNavPath: no world available"));
		return false;
	}
	...
	if (!NavSys)
	{
		UE_LOG(LogTetherAgent, Warning, TEXT("FindNavPath: no navigation system in world"));
		return false;
	}
...
bool UTetherGameplayLibrary::ApplyMovementInput(const FVector& WorldDirection, float ScaleValue, bool bForce)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	if (!Pawn)
...
bool UTetherGameplayLibrary::ApplyLookInput(float YawDelta, float PitchDelta)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	if (!World)
	{
...
bool UTetherGameplayLibrary::SetControlRotation(const FRotator& NewRotation)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	if (!World)
	{
...
bool UTetherGameplayLibrary::Jump()
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
...
bool UTetherGameplayLibrary::StopJumping()
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	ACharacter* Char = Cast<ACharacter>(Pawn);
...
bool UTetherGameplayLibrary::InjectEnhancedInputAxis(const FString& InputActionPath, const FVector& AxisValue)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	if (!World)
	{
		UE_LOG(LogTetherAgent, Warning, TEXT("InjectEnhancedInputAxis: no ready local-player PIE world"));
		return false;
	}
	APlayerController* PC = World->GetFirstPlayerController();
	if (!PC || !PC->GetLocalPlayer())
	{
		UE_LOG(LogTetherAgent, Warning, TEXT("InjectEnhancedInputAxis: no local player"));
		return false;
	}
	...
	UE_LOG(LogTetherAgent, VeryVerbose,
...
bool UTetherGameplayLibrary::SetStickyInput(const FString& InputActionPath, const FVector& AxisValue)
{
	UInputAction* Action = LoadObject<UInputAction>(nullptr, *InputActionPath);
	if (!Action)
	{
	...
	TetherAgentImpl::FStickyEntry Entry;
	Entry.Action = Action;
	Entry.Value = AxisValue;
	Entry.World = World;
	Entry.PlayerController = PlayerController;
	Entry.LocalPlayer = LocalPlayer;
	TetherAgentImpl::GStickyInputs.Add(InputActionPath, Entry);
	TetherAgentImpl::EnsureStickyTickerRunning();
	UE_LOG(LogTetherAgent, Log, TEXT("SetStickyInput: %s = (%.2f, %.2f, %.2f)"),
		*InputActionPath, AxisValue.X, AxisValue.Y, AxisValue.Z);
	return true;
}

bool UTetherGameplayLibrary::ClearStickyInput(const FString& InputActionPath)
{
	if (InputActionPath.IsEmpty())
	{
		const int32 N = TetherAgentImpl::GStickyInputs.Num();
		TetherAgentImpl::GStickyInputs.Reset();
		UE_LOG(LogTetherAgent, Log, TEXT("ClearStickyInput: removed all %d entries"), N);
		TetherAgentImpl::StopStickyTickerIfIdle();
		return N > 0;
	}
	const int32 Removed = TetherAgentImpl::GStickyInputs.Remove(InputActionPath);
	TetherAgentImpl::StopStickyTickerIfIdle();
	return Removed > 0;
}
...
bool UTetherGameplayLibrary::GetInputActionTriggers(
	...
	TArray<float>& OutThresholdSeconds)
{
	...
	{
		OutTriggerNames.Add(TetherAgentImpl::TriggerShortName(Trigger));
		OutThresholdSeconds.Add(TetherAgentImpl::TriggerThresholdSeconds(Trigger));
	}
	return true;
}

bool UTetherGameplayLibrary::TriggerInputAction(const FString& InputActionPath, float HoldSeconds)
{
	UInputAction* Action = LoadObject<UInputAction>(nullptr, *InputActionPath);
	if (!Action)
	{
...
bool UTetherGameplayLibrary::GetControlRotation(FRotator& OutRotation)
{
...
bool UTetherGameplayLibrary::TeleportPawn(
...
int32 UTetherGameplayLibrary::GetStickyInputs(TArray<FString>& OutPaths, TArray<FVector>& OutValues)
{
	OutPaths.Reset();
	OutValues.Reset();
	OutPaths.Reserve(TetherAgentImpl::GStickyInputs.Num());
	OutValues.Reserve(TetherAgentImpl::GStickyInputs.Num());
	for (const TPair<FString, TetherAgentImpl::FStickyEntry>& Pair : TetherAgentImpl::GStickyInputs)
	{
...
bool UTetherGameplayLibrary::IsInPIE()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	return World != nullptr && TetherAgentImpl::GetPlayerPawn(World) != nullptr;
}
...
FString UTetherGameplayLibrary::GetCameraHitActor(float MaxDistance)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	FVector CamLoc; FRotator CamRot;
	if (!TetherAgentImpl::GetCameraView(World, CamLoc, CamRot))
	{
		return FString();
	}
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherCameraAim), /*bTraceComplex=*/ false);
...
bool UTetherGameplayLibrary::GetCameraHitLocation(float MaxDistance, FVector& OutHitLocation)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherCameraAimLoc), /*bTraceComplex=*/ false);
	if (Pawn) Params.AddIgnoredActor(Pawn);
	FHitResult Hit;
...
bool UTetherGameplayLibrary::IsActorVisibleFromCamera(const FString& ActorName, float MaxDistance)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	FVector CamLoc; FRotator CamRot;
	if (!TetherAgentImpl::GetCameraView(World, CamLoc, CamRot))
	...
	AActor* Target = TetherAgentImpl::FindPIEActor(World, ActorName);
	if (!Target)
	{
		return false;
	}
	...
	for (const FVector& P : SamplePoints)
	{
		...
		FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherCameraVis), /*bTraceComplex=*/ false);
		if (Pawn) Params.AddIgnoredActor(Pawn);
		FHitResult Hit;
...
float UTetherGameplayLibrary::GetPawnGroundHeight(float MaxDistance)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	if (!Pawn)
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherPawnGround), /*bTraceComplex=*/ false, Pawn);
	FHitResult Hit;
...
bool UTetherGameplayLibrary::ProjectPointToNavmesh(const FVector& Point, const FVector& SearchExtent, FVector& OutProjected)
{
	OutProjected = FVector::ZeroVector;
	UWorld* World = TetherAgentImpl::GetNavWorld();
	if (!World)
	{
		return false;
	}
...
bool UTetherGameplayLibrary::IsPointOnNavmesh(const FVector& Point, float Tolerance)
{
	FVector Dummy;
...
bool UTetherGameplayLibrary::GetNavMeshBounds(FVector& OutMin, FVector& OutMax)
{
...
bool UTetherGameplayLibrary::GetRandomReachablePointInRadius(const FVector& Origin, float Radius, FVector& OutPoint)
{
...
bool UTetherGameplayLibrary::GetPIEViewportSize(FVector2D& OutSize)
{
...
bool UTetherGameplayLibrary::DeprojectScreenToWorld(float NormalizedX, float NormalizedY, FVector& OutOrigin, FVector& OutDirection)
{
...
bool UTetherGameplayLibrary::ProjectWorldToScreen(const FVector& WorldLocation, FVector2D& OutNormalized)
{
...
float UTetherGameplayLibrary::GetCameraFOV()
{
	APlayerCameraManager* Cam = TetherAgentImpl::GetPIECameraManager();
	return Cam ? Cam->GetFOVAngle() : -1.0f;
}

bool UTetherGameplayLibrary::SetCameraFOV(float FOV)
{
...
bool UTetherGameplayLibrary::UnlockCameraFOV()
{
	APlayerCameraManager* Cam = TetherAgentImpl::GetPIECameraManager();
	if (!Cam)
	{
...
float UTetherGameplayLibrary::GetPawnMaxWalkSpeed()
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	UCharacterMovementComponent* Move = TetherAgentImpl::GetCharacterMovement(World);
	return Move ? Move->MaxWalkSpeed : -1.0f;
}

bool UTetherGameplayLibrary::SetPawnMaxWalkSpeed(float Speed)
{
	if (Speed < 0.0f)
	{
		return false;
...
bool UTetherGameplayLibrary::SetPawnGravityScale(float Scale)
{
	if (Scale < 0.0f)
	{
		return false;
...
float UTetherGameplayLibrary::GetPawnSpeed()
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	if (!Pawn)
...
bool UTetherGameplayLibrary::GetPawnCapabilities(
	...
	bool& bCanCrouch, bool& bCanJump)
{
	...
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	ACharacter* Char = Cast<ACharacter>(TetherAgentImpl::GetPlayerPawn(World));
	if (!Char)
	{
		return false;
	}
...
bool UTetherGameplayLibrary::SimulateJumpArc(
	...
	FVector& OutLandLocation, FString& OutLandActorLabel)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherJumpArc), /*bTraceComplex*/ true);
	// Exclude the pawn itself so the trace doesn't immediately hit our capsule.
	if (APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World))
	{
...
FString UTetherGameplayLibrary::SpawnActorInPIE(const FString& ClassPath, const FVector& Location, const FRotator& Rotation)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
	...
	UClass* Cls = TetherAgentImpl::LoadActorClass(ClassPath);
...
bool UTetherGameplayLibrary::DestroyActorInPIE(const FString& ActorName)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	AActor* Target = TetherAgentImpl::FindPIEActor(World, ActorName);
	if (!Target)
...
bool UTetherGameplayLibrary::GetPIEActorLocation(const FString& ActorName, FVector& OutLocation)
{
...
float UTetherGameplayLibrary::GetGlobalTimeDilation()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
...
bool UTetherGameplayLibrary::SetGlobalTimeDilation(float Scale)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
...
float UTetherGameplayLibrary::GetActorTimeDilation(const FString& ActorName)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	AActor* Target = TetherAgentImpl::FindPIEActor(World, ActorName);
	if (!Target)
...
bool UTetherGameplayLibrary::SetActorTimeDilation(const FString& ActorName, float Scale)
{
	if (Scale <= 0.0f)
	{
		return false;
...
bool UTetherGameplayLibrary::PlaySound2D(const FString& SoundAssetPath, float VolumeMultiplier, float PitchMultiplier)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
...
bool UTetherGameplayLibrary::PlaySoundAtLocation(const FString& SoundAssetPath, const FVector& Location, float VolumeMultiplier, float PitchMultiplier)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
...
float UTetherGameplayLibrary::ApplyDamageToActor(const FString& TargetActorName, float DamageAmount)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	AActor* Target = TetherAgentImpl::FindPIEActor(World, TargetActorName);
	if (!Target)
	{
		return -1.0f;
	}
...
TArray<FString> UTetherGameplayLibrary::GetAllPawns()
{
	TArray<FString> Out;
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
...
TArray<FString> UTetherGameplayLibrary::GetAIPawns()
{
	TArray<FString> Out;
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
...
FString UTetherGameplayLibrary::GetActorController(const FString& ActorName)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	AActor* A = TetherAgentImpl::FindPIEActor(World, ActorName);
...
bool UTetherGameplayLibrary::IsActorAIControlled(const FString& ActorName)
{
...
bool UTetherGameplayLibrary::AddOnScreenDebugMessage(const FString& Message, float DurationSeconds, float R, float G, float B)
{
...
bool UTetherGameplayLibrary::ClearOnScreenDebugMessages()
{
	if (!GEngine)
	{
		return false;
...
bool UTetherGameplayLibrary::DrawDebugLine(const FVector& Start, const FVector& End, float Thickness, float DurationSeconds)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
...
bool UTetherGameplayLibrary::DrawDebugSphereAt(const FVector& Center, float Radius, float Thickness, float DurationSeconds)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
...
bool UTetherGameplayLibrary::AddMappingContext(const FString& MappingContextPath, int32 Priority)
{
	UEnhancedInputLocalPlayerSubsystem* Sub = TetherAgentImpl::GetEILocalPlayerSub();
	if (!Sub)
	{
		return false;
	}
...
bool UTetherGameplayLibrary::RemoveMappingContext(const FString& MappingContextPath)
{
	UEnhancedInputLocalPlayerSubsystem* Sub = TetherAgentImpl::GetEILocalPlayerSub();
	if (!Sub)
	{
...
bool UTetherGameplayLibrary::IsMappingContextActive(const FString& MappingContextPath)
{
	UEnhancedInputLocalPlayerSubsystem* Sub = TetherAgentImpl::GetEILocalPlayerSub();
	if (!Sub)
	{
...
FString UTetherGameplayLibrary::GetInputActionValueType(const FString& InputActionPath)
{
...
bool UTetherGameplayLibrary::DrawDebugBoxAt(const FVector& Center, const FVector& Extent, float Thickness, float DurationSeconds)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return false;
...
bool UTetherGameplayLibrary::DrawDebugArrow(const FVector& Start, const FVector& End, float ArrowSize, float DurationSeconds)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return false;
...
bool UTetherGameplayLibrary::DrawDebugString(const FString& Text, const FVector& Location, float DurationSeconds)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
...
bool UTetherGameplayLibrary::FlushPersistentDebugDraws()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return false;
...
bool UTetherGameplayLibrary::GetPawnForwardVector(FVector& OutForward)
{
...
bool UTetherGameplayLibrary::GetPawnRightVector(FVector& OutRight)
{
...
bool UTetherGameplayLibrary::GetPawnUpVector(FVector& OutUp)
{
...
float UTetherGameplayLibrary::GetDistanceToPawn(const FVector& Location)
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	if (!Pawn) return -1.0f;
...
int64 UTetherGameplayLibrary::GetPIEFrameNumber()
{
	return static_cast<int64>(GFrameCounter);
}

float UTetherGameplayLibrary::GetPIEDeltaSeconds()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	return World ? World->GetDeltaSeconds() : 0.0f;
}

int32 UTetherGameplayLibrary::GetPIENumPlayers()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return 0;
...
int32 UTetherGameplayLibrary::GetPIENumAIControllers()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return 0;
	int32 Count = 0;
...
bool UTetherGameplayLibrary::AddImpulseToPIEActor(const FString& ActorName, const FVector& Impulse, bool bVelocityChange)
{
	UPrimitiveComponent* Prim = TetherAgentImpl::GetPIEPrimitive(ActorName);
	if (!Prim) return false;
	Prim->AddImpulse(Impulse, NAME_None, bVelocityChange);
	return true;
}

bool UTetherGameplayLibrary::AddForceToPIEActor(const FString& ActorName, const FVector& Force)
{
	UPrimitiveComponent* Prim = TetherAgentImpl::GetPIEPrimitive(ActorName);
	if (!Prim) return false;
	Prim->AddForce(Force);
...
bool UTetherGameplayLibrary::WakePIEActorPhysics(const FString& ActorName)
{
	UPrimitiveComponent* Prim = TetherAgentImpl::GetPIEPrimitive(ActorName);
	if (!Prim) return false;
...
bool UTetherGameplayLibrary::GetPIEActorLinearVelocity(const FString& ActorName, FVector& OutVelocity)
{
...
bool UTetherGameplayLibrary::StartCameraShake(const FString& ShakeClassPath, float Scale)
{
	APlayerCameraManager* Cam = TetherAgentImpl::GetPIECameraManager();
	if (!Cam) return false;
	TSubclassOf<UCameraShakeBase> Cls = TetherAgentImpl::LoadShakeClass(ShakeClassPath);
	if (!Cls) return false;
...
bool UTetherGameplayLibrary::StopCameraShakeByClass(const FString& ShakeClassPath, bool bImmediately)
{
	APlayerCameraManager* Cam = TetherAgentImpl::GetPIECameraManager();
	if (!Cam) return false;
	TSubclassOf<UCameraShakeBase> Cls = TetherAgentImpl::LoadShakeClass(ShakeClassPath);
...
bool UTetherGameplayLibrary::StopAllCameraShakes(bool bImmediately)
{
	APlayerCameraManager* Cam = TetherAgentImpl::GetPIECameraManager();
	if (!Cam) return false;
...
bool UTetherGameplayLibrary::PlayWorldCameraShake(const FString& ShakeClassPath, const FVector& Epicenter, float InnerRadius, float OuterRadius, float ScaleMultiplier)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return false;
	TSubclassOf<UCameraShakeBase> Cls = TetherAgentImpl::LoadShakeClass(ShakeClassPath);
...
FString UTetherGameplayLibrary::GetPlayerPawnActorName()
{
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
...
USkeletalMeshComponent* UTetherGameplayLibrary::GetPlayerSkeletalMeshComponent(const FString& ComponentName)
{
	return TetherAgentImpl::GetPlayerSkeletalMeshComponent(TetherAgentImpl::GetLocalPlayerPIEWorld(), ComponentName);
}

UAnimInstance* UTetherGameplayLibrary::GetPlayerAnimInstance(const FString& ComponentName)
{
...
FString UTetherGameplayLibrary::GetPlayerStartActorName()
{
	AActor* Start = TetherAgentImpl::FindPlayerStart();
	return Start ? Start->GetFName().ToString() : FString();
}

bool UTetherGameplayLibrary::GetPlayerStartTransform(FVector& OutLocation, FRotator& OutRotation)
{
	OutLocation = FVector::ZeroVector;
	OutRotation = FRotator::ZeroRotator;
	AActor* Start = TetherAgentImpl::FindPlayerStart();
...
bool UTetherGameplayLibrary::RespawnPlayerPawn()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	APawn* Pawn = TetherAgentImpl::GetPlayerPawn(World);
	if (!Pawn) return false;
...
bool UTetherGameplayLibrary::PauseGame(bool bPaused)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return false;
...
bool UTetherGameplayLibrary::IsGamePaused()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return false;
...
FString UTetherGameplayLibrary::GetGameModeClassName()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
...
FString UTetherGameplayLibrary::GetGameStateClassName()
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World) return FString();
...
int32 UTetherGameplayLibrary::ApplyRadialDamage(const FVector& Origin, float DamageAmount, float InnerRadius, float OuterRadius)
{
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
	{
...
TArray<FString> UTetherGameplayLibrary::FindPIEActorsByClass(const FString& ClassPath)
{
	TArray<FString> Out;
	UWorld* World = TetherAgentImpl::GetPIEWorld();
	if (!World)
...
bool UTetherGameplayLibrary::GetCameraViewPoint(FVector& OutLocation, FRotator& OutRotation)
{
	OutLocation = FVector::ZeroVector;
	OutRotation = FRotator::ZeroRotator;
	UWorld* World = TetherAgentImpl::GetLocalPlayerPIEWorld();
...
FString UTetherGameplayLibrary::GetActorAtScreenPosition(float NormalizedX, float NormalizedY, float MaxDistance)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherScreenHit), /*bTraceComplex=*/ false);
	if (Pawn) Params.AddIgnoredActor(Pawn);
	FHitResult Hit;
...
bool UTetherGameplayLibrary::PressKey(const FString& KeyName, bool bPressed)
{
	...
	if (!Key.IsValid())
	{
		UE_LOG(LogTetherAgent, Warning, TEXT("PressKey: invalid key name '%s'"), *KeyName);
		return false;
	}
...
namespace TetherInputAuthoringImpl
{
...
TArray<FString> UTetherGameplayLibrary::ListInputActions(
	const FString& ContentPathFilter, int32 MaxResults)
{
	return TetherInputAuthoringImpl::ListAssetsOfClass(
		UInputAction::StaticClass(), ContentPathFilter, MaxResults);
}

TArray<FString> UTetherGameplayLibrary::ListInputMappingContexts(
	const FString& ContentPathFilter, int32 MaxResults)
{
	return TetherInputAuthoringImpl::ListAssetsOfClass(
...
TArray<FTetherIMCMapping> UTetherGameplayLibrary::GetInputMappingContextMappings(
	const FString& MappingContextPath)
{
	TArray<FTetherIMCMapping> Result;
...
bool UTetherGameplayLibrary::AddIAMappingToIMC(
	const FString& MappingContextPath,
	const FString& InputActionPath,
	const FString& KeyName)
{
	...
	if (KeyName.IsEmpty())
	{
		UE_LOG(LogTetherAgent, Warning, TEXT("AddIAMappingToIMC: KeyName is empty"));
		return false;
	}
...
bool UTetherGameplayLibrary::RemoveIAMappingFromIMC(
	const FString& MappingContextPath,
	const FString& InputActionPath,
	const FString& KeyName)
{
...
FString UTetherGameplayLibrary::CreateInputAction(
	const FString& PackagePath, const FString& ValueType,
	const FString& Description, bool bSave)
{
	FString ObjectPath;
	UInputAction* IA = TetherInputAuthoringImpl::CreateInputAsset<UInputAction>(PackagePath, ObjectPath);
	if (!IA) return FString();

	IA->ValueType = TetherInputAuthoringImpl::ParseValueType(ValueType);
...
FString UTetherGameplayLibrary::CreateInputMappingContext(
	const FString& PackagePath, const FString& Description, bool bSave)
{
	FString ObjectPath;
	UInputMappingContext* IMC = TetherInputAuthoringImpl::CreateInputAsset<UInputMappingContext>(PackagePath, ObjectPath);
...
int32 UTetherGameplayLibrary::AddTriggerToIA(
	...
	const FString& ParamsJson, bool bSave)
{
	...
	UClass* Cls = TetherInputAuthoringImpl::ResolveSubclass(UInputTrigger::StaticClass(), TriggerClass);
	...
	if (!TetherInputAuthoringImpl::ApplyJsonParamsToObject(Instance, ParamsJson))
	{
...
int32 UTetherGameplayLibrary::AddModifierToIA(
	...
	const FString& ParamsJson, bool bSave)
{
	...
	UClass* Cls = TetherInputAuthoringImpl::ResolveSubclass(UInputModifier::StaticClass(), ModifierClass);
	if (!Cls || Cls->HasAnyClassFlags(CLASS_Abstract))
	{
		UE_LOG(LogTetherAgent, Warning,
	...
	TetherInputAuthoringImpl::ApplyJsonParamsToObject(Instance, ParamsJson);
...
bool UTetherGameplayLibrary::RemoveTriggerFromIA(
	const FString& InputActionPath, int32 Index, bool bSave)
{
	UInputAction* IA = LoadObject<UInputAction>(nullptr, *InputActionPath);
	if (!IA) return false;
	const int32 Norm = TetherInputAuthoringImpl::NormalizeIndex(Index, IA->Triggers.Num());
...
bool UTetherGameplayLibrary::RemoveModifierFromIA(
	const FString& InputActionPath, int32 Index, bool bSave)
{
	UInputAction* IA = LoadObject<UInputAction>(nullptr, *InputActionPath);
	if (!IA) return false;
	const int32 Norm = TetherInputAuthoringImpl::NormalizeIndex(Index, IA->Modifiers.Num());
	if (Norm == INDEX_NONE) return false;
...
TArray<FTetherInputComponentInstance> UTetherGameplayLibrary::GetInputActionTriggersFull(
	const FString& InputActionPath)
{
	TArray<FTetherInputComponentInstance> Out;
	UInputAction* IA = LoadObject<UInputAction>(nullptr, *InputActionPath);
	if (!IA) return Out;
	for (UInputTrigger* T : IA->Triggers)
	{
		if (!T) continue;
		FTetherInputComponentInstance E;
		E.ClassName = T->GetClass()->GetName();
		E.ClassPath = T->GetClass()->GetPathName();
		E.ParamsJson = TetherInputAuthoringImpl::SerializeUObjectToJson(T);
		Out.Add(MoveTemp(E));
	}
	return Out;
}

TArray<FTetherInputComponentInstance> UTetherGameplayLibrary::GetInputActionModifiersFull(
	const FString& InputActionPath)
{
	TArray<FTetherInputComponentInstance> Out;
	UInputAction* IA = LoadObject<UInputAction>(nullptr, *InputActionPath);
	...
	for (UInputModifier* M : IA->Modifiers)
	{
		...
		E.ParamsJson = TetherInputAuthoringImpl::SerializeUObjectToJson(M);
		Out.Add(MoveTemp(E));
	}
	return Out;
}

TArray<FString> UTetherGameplayLibrary::ListInputActionsByValueType(
	const FString& ContentPathFilter, const FString& ValueTypeFilter, int32 MaxResults)
{
	TArray<FString> All = TetherInputAuthoringImpl::ListAssetsOfClass(
	...
	const EInputActionValueType Want = TetherInputAuthoringImpl::ParseValueType(ValueTypeFilter);
	TArray<FString> Out;
...
bool UTetherGameplayLibrary::SetInputActionProperty(
	const FString& InputActionPath, const FString& PropertyName, const FString& JsonValue, bool bSave)
{
	...
	if (PropertyName == TEXT("ValueType"))
	{
		...
		IA->ValueType = TetherInputAuthoringImpl::ParseValueType(Trim);
	}
	else
	{
...
bool UTetherGameplayLibrary::SetIMCMappingPlayerMappableKeySettings(
	...
	const FString& PlayerMappableKeySettingsPath, bool bSave)
{
	...
	const int32 Idx = TetherInputAuthoringImpl::FindMappingIndex(IMC, IA, Key);
	...
	IMC->Modify();
	FEnhancedActionKeyMapping* M = TetherInputAuthoringImpl::GetMutableMapping(IMC, Idx);
	if (!M) return false;
...
FString UTetherGameplayLibrary::DuplicateInputAction(const FString& SourcePath, const FString& DestPath, bool bSave)
{
	return TetherInputAuthoringImpl::DuplicateAsset<UInputAction>(SourcePath, DestPath, bSave);
}

FString UTetherGameplayLibrary::DuplicateInputMappingContext(const FString& SourcePath, const FString& DestPath, bool bSave)
{
	return TetherInputAuthoringImpl::DuplicateAsset<UInputMappingContext>(SourcePath, DestPath, bSave);
}
...
int32 UTetherGameplayLibrary::AddTriggerToIMCMapping(
	const FString& MappingContextPath, const FString& InputActionPath, const FString& KeyName,
	const FString& TriggerClass, const FString& ParamsJson, bool bSave)
{
	...
	int32 MappingIdx;
	FEnhancedActionKeyMapping* M = TetherInputAuthoringImpl::ResolveMapping(IMC, IA, Key, MappingIdx);
	if (!M) return -1;
	...
	if (!Inst) return -1;
	TetherInputAuthoringImpl::ApplyJsonParamsToObject(Inst, ParamsJson);
...
int32 UTetherGameplayLibrary::AddModifierToIMCMapping(
	const FString& MappingContextPath, const FString& InputActionPath, const FString& KeyName,
...
bool UTetherGameplayLibrary::RemoveTriggerFromIMCMapping(
	const FString& MappingContextPath, const FString& InputActionPath, const FString& KeyName,
	int32 Index, bool bSave)
{
	...
	const int32 Norm = TetherInputAuthoringImpl::NormalizeIndex(Index, M->Triggers.Num());
	if (Norm == INDEX_NONE) return false;
...
bool UTetherGameplayLibrary::RemoveModifierFromIMCMapping(
	const FString& MappingContextPath, const FString& InputActionPath, const FString& KeyName,
	int32 Index, bool bSave)
{
	UInputMappingContext* IMC = LoadObject<UInputMappingContext>(nullptr, *MappingContextPath);
	...
	const int32 Norm = TetherInputAuthoringImpl::NormalizeIndex(Index, M->Modifiers.Num());
	if (Norm == INDEX_NONE) return false;
...
namespace TetherInputSearchImpl
{
...
TArray<FTetherInputReference> UTetherGameplayLibrary::FindInputActionReferences(
	const FString& InputActionPath, const FString& BlueprintPackagePathFilter)
{
	TArray<FTetherInputReference> Out;
	UInputAction* IA = LoadObject<UInputAction>(nullptr, *InputActionPath);
	if (!IA) return Out;
	...
	TetherInputSearchImpl::EnumerateBlueprints(BlueprintPackagePathFilter, BPs);
	for (UBlueprint* BP : BPs)
	{
		TetherInputSearchImpl::ForEachGraph(BP, [&](UEdGraph* G)
		{
...
TArray<FTetherInputReference> UTetherGameplayLibrary::FindInputMappingContextReferences(
...
namespace TetherInputRuntimeImpl
{
...
TArray<FTetherMappingContextEntry> UTetherGameplayLibrary::GetActiveMappingContextStack()
{
	TArray<FTetherMappingContextEntry> Out;
	UEnhancedInputLocalPlayerSubsystem* Sub = TetherInputRuntimeImpl::GetActiveSubsystem();
	if (!Sub) return Out;
	...
	for (const FAssetData& A : Data)
	{
		...
		{
			FTetherMappingContextEntry E;
			E.MappingContextPath = IMC->GetPathName();
			E.Priority = Priority;
			Out.Add(MoveTemp(E));
		}
	}
	Out.Sort([](const FTetherMappingContextEntry& A, const FTetherMappingContextEntry& B) { return A.Priority > B.Priority; });
	return Out;
}

FTetherInputActionState UTetherGameplayLibrary::GetCurrentInputActionState(const FString& InputActionPath)
{
	FTetherInputActionState Out;
...
int32 UTetherGameplayLibrary::DumpInjectedInputQueue(
...
bool UTetherGameplayLibrary::SimulateKeyEvent(const FString& KeyName, bool bPressed, int32 UserIndex)
{
	FKey Key(*KeyName);
...
TArray<FTetherLegacyAxisMapping> UTetherGameplayLibrary::ListLegacyAxisMappings()
{
	TArray<FTetherLegacyAxisMapping> Out;
	const UInputSettings* S = UInputSettings::GetInputSettings();
	if (!S) return Out;
	for (const FInputAxisKeyMapping& M : S->GetAxisMappings())
	{
		FTetherLegacyAxisMapping E;
...
TArray<FTetherLegacyActionMapping> UTetherGameplayLibrary::ListLegacyActionMappings()
{
	TArray<FTetherLegacyActionMapping> Out;
	const UInputSettings* S = UInputSettings::GetInputSettings();
	if (!S) return Out;
	for (const FInputActionKeyMapping& M : S->GetActionMappings())
	{
		FTetherLegacyActionMapping E;
...
bool UTetherGameplayLibrary::AddLegacyAxisMapping(const FString& MappingName, const FString& KeyName, float Scale)
{
...
bool UTetherGameplayLibrary::AddLegacyActionMapping(
...
bool UTetherGameplayLibrary::RemoveLegacyAxisMapping(const FString& MappingName, const FString& KeyName)
{
	UInputSettings* S = UInputSettings::GetInputSettings();
	if (!S) return false;
	FKey Key(*KeyName); if (!Key.IsValid()) return false;
...
bool UTetherGameplayLibrary::RemoveLegacyActionMapping(const FString& MappingName, const FString& KeyName)
{
	UInputSettings* S = UInputSettings::GetInputSettings();
	if (!S) return false;
	FKey Key(*KeyName); if (!Key.IsValid()) return false;
...
TArray<FTetherInputBindingIssue> UTetherGameplayLibrary::ValidateInputBindings(
	const FString& BlueprintPackagePathFilter)
{
	TArray<FTetherInputBindingIssue> Out;
...
TArray<FTetherKeyConflict> UTetherGameplayLibrary::DetectKeyConflicts(const TArray<FString>& MappingContextPaths)
{
	TArray<FTetherKeyConflict> Out;
