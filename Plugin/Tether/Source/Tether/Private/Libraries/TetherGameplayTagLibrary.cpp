#include "TetherGameplayTagLibrary.h"
...
namespace TetherGameplayTagOps
{
...
TArray<FString> UTetherGameplayTagLibrary::FindAssetsReferencingTag(
...
TArray<FString> UTetherGameplayTagLibrary::ListAllRegisteredTags(
...
FTetherTagSourceInfo UTetherGameplayTagLibrary::GetTagSourceInfo(const FString& TagString)
{
	FTetherTagSourceInfo Info;
	...
	{
		Info.SourceType = TetherGameplayTagOps::SourceTypeToString(PrimarySource->SourceType);
		Info.SourceLocation = TetherGameplayTagOps::ResolveSourceLocation(PrimarySource);
	}
	else
	{
		Info.SourceType = TEXT("Unknown");
...
TArray<FTetherTagSourceListing> UTetherGameplayTagLibrary::ListTagSourceInis(
	const FString& FilterType)
{
	TArray<FTetherTagSourceListing> Result;
	UGameplayTagsManager& TagsMgr = UGameplayTagsManager::Get();
	...
	{
		const FString TypeName = TetherGameplayTagOps::SourceTypeToString(Type);
		...
		{
			if (!Source) continue;
			FTetherTagSourceListing Entry;
			Entry.SourceName = Source->SourceName.ToString();
			Entry.SourceType = TypeName;
			Entry.ConfigFilePath = TetherGameplayTagOps::ResolveSourceLocation(Source);
...
bool UTetherGameplayTagLibrary::AddGameplayTag(
	...
	const FString& Comment, bool bIsRestricted)
{
	...
	if (bOk)
	{
		...
		{
			ResolvedSource = TetherGameplayTagOps::ResolveTagSourceName(NewTag);
		}
		TetherGameplayTagOps::EnsureSourceRedirectsPersisted(ResolvedSource);
	}

	return bOk;
}

bool UTetherGameplayTagLibrary::RenameGameplayTag(
	const FString& OldTag, const FString& NewTag, bool bRenameChildren)
{
	...
	if (bOk)
	{
		const FName SourceName = TetherGameplayTagOps::ResolveTagSourceName(NewTag);
		TetherGameplayTagOps::EnsureSourceRedirectsPersisted(SourceName);
	}

	return bOk;
}

bool UTetherGameplayTagLibrary::RemoveGameplayTag(const FString& TagString)
{
...
bool UTetherGameplayTagLibrary::RemoveGameplayTagRedirect(
	const FString& OldTag, const FString& NewTag)
{
	...
	// Restricted is included even though current tether mutations don't write
...
TArray<FTetherTagRedirectEntry> UTetherGameplayTagLibrary::ListGameplayTagRedirects(
	const FString& SourceIniFilter, const FString& OldTagPrefixFilter)
{
	TArray<FTetherTagRedirectEntry> Result;
	UGameplayTagsManager& TagsMgr = UGameplayTagsManager::Get();
	...
	Result.Sort([](const FTetherTagRedirectEntry& A, const FTetherTagRedirectEntry& B)
	{
