...
#define LOCTEXT_NAMESPACE "TetherPoseSearch"

namespace TetherPoseSearchImpl
{
	UPoseSearchSchema* LoadSchema(const FString& Path)
	{
		UPoseSearchSchema* PSS = LoadObject<UPoseSearchSchema>(nullptr, *Path);
		if (!PSS)
			UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load PoseSearchSchema '%s'"), *Path);
		return PSS;
	}
	...
	UPoseSearchDatabase* LoadDatabase(const FString& Path)
	{
		...
		if (!PSD)
			UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load PoseSearchDatabase '%s'"), *Path);
		return PSD;
	}
	...
	UObject* LoadAnyAnim(const FString& Path)
	{
		...
		if (!Asset)
			UE_LOG(LogTemp, Warning, TEXT("Tether: Could not load anim asset '%s'"), *Path);
		return Asset;
	}
...
{
	using namespace TetherPoseSearchImpl;
	FTetherPSSInfo Out;
...
FTetherPSSCrashingLegsChannelResult UTetherPoseSearchLibrary::SetCrashingLegsChannel(
...
{
	using namespace TetherPoseSearchImpl;
	FTetherPSSCrashingLegsChannelResult Out;
...
{
	using namespace TetherPoseSearchImpl;
	TArray<FTetherPSDAnimEntry> Out;
...
namespace TetherPoseSearchImpl
{
	...
	FTetherPSDAddResult MakeAddError(const FString& Msg)
	{
		FTetherPSDAddResult R;
		R.Index = -1;
		R.Error = Msg;
		UE_LOG(LogTemp, Warning, TEXT("Tether: %s"), *Msg);
		return R;
	}
}

FTetherPSDAddResult UTetherPoseSearchLibrary::AddAnimationToDatabase(const FString& DatabasePath, const FString& AnimationAssetPath,
...
FTetherPSDAddResult UTetherPoseSearchLibrary::AddBlendSpaceToDatabase(const FString& DatabasePath, const FString& BlendSpacePath,
...
bool UTetherPoseSearchLibrary::RemoveDatabaseAnimationAt(const FString& DatabasePath, int32 Index)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: RemoveDatabaseAnimationAt index %d out of range [0, %d)"),
...
bool UTetherPoseSearchLibrary::SetDatabaseAnimationEnabled(const FString& DatabasePath, int32 Index, bool bEnabled)
{
	...
	if (!E)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SetDatabaseAnimationEnabled index %d invalid"), Index);
		return false;
	}
...
bool UTetherPoseSearchLibrary::SetDatabaseAnimationSamplingRange(const FString& DatabasePath, int32 Index,
	float SamplingRangeMin, float SamplingRangeMax)
{
#if WITH_EDITORONLY_DATA
	using namespace TetherPoseSearchImpl;
...
bool UTetherPoseSearchLibrary::SetDatabaseBlendSpaceSampling(const FString& DatabasePath, int32 Index,
	int32 HSamples, int32 VSamples,
	bool bUseGridForSampling, bool bUseSingleSample,
	float BlendParamX, float BlendParamY)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: SetDatabaseBlendSpaceSampling entry %d is not a BlendSpace"), Index);
		return false;
	}
