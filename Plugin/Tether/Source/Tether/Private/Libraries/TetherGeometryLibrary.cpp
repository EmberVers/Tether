...
#define LOCTEXT_NAMESPACE "TetherGeometry"

namespace TetherGeometryImpl
{
	...
	UDynamicMesh* ResolveHandle(int32 Handle)
	{
		...
		if (!Found)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: handle %d is not registered"), Handle);
			return nullptr;
		}
		UDynamicMesh* Mesh = Found->Get();
		if (!Mesh)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: handle %d resolved to null (already collected)"), Handle);
			return nullptr;
		}
		return Mesh;
	}
	...
	// Mirror TetherLevelLibrary actor-lookup conventions: try FName first,
	...
	FGeometryScriptMeshSelection* ResolveSelection(int32 SelectionId)
	{
		...
		if (!Found)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: selection %d is not registered"), SelectionId);
		}
		return Found;
	}
	...
	bool ParseBooleanOp(const FString& Op, EGeometryScriptBooleanOperation& Out)
	{
		...
		UE_LOG(LogTemp, Warning,
			TEXT("Tether|Geometry: unknown boolean op '%s' (expected union / intersect / subtract)"),
			*Op);
		return false;
	}
...
{
	using namespace TetherGeometryImpl;
	UDynamicMesh* NewMesh = NewObject<UDynamicMesh>(GetTransientPackage());
	if (!NewMesh)
	{
		UE_LOG(LogTemp, Error, TEXT("Tether|Geometry: NewObject<UDynamicMesh> returned null"));
		return 0;
	}
	FHandlePool& Pool = GetPool();
...
bool UTetherGeometryLibrary::LoadMeshFromStaticMesh(int32 Handle, const FString& AssetPath, int32 Lod)
{
	...
	if (!Source)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: could not load StaticMesh '%s'"), *AssetPath);
		return false;
	}
...
bool UTetherGeometryLibrary::LoadMeshFromComponent(const FString& ActorLabel, const FString& ComponentName, int32 Handle)
{
	...
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: actor '%s' not found in editor world"), *ActorLabel);
		return false;
	}
	USceneComponent* Component = FindComponent(Actor, ComponentName);
	if (!Component)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: component '%s' not found on actor '%s'"),
...
FString UTetherGeometryLibrary::SaveMeshToNewStaticMesh(int32 Handle, const FString& NewAssetPath, const TArray<UMaterialInterface*>& MaterialList)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: SaveMeshToNewStaticMesh requires WITH_EDITOR"));
	return FString{};
#endif
}
...
bool UTetherGeometryLibrary::SaveMeshToExistingStaticMesh(int32 Handle, const FString& ExistingAssetPath, bool bReplaceMaterials)
{
	...
	if (!Target)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: could not load existing StaticMesh '%s'"), *ExistingAssetPath);
		return false;
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: SaveMeshToExistingStaticMesh requires WITH_EDITOR"));
	return false;
#endif
}
...
bool UTetherGeometryLibrary::MeshDisplaceFromTexture(int32 Handle, const FString& TexturePath, float Magnitude, int32 UVChannel)
{
	...
	if (!Tex)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: could not load Texture2D '%s'"), *TexturePath);
		return false;
	}
...
bool UTetherGeometryLibrary::MeshVoxelMerge(const TArray<int32>& Handles, float CellSizeCm)
{
	...
	if (Handles.Num() == 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: MeshVoxelMerge requires ≥1 handle"));
		return false;
	}
	...
	for (int32 i = 1; i < Handles.Num(); ++i)
	{
		...
		if (!Source)
		{
			UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: MeshVoxelMerge skipping invalid handle %d at index %d"), Handles[i], i);
			continue;
		}
...
int32 UTetherGeometryLibrary::SelectByNormalDirection(int32 Handle, FVector Normal, float MaxAngleDeg)
{
	...
	if (!Normal.Normalize())
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: SelectByNormalDirection received zero normal"));
		return 0;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: SelectByNormalDirection produced empty selection"));
		return 0;
	}
...
bool UTetherGeometryLibrary::SweepAlongSpline(
	int32 Handle,
	...
	int32 NumPathSamples)
{
	...
	if (ProfileXY.Num() < 3)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: SweepAlongSpline requires ≥3 profile vertices"));
		return false;
	}
	...
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: SweepAlongSpline could not find actor '%s'"), *ActorLabel);
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether|Geometry: SweepAlongSpline could not find spline component '%s' on actor '%s'"),
			*ComponentName, *ActorLabel);
		return false;
	}
	...
	if (TotalLength <= 0.f)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: SweepAlongSpline got zero-length spline"));
		return false;
	}
...
FString UTetherGeometryLibrary::BakeNormalsToTexture(int32 Handle, const FString& NewTexturePath, int32 Resolution)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: BakeNormalsToTexture produced no texture"));
		return FString{};
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: BakeNormalsToTexture requires WITH_EDITOR"));
	return FString{};
#endif
}
...
bool UTetherGeometryLibrary::MeshUVUnwrap(int32 Handle, const FString& Method)
{
	...
	if (!Bounds.IsValid)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether|Geometry: MeshUVUnwrap on empty mesh"));
		return false;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether|Geometry: unknown UV unwrap method '%s' (expected box / cylinder / plane)"),
			*Method);
		return false;
	}
