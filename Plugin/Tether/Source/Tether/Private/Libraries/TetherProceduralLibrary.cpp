#include "TetherProceduralLibrary.h"
...
namespace TetherProceduralImpl
{
	 ...
	 *  TetherLevelLibrary so callers can pass either form interchangeably. */
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsGrid(
	FBox Bounds, float Spacing, float JitterRatio, int32 Seed)
{
	...
	if (Total > TetherProceduralImpl::MaxPointCount)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsGrid would yield %lld points (cap %d) — refusing. "
				 "Increase Spacing or shrink Bounds."),
			Total, TetherProceduralImpl::MaxPointCount);
		return Out;
	}
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsOnSurface(
	const FString& ActorLabel, int32 Count, int32 Seed, float MaxBounceUp)
{
	...
	if (Count > TetherProceduralImpl::MaxPointCount)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnSurface Count=%d exceeds cap %d — refusing."),
			Count, TetherProceduralImpl::MaxPointCount);
		return Out;
	}

	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Target = TetherProceduralImpl::FindActor(World, ActorLabel);
	if (!Target)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnSurface — actor '%s' not found"), *ActorLabel);
		return Out;
	}
	...
	FRandomStream Rng(Seed);
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherSampleOnSurface), /*bTraceComplex=*/true);
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsOnLandscape(
	const FString& LandscapeLabel, FBox Bounds2D, int32 Count, int32 Seed)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnLandscape Count=%d exceeds cap %d — refusing."),
			Count, TetherProceduralImpl::MaxPointCount);
		return Out;
	}
	if (!Bounds2D.IsValid)
	...
	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Target = TetherProceduralImpl::FindActor(World, LandscapeLabel);
	ALandscapeProxy* RootProxy = Cast<ALandscapeProxy>(Target);
	if (!RootProxy)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnLandscape — '%s' not a Landscape actor"),
			*LandscapeLabel);
		return Out;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnLandscape — landscape '%s' has no LandscapeInfo "
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsPoissonDisk2D(
	FBox Bounds, float MinRadius, int32 MaxAttempts, int32 Seed)
{
	...
	if (GridCells > TetherProceduralImpl::MaxPointCount * 4)
	{
		// Worst-case grid memory grows with area / R²; for very small R relative
		// to bounds this becomes an OOM hazard. Refuse early.
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsPoissonDisk2D grid (%d × %d = %lld cells) too large; "
	...
	FRandomStream Rng(Seed);
	TArray<int32> Active;
	Out.Reserve(FMath::Min<int32>(static_cast<int32>(GridCells), TetherProceduralImpl::MaxPointCount));
	...
	while (Active.Num() > 0)
	{
		if (Out.Num() >= TetherProceduralImpl::MaxPointCount)
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: SamplePointsPoissonDisk2D hit cap %d, truncating."),
				TetherProceduralImpl::MaxPointCount);
			break;
		}
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsPoissonDisk3D(
	FBox Bounds, float MinRadius, int32 MaxAttempts, int32 Seed)
{
	TArray<FVector> Out;
	...
	if (Cells > static_cast<int64>(TetherProceduralImpl::MaxPointCount) * 4)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsPoissonDisk3D grid (%d × %d × %d = %lld cells) "
	...
	FRandomStream Rng(Seed);
	TArray<int32> Active;
	Out.Reserve(FMath::Min<int32>(static_cast<int32>(Cells), TetherProceduralImpl::MaxPointCount));
	...
	while (Active.Num() > 0)
	{
		...
		{
			UE_LOG(LogTemp, Warning,
				TEXT("Tether: SamplePointsPoissonDisk3D hit cap %d, truncating."),
				TetherProceduralImpl::MaxPointCount);
			break;
		}
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsOnSpline(
	...
	const FString& Mode, float CountOrSpacing)
{
	...
	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Actor = TetherProceduralImpl::FindActor(World, SplineActorLabel);
	USplineComponent* Spline = TetherProceduralImpl::FindSplineComponent(Actor, ComponentName);
	if (!Spline)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnSpline — spline '%s.%s' not found"),
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnSpline — bad Mode '%s' (expected 'by_count' or 'by_distance')"),
			*Mode);
		return Out;
	}

	if (N > TetherProceduralImpl::MaxPointCount)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsOnSpline N=%d exceeds cap %d — refusing."),
			N, TetherProceduralImpl::MaxPointCount);
		return Out;
	}
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsInVolume(
	const FString& VolumeActorLabel, int32 Count, int32 Seed, int32 MaxAttempts)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsInVolume Count=%d exceeds cap %d — refusing."),
			Count, TetherProceduralImpl::MaxPointCount);
		return Out;
	}
	if (MaxAttempts <= 0)
	...
	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Actor = TetherProceduralImpl::FindActor(World, VolumeActorLabel);
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsInVolume — actor '%s' not found"), *VolumeActorLabel);
		return Out;
	}
	...
	for (int32 attempt = 0; attempt < BudgetTotal && Out.Num() < Count; ++attempt)
	{
		...
		if (TetherProceduralImpl::IsPointInsideActor(Actor, P))
		{
			Out.Add(P);
		}
	}
...
TArray<FVector> UTetherProceduralLibrary::SamplePointsJitterStratified(
	FBox Bounds, int32 GridResolution, int32 Seed)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SamplePointsJitterStratified %lld points exceeds cap %d."),
			Total, TetherProceduralImpl::MaxPointCount);
		return Out;
	}
...
TArray<FTransform> UTetherProceduralLibrary::SampleTransformsAlongSpline(
	const FString& SplineActorLabel, const FString& ComponentName,
	const FString& Mode, float CountOrSpacing)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SampleTransformsAlongSpline — spline '%s.%s' not found"),
			*SplineActorLabel, *ComponentName);
		return Out;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: SampleTransformsAlongSpline N=%d exceeds cap %d."),
			N, TetherProceduralImpl::MaxPointCount);
		return Out;
	}
...
TArray<FTransform> UTetherProceduralLibrary::JitterTransforms(
	...
	FVector ScaleMin, FVector ScaleMax, int32 Seed)
{
	...
	for (const FTransform& X : Xs)
	{
		const FVector Pos = X.GetLocation()
			+ FVector(TetherProceduralImpl::Gauss(Rng) * PosSigma,
			          TetherProceduralImpl::Gauss(Rng) * PosSigma,
			          TetherProceduralImpl::Gauss(Rng) * PosSigma);

		FRotator R = X.Rotator();
		R.Pitch += TetherProceduralImpl::Gauss(Rng) * RotSigma;
		R.Yaw   += TetherProceduralImpl::Gauss(Rng) * RotSigma;
		R.Roll  += TetherProceduralImpl::Gauss(Rng) * RotSigma;
...
TArray<FVector> UTetherProceduralLibrary::FilterPointsBySlope(
	const TArray<FVector>& In, float MaxSlopeDeg, float BounceUp)
{
	...
	Out.Reserve(In.Num());
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherFilterBySlope), /*bTraceComplex=*/true);
...
TArray<FVector> UTetherProceduralLibrary::FilterPointsByMinDistance(
	const TArray<FVector>& In, float MinDist)
{
	...
	if (Cells > static_cast<int64>(TetherProceduralImpl::MaxPointCount) * 10)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FilterPointsByMinDistance grid (%d × %d = %lld cells) too large; "
...
TArray<FVector> UTetherProceduralLibrary::FilterPointsByOverlap(
	const TArray<FVector>& Pts, const TArray<FString>& BlockingClassPaths, float Radius)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherFilterByOverlap), /*bTraceComplex=*/false);
...
TArray<FVector> UTetherProceduralLibrary::FilterPointsByDensityMask(
	...
	int32 ChannelIndex, float Threshold, int32 Seed)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FilterPointsByDensityMask — texture '%s' failed to load"),
			*TextureAsset);
		return Out;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FilterPointsByDensityMask — texture '%s' has CompressionSettings=%d "
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FilterPointsByDensityMask — mip 0 BulkData lock failed for '%s'"),
			*TextureAsset);
		return Out;
	}
...
TArray<FVector> UTetherProceduralLibrary::FilterPointsInsideActor(
	const TArray<FVector>& Pts, const FString& ContainerActorLabel, bool bInside)
{
	...
	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Container = TetherProceduralImpl::FindActor(World, ContainerActorLabel);
	if (!Container)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FilterPointsInsideActor — actor '%s' not found"),
			*ContainerActorLabel);
		return Out;
	}
	...
	for (const FVector& P : Pts)
	{
		const bool bIsInside = TetherProceduralImpl::IsPointInsideActor(Container, P);
...
TArray<FVector> UTetherProceduralLibrary::FilterPointsByLandscapeLayer(
	const TArray<FVector>& Pts, const FString& LandscapeLabel, FName LayerName, float Threshold)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FilterPointsByLandscapeLayer — '%s' not a Landscape actor"),
			*LandscapeLabel);
		return Out;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: FilterPointsByLandscapeLayer — no LandscapeInfo for '%s'"),
			*LandscapeLabel);
		return Out;
	}
...
TArray<FVector> UTetherProceduralLibrary::ProjectPointsToSurface(
	const TArray<FVector>& In, float BounceUp, float BounceDown, TArray<FVector>& OutHitNormals)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(TetherProjectToSurface), /*bTraceComplex=*/true);
...
FString UTetherProceduralLibrary::EnsureProceduralISMActor(
	const FString& Tag, const FString& MeshPath, bool bUseHISM)
{
	...
	if (TagName.IsNone())
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: EnsureProceduralISMActor — empty Tag refused"));
		return FString();
	}
	...
	if (AActor* Existing = TetherProceduralImpl::FindFirstActorWithTag(World, TagName))
	{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: EnsureProceduralISMActor — mesh '%s' failed to load"), *MeshPath);
		return FString();
	}
	...
	if (!New)
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether: EnsureProceduralISMActor — SpawnActor failed"));
		return FString();
	}
	New->Modify();
...
TArray<int32> UTetherProceduralLibrary::AddInstancesByTransforms(
	const FString& ActorName, const TArray<FTransform>& Xs, bool bWorldSpace)
{
	...
	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Actor = TetherProceduralImpl::FindActor(World, ActorName);
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddInstancesByTransforms — actor '%s' not found"), *ActorName);
		return Out;
	}

	UInstancedStaticMeshComponent* ISMC = TetherProceduralImpl::FindISMComponent(Actor);
	if (!ISMC)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: AddInstancesByTransforms — actor '%s' has no [H]ISMC"), *ActorName);
		return Out;
	}
...
bool UTetherProceduralLibrary::ClearInstances(const FString& ActorName)
{
	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Actor = TetherProceduralImpl::FindActor(World, ActorName);
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: ClearInstances — actor '%s' not found"), *ActorName);
		return false;
	}
...
bool UTetherProceduralLibrary::RemoveInstancesByIds(
...
bool UTetherProceduralLibrary::UpdateInstanceTransformsByIds(
	...
	const TArray<FTransform>& NewXs, bool bWorldSpace)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: UpdateInstanceTransformsByIds — Ids (%d) and NewXs (%d) length mismatch"),
			Ids.Num(), NewXs.Num());
		return false;
	}
...
bool UTetherProceduralLibrary::RebuildProceduralNavigation(const FString& ActorName)
{
	UWorld* World = TetherProceduralImpl::GetEditorWorld();
	AActor* Actor = TetherProceduralImpl::FindActor(World, ActorName);
	if (!Actor)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether: RebuildProceduralNavigation — actor '%s' not found"), *ActorName);
		return false;
	}
