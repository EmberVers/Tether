...
#define LOCTEXT_NAMESPACE "TetherStateTree"

namespace TetherStateTreeImpl
{
...
{
	LastError = Message;
	UE_LOG(LogTemp, Warning, TEXT("Tether StateTree: %s"), *Message);
	return false;
}
...
static FTetherStateTreeNodeInfo MakeNodeInfo(const FStateTreeEditorNode& Node, const FString& Scope,
	const FGuid& OwnerId, int32 Index)
{
	FTetherStateTreeNodeInfo Info;
...
static FTetherStateTreePropertyResult ReadProperty(FPropertyBindingDataView View, const FString& PropertyPath, UObject* Owner)
{
	FTetherStateTreePropertyResult Result;
...
{
	return TetherStateTreeImpl::LastError;
}

FTetherStateTreeCreateResult UTetherStateTreeLibrary::CreateStateTree(
	const FString& AssetPath, const FString& SchemaClassPath)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	FTetherStateTreeCreateResult Result;
...
FTetherStateTreeAssetInfo UTetherStateTreeLibrary::GetStateTreeInfo(const FString& AssetPath)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	FTetherStateTreeAssetInfo Info;
...
bool UTetherStateTreeLibrary::ValidateStateTree(const FString& AssetPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("ValidateStateTree", "Tether: Validate StateTree"));
	Tree->Modify();
...
FTetherStateTreeCompileResult UTetherStateTreeLibrary::CompileStateTree(
	const FString& AssetPath, bool bRunValidation)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	FTetherStateTreeCompileResult Result;
	UStateTree* Tree = LoadStateTree(AssetPath);
	if (!Tree || !GetEditorData(Tree))
	{
		Result.Error = LastError;
	...
	{
		FTetherStateTreeCompileMessage Out;
...
TArray<FTetherStateTreeStateInfo> UTetherStateTreeLibrary::ListStateTreeStates(const FString& AssetPath)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeStateInfo> Result;
	...
	EditorData->VisitHierarchy([&](UStateTreeState& State, UStateTreeState* Parent)
	{
		...
		FTetherStateTreeStateInfo Info;
...
FString UTetherStateTreeLibrary::AddStateTreeState(const FString& AssetPath,
	const FString& ParentStateId, const FString& Name, const FString& StateType, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddStateTreeState", "Tether: Add StateTree State"));
	Tree->Modify();
	EditorData->Modify();
...
bool UTetherStateTreeLibrary::RemoveStateTreeState(const FString& AssetPath, const FString& StateId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveStateTreeState", "Tether: Remove StateTree State"));
	Tree->Modify();
	State->Modify();
...
bool UTetherStateTreeLibrary::MoveStateTreeState(const FString& AssetPath, const FString& StateId,
	const FString& NewParentStateId, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("MoveStateTreeState", "Tether: Move StateTree State"));
	Tree->Modify();
	EditorData->Modify();
	State->Modify();
...
bool UTetherStateTreeLibrary::SetStateTreeStateType(const FString& AssetPath,
	const FString& StateId, const FString& StateType)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeStateType", "Tether: Set StateTree State Type"));
	Tree->Modify();
	State->Modify();
	State->Type = Type;
...
FTetherStateTreePropertyResult UTetherStateTreeLibrary::GetStateTreeStateProperty(
...
bool UTetherStateTreeLibrary::SetStateTreeStateProperty(const FString& AssetPath,
	const FString& StateId, const FString& PropertyPath, const FString& Value)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeStateProperty", "Tether: Set StateTree State Property"));
	Tree->Modify();
...
bool UTetherStateTreeLibrary::SetStateTreeLinkedState(const FString& AssetPath,
	const FString& StateId, const FString& LinkedStateId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeLinkedState", "Tether: Set Linked State"));
	Tree->Modify();
	State->Modify();
...
bool UTetherStateTreeLibrary::SetStateTreeLinkedAsset(const FString& AssetPath,
	const FString& StateId, const FString& LinkedAssetPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeLinkedAsset", "Tether: Set Linked StateTree Asset"));
	Tree->Modify();
	State->Modify();
	if (Linked)
	{
...
static void GatherNodeTypes(FStateTreeNodeClassCache& Cache, UStateTreeEditorData* EditorData,
	...
	bool bIncludeDisallowed, TSet<FString>& Seen, TArray<FTetherStateTreeNodeTypeInfo>& Out)
{
	auto AddData = [&](const TArray<TSharedPtr<FStateTreeNodeClassData>>& Data)
	{
		for (const TSharedPtr<FStateTreeNodeClassData>& Entry : Data)
		{
			...
			Seen.Add(Path);
			FTetherStateTreeNodeTypeInfo Info;
			Info.Kind = Kind;
			Info.TypePath = Path;
...
TArray<FTetherStateTreeNodeTypeInfo> UTetherStateTreeLibrary::ListStateTreeNodeTypes(
	const FString& AssetPath, const FString& Kind, bool bIncludeDisallowed)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeNodeTypeInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	if (!EditorData)
	{
		return Result;
	...
	Result.Sort([](const FTetherStateTreeNodeTypeInfo& A, const FTetherStateTreeNodeTypeInfo& B)
	{
...
TArray<FTetherStateTreeNodeInfo> UTetherStateTreeLibrary::ListStateTreeNodes(
	const FString& AssetPath, const FString& Scope)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeNodeInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	if (!EditorData)
	{
		return Result;
...
TArray<FTetherStateTreePropertyInfo> UTetherStateTreeLibrary::ListStateTreeNodeProperties(
	const FString& AssetPath, const FString& NodeId, const FString& DataSource, bool bIncludeInherited)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreePropertyInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	FGuid Guid;
	...
	for (TFieldIterator<FProperty> It(Struct, Flags); It; ++It)
	{
		...
		FTetherStateTreePropertyInfo Info;
...
FString UTetherStateTreeLibrary::AddStateTreeNode(const FString& AssetPath, const FString& OwnerId,
	const FString& Scope, const FString& TypePath, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddStateTreeNode", "Tether: Add StateTree Node"));
	Tree->Modify();
	EditorData->Modify();
...
bool UTetherStateTreeLibrary::RemoveStateTreeNode(const FString& AssetPath, const FString& NodeId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveStateTreeNode", "Tether: Remove StateTree Node"));
	Tree->Modify();
	EditorData->Modify();
...
bool UTetherStateTreeLibrary::MoveStateTreeNode(const FString& AssetPath,
	const FString& NodeId, int32 NewIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("MoveStateTreeNode", "Tether: Move StateTree Node"));
	Tree->Modify();
	EditorData->Modify();
	if (Handle.OwnerState) Handle.OwnerState->Modify();
...
FTetherStateTreePropertyResult UTetherStateTreeLibrary::GetStateTreeNodeProperty(
...
bool UTetherStateTreeLibrary::SetStateTreeNodeProperty(const FString& AssetPath,
	const FString& NodeId, const FString& DataSource, const FString& PropertyPath, const FString& Value)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeNodeProperty", "Tether: Set StateTree Node Property"));
	Tree->Modify();
	EditorData->Modify();
	if (Handle.OwnerState) Handle.OwnerState->Modify();
...
TArray<FTetherStateTreeTransitionInfo> UTetherStateTreeLibrary::ListStateTreeTransitions(
	const FString& AssetPath, const FString& StateId)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeTransitionInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	...
	EditorData->VisitHierarchy([&](UStateTreeState& State, UStateTreeState*)
	{
		...
		for (int32 Index = 0; Index < State.Transitions.Num(); ++Index)
		{
			...
			FTetherStateTreeTransitionInfo Info;
...
FString UTetherStateTreeLibrary::AddStateTreeTransition(const FString& AssetPath,
	...
	const FString& TargetStateId, const FString& RequiredEventTag, int32 InsertIndex)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddStateTreeTransition", "Tether: Add StateTree Transition"));
	Tree->Modify();
	State->Modify();
...
bool UTetherStateTreeLibrary::RemoveStateTreeTransition(const FString& AssetPath,
	const FString& TransitionId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveStateTreeTransition", "Tether: Remove StateTree Transition"));
	Tree->Modify();
...
bool UTetherStateTreeLibrary::MoveStateTreeTransition(const FString& AssetPath,
	const FString& TransitionId, int32 NewIndex)
{
	...
	if (TargetIndex == Handle.Index) return true;
	FScopedTransaction Transaction(LOCTEXT("MoveStateTreeTransition", "Tether: Move StateTree Transition"));
	Tree->Modify();
	Handle.OwnerState->Modify();
...
FTetherStateTreePropertyResult UTetherStateTreeLibrary::GetStateTreeTransitionProperty(
...
bool UTetherStateTreeLibrary::SetStateTreeTransitionProperty(const FString& AssetPath,
	const FString& TransitionId, const FString& PropertyPath, const FString& Value)
{
	...
	if (PropertyKey == TEXT("id") || PropertyKey == TEXT("conditions") || PropertyKey == TEXT("state"))
		return SetError(FString::Printf(TEXT("transition property '%s' is structural; use the dedicated StateTree API"),
			*PropertyPath));
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeTransitionProperty", "Tether: Set StateTree Transition Property"));
	Tree->Modify();
	Handle.OwnerState->Modify();
...
bool UTetherStateTreeLibrary::SetStateTreeTransitionTarget(const FString& AssetPath,
	const FString& TransitionId, const FString& TargetType, const FString& TargetStateId)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeTransitionTarget", "Tether: Set StateTree Transition Target"));
	Tree->Modify();
	Handle.OwnerState->Modify();
...
TArray<FTetherStateTreeBindingInfo> UTetherStateTreeLibrary::ListStateTreeBindings(
	const FString& AssetPath)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeBindingInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	if (!EditorData) return Result;
	for (const FStateTreePropertyPathBinding& Binding : EditorData->EditorBindings.GetBindings())
	{
		FTetherStateTreeBindingInfo Info;
...
TArray<FTetherStateTreeBindableStructInfo> UTetherStateTreeLibrary::ListStateTreeBindableStructs(
	const FString& AssetPath, const FString& TargetId)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeBindableStructInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	if (!EditorData) return Result;
	FGuid TargetGuid;
	...
	for (const TInstancedStruct<FPropertyBindingBindableStructDescriptor>& Instanced : Descriptors)
	{
		...
		FTetherStateTreeBindableStructInfo Info;
...
bool UTetherStateTreeLibrary::AddStateTreeBinding(const FString& AssetPath,
	...
	const FString& TargetPath, bool bOutputBinding)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddStateTreeBinding", "Tether: Add StateTree Binding"));
	Tree->Modify();
	EditorData->Modify();
	if (bOutputBinding)
	{
...
bool UTetherStateTreeLibrary::RemoveStateTreeBinding(const FString& AssetPath,
	const FString& TargetId, const FString& TargetPath)
{
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveStateTreeBinding", "Tether: Remove StateTree Binding"));
	Tree->Modify();
	EditorData->Modify();
...
int32 UTetherStateTreeLibrary::ClearStateTreeBindingsForItem(const FString& AssetPath,
	const FString& ItemId)
{
	...
	const int32 Before = EditorData->EditorBindings.GetBindings().Num();
	FScopedTransaction Transaction(LOCTEXT("ClearStateTreeBindingsForItem", "Tether: Clear StateTree Bindings"));
	Tree->Modify();
	EditorData->Modify();
...
TArray<FTetherStateTreeParameterInfo> UTetherStateTreeLibrary::ListStateTreeParameters(
	const FString& AssetPath, const FString& ScopeId)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeParameterInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	if (!EditorData) return Result;
	...
	{
		FTetherStateTreeParameterInfo Info;
		Info.Id = GuidToString(Desc.ID);
		Info.Name = Desc.Name.ToString();
...
FString UTetherStateTreeLibrary::AddStateTreeRootParameter(const FString& AssetPath,
	const FString& Name, const FString& Type, const FString& DefaultValue)
{
	...
	FScopedTransaction Transaction(LOCTEXT("AddStateTreeRootParameter", "Tether: Add StateTree Root Parameter"));
	Tree->Modify();
	EditorData->Modify();
...
bool UTetherStateTreeLibrary::RemoveStateTreeRootParameter(const FString& AssetPath,
	const FString& Name)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	...
	FScopedTransaction Transaction(LOCTEXT("RemoveStateTreeRootParameter", "Tether: Remove StateTree Root Parameter"));
	Tree->Modify();
	EditorData->Modify();
...
bool UTetherStateTreeLibrary::RenameStateTreeRootParameter(const FString& AssetPath,
	const FString& OldName, const FString& NewName)
{
	...
	FInstancedPropertyBag& Bag = GetMutableRootBag(EditorData);
	FScopedTransaction Transaction(LOCTEXT("RenameStateTreeRootParameter", "Tether: Rename StateTree Root Parameter"));
	Tree->Modify();
	EditorData->Modify();
...
bool UTetherStateTreeLibrary::SetStateTreeParameterValue(const FString& AssetPath,
	const FString& ScopeId, const FString& Name, const FString& Value, bool bMarkOverridden)
{
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeParameterValue", "Tether: Set StateTree Parameter"));
	Tree->Modify();
	EditorData->Modify();
...
static FTetherStateTreeComponentInfo MakeComponentInfo(UStateTreeComponent* Component)
{
	FTetherStateTreeComponentInfo Info;
...
TArray<FTetherStateTreeBreakpointInfo> UTetherStateTreeLibrary::ListStateTreeBreakpoints(
	const FString& AssetPath)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeBreakpointInfo> Result;
	UStateTreeEditorData* EditorData = GetEditorData(LoadStateTree(AssetPath));
	if (!EditorData) return Result;
	for (const FStateTreeEditorBreakpoint& Breakpoint : EditorData->Breakpoints)
	{
		FTetherStateTreeBreakpointInfo Info;
...
bool UTetherStateTreeLibrary::SetStateTreeBreakpoint(const FString& AssetPath,
...
TArray<FTetherStateTreeComponentInfo> UTetherStateTreeLibrary::ListStateTreeComponents(bool bPIEOnly)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	TArray<FTetherStateTreeComponentInfo> Result;
	for (TObjectIterator<UStateTreeComponent> It; It; ++It)
	{
	...
	Result.Sort([](const FTetherStateTreeComponentInfo& A, const FTetherStateTreeComponentInfo& B)
	{
...
FTetherStateTreeComponentInfo UTetherStateTreeLibrary::GetStateTreeComponentInfo(
...
bool UTetherStateTreeLibrary::SetStateTreeComponentAsset(const FString& ComponentPath,
	const FString& AssetPath)
{
	using namespace TetherStateTreeImpl;
	ClearError();
	...
	FScopedTransaction Transaction(LOCTEXT("SetStateTreeComponentAsset", "Tether: Set StateTree Component Asset"));
	Component->Modify();
...
bool UTetherStateTreeLibrary::ControlStateTreeComponent(const FString& ComponentPath,
...
bool UTetherStateTreeLibrary::SendStateTreeComponentEvent(const FString& ComponentPath,
