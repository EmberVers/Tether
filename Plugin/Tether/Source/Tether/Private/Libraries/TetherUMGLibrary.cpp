...
namespace TetherUMGImpl
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Tether UMG.%s: %s"), FunctionName, *Message);
	}

	static void LogMVVMUnavailable(const TCHAR* FunctionName)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("Tether UMG.%s: MVVM authoring/runtime support requires UE 5.7+; this call is a safe no-op on the current engine"),
			FunctionName);
	}
	...
	static void GatherWidgets(UWidget* Widget, const FString& ParentName, TArray<FTetherWidgetInfo>& Out)
	{
		if (!Widget)
		{
			return;
		}
		FTetherWidgetInfo Info;
	...
	static void GatherLiveWidgets(UWidget* Widget, const FString& ParentName, TArray<FTetherLiveWidgetInfo>& Out)
	{
		if (!Widget)
		{
			return;
	...
	static TArray<FTetherWidgetPropertyValue> GatherDifferentProperties(UObject* Object)
	{
		TArray<FTetherWidgetPropertyValue> Result;
		if (!Object)
		{
			return Result;
		}
		...
		for (TFieldIterator<FProperty> It(Object->GetClass()); It; ++It)
		{
			...
			FTetherWidgetPropertyValue Info;
		...
		Result.Sort([](const FTetherWidgetPropertyValue& A, const FTetherWidgetPropertyValue& B)
		{
	...
	static void AddIssue(FTetherWidgetValidationReport& Report, const FString& Severity,
		const FString& Code, const FString& WidgetName, const FString& Message)
	{
		FTetherWidgetValidationIssue Issue;
...
TArray<FTetherWidgetClassInfo> UTetherUMGLibrary::ListWidgetClasses(
	const FString& Query, bool bIncludeAbstract, int32 MaxResults)
{
	TArray<FTetherWidgetClassInfo> Result;
	for (TObjectIterator<UClass> It; It; ++It)
	{
		UClass* Class = *It;
	...
	Result.Sort([](const FTetherWidgetClassInfo& A, const FTetherWidgetClassInfo& B)
	{
...
FTetherWidgetOperationResult UTetherUMGLibrary::CreateWidgetBlueprint(
	const FString& AssetPath, const FString& ParentClassPath)
{
	using namespace TetherUMGImpl;
	FTetherWidgetOperationResult Result;
	FString PackagePath;
	FString AssetName;
...
FTetherWidgetOperationResult UTetherUMGLibrary::AddWidget(
	...
	const FString& WidgetName, const FString& ParentName, int32 InsertIndex)
{
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Add Widget")));
	WBP->Modify();
...
bool UTetherUMGLibrary::RemoveWidget(const FString& WidgetBlueprintPath, const FString& WidgetName)
{
	using namespace TetherUMGImpl;
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Remove Widget")));
	WBP->Modify();
	WBP->WidgetTree->Modify();
...
bool UTetherUMGLibrary::RenameWidget(
	const FString& WidgetBlueprintPath, const FString& WidgetName, const FString& NewName)
{
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Rename Widget")));
	WBP->Modify();
	Widget->Modify();
...
bool UTetherUMGLibrary::ReparentWidget(
	...
	const FString& NewParentName, int32 InsertIndex)
{
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Reparent Widget")));
	WBP->Modify();
	Widget->Modify();
...
bool UTetherUMGLibrary::SetWidgetIsVariable(
...
TArray<FTetherWidgetInfo> UTetherUMGLibrary::GetWidgetTree(const FString& WidgetBlueprintPath)
{
	TArray<FTetherWidgetInfo> Result;
	if (UWidgetBlueprint* WBP = TetherUMGImpl::LoadWBP(WidgetBlueprintPath, TEXT("get_widget_tree")))
	{
		if (WBP->WidgetTree) TetherUMGImpl::GatherWidgets(WBP->WidgetTree->RootWidget, FString(), Result);
	}
	return Result;
}

TArray<FTetherWidgetPropertyValue> UTetherUMGLibrary::GetWidgetProperties(
	const FString& WidgetBlueprintPath, const FString& WidgetName)
{
	UWidgetBlueprint* WBP = TetherUMGImpl::LoadWBP(WidgetBlueprintPath, TEXT("get_widget_properties"));
	return TetherUMGImpl::GatherDifferentProperties(TetherUMGImpl::FindWidgetByName(WBP, WidgetName));
}

TArray<FTetherWidgetPropertyValue> UTetherUMGLibrary::GetWidgetSlotProperties(
	const FString& WidgetBlueprintPath, const FString& WidgetName)
{
	UWidgetBlueprint* WBP = TetherUMGImpl::LoadWBP(WidgetBlueprintPath, TEXT("get_widget_slot_properties"));
	UWidget* Widget = TetherUMGImpl::FindWidgetByName(WBP, WidgetName);
	return TetherUMGImpl::GatherDifferentProperties(Widget ? Widget->Slot : nullptr);
}

TArray<FTetherWidgetInfo> UTetherUMGLibrary::SearchWidgets(
	const FString& WidgetBlueprintPath, const FString& Query)
{
	TArray<FTetherWidgetInfo> Result;
	for (const FTetherWidgetInfo& Info : GetWidgetTree(WidgetBlueprintPath))
	{
...
bool UTetherUMGLibrary::SetWidgetProperty(
	const FString& WidgetBlueprintPath, const FString& WidgetName,
...
bool UTetherUMGLibrary::SetWidgetSlotProperty(
	const FString& WidgetBlueprintPath, const FString& WidgetName,
	const FString& PropertyName, const FString& Value)
{
	using namespace TetherUMGImpl;
...
bool UTetherUMGLibrary::SetCanvasSlotLayout(
	const FString& WidgetBlueprintPath, const FString& WidgetName,
...
bool UTetherUMGLibrary::SetWidgetBrush(
	const FString& WidgetBlueprintPath, const FString& WidgetName,
...
FTetherWidgetValidationReport UTetherUMGLibrary::CompileAndValidateWidgetBlueprint(
	const FString& WidgetBlueprintPath, bool bSave, bool bCheckAccessibility)
{
	using namespace TetherUMGImpl;
	FTetherWidgetValidationReport Report;
...
TArray<FTetherWidgetAnimationInfo> UTetherUMGLibrary::GetWidgetAnimations(
	const FString& WidgetBlueprintPath)
{
	using namespace TetherUMGImpl;
	TArray<FTetherWidgetAnimationInfo> Result;
	UWidgetBlueprint* WBP = LoadWBP(WidgetBlueprintPath, TEXT("get_widget_animations"));
	if (!WBP) return Result;
	for (UWidgetAnimation* Animation : WBP->Animations)
	{
		if (!Animation) continue;
		FTetherWidgetAnimationInfo Info;
		...
		for (const FMovieSceneBinding& Binding : Bindings)
		{
			...
			{
				if (!Track) continue;
				FTetherWidgetAnimTrack TrackInfo;
...
bool UTetherUMGLibrary::CreateWidgetAnimation(
	...
	float DurationSeconds, int32 DisplayRate)
{
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Create Widget Animation")));
	WBP->Modify();
...
bool UTetherUMGLibrary::RemoveWidgetAnimation(
	const FString& WidgetBlueprintPath, const FString& AnimationName)
{
	using namespace TetherUMGImpl;
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Remove Widget Animation")));
	WBP->Modify();
...
bool UTetherUMGLibrary::AddWidgetAnimationFloatKeys(
	const FString& WidgetBlueprintPath, const FString& AnimationName,
	...
	const TArray<FTetherWidgetFloatKey>& Keys, const FString& Interpolation)
{
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Add Widget Float Animation Keys")));
	Animation->Modify();
	UMovieScene* Scene = Animation->GetMovieScene();
	Scene->Modify();
	...
	Section->Modify();
	for (const FTetherWidgetFloatKey& Key : Keys)
	{
...
bool UTetherUMGLibrary::AddWidgetAnimationColorKeys(
	const FString& WidgetBlueprintPath, const FString& AnimationName,
	const FString& WidgetName, const FString& PropertyName,
	const TArray<FTetherWidgetColorKey>& Keys, const FString& Interpolation)
{
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Add Widget Color Animation Keys")));
	Animation->Modify();
	UMovieScene* Scene = Animation->GetMovieScene();
	Scene->Modify();
	...
	for (const FTetherWidgetColorKey& Key : Keys)
	{
...
bool UTetherUMGLibrary::AddWidgetAnimationTransformKeys(
	const FString& WidgetBlueprintPath, const FString& AnimationName,
	...
	const FString& Interpolation)
{
	...
	const FScopedTransaction Transaction(FText::FromString(TEXT("Tether Add Widget Transform Animation Keys")));
	Animation->Modify();
	UMovieScene* Scene = Animation->GetMovieScene();
	Scene->Modify();
	...
	for (const FTetherWidgetTransformKey& Key : Keys)
	{
		const FFrameNumber Frame = SecondsToFrame(Scene, Key.Time);
...
int32 UTetherUMGLibrary::RemoveWidgetAnimationTrack(
	const FString& WidgetBlueprintPath, const FString& AnimationName,
	const FString& WidgetName, const FString& PropertyName)
{
	using namespace TetherUMGImpl;
...
TArray<FTetherWidgetBindingInfo> UTetherUMGLibrary::GetWidgetBindings(
	const FString& WidgetBlueprintPath)
{
	TArray<FTetherWidgetBindingInfo> Result;
	UWidgetBlueprint* WBP = TetherUMGImpl::LoadWBP(WidgetBlueprintPath, TEXT("get_widget_bindings"));
	if (!WBP) return Result;
	for (const FDelegateEditorBinding& Binding : WBP->Bindings)
	{
		FTetherWidgetBindingInfo Info;
...
TArray<FTetherWidgetEventInfo> UTetherUMGLibrary::GetWidgetEvents(
	const FString& WidgetBlueprintPath)
{
	TArray<FTetherWidgetEventInfo> Result;
	UWidgetBlueprint* WBP = TetherUMGImpl::LoadWBP(WidgetBlueprintPath, TEXT("get_widget_events"));
	if (!WBP) return Result;
	for (UEdGraph* Graph : WBP->UbergraphPages)
	{
		if (!Graph) continue;
...
FTetherWidgetOperationResult UTetherUMGLibrary::CreateMVVMViewModelBlueprint(
	const FString& AssetPath, const FString& ParentClassPath)
{
	using namespace TetherUMGImpl;
	FTetherWidgetOperationResult Result;
	...
	UBlueprint* BP = FKismetEditorUtilities::CreateBlueprint(
		...
		UBlueprint::StaticClass(), UBlueprintGeneratedClass::StaticClass(), FName(TEXT("Tether")));
	if (!BP)
	{
...
bool UTetherUMGLibrary::SetViewModelFieldNotify(
...
FString UTetherUMGLibrary::AddMVVMViewModel(
...
bool UTetherUMGLibrary::RemoveMVVMViewModel(
	const FString& WidgetBlueprintPath, const FString& ViewModelName)
{
	using namespace TetherUMGImpl;
#if UE_VERSION_OLDER_THAN(5, 7, 0)
...
TArray<FTetherMVVMViewModelInfo> UTetherUMGLibrary::GetMVVMViewModels(const FString& WidgetBlueprintPath)
{
	using namespace TetherUMGImpl;
	TArray<FTetherMVVMViewModelInfo> Result;
#if UE_VERSION_OLDER_THAN(5, 7, 0)
...
FString UTetherUMGLibrary::AddMVVMBinding(
	const FString& WidgetBlueprintPath, const FString& ViewModelName,
...
bool UTetherUMGLibrary::RemoveMVVMBinding(
...
TArray<FTetherMVVMBindingInfo> UTetherUMGLibrary::GetMVVMBindings(const FString& WidgetBlueprintPath)
{
	using namespace TetherUMGImpl;
	TArray<FTetherMVVMBindingInfo> Result;
#if UE_VERSION_OLDER_THAN(5, 7, 0)
...
bool UTetherUMGLibrary::SetMVVMViewSettings(
...
FString UTetherUMGLibrary::SpawnWidgetInstance(const FString& WidgetBlueprintPath, int32 ZOrder)
{
	using namespace TetherUMGImpl;
...
bool UTetherUMGLibrary::RemoveWidgetInstance(const FString& InstanceHandle)
{
	using namespace TetherUMGImpl;
...
int32 UTetherUMGLibrary::RemoveAllWidgetInstances()
{
	using namespace TetherUMGImpl;
	int32 Removed = 0;
...
TArray<FTetherLiveWidgetInfo> UTetherUMGLibrary::GetLiveWidgetTree(const FString& InstanceHandle)
{
	using namespace TetherUMGImpl;
	TArray<FTetherLiveWidgetInfo> Result;
...
FString UTetherUMGLibrary::GetLiveWidgetProperty(
...
bool UTetherUMGLibrary::SetLiveWidgetProperty(
...
bool UTetherUMGLibrary::ClickLiveButton(const FString& InstanceHandle, const FString& WidgetName)
{
	using namespace TetherUMGImpl;
...
bool UTetherUMGLibrary::SetLiveWidgetText(
...
bool UTetherUMGLibrary::SetLiveWidgetValue(
...
bool UTetherUMGLibrary::SetLiveWidgetChecked(
...
bool UTetherUMGLibrary::FocusLiveWidget(const FString& InstanceHandle, const FString& WidgetName)
{
	using namespace TetherUMGImpl;
...
bool UTetherUMGLibrary::PlayLiveWidgetAnimation(
...
bool UTetherUMGLibrary::StopLiveWidgetAnimation(
	const FString& InstanceHandle, const FString& AnimationName)
{
	using namespace TetherUMGImpl;
...
bool UTetherUMGLibrary::SetLiveWidgetMaterialScalar(
	const FString& InstanceHandle, const FString& WidgetName,
...
bool UTetherUMGLibrary::SetLiveWidgetMaterialVector(
	const FString& InstanceHandle, const FString& WidgetName,
...
FString UTetherUMGLibrary::GetLiveViewModelProperty(
...
bool UTetherUMGLibrary::SetLiveViewModelProperty(
