...
namespace TetherPerfImpl
{
	...
	static void FinalizeBreakdownRows(
		TArray<FTetherPerfBreakdownRow>& Rows,
		int32 MaxGroups)
	{
		Rows.Sort([](const FTetherPerfBreakdownRow& A, const FTetherPerfBreakdownRow& B)
		{
			if (A.Count != B.Count)
			{
	...
	static void AccumulateBucket(
		TMap<FString, FTetherPerfBreakdownRow>& Buckets,
		const FString& Key,
		int64 BytesToAdd,
		const FString& SamplePath)
	{
		FTetherPerfBreakdownRow& Bucket = Buckets.FindOrAdd(Key);
...
FTetherFrameTiming UTetherPerfLibrary::GetFrameTiming()
{
	FTetherFrameTiming Out;
	...
	if (FStatUnitData* StatUnit = TetherPerfImpl::GetActiveViewportStatUnit())
	{
		if (StatUnit->FrameTime > 0.f)
		{
			...
			Out.GpuMs = TetherPerfImpl::SumGpuMs(StatUnit->GPUFrameTime);
...
FTetherRenderCounters UTetherPerfLibrary::GetRenderCounters()
{
	FTetherRenderCounters Out;
...
FTetherMemoryStats UTetherPerfLibrary::GetMemoryStats()
{
	FTetherMemoryStats Out;

	const FPlatformMemoryStats Stats = FPlatformMemory::GetStats();
	const FPlatformMemoryConstants& Constants = FPlatformMemory::GetConstants();

	Out.UsedPhysicalMb = TetherPerfImpl::BytesToMb(Stats.UsedPhysical);
	Out.UsedVirtualMb = TetherPerfImpl::BytesToMb(Stats.UsedVirtual);
	Out.PeakUsedPhysicalMb = TetherPerfImpl::BytesToMb(Stats.PeakUsedPhysical);
	Out.PeakUsedVirtualMb = TetherPerfImpl::BytesToMb(Stats.PeakUsedVirtual);
	Out.AvailablePhysicalMb = TetherPerfImpl::BytesToMb(Stats.AvailablePhysical);
	Out.AvailableVirtualMb = TetherPerfImpl::BytesToMb(Stats.AvailableVirtual);
	Out.TotalPhysicalMb = TetherPerfImpl::BytesToMb(Constants.TotalPhysical);
...
FTetherUObjectStats UTetherPerfLibrary::GetUObjectStats(int32 TopN)
{
	FTetherUObjectStats Out;
...
FTetherPerfSnapshot UTetherPerfLibrary::GetPerfSnapshot(bool bIncludeUObjectStats, int32 UObjectTopN)
{
	FTetherPerfSnapshot Out;
...
TArray<FTetherPerfBreakdownRow> UTetherPerfLibrary::GetWorldActorBreakdown(
	const FString& LevelFilter,
	const FString& GroupBy,
	int32 MaxGroups)
{
	TArray<FTetherPerfBreakdownRow> Out;

	UWorld* World = TetherPerfImpl::GetEditorWorldForPerf();
	if (!World)
	{
		return Out;
	}
	...
	if (bIsWorldPartition)
	{
		UE_LOG(LogTemp, Verbose,
			TEXT("TetherPerf: GetWorldActorBreakdown — World Partition detected; ")
			TEXT("only loaded actors counted (unloaded descs not yet supported)"));
	}

	// Walk levels, build (level, class) → count + samples map.
	TMap<TetherPerfImpl::FActorBreakdownKey, TetherPerfImpl::FActorBreakdownAcc> Acc;
	Acc.Reserve(256);
	...
	for (ULevel* Level : World->GetLevels())
	{
		...
		const FString LevelShort = TetherPerfImpl::GetLevelShortName(Level);
		...
		for (AActor* Actor : Level->Actors)
		{
			...
			// aren't grouping on.
			TetherPerfImpl::FActorBreakdownKey MapKey;
			if (bGroupByLevel)
			{
				MapKey = TetherPerfImpl::FActorBreakdownKey(LevelFName, NAME_None);
			}
			else if (bGroupByLevelClass)
			{
				MapKey = TetherPerfImpl::FActorBreakdownKey(LevelFName, Cls->GetFName());
			}
			else // bGroupByClass
			{
				MapKey = TetherPerfImpl::FActorBreakdownKey(NAME_None, Cls->GetFName());
			}

			TetherPerfImpl::FActorBreakdownAcc& Bucket = Acc.FindOrAdd(MapKey);
			++Bucket.Count;
	...
	Out.Reserve(Acc.Num());
	for (const TPair<TetherPerfImpl::FActorBreakdownKey, TetherPerfImpl::FActorBreakdownAcc>& Entry : Acc)
	{
		const FName LevelKeyComp = Entry.Key.Key;
		const FName ClassKeyComp = Entry.Key.Value;

		FTetherPerfBreakdownRow Row;
		if (bGroupByLevel)
		{
	...
	TetherPerfImpl::FinalizeBreakdownRows(Out, MaxGroups);
	return Out;
}
...
TArray<FTetherPerfBreakdownRow> UTetherPerfLibrary::GetTextureMemoryBreakdown(
	const FString& GroupBy,
	const FString& Mode,
	int32 MaxGroups)
{
	...
	TetherPerfImpl::ETextureGroupBy ModeEnum = TetherPerfImpl::ETextureGroupBy::Folder;
	if (!TetherPerfImpl::ParseTextureGroupBy(GroupBy, ModeEnum))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetTextureMemoryBreakdown bad group_by '%s' — expected ")
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetTextureMemoryBreakdown bad mode '%s' — expected disk | runtime"),
			*Mode);
		return Out;
	}
	...
	if (bRuntimeMode)
	{
		...
		for (TObjectIterator<UTexture> It; It; ++It)
		{
			...
			const FString Key = TetherPerfImpl::GetTextureGroupKeyFromObject(Tex, ModeEnum);
			TetherPerfImpl::AccumulateBucket(Buckets, Key, Bytes, Tex->GetPathName());
		}
	}
	else
	{
		...
		for (const FAssetData& Data : Assets)
		{
			...
			const int64 Bytes = TetherPerfImpl::GetPackageDiskSize(Data.PackageName);
			if (Bytes <= 0)
			{
				// Never-saved or missing on disk. Skip (matches doc contract).
				continue;
			}
			const FString Key = TetherPerfImpl::GetTextureGroupKeyFromAssetData(Data, ModeEnum);
			TetherPerfImpl::AccumulateBucket(Buckets, Key, Bytes, Data.GetSoftObjectPath().ToString());
		}
	}

	Out.Reserve(Buckets.Num());
	for (const TPair<FString, FTetherPerfBreakdownRow>& Pair : Buckets)
	{
		Out.Add(Pair.Value);
	}
	TetherPerfImpl::FinalizeBreakdownRowsByBytes(Out, MaxGroups);
	return Out;
}
...
TArray<FTetherPerfBreakdownRow> UTetherPerfLibrary::GetMeshMemoryBreakdown(
	const FString& GroupBy,
	...
	int32 MaxGroups)
{
	...
	TetherPerfImpl::EMeshGroupBy ModeEnum = TetherPerfImpl::EMeshGroupBy::Folder;
	if (!TetherPerfImpl::ParseMeshGroupBy(GroupBy, ModeEnum))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetMeshMemoryBreakdown bad group_by '%s' — expected ")
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetMeshMemoryBreakdown bad mesh_type '%s' — expected static | skeletal | all"),
			*MeshType);
		return Out;
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetMeshMemoryBreakdown bad mode '%s' — expected disk | runtime"),
			*Mode);
		return Out;
	}
	...
	if (bRuntimeMode)
	{
		if (bWantStatic)
		{
			for (TObjectIterator<UStaticMesh> It; It; ++It)
			{
				...
				switch (ModeEnum)
				{
				case TetherPerfImpl::EMeshGroupBy::Folder:
				{
					if (UPackage* Pkg = SM->GetOutermost())
					{
						...
						if (PackageName.FindLastChar(TEXT('/'), LastSlash))
						{
							Key = TetherPerfImpl::GetTopLevelFolder(PackageName.Left(LastSlash));
						}
						else
						{
							Key = PackageName;
				...
				case TetherPerfImpl::EMeshGroupBy::VertexCountBucket:
				{
					...
					Key = TetherPerfImpl::VertexCountToBucket(TotalVerts);
					break;
				}
				}
				TetherPerfImpl::AccumulateBucket(Buckets, Key, Bytes, SM->GetPathName());
			}
		}
		if (bWantSkeletal)
		{
			for (TObjectIterator<USkeletalMesh> It; It; ++It)
			{
				...
				TetherPerfImpl::AccumulateBucket(Buckets, Key, Bytes, SK->GetPathName());
			}
		}
	}
	else
	{
		...
		auto WalkAssets = [&Buckets, ModeEnum, &AR](UClass* Cls)
		{
			...
			for (const FAssetData& Data : Assets)
			{
				...
				const int64 Bytes = TetherPerfImpl::GetPackageDiskSize(Data.PackageName);
				if (Bytes <= 0) continue;
				const FString Key = TetherPerfImpl::GetMeshGroupKeyFromAssetData(Data, ModeEnum);
				TetherPerfImpl::AccumulateBucket(Buckets, Key, Bytes, Data.GetSoftObjectPath().ToString());
			}
		};
...
TArray<FTetherPerfBreakdownRow> UTetherPerfLibrary::GetUObjectMemoryBreakdown(int32 TopN)
{
	TArray<FTetherPerfBreakdownRow> Out;
	const int32 ClampedTopN = FMath::Clamp(TopN, 1, 200);
	...
	TMap<FName, FTetherPerfBreakdownRow> ByClass;
	...
	for (TObjectIterator<UObject> It; It; ++It)
	{
		...
		FTetherPerfBreakdownRow& Bucket = ByClass.FindOrAdd(Cls->GetFName());
		if (Bucket.Key.IsEmpty())
		{
	...
	for (const TPair<FName, FTetherPerfBreakdownRow>& Pair : ByClass)
	{
		Out.Add(Pair.Value);
	}
	TetherPerfImpl::FinalizeBreakdownRowsByBytes(Out, ClampedTopN);
	return Out;
}
...
TArray<FTetherPerfBreakdownRow> UTetherPerfLibrary::GetAudioMemoryBreakdown(
	const FString& GroupBy,
	const FString& Mode,
	int32 MaxGroups)
{
	...
	TetherPerfImpl::EAudioGroupBy ModeEnum = TetherPerfImpl::EAudioGroupBy::CompressionFormat;
	if (!TetherPerfImpl::ParseAudioGroupBy(GroupBy, ModeEnum))
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetAudioMemoryBreakdown bad group_by '%s' — expected ")
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetAudioMemoryBreakdown bad mode '%s' — expected disk | runtime"),
			*Mode);
		return Out;
	}
	...
	if (bRuntimeMode)
	{
		for (TObjectIterator<USoundWave> It; It; ++It)
		{
			...
			const FString Key = TetherPerfImpl::GetAudioGroupKeyFromObject(SW, ModeEnum);
			TetherPerfImpl::AccumulateBucket(Buckets, Key, Bytes, SW->GetPathName());
		}
	}
	else
	{
		...
		for (const FAssetData& Data : Assets)
		{
			...
			const int64 Bytes = TetherPerfImpl::GetPackageDiskSize(Data.PackageName);
			if (Bytes <= 0) continue;
			const FString Key = TetherPerfImpl::GetAudioGroupKeyFromAssetData(Data, ModeEnum);
			TetherPerfImpl::AccumulateBucket(Buckets, Key, Bytes, Data.GetSoftObjectPath().ToString());
		}
	}
...
TArray<FTetherPerfBreakdownRow> UTetherPerfLibrary::GetAssetSizeTopN(
	...
	int32 TopN)
{
	...
	else
	{
		...
		UClass* Cls = nullptr;
		if (!TetherPerfImpl::ResolveClassFilter(TrimmedFilter, ClassPath, Cls))
		{
			UE_LOG(LogTemp, Warning,
				TEXT("TetherPerf: GetAssetSizeTopN can't resolve class_filter '%s' — ")
...
namespace TetherPerfFrameHook
{
	...
	static TArray<FTetherHitchEntry> Hitches;
	...
	static TArray<FTetherAutoHitchEntry> AutoHitchBuffer;
	...
	static TArray<FTetherHitchEntry> SnapshotHitches()
	{
		FScopeLock L(&State_Lock);
		return Hitches;
	}
...
TArray<FTetherHistogramBucket> UTetherPerfLibrary::GetFrameTimeHistogram(
	float BucketMs,
	float MaxBucketMs)
{
	TArray<FTetherHistogramBucket> Out;

	// Snap caller's bucket width up to the internal resolution.
	float Width = FMath::Clamp(BucketMs, TetherPerfFrameHook::FineBucketWidthMs, 50.f);
	...
	const float Snapped = FMath::Max(TetherPerfFrameHook::FineBucketWidthMs,
		FMath::RoundToFloat(Width / TetherPerfFrameHook::FineBucketWidthMs)
		* TetherPerfFrameHook::FineBucketWidthMs);
	Width = Snapped;

	const float Max = FMath::Clamp(MaxBucketMs, Width, TetherPerfFrameHook::MaxFineMs);

	int32 SnapshotBuckets[TetherPerfFrameHook::FineBucketCount];
	int64 Total = 0;
	TetherPerfFrameHook::SnapshotFineBuckets(SnapshotBuckets, Total);

	const int32 OutBucketCount = FMath::CeilToInt(Max / Width);
	Out.Reserve(OutBucketCount + 1);
	for (int32 i = 0; i < OutBucketCount; ++i)
	{
		FTetherHistogramBucket Row;
	...
	{
		FTetherHistogramBucket Overflow;
	...
	const float FineWidth = TetherPerfFrameHook::FineBucketWidthMs;
	for (int32 FineIdx = 0; FineIdx < TetherPerfFrameHook::FineBucketCount; ++FineIdx)
	{
		...
		int32 OutIdx;
		if (FineLowerMs >= Max || FineIdx == TetherPerfFrameHook::FineBucketCount - 1)
		{
	...
	if (Total > 0)
	{
		...
		for (FTetherHistogramBucket& Row : Out)
		{
...
TArray<FTetherHitchEntry> UTetherPerfLibrary::GetHitchLog(
	float ThresholdMs,
	int32 MaxEntries)
{
	const int32 ClampedMax = FMath::Clamp(MaxEntries, 1, TetherPerfFrameHook::MaxHitchEntries);
	const float Threshold = FMath::Max(ThresholdMs, 0.f);

	TArray<FTetherHitchEntry> All = TetherPerfFrameHook::SnapshotHitches();

	// Filter by threshold (the captured set already has TotalMs >= HitchMinMs;
	// caller's threshold tightens that further).
	TArray<FTetherHitchEntry> Out;
	Out.Reserve(FMath::Min(All.Num(), ClampedMax));
	for (const FTetherHitchEntry& E : All)
	{
...
void UTetherPerfLibrary::ResetFrameTimeHistogram()
{
	TetherPerfFrameHook::ResetHistogram();
}

void UTetherPerfLibrary::ClearHitchLog()
{
	TetherPerfFrameHook::ClearHitches();
}

// ─── M8-1: Auto-hitch capture ───────────────────────────────

bool UTetherPerfLibrary::BeginAutoHitchCapture(float ThresholdMs, int32 MaxEntries)
{
	const float ClampedThreshold = FMath::Clamp(ThresholdMs, 10.f, 5000.f);
	const int32 ClampedMax       = FMath::Clamp(MaxEntries, 1, 1000);

	FScopeLock L(&TetherPerfFrameHook::State_Lock);
	TetherPerfFrameHook::bAutoHitchActive    = true;
	TetherPerfFrameHook::AutoHitchThresholdMs = ClampedThreshold;
	TetherPerfFrameHook::AutoHitchMaxEntries = ClampedMax;
	TetherPerfFrameHook::AutoHitchStartedAtUtc = FDateTime::UtcNow().ToIso8601();
	TetherPerfFrameHook::AutoHitchBuffer.Reset();
	return true;
}

TArray<FTetherAutoHitchEntry> UTetherPerfLibrary::EndAutoHitchCapture()
{
	FScopeLock L(&TetherPerfFrameHook::State_Lock);
	TetherPerfFrameHook::bAutoHitchActive = false;
	TArray<FTetherAutoHitchEntry> Out = TetherPerfFrameHook::AutoHitchBuffer;
	TetherPerfFrameHook::AutoHitchBuffer.Reset();
	return Out;
}

FTetherAutoHitchState UTetherPerfLibrary::GetAutoHitchState()
{
	FTetherAutoHitchState Out;
	FScopeLock L(&TetherPerfFrameHook::State_Lock);
	Out.bActive          = TetherPerfFrameHook::bAutoHitchActive;
	Out.ThresholdMs      = TetherPerfFrameHook::AutoHitchThresholdMs;
	Out.EntriesBuffered  = TetherPerfFrameHook::AutoHitchBuffer.Num();
	Out.MaxEntries       = TetherPerfFrameHook::AutoHitchMaxEntries;
	Out.StartedAtUtc     = TetherPerfFrameHook::AutoHitchStartedAtUtc;
	return Out;
}

// ─── M5-4: GetFrameTimePercentiles ──────────────────────────

TArray<float> UTetherPerfLibrary::GetFrameTimePercentiles(const TArray<float>& Percentiles)
{
	TArray<float> Out;
	...
	const float FineWidth = TetherPerfFrameHook::FineBucketWidthMs;
	const int32 LastIdx   = TetherPerfFrameHook::FineBucketCount - 1;
	...
	for (float Raw : Percentiles)
	{
		...
		for (int32 i = 0; i < TetherPerfFrameHook::FineBucketCount; ++i)
		{
...
namespace TetherPerfSampler
{
	 ...
	 * and the captured FTetherPerfSnapshot ring buffer. Access is funneled
	...
	static TArray<FTetherPerfSnapshot> Buffer;

	/** Append one snapshot, evicting the oldest when the ring fills. */
	static void AppendSnapshot(FTetherPerfSnapshot&& Snap)
	{
	...
	static bool Tick(float /*DeltaTime*/)
	{
		...
		FTetherPerfSnapshot Snap = UTetherPerfLibrary::GetPerfSnapshot(bWantUObjects, /*UObjectTopN*/ 20);
	...
	static TArray<FTetherPerfSnapshot> StopAndDrain()
	{
		TArray<FTetherPerfSnapshot> Out;
		{
			FScopeLock L(&Sampler_Lock);
	...
	FTetherPerfSamplingState GetState()
	{
		FTetherPerfSamplingState Out;
		FScopeLock L(&Sampler_Lock);
		Out.bActive = bActive;
	...
	TArray<FTetherPerfSnapshot> SnapshotBuffer()
	{
		FScopeLock L(&Sampler_Lock);
		return Buffer;
	}
...
bool UTetherPerfLibrary::StartPerfSampling(
	int32 PeriodMs,
	int32 MaxSamples,
	bool bIncludeUObjectStats)
{
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: StartPerfSampling include_uobject_stats=true with period_ms=%d ")
			TEXT("(<5000) will cause every tick to spend 50-300 ms in TObjectIterator — proceeding ")
			TEXT("but this is expensive."), ClampedPeriod);
	}
	return TetherPerfSampler::Start(ClampedPeriod, ClampedMax, bIncludeUObjectStats);
}

TArray<FTetherPerfSnapshot> UTetherPerfLibrary::StopPerfSampling()
{
	return TetherPerfSampler::StopAndDrain();
}

FTetherPerfSamplingState UTetherPerfLibrary::GetPerfSamplingState()
{
	return TetherPerfSampler::GetState();
}
...
namespace TetherPerfImpl
{
	 ...
	 *   - empty → <Project>/Saved/Tether/perf_samples_<unix>.csv
...
bool UTetherPerfLibrary::ExportPerfSamplesToCsv(const FString& OutputPath)
{
	TArray<FTetherPerfSnapshot> Samples = TetherPerfSampler::SnapshotBuffer();
	if (Samples.Num() == 0)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: ExportPerfSamplesToCsv — sample buffer empty, nothing to write."));
		return false;
	}

	const FString ResolvedPath = TetherPerfImpl::ResolveCsvOutputPath(OutputPath);
	...
	for (const FTetherPerfSnapshot& S : Samples)
	{
		Csv += TetherPerfImpl::CsvEscape(S.CaptureTimeUtc);
		Csv += TEXT(",");
		...
		Csv += TEXT(",");
		Csv += TetherPerfImpl::CsvEscape(S.EngineVersion);
		Csv += TEXT("\r\n");
	}
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: ExportPerfSamplesToCsv — failed to write '%s' (%d samples)"),
			*ResolvedPath, Samples.Num());
		return false;
	}

	UE_LOG(LogTemp, Log,
		TEXT("TetherPerf: wrote %d perf samples to '%s'"),
...
namespace TetherPerfRender
{
	...
	static FTetherActorRenderCost BuildActorCostFromComponents(
		const AActor* Actor,
		const TArray<UPrimitiveComponent*>& Comps)
	{
		FTetherActorRenderCost Out;
...
TArray<FTetherMaterialRenderRow> UTetherPerfLibrary::GetVisiblePrimitivesByMaterial(
	int32 ViewportIndex,
	int32 TopN)
{
	(void)ViewportIndex; // reserved for future RT-side enrichment

	TArray<FTetherMaterialRenderRow> Out;
	const int32 ClampedTopN = FMath::Clamp(TopN, 1, 1000);
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		for (UPrimitiveComponent* Comp : Prims)
		{
			...
			const int64 CompTriangles =
				TetherPerfRender::EstimateComponentTriangles(Comp);
	...
	Out.Sort([](const FTetherMaterialRenderRow& A, const FTetherMaterialRenderRow& B)
	{
...
FTetherActorRenderCost UTetherPerfLibrary::GetActorRenderCost(const FString& ActorPath)
{
	FTetherActorRenderCost Out;
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("TetherPerf: GetActorRenderCost — no editor world available"));
		return Out;
	}
	...
	TArray<UPrimitiveComponent*> Prims;
	Actor->GetComponents<UPrimitiveComponent>(Prims);
	return TetherPerfRender::BuildActorCostFromComponents(Actor, Prims);
}

// ─── M3-3: get_lod_distribution ─────────────────────────────

TArray<FTetherPerfBreakdownRow> UTetherPerfLibrary::GetLodDistribution(
	const FString& ClassFilter,
	const FString& ActorFilter)
{
	...
	for (ULevel* Level : World->GetLevels())
	{
		...
		for (AActor* Actor : Level->Actors)
		{
			...
			for (UPrimitiveComponent* Comp : Prims)
			{
				...
				const FString MeshPath = TetherPerfRender::GetMeshAssetPathForLod(Comp);
				if (MeshPath.IsEmpty())
				{
					continue;
				}
				const int32 LodIdx = TetherPerfRender::ResolveComponentLodIndex(Comp);
				if (LodIdx < 0)
				{
					continue;
				}
	...
	TetherPerfImpl::FinalizeBreakdownRows(Out, /*MaxGroups*/ 1000);
	return Out;
}
...
FTetherLumenDiagnostics UTetherPerfLibrary::GetLumenDiagnostics()
{
	FTetherLumenDiagnostics Out;
	Out.EngineVersion = FEngineVersion::Current().ToString();
	...
	Out.ActiveVisualizationMode = TetherPerfRender::GetCVarString(
	...
	const int32 GiMethod = TetherPerfRender::GetCVarInt(
		TEXT("r.DynamicGlobalIlluminationMethod"), 0);
	const int32 ReflMethod = TetherPerfRender::GetCVarInt(
...
FTetherNaniteStats UTetherPerfLibrary::GetNaniteStats()
{
	FTetherNaniteStats Out;
	Out.EngineVersion = FEngineVersion::Current().ToString();
	...
	int32 NaniteCount = 0;
	if (UWorld* World = TetherPerfImpl::GetEditorWorldForPerf())
	{
		for (TActorIterator<AActor> It(World); It; ++It)
		{
			AActor* Actor = *It;
...
TArray<FTetherActorRenderCost> UTetherPerfLibrary::GetShadowCasterBreakdown(int32 TopN)
{
	TArray<FTetherActorRenderCost> Out;
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		FTetherActorRenderCost Row =
			TetherPerfRender::BuildActorCostFromComponents(Actor, ShadowCasters);
	...
	Out.Sort([](const FTetherActorRenderCost& A, const FTetherActorRenderCost& B)
	{
...
namespace TetherPerfTrace
{
	 ...
	 *  empty → default Saved/Tether/Traces/<unix>.utrace; existing or
...
FTetherTraceStartResult UTetherPerfLibrary::StartTraceCapture(
	const TArray<FString>& Channels,
	const FString& OutputDir,
	int32 MaxSizeMb)
{
	FTetherTraceStartResult Out;
	const int32 ClampedMaxMb = FMath::Clamp(MaxSizeMb, 0, 65536);

	{
		FScopeLock L(&TetherPerfTrace::State_Lock);
		if (TetherPerfTrace::bActive)
		{
			Out.bSuccess = false;
			Out.Error = TEXT("already_active");
			Out.Path = TetherPerfTrace::ActivePath;
			Out.Channels = TetherPerfTrace::ActiveChannels;
			return Out;
		}
	}

	const FString FilePath = TetherPerfTrace::ResolveTraceFilePath(OutputDir);
	if (!TetherPerfTrace::EnsureParentDir(FilePath))
	{
		Out.bSuccess = false;
	...
	FString ChannelStr = TetherPerfTrace::JoinChannels(Channels);
	if (ChannelStr.IsEmpty())
	{
		ChannelStr = TetherPerfTrace::DefaultChannelString();
	}
	...
	{
		FScopeLock L(&TetherPerfTrace::State_Lock);
		TetherPerfTrace::bActive = true;
		TetherPerfTrace::ActivePath = FilePath;
		TetherPerfTrace::ActiveMaxSizeMb = ClampedMaxMb;
		TetherPerfTrace::ActiveStartedAtUtc = FDateTime::UtcNow().ToIso8601();
		TetherPerfTrace::ActiveStartSeconds = FApp::GetCurrentTime();

		// Split the joined string back into entries for the result struct.
		// Use the original list when present, otherwise echo "default".
		TetherPerfTrace::ActiveChannels.Reset();
		if (Channels.Num() > 0)
		{
			for (const FString& C : Channels)
			{
				...
				if (!T.IsEmpty())
				{
					TetherPerfTrace::ActiveChannels.Add(T);
				}
			}
		}
		if (TetherPerfTrace::ActiveChannels.Num() == 0)
		{
			TetherPerfTrace::ActiveChannels.Add(TetherPerfTrace::DefaultChannelString());
		}
...
FTetherTraceStopResult UTetherPerfLibrary::StopTraceCapture()
{
	FTetherTraceStopResult Out;
	...
	{
		FScopeLock L(&TetherPerfTrace::State_Lock);
		bWasActive = TetherPerfTrace::bActive;
		PathSnapshot = TetherPerfTrace::ActivePath;
		StartSecondsSnapshot = TetherPerfTrace::ActiveStartSeconds;
	}
	...
	const int64 SizeBytes = TetherPerfTrace::QueryFileSize(PathSnapshot);
...

	{
		...
		// last run.
		TetherPerfTrace::bActive = false;
		TetherPerfTrace::ActiveStartSeconds = 0.0;
	}
...
FTetherTraceState UTetherPerfLibrary::GetTraceState()
{
	FTetherTraceState Out;
	{
		FScopeLock L(&TetherPerfTrace::State_Lock);
		Out.bActive = TetherPerfTrace::bActive;
		Out.Path = TetherPerfTrace::ActivePath;
		Out.Channels = TetherPerfTrace::ActiveChannels;
		Out.StartedAtUtc = TetherPerfTrace::ActiveStartedAtUtc;
	}
	...
	Out.CurrentSizeBytes = TetherPerfTrace::QueryFileSize(Out.Path);
	return Out;
}
...
namespace TetherPerfTrace
{
	...
	{
		auto* Sink = static_cast<TArray<FTetherTraceChannelInfo>*>(User);
		if (!Sink || !Name)
		{
			return;
		}
		FTetherTraceChannelInfo Row;
...
TArray<FTetherTraceChannelInfo> UTetherPerfLibrary::ListTraceChannels()
{
	TArray<FTetherTraceChannelInfo> Out;

#if !UE_VERSION_OLDER_THAN(5, 4, 0)
	UE::Trace::EnumerateChannels(&TetherPerfTrace::EnumerateChannelCallback, &Out);
	...
	Out.Sort([](const FTetherTraceChannelInfo& A, const FTetherTraceChannelInfo& B)
	{
...
namespace TetherPerfTraceImpl
{
	...
	/**
	 * Convert a TimerId → FAgg map into an FTetherPerfHotScope array, ranked
	 * by TotalMs descending and capped at TopN. `TimerNames` resolves the
	 * display name; missing entries render as `<TimerIndex N>`.
	 */
	static TArray<FTetherPerfHotScope> RankAndConvert(
		...
		int32 TopN)
	{
		...
		TArray<FTetherPerfHotScope> Result;
		Result.Reserve(Take);
		for (int32 i = 0; i < Take; ++i)
		{
			FTetherPerfHotScope Row;
...
FTetherPerfTraceSummary UTetherPerfLibrary::ParseTraceToSummary(
	const FString& UtracePath,
	int32 TopN,
	int32 TopNPerThread,
	int32 TopNCounters)
{
	FTetherPerfTraceSummary Out;
	...
	{
		using namespace TetherPerfTraceImpl;
		...
		if (TopNPerThread > 0)
		{
			...
			Out.PerThreadHotScopes.Sort(
				[](const FTetherPerThreadHotScopes& A, const FTetherPerThreadHotScopes& B)
				{
	...
	if (TopNCounters > 0)
	{
		...
		TArray<FTetherPerfCounter> AllCounters;
		...
		AllCounters.Sort(
			[](const FTetherPerfCounter& A, const FTetherPerfCounter& B)
			{
	...
	if (const TraceServices::ILoadTimeProfilerProvider* LoadProv = TraceServices::ReadLoadTimeProfilerProvider(*Session))
	{
		...
		if (PkgTable)
		{
			...
			if (Reader)
			{
				TArray<FTetherPerfLoadTimeRow> AllRows;
				...
				AllRows.Sort(
					[](const FTetherPerfLoadTimeRow& A, const FTetherPerfLoadTimeRow& B)
					{
...
FTetherPerfAllocSummary UTetherPerfLibrary::ParseAllocTraceToSummary(const FString& UtracePath)
{
	FTetherPerfAllocSummary Out;
	Out.TracePath = UtracePath;
	...
	for (FTetherPerfAllocTag& T : Out.Tags)
	{
...
FTetherPerfNetSummary UTetherPerfLibrary::ParseNetTraceToSummary(const FString& UtracePath)
{
	FTetherPerfNetSummary Out;
	Out.TracePath = UtracePath;
	...
	NetProv->ReadGameInstances(
		...
		{
			FTetherPerfNetGameInstance Row;
			...
			NetProv->ReadConnections(Instance.GameInstanceIndex,
				...
				{
					FTetherPerfNetConnection ConnRow;
...
FTetherTextureStreamingState UTetherPerfLibrary::GetTextureStreamingResidency(int32 TopN)
{
	FTetherTextureStreamingState Out;
	const int32 ClampedTopN = FMath::Clamp(TopN, 1, 1000);
	...
	TArray<FTetherTextureStreamingRow> AllRows;
	...
	for (TObjectIterator<UTexture2D> It; It; ++It)
	{
		...
		FTetherTextureStreamingRow Row;
	...
	AllRows.Sort(
		[](const FTetherTextureStreamingRow& A, const FTetherTextureStreamingRow& B)
		{
...
FTetherRenderTargetMemory UTetherPerfLibrary::GetRenderTargetMemory(int32 TopN)
{
	FTetherRenderTargetMemory Out;
	const int32 ClampedTopN = FMath::Clamp(TopN, 1, 1000);

	TArray<FTetherRenderTargetEntry> AllEntries;
	...
	AllEntries.Sort(
		[](const FTetherRenderTargetEntry& A, const FTetherRenderTargetEntry& B)
		{
...
FTetherGpuPassTimings UTetherPerfLibrary::GetPerPassGpuTimings()
{
	FTetherGpuPassTimings Out;
	...
	Out.Passes.Sort(
		[](const FTetherGpuPassTiming& A, const FTetherGpuPassTiming& B)
		{
...
FTetherInsightsLaunchResult UTetherPerfLibrary::BeginInsightsForTrace(const FString& UtracePath)
{
	FTetherInsightsLaunchResult Out;
	Out.TracePath = UtracePath;
...
FTetherPerfSnapshotDelta UTetherPerfLibrary::ComparePerfSnapshots(
	const FTetherPerfSnapshot& Before,
	const FTetherPerfSnapshot& After,
	float RegressionThreshold)
{
	FTetherPerfSnapshotDelta Out;
...
FTetherPerfCookSummary UTetherPerfLibrary::ParseCookTraceToSummary(const FString& UtracePath, int32 TopN)
{
	FTetherPerfCookSummary Out;
	Out.TracePath = UtracePath;
	...
	Rows.Sort(
		[](const FTetherPerfCookRow& A, const FTetherPerfCookRow& B)
		{
...
FTetherAllMaterialsAnalysis UTetherPerfLibrary::AnalyzeAllMaterials(int32 TopN)
{
	FTetherAllMaterialsAnalysis Out;
	const int32 ClampedTopN = FMath::Clamp(TopN, 1, 1000);
	...
	TArray<FTetherMaterialPerfRow> AllRows;
	...
	AllRows.Sort(
		[](const FTetherMaterialPerfRow& A, const FTetherMaterialPerfRow& B)
		{
...
#endif // !UE_VERSION_OLDER_THAN(5, 7, 0) — pre-5.7 path lives in TetherPerfLibrary_Stubs.cpp
