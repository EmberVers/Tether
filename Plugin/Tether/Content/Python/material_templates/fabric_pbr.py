...
two Matrix-demo patterns relevant to cloth authoring:
  ...
  * **4-point roughness remap** (Low / SculptLow / SculptHigh / High) driven by
    ``TetherRoughnessRemap4Pt`` — replaces the v1 2-point Min/Max lerp. Matches
    how Matrix's `M_Clothing` tunes per-layer roughness envelopes.
  * **Charlie sheen grazing rim** gated by ``DoCharlieSheen`` — adds
    ``TetherCharlieSheenGrazing(N, V, SheenRoughness) × SheenColor × SheenIntensity``
...
def build(master_path: str = "/Game/TetherTemplates/M_Fabric_PBR",
          mi_path: Optional[str] = "/Game/TetherTemplates/MI_Fabric_PBR_Test",
    ...
    if not unreal.EditorAssetLibrary.does_asset_exist(linear_white):
        raise RuntimeError(
            ...
            "in a separate tether.exec before building M_Fabric_PBR."
        )
