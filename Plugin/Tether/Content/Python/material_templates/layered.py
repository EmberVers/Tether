...
``/Game/TetherTemplates/M_Layered_Base``:
...
that already bit us on the Texture2DFactoryNew path. The pattern:
  ...
  1. First tether exec → ``ensure_layer_stack_assets(rebuild=...)`` —
     creates the 5 MFs, saves, returns.
  2. Second tether exec → ``build(rebuild=...)`` — master + MAL
...
ML_METAL_PATH  = "/Game/TetherTemplates/ML_Layer_Metal"
ML_FABRIC_PATH = "/Game/TetherTemplates/ML_Layer_Fabric"
ML_DIRT_PATH   = "/Game/TetherTemplates/ML_Layer_Dirt"

MLB_VC_R_PATH  = "/Game/TetherTemplates/MLB_VertexColorR"
MLB_VC_G_PATH  = "/Game/TetherTemplates/MLB_VertexColorG"
...
def ensure_layer_stack_assets(rebuild: bool = False) -> Dict[str, str]:
    ...
    MUST be called from its own tether exec, before ``build()``. Bundling
...
def _assert_stack_assets_exist() -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            "separate tether exec before building M_Layered_Stack.")


def build(master_path: str = "/Game/TetherTemplates/M_Layered_Stack",
          mi_path: Optional[str] = "/Game/TetherTemplates/MI_Layered_Stack_Test",
          compile: bool = True,
    ...
    # AddDefaultBackgroundLayer(). Then configure the stack via the tether's
    ...
    # Delegate layer-stack config to the tether C++ layer. Blends is one
