...
Two noise contributions added additively to the post-tonemap image:
  ...
  1. **Grain** — ``TetherHash21(UV * GrainScale + t * TimeSpeed * (1, 0.37))``
  ...
  2. **Dither** — second ``TetherHash21`` tap at high frequency (pixel-
...
Both hashes trampoline through the shared ``TetherSnippets.ush`` helper
so M5-11 (trivial-Custom) skips them.

Final:

    grain    = (TetherHash21(uv_grain) - 0.5) * 2 * GrainIntensity
    midmask  = 4 * L * (1 - L)                       # parabola peak at L=0.5
    grain_m  = grain * lerp(1, midmask, LuminanceWeight)
    dither   = (TetherHash21(uv_dither) - 0.5) * DitherStrength / 127.5
...
def build(master_path: str = "/Game/TetherTemplates/PP_Film_Grain_AA",
          apply_to_volume: str = "",
    ...
    grain_custom = L.add_custom_expression(
        ...
        ["P"],
        "Float1",
        "return TetherHash21(P);",
        ["/Plugin/Tether/TetherSnippets.ush"],
        "Film-grain hash tap")
