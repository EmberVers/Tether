...
This revision absorbs the Matrix-demo clothing master's authoring patterns:
  ...
  * **4-point roughness remap** — Low / SculptLow / SculptHigh / High
    (driven by ``TetherRoughnessRemap4Pt`` HLSL snippet). Matrix's clothing
  ...
  * **Bent normals** (optional, gated by ``A_DoBentNormal``) — extra normal
    texture decoded through ``TetherBentNormalAO`` and multiplied into AO.
  ...
  * **Charlie sheen grazing** (optional, gated by ``A_DoCharlieSheen``) —
    ...
    ``TetherCharlieSheenGrazing`` and ADD it (× sheen color × intensity) to
...
SNIPPETS_PATH = "/Plugin/Tether/TetherSnippets.ush"
...
def build(master_path: str = "/Game/TetherTemplates/M_Character_Armor",
          mi_path: Optional[str] = "/Game/TetherTemplates/MI_Character_Armor_Test",
    ...
    custom_blend = L.add_custom_expression(
        ...
        "Float3",
        "return TetherBlendAngleCorrectedNormals(BaseN, DetailN, Strength);",
        [SNIPPETS_PATH],
    ...
    custom_remap = L.add_custom_expression(
        ...
        "Float1",
        "return TetherRoughnessRemap4Pt(value, low, high, sculptLow, sculptHigh);",
        [SNIPPETS_PATH],
    ...
    custom_sheen = L.add_custom_expression(
        ...
        ["N", "V", "roughness"],
        "Float1",
        "return TetherCharlieSheenGrazing(N, V, roughness);",
        [SNIPPETS_PATH],
    ...
    custom_bent = L.add_custom_expression(
        ...
        ["BentN", "V"],
        "Float1",
        "return TetherBentNormalAO(BentN, V);",
        [SNIPPETS_PATH],
    ...
    # AO = saturate(ORM.R * (1 - Strength) + ORM.R * TetherBentNormalAO(BentN,V) * Strength)
    ...
    # sheen_contrib = TetherCharlieSheenGrazing(N, V, sheenRoughness) × sheenColor × sheenIntensity
    ...
    if mi_path:
        if unreal.EditorAssetLibrary.does_asset_exist(mi_path):
            ...
            mi_result = unreal.TetherMaterialLibrary.create_material_instance(
                master_path, mi_path)
