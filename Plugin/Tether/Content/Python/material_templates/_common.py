...
- ``OpList``  — name-tracking wrapper around ``apply_material_graph_ops``
  ...
  by name on connect / set_prop ops. The list is flushed to the tether
...
DEFAULT_MASKS_TEX = "/Game/TetherTemplates/_System/T_White_Masks.T_White_Masks"
DEFAULT_LINEAR_WHITE_TEX = "/Game/TetherTemplates/_System/T_White_Linear.T_White_Linear"
...
def ensure_default_masks_texture() -> str:
    """Idempotent: ensure ``/Game/TetherTemplates/_System/T_White_Masks`` exists
...
def ensure_default_linear_texture() -> str:
    """Idempotent: ensure ``/Game/TetherTemplates/_System/T_White_Linear``
...
def add_roughness_remap_4pt(ops: OpList, prefix: str, x: int, y: int,
    ...
    """Emit the 4 scalar parameters driving ``TetherRoughnessRemap4Pt``.
    ...
    to Low / High respectively (see TetherSnippets TetherRoughnessRemap4Pt).
    """
    names = {
...
def add_charlie_sheen_scalars(ops: OpList, prefix: str, x: int, y: int,
    ...
    (``TetherCharlieSheenGrazing``) multiplied by a SheenColor vector and
...
def clear_material_graph(material_path: str) -> int:
    ...
    def append_op(kind: str, **kwargs: Any) -> None:
        operation = unreal.TetherMaterialGraphOp()
        operation.op = kind
