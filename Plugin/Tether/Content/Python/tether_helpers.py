"""
Tether helper functions for common editor operations.

These are pre-loaded in the UE Python environment and can be called
directly from scripts sent via the tether.

Usage from tether:
    python tether.py exec "from tether_helpers import *; print(list_assets('/Game/Props'))"
"""
...
def scaffold_enhanced_input_pawn(bp_path, ia_action_map, parent_class=None,
    ...
    """
    bp_lib    = unreal.TetherBlueprintLibrary
    ...
    #    Tether.get_blueprint_functions is the read API; FunctionGraphs UPROPERTY
...
def export_input_bindings_to_json(content_path_filter="/Game", include_imcs=True,
    ...
    """
    gp = unreal.TetherGameplayLibrary
    asset_lib = unreal.EditorAssetLibrary
...
def detect_trigger_conflicts(input_action_path):
    ...
    """
    triggers = unreal.TetherGameplayLibrary.get_input_action_triggers(input_action_path)
