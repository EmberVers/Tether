"""Focused stdlib tests for Tether UDP discovery."""
...
MODULE_PATH = (
    ...
    / ".claude" / "skills" / "tether" / "scripts" / "tether_discovery.py"
)
SPEC = importlib.util.spec_from_file_location("tether_discovery", MODULE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError(f"Cannot load discovery module from {MODULE_PATH}")
tether_discovery = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = tether_discovery
SPEC.loader.exec_module(tether_discovery)
...
class FakeDiscoverySocket:
    ...
    def _valid_response(self):
        return {
            "v": tether_discovery.PROTOCOL_VERSION,
            "protocol_version": tether_discovery.PROTOCOL_VERSION,
            "type": "response",
            ...
            "tcp_port": 32123,
            "token_fingerprint": "",
            "capabilities": list(tether_discovery.EXACT_CAPABILITIES),
        }
...
class DiscoveryTests(unittest.TestCase):
    def run_discovery(self, fake_socket, **kwargs):
        with mock.patch.object(tether_discovery.socket, "socket", return_value=fake_socket):
            return tether_discovery.discover(timeout_ms=10, **kwargs)
    ...
    def test_local_scope_is_default_and_uses_zero_multicast_ttl(self):
        ...
        self.assertIn(
            (socket.IPPROTO_IP, socket.IP_MULTICAST_TTL,
             tether_discovery.LOCAL_MULTICAST_TTL),
    ...
    def test_lan_scope_is_explicit_and_uses_one_multicast_ttl(self):
        ...
        self.assertIn(
            (socket.IPPROTO_IP, socket.IP_MULTICAST_TTL,
             tether_discovery.LAN_MULTICAST_TTL),
            fake_socket.sockopts,
        )

    def test_non_multicast_group_and_invalid_scope_fail_before_socket_creation(self):
        with mock.patch.object(tether_discovery.socket, "socket") as socket_factory:
            with self.assertRaises(tether_discovery.DiscoveryError):
                tether_discovery.discover(group="192.0.2.10")
            with self.assertRaises(tether_discovery.DiscoveryError):
                tether_discovery.discover(scope="wide-area")
    ...
    def test_windows_udp_connreset_behavior_is_disabled_when_supported(self):
        fake_socket = FakeDiscoverySocket(response_count=0)
        with (mock.patch.object(tether_discovery.sys, "platform", "win32"),
              mock.patch.object(tether_discovery.socket, "SIO_UDP_CONNRESET",
    ...
    def test_multicast_and_loopback_responses_are_deduplicated_by_instance(self):
        ...
        self.assertEqual(fake_socket.targets, [
            (tether_discovery.DEFAULT_DISCOVERY_GROUP, 9876),
            (tether_discovery.LOCAL_DISCOVERY_HOST, 9876),
        ])
    ...
    def test_loopback_still_discovers_when_multicast_send_fails(self):
        fake_socket = FakeDiscoverySocket(fail_hosts={tether_discovery.DEFAULT_DISCOVERY_GROUP})
    ...
    def test_pid_reuse_with_two_instance_ids_stays_ambiguous(self):
        fields = dict(
            ...
            tcp_port=32123,
            token_fingerprint="",
            capabilities=tether_discovery.EXACT_CAPABILITIES,
            response_host="127.0.0.1",
        )
        endpoints = [
            tether_discovery.Endpoint(instance_id="old-instance", **fields),
            tether_discovery.Endpoint(instance_id="new-instance", **fields),
        ]
        with self.assertRaises(tether_discovery.DiscoveryError):
            tether_discovery.select(endpoints, project_filter="TestProject")
    ...
    def test_malformed_datagrams_do_not_suppress_later_valid_response(self):
        malformed = [
            ...
            {"capabilities": list(tether_discovery.EXACT_CAPABILITIES) + [7]},
            {"capabilities": list(tether_discovery.EXACT_CAPABILITIES) + ["exact_exec"]},
    ...
    def test_minimum_capabilities_allow_unique_forward_compatible_extras(self):
        capabilities = list(tether_discovery.EXACT_CAPABILITIES) + ["exact_future_read"]
    ...
    def test_optional_editor_status_capability_does_not_hide_base_v2_endpoint(self):
        capabilities = list(tether_discovery.REQUIRED_EXACT_CAPABILITIES)
        endpoints = self.run_discovery(FakeDiscoverySocket(
            response_overrides={"capabilities": capabilities}
        ))
        self.assertEqual(len(endpoints), 1)
        self.assertEqual(endpoints[0].capabilities, tuple(capabilities))
        self.assertNotIn(tether_discovery.EXACT_EDITOR_STATUS_CAPABILITY, capabilities)
    ...
    def test_discovery_raises_only_when_every_send_path_fails(self):
        fake_socket = FakeDiscoverySocket(
            fail_hosts={tether_discovery.DEFAULT_DISCOVERY_GROUP,
                        tether_discovery.LOCAL_DISCOVERY_HOST},
            response_count=0,
        )
