...
SPEC = importlib.util.spec_from_file_location("tether_cli_for_health_tests", MODULE_PATH)
...
class HealthClientTests(unittest.TestCase):
    def test_human_render_preserves_v1_status_fields(self):
        rendered = "\n".join(tether._status_lines(sample_health()))
    ...
    def test_never_observed_age_preserves_v1_minus_one_sentinel(self):
        ...
        rendered = "\n".join(tether._status_lines(response))
    ...
    def test_command_selects_exact_cached_status_with_frozen_identity(self):
        ...
        with mock.patch.object(
            ...
            tether, "send_request", return_value=sample_health()
        ) as send, redirect_stdout(stdout):
            exit_code = tether.cmd_status(args)
        ...
        wire = tether._build_exact_payload(payload, identity)
    ...
    def test_status_requires_its_capability_without_hiding_base_endpoint(self):
        ...
        base_capabilities = tuple(
            capability for capability in tether.EXACT_CAPABILITIES
            if capability != tether.EXACT_EDITOR_STATUS_CAPABILITY
        )
        ...
        with mock.patch.object(
            ...
            "resolve_target",
            return_value=("127.0.0.1", 12345, None, None, identity),
        ), mock.patch.object(tether, "send_request") as send, redirect_stdout(stdout):
            exit_code = tether.cmd_status(args)
        ...
        for malformed, expected_error in cases:
            with self.subTest(expected_error=expected_error):
                ...
                with mock.patch.object(
                    ...
                    tether, "send_request", return_value=copy.deepcopy(malformed)
