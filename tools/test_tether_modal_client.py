"""Focused stdlib tests for Tether modal diagnostics and redaction."""
...
SPEC = importlib.util.spec_from_file_location("tether_cli_for_modal_tests", MODULE_PATH)
...
class ModalClientTests(unittest.TestCase):
    def test_human_render_redacts_password_even_if_server_value_is_wrong(self):
        rendered = "\n".join(tether._modal_lines(sample_modal()))
    ...
    def test_timeout_diagnostic_is_structured_and_action_neutral(self):
        ...
        tether._attach_modal_diagnostic(response, sample_modal())
    ...
    def test_probe_uses_modal_bypass_command(self):
        ...
        with mock.patch.object(tether, "send_request", return_value=response) as send:
            modal = tether._probe_modal("127.0.0.1", 12345, None, sample_identity())
    ...
    def test_absent_modal_does_not_relabel_timeout(self):
        ...
        tether._attach_modal_diagnostic(response, {"present": False})
    ...
    def test_exec_timeout_automatically_attaches_modal_snapshot(self):
        ...
        with ExitStack() as stack:
            stack.enter_context(mock.patch.object(
                tether, "resolve_target", return_value=(
                    "127.0.0.1", 12345, None,
                    "C:/Projects/TestProject/TestProject.uproject", sample_identity()
                )
            ))
            stack.enter_context(mock.patch.object(
                tether, "send_request", return_value=server_timeout
            ))
            probe = stack.enter_context(mock.patch.object(
                tether, "_probe_modal", return_value=sample_modal()
            ))
            stack.enter_context(mock.patch.object(tether, "_audit"))
            stack.enter_context(redirect_stdout(stdout))
            exit_code = tether._execute(args, "print('test')")
