...
盘点 Tether 作为 agent ↔ UE 编辑器桥梁的能力缺口，按"能补 / 代价高但理论可行 / 根本补不了"三档划分。用于指导后续投资方向。

最后更新：2026-05-22（A2-#10b UserDefinedStruct 写交付；前序已交付：A1-#2 GBuffer + A1-#3 Perf 快照 + A7-#28 TODO 审计 + A12-#45 Tether 调用日志 + A12-#46 Signature registry dump + A11-#42 Webhook 通知 + A2-#8 AnimGraph/SM 写 + A2-#9 GA/GE/GC 配置）

---

## 现状基线（2026-04-20）

Tether 已覆盖的主要领域（详见各 `tether-*-api.md`）：
...
| 10b | **UserDefinedStruct 写** ✅ | — | — | **已交付 2026-05-22**：新增 `TetherStructLibrary`（22 个库 → 22 个库 +UDS，1033 UFUNCTION）。`create_user_defined_struct` 走 `UserDefinedStructureFactory` + `AssetTools::CreateAsset`；字段 CRUD 全套（add / remove / rename / change_type / set_default / move）走 `FStructureEditorUtils`；元数据 op (tooltip × 2、edit_on_instance) 用 `ChangeVariableTooltip` / `ChangeTooltip` / `ChangeEditableOnBPInstance` 官方 API。底层 `UserDefinedStructureFactory` 与 `StructureEditorUtils` 在 native Python 都没暴露 —— 这是程序化构造 DataTable rowtype + BP 变量类型的唯一通路。`ParseTypeString` 已抽到 `TetherTypeParse` 共用模块，BP 库 5 处调用一并改造 —— 后续给 BP 变量加 Set/Map/Enum 类型支持会同时惠及 struct 字段。|
...
| 13 | **Crash dump 解析** | 中-大 | 低-中 | editor 崩 tether 就断；`Saved/Crashes/` 堆 `.dmp` 没有 API 解析。做法：`rebuild_relaunch.py` 启动前检测最新 crash、读 `.log` + `dbghelp.dll` 符号化栈。 |
...
| 18 | **Enhanced Input IA/IMC 辅助** | 中 | 高 | 现代 UE 输入层没有 tether 封装，只能 raw `unreal.*`。 |
...
| 20 | **Plugin 依赖声明扫描** | 小 | 中 | 构建警告里 `Plugin 'Tether' does not list plugin 'EnhancedInput' as a dependency` 这种应该自动扫 + 修 .uplugin。 |
...
| 28 | **TODO / debug print 审计** ✅ | — | — | **已交付 2026-04-20**：`TetherBlueprintLibrary.find_blueprint_debug_prints` 跨 BP 扫 `PrintString`/`PrintText`/`PrintWarning` 并提取静态字面量；`scripts/audit_tech_debt.py` 编排 BP 扫 + 文件系统 regex 扫 `TODO`/`HACK`/`FIXME`/`XXX` + `UE_LOG(LogTemp`，文本 + JSON 双输出。首次跑 plugin 自身代码即找出 75 处 LOG_TEMP + 1 TODO。 |
...
| 41 | **Named workflow / macro 录制** | 小-中 | 中 | 把一串 tether 调用 + 参数打包成一个"macro"，下次一个 call 复用。Reactive handlers 框架已开了半个头。 |
...
| # | 项目 | 工程量 | 频次 | 说明 |
|---|---|---|---|---|
| 42 | **Webhook on completion** ✅ | — | — | **已交付 2026-04-20**：`scripts/notify.py` — stdlib-only POST，URL 自动识别 Slack / Discord / generic 格式；`--exit-status $?` 自动 success/failure；`TETHER_WEBHOOK_URL` env 默认。 |
...
| 44 | **Perforce / Git LFS 辅助** | 中 | 中-高 | check-out、submit、conflict 检测的 tether 封装（现在的 SourceControl API 太薄，只有 state 查询 + checkout）。大项目必需。 |

### A12. Tether 自观测 / 元工具

对 Tether 本身的自动化，基础设施投入。

| # | 项目 | 工程量 | 频次 | 说明 |
|---|---|---|---|---|
| 45 | **Tether 调用日志 + 性能** ✅ | — | — | **已交付 2026-04-20**：`TetherServer` 在 HandleClient 末尾追加到 `FTetherCallLog` ring buffer（默认 500 条，可调 [10, 5000]）；`TetherEditorLibrary` 暴露 `get_tether_call_log` / `get_tether_call_stats`（avg/min/max/p95 + 总丢弃数）/ `clear_tether_call_log` / 容量 get/set。每条记 request_id, command, script_preview, total/exec ms, success, output/error bytes, endpoint, error_preview。 |
| 46 | **Signature registry dump** ✅ | — | — | **已交付 2026-04-20**：`TetherEditorLibrary.dump_tether_signature_registry()` 一次返回 JSON 描述全部 13 个 `UTether*Library` 的 ~688 个 BlueprintCallable UFUNCTION（class、function、python_name、category、tooltip、params 含 type/default/is_out/is_return）。 |
| 47 | **Tether API changelog 自动生成** | 小 | 低-中 | 每次合并时 diff `Tether*Library.h` 之间的 UFUNCTION 列表，生成 Markdown changelog。不再靠记忆回忆"上周加了啥"。 |
| 48 | **单 call dry-run / trace** | 中 | 低-中 | "这个 tether 调用会动哪些 asset / 改哪些 UObject"，真跑前预览。蓝图 dry-run（#15）的通用版本。 |
...
| **DCC 联动（Blender / Maya / Houdini）** | 跨进程跨沙箱。需要独立 MCP 或单独的桥。Tether 够不着。 |
...
| **自动新增游戏项目 C++ 模块** | 能改 plugin 自己，但往 `<ue-project>/Source/` 塞新 module + 改 .uproject + 让 UBT 识别，这套流程 tether 没做过。 |

---

## Tier C — 根本做不了（约束而非工程量）

1. **听游戏音频** — 编辑器不把 `USoundSubmix` 的原始 buffer 暴露给 BP/Python。要改引擎或注入 NRT 录音插件，超出 tether 范围。
...
8. **编辑器崩溃后的现场分析** — crash 后 tether 断连，重启后进程现场全丢。只能读事后 .dmp。
...
2. **需要调用外部进程并拿结果** — `ffmpeg`、`blender --python`、`git lfs`。`tether.exec` 卡 GameThread，长命令死锁。解法：在 tether 里加 `run_external_process(cmd, timeout)` 走 `FPlatformProcess::CreateProc` + 异步 pipe 读，不是不能做，只是每次都要临时起架子。
