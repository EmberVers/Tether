---
name: tether
description: Execute Python scripts inside a running Unreal Engine 5.3+ editor via TCP tether. Use when the user asks to interact with UE, manipulate assets, query scenes, automate workflows, or run Python in Unreal.
...
If `tether.py` returns `discovery: no Tether editors found`, walk these in order — don't troubleshoot Python or firewalls first:

1. **Plugin and matching skill installed** with `sync_project.bat <UEProjectRoot>`. Check: `<UEProject>/Plugins/Tether/Tether.uplugin` exists. The legacy `sync_plugin.bat` command name accepts the same project-root argument.
2. **Plugin enabled** — check `<UEProject>/<Project>.uproject` `"Plugins"` block for `{"Name":"Tether", "Enabled":false}` and flip if present.
3. **Editor up and ready** — `tether.py ping` returns `"ready": true`. `false` means MainFrame still loading; wait 10–60s.

The loopback probe keeps local discovery working when multicast is blocked by a VPN, virtual NIC, or Windows firewall policy. The discovery group must be an IPv4 multicast address; direct unicast mode is intentionally fail-closed: `--endpoint`, `--instance-id`, `--expected-pid`, and `--expected-project-path` (or matching `UNREAL_TETHER_*` variables) are one inseparable tuple. Copy the identity values verbatim from one discovery response or Server startup line; `project_path` is case- and representation-sensitive on every OS. A host/port alone can be stale and is never used as a legacy fallback. Python 3.9+ stdlib only.
...
i=0
until python .claude/skills/tether/scripts/tether.py --json --timeout 3 ping 2>/dev/null | grep -q '"ready": *true'; do
  now=$(date +%s)
...
- After the loop returns, do **one foreground** `tether.py ping` before real
...
- Skip this entirely when invoking `rebuild_relaunch.py` — it polls
  internally and prints `[rebuild] tether is ready.` on success.

## Tether CLI

```bash
python "${CLAUDE_SKILL_DIR}/scripts/tether.py" [options] <command> [args]
```
...
| `preflight <path>` | Lint a script for tether-call errors WITHOUT sending to UE |
| `suggest [pattern]` | Look up the tether equivalent for a raw `unreal.*` fallback |
...
Tether's modal commands use a separate GameThread task path, so they keep
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/tether.py" --json modal-status
python "${CLAUDE_SKILL_DIR}/scripts/tether.py" modal-click 8f1a2b3c4d5e6f70 0
```
...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/tether.py" exec --stdin <<'EOF'
from tether import Asset
...
**Windows path gotcha (Bash tool only):** unquoted backslashes get eaten by bash word-splitting — `python G:\Claude\Tether\.claude\skills\tether\scripts\tether.py exec ...` arrives as `G:ClaudeTether.claudeskillstetherscriptstether.py` and fails with "No such file or directory". Use forward slashes (`G:/Claude/...`) or single-quote the path (`'G:\Claude\...'`). The body inside `<<'EOF' ... EOF` is literal and unaffected — only the arg before the heredoc matters. PowerShell tool is unaffected.

## API surface — use the wrapper module first

```python
from tether import Asset, Level, Blueprint, Editor, Anim, Rig, Niagara, Material, PoseSearch, Chooser, StateTree, SmartObject, ...
paths, _ = Asset.search_assets_in_all_content(query="Hero", max_results=20)
```

The wrapper has 26 classes (one per `Tether*Library`) with **kwargs-only signatures** — positional args raise `TypeError` immediately, no UE round-trip. This is the structural fix for positional-arg-order hallucinations. Regenerate after C++ header changes via `python tools/gen_manifest.py`.

Fallback: raw `unreal.Tether*Library.foo(...)` works (preflight catches errors), but prefer the wrapper.

## AST preflight (automatic — safety net only)

Every `exec*` call runs an AST preflight in the tether client BEFORE sending. Reads `scripts/tether_manifest.json` and rejects locally (exit 3, no UE round-trip) when:

- Function/library name doesn't exist (with did-you-mean)
- Required args missing / unknown kwarg / param given twice
- `unreal.TetherXxx.YYY` enum member doesn't exist
...
| Symptom | Fix |
|---------|-----|
| Tether call returns `[]` for an asset you know exists | Wrong scope: `PROJECT` covers `/Game` only; plugin assets need `ALL_ASSETS`. Use `Asset.search_assets_in_all_content(...)`. |
...
| Pawn movement script freezes the editor | `time.sleep` inside `exec` blocks GameThread — see `tether-gameplay-api.md` "chase a target" pattern (use `register_runtime_timer`). |
...
| Need "where is this GameplayTag used?" / Find References on a tag | `unreal.TetherGameplayTagLibrary.find_assets_referencing_tag(tag, include_children, ...)`. Mutations: `add_gameplay_tag` / `rename_gameplay_tag` (auto-redirect) / `remove_gameplay_tag`; pick the target ini via `list_tag_source_inis(...)`. For `PrimaryAssetId` / other named-value structs use the generic `TetherAssetLibrary.find_assets_referencing_searchable_name(struct_type, value, ...)`. See `tether-gameplaytag-api.md`. |
...
1. **Tether function names** — `grep scripts/tether_manifest.json` or read the relevant `references/tether-*-api.md`. Don't translate the C++ name into snake_case in your head and assume.
2. **Tether USTRUCT return-value fields** — return types like `TetherGameplayEffectInfo` keep a *minimum* set of fields (not exhaustive); UE 5.x evolves struct shape between versions. Look up the fields in `tether_manifest.json` `structs` section, OR run a one-off probe `print([a for a in dir(obj) if not a.startswith('_')])` BEFORE writing the consumer code.
...
4. **Function signatures** — when unsure: `unreal.TetherXxx.fn.__doc__` first line is the signature. Cheaper than a guess + correction round-trip.
...
If a tether call returns empty / errors, suspect **your parameters** (wrong scope / path / filter), not the wrapper. Falling back to `unreal.AssetRegistryHelpers` etc. bypasses preflight, audit logging, and often walks 100k–2M registry entries.

Preflight emits `[WARN]` on known raw-fallback patterns and shows the tether equivalent inline. To look up before writing: `tether.py suggest [pattern]`. Patterns currently mapped:

| Raw pattern | Tether equivalent |
|---|---|
...
If you genuinely believe no tether function covers your case, **ask the user before reaching for raw `unreal.*`**.
...
|-------|------|------|
| Blueprint queries + authoring | `references/tether-blueprint-api.md` | Class hierarchy, variables/functions/components, node search, write ops, auto-layout flow, lint loop |
| Asset queries | `references/tether-asset-api.md` | Asset lookup, search, references/dependencies, **`SoftObjectPath` stringification** (top-of-file block) |
| UMG / Widget | `references/tether-umg-api.md` | **Read before any Widget Blueprint write.** Asset/tree/layout/style authoring, UI-material brushes, batched widget animations, UE 5.7+ MVVM, compile validation, and PIE functional verification. |
| Animation | `references/tether-anim-api.md` | ABP state machines, slots, sequences, montages, blend spaces. **Authoring an ABP? Read the "Authoring an Animation Blueprint (agent workflow)" section first.** |
| **Control Rig / IK Rig / IK Retargeter** | `references/tether-rig-api.md` | **Read before any rig or retarget asset write.** Type discovery, hierarchy and RigVM authoring, solver/goal/chain setup, retarget ops/mapping/poses/profiles, batch retargeting, compile/processor validation, transient evaluation, animation-quality review, and cleanup. Functional on UE 5.7+. |
| **Niagara / VFX** | `references/tether-niagara-api.md` | **Read before any Niagara asset write.** Template/script discovery, System/Emitter recipes, module inputs, User parameters, renderers/materials/bindings, compile/audit gates, Trail/Sparks/Explosion/Dissolve presets, moving transient previews, and cleanup. Functional on UE 5.7+. |
| DataTable | `references/tether-datatable-api.md` | Schema, rows, fields, search, CSV |
| Material | `references/tether-material-api.md` | Material/master/function inspection and delivery: dependency-complete local graph reads, fail-closed/preflighted graph mutation, compile diagnostics, instances, layers, textures, templates, and previews |
| Level / Actor | `references/tether-level-api.md` | Level queries, spawn/destroy/move, property get/set, selection |
| Editor session | `references/tether-editor-api.md` | Asset open/save, viewport camera, PIE start/stop, console/CVars, BP compile |
| GameplayAbility | `references/tether-gameplayability-api.md` | GA Blueprint metadata |
| **Generic UPROPERTY read/write (privileged)** | `references/tether-property-api.md` | When `get_editor_property` rejects with "protected" / `set_editor_property` rejects EditDefaultsOnly sub-fields / you need to read `private:` / bare `UPROPERTY()` / write nested struct values via export-text. `list_u_properties` / `get_u_property_as_export_text(path, "Modifiers[0].Foo.Bar")` / `set_u_property_from_export_text` (with `fire_change_notify` for editor refresh; native `/Script/` CDO targets need `allow_native_cdo=True`) / `array_append_u_property` (auto-handles FGameplayTagContainer ParentTags) / `array_remove_u_property` / `array_clear_u_property` / `get_asset_cdo_path`. **Privileged** — bypasses UE editor safety net. Use dedicated APIs (`add_gameplay_tag`, `add_ge_modifier_scalable`) when they exist. |
| **GameplayTag references / sources / mutations** | `references/tether-gameplaytag-api.md` | Read: `find_assets_referencing_tag` (with child-tag expansion), `list_all_registered_tags`, `get_tag_source_info`. Write: `add_gameplay_tag(tag, source_ini='', comment='')`, `rename_gameplay_tag(old, new, rename_children=True)` (auto-inserts redirect, redirect lifetime hardened — survives subsequent mutations + cold restart), `remove_gameplay_tag(tag)`. Source enum: `list_tag_source_inis(filter_type='')`. Redirect ops: `list_gameplay_tag_redirects(source='', old_prefix='')` + `remove_gameplay_tag_redirect(old, new)` for enumerate-then-sweep. Built on the AssetRegistry SearchableName index + IGameplayTagsEditorModule. `tether-asset-api.md` "SearchableName Index" has the generic read version (PrimaryAssetId, custom named-value structs). |
| **Motion Matching — PoseSearch** | `references/tether-pose-search-api.md` | **Read before any PSS / PSD read or write** — `DatabaseAnimationAssets` / `Channels` are `private:` and unreachable via `get_editor_property`; this lib is the only path. Includes `wait-pose-index` CLI. |
| **Motion Matching — Chooser** | `references/tether-chooser-api.md` | **Read before any CHT read or write** — `ResultsStructs` / `DisabledRows` etc. are `private:`; this lib is the only path. Covers NestedChooser `:Name` paths, `matched_row=-1` caveat, and the auto Compile+PostEditChange contract. |
| Curve / CurveTable | `references/tether-curve-api.md` | Batch evaluating a curve channel at N points, editing `UCurveTable` rows, atomic tangent writes in one transaction, refreshing open Curve Editor tabs. Single `UCurveFloat` key edits work via raw reflection — read the "when to use this" table first. |
| User-defined structs | `references/tether-struct-api.md` | Create/edit `UUserDefinedStruct` assets — the only path to add/remove/rename/retype/reorder UDS fields (no native Python entry point); also the rowtype for `create_data_table_from_json` and `add_blueprint_variable`. |
| **StateTree authoring + runtime** | `references/tether-statetree-api.md` | **Read before any StateTree write.** GUID-first hierarchy/node/transition workflows, schema-filtered type discovery, reflected node-property discovery, bindings, parameters, compile diagnostics, transient breakpoints, and PIE component control. Functional on UE 5.7+. |
| **Smart Object authoring + runtime** | `references/tether-smartobject-api.md` | **Read before any Smart Object write or claim.** Definition/slot/behavior/data/annotation editing, World Conditions, parameters/bindings, world components and persistent collections, runtime query/claim/occupy/release, tags/events, and entrance validation. Functional on UE 5.7+; claim tokens are session-local and must be released. |
| Reactive handlers | `references/tether-reactive.md` | Register Python on UE events (GameplayEvent / AnimNotify / MovementMode / Attribute / ActorLifecycle / InputAction). |
| Navigation | `references/tether-navigation-api.md` | NavMesh OBJ export |
| **Geometry Script (mesh ops)** | `references/tether-geometry-api.md` | **Read before any dynamic-mesh op.** `UDynamicMesh` handle pool (release when done), load/boolean/smooth/decimate/primitives/save pipeline. UE 5.7+; editor-world only. **Never chain create+load+save in one exec** — the save opens a reference-completing modal that deadlocks the tether. |
| **PCG (read + trigger)** | `references/tether-pcg-api.md` | Read-only PCG graph access, component override edit, Generate/Cleanup trigger. UE 5.7+; editor-world only. No graph editing. **`wait_for_pcg_generate` cannot actually wait** (GT poll loop) — poll across separate execs instead. |
| **Procedural placement** | `references/tether-procedural-api.md` | Level-dressing primitives: deterministic sampling (grid/poisson/surface/landscape/spline/volume, 100k-point cap), filtering, ISM instancing. Combine with Geometry for mesh generation. |
| Perf snapshots | `references/tether-perf-api.md` | Structured FPS / GT / RT / GPU / draw calls / mem / UObject histogram |
| Agent / Gameplay | `references/tether-gameplay-api.md` | **Mandatory before driving the player pawn** — sticky inputs, camera steering, navmesh path planning. |
...
The exact API sequence, MVVM modes, animation key structs, runtime semantics, and cleanup checklist are in `references/tether-umg-api.md`.
...
checklist are in `references/tether-niagara-api.md`.

> **Pawn control is non-obvious — read `tether-gameplay-api.md` first.** Driving the player (sticky `IA_Move`, `apply_look_input`, navigating to a moving target, holding an input over time) has hard constraints a fresh API read won't reveal: `tether.exec` runs on GameThread so `time.sleep` inside one `exec` freezes the engine and stops the sticky ticker; continuous steering must run from a reactive `register_runtime_timer` callback, not a Python `while` loop; `IA_Move` is camera-relative and the forward-axis convention varies per project. The "Pattern: chase a (possibly moving) target and stop on arrival" section has the working template.

> **Asset lookup by name defaults to `search_assets_in_all_content(name, max_results)`.** When the user names an asset without a path, do **not** call `unreal.AssetRegistryHelpers.get_assets_by_path('/Game', recursive=True)` and filter — that walks 100k–2M+ entries and times out. The full `search_assets` form needs `TetherAssetSearchScope.ALL_ASSETS` (not `PROJECT`) when the asset might live in a plugin mount (`/PluginName/...`). `PROJECT` covers `/Game` only; using it for a plugin asset returns `[]` silently. Valid scope members: `ALL_ASSETS`, `PROJECT`, `CUSTOM_PACKAGE_PATH` — there is no `GAME_FOLDER`.
...
1. State the tradeoff. Tether has full graph-write primitives but agent-Graphs are weaker than C++.
2. Offer alternative: a C++ `UFUNCTION(BlueprintCallable)` with thin BP wrapper, or another tether surface (Material / Anim / UMG / DataTable / Level).
...
3. auto_layout — MANDATORY after ANY graph mutation; use 'pin_aligned' (3-exec flow) or 'exec_flow' (bulk).
                 See tether-blueprint-api.md for the exact open→sleep→layout sequence.
...
Tether is UTF-8 byte-perfect end-to-end since 2026-05-04. **Mojibake = display, not data.** Confirm with hex:

```python
print(got.encode('utf-8').hex(), '==', '测试'.encode('utf-8').hex())
```

If hex matches, it's a Windows cp936/cp1252 console issue — fix the *display*, not the tether:

| Situation | Fix |
|---|---|
| Piping `tether.py ... > out.txt` writes cp936 | prefix `PYTHONIOENCODING=utf-8` (bash) / `$env:PYTHONIOENCODING='utf-8'` + `\| Out-File -Encoding utf8` (PS) |
| Reading saved file back | `open(p, encoding='utf-8')` (or `utf-8-sig` for PS BOM) |
| Calling tether.py from another Python process | set `PYTHONIOENCODING=utf-8` in subprocess env |
...
- **Exec response semantics:** `success` reflects execution only — stderr (warnings, log noise) is preserved in `error` and may be non-empty with `success=true`, so don't treat "error text present" as failure on its own. Each of `output`/`error` is capped at 8 MiB of UTF-8 bytes; a `"truncated": true` field means the field was clipped — page or reduce the output rather than re-running. The client rejects response frames over 10 MiB as a protocol error; if you hit it, lower the print volume.
