...
Reads `TETHER_WEBHOOK_URL` as the default `--url`, so a one-shot
...
def main() -> int:
    ...
    p.add_argument(
        "--url",
        default=os.environ.get("TETHER_WEBHOOK_URL", ""),
        help="Webhook URL. Defaults to TETHER_WEBHOOK_URL env var.",
    )
    ...
    p.add_argument("--source", default=f"Tether@{socket.gethostname()}")
    p.add_argument("--timeout", type=float, default=10.0)
    p.add_argument("--quiet", action="store_true")
    args = p.parse_args()

    if not args.url:
        print("ERROR: --url is required (or set TETHER_WEBHOOK_URL).", file=sys.stderr)
        return 2
