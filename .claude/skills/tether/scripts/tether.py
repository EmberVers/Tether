#!/usr/bin/env python3
"""
Tether CLI — execute Python in a running Unreal Editor over a TCP tether.

Usage:
    python tether.py ping                          # Check connection
    python tether.py exec "print('hello')"         # Execute inline code (single statement)
    python tether.py exec --stdin <<'EOF'          # Multi-step, one-shot, no disk file
    import unreal
    print(unreal.SystemLibrary.get_project_directory())
    EOF
    python tether.py exec -                        # `-` is shorthand for --stdin
    python tether.py exec-file script.py           # Multi-step, will iterate / keep on disk
    python tether.py exec "code" --json            # Machine-readable output
    python tether.py status                        # Read cached Engine/Slate health
    python tether.py modal-status                  # Inspect a blocking Slate dialog
    python tether.py modal-click <snapshot> 0      # Click after semantic review
...
Saved/Tether/exec.log (ring-buffered, 5 MB × 3 backups = 20 MB hard cap).
...
Environment variable fallbacks:
    UNREAL_TETHER_PROJECT, UNREAL_TETHER_ENDPOINT, UNREAL_TETHER_TOKEN,
    UNREAL_TETHER_INSTANCE_ID, UNREAL_TETHER_PID, UNREAL_TETHER_PROJECT_PATH,
    UNREAL_TETHER_DISCOVERY_GROUP, UNREAL_TETHER_DISCOVERY_SCOPE
...
try:
    ...
    # Support being launched as `python /abs/path/tether.py` — add scripts/
    # dir to sys.path and retry.
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from tether_discovery import (  # noqa: E402
        DEFAULT_DISCOVERY_GROUP,
        DEFAULT_DISCOVERY_PORT,
        DEFAULT_DISCOVERY_SCOPE,
        DEFAULT_DISCOVERY_TIMEOUT_MS,
...
def _resolve_discovery_scope(args) -> str:
    ...
    raw_scope = (getattr(args, "discovery_scope", None)
                 or os.environ.get("UNREAL_TETHER_DISCOVERY_SCOPE")
...
def resolve_target(args) -> "tuple[str, int, str | None, str | None, Endpoint]":
    ...
    log under Saved/Tether/. None when --endpoint skipped discovery.

    Precedence:
      1. --endpoint=host:port (or UNREAL_TETHER_ENDPOINT)
      2. Host-local UDP multicast + loopback discovery, filtered by --project
         (or UNREAL_TETHER_PROJECT)
    """
    ...
    # 2. Discovery.
    project = getattr(args, "project", None) or os.environ.get("UNREAL_TETHER_PROJECT") or "*"
    group_str = (getattr(args, "discovery_group", None)
                 or os.environ.get("UNREAL_TETHER_DISCOVERY_GROUP")
    ...
    try:
        ...
        token = load_token(ep, explicit_token=getattr(args, "token", None)
                           or os.environ.get("UNREAL_TETHER_TOKEN"))
...
def _audit_log_path(project_path: "str | None") -> "str | None":
    ...
    Priority: UNREAL_TETHER_AUDIT_LOG env override → <project>/Saved/Tether/exec.log
    → None (disabled) when neither is available (e.g. --endpoint without env override).
    """
    override = os.environ.get("UNREAL_TETHER_AUDIT_LOG")
    if override:
        return override
    if project_path:
        saved_dir = os.path.join(os.path.dirname(project_path), "Saved", "Tether")
...
def _get_audit_logger(log_path: str) -> logging.Logger:
    ...
    Multiple tether.py invocations can race on rotation (RotatingFileHandler
    ...
    logger = logging.getLogger(f"tether.audit.{log_path}")
...
if not __ub_os_for_json.environ.get("UNREAL_TETHER_NO_JSON_COMPACT") and not getattr(__ub_json, "__ub_patched_marker__", False):
...
def __ub_resolve_for_enrich(__ub_exc):
    ...
    # Stage: UPROPERTYs via reflection (PascalCase) — uses tether function we already shipped
    __ub_uprops = []
    if __ub_cls_path and __ub_cls_name != "unreal":
        try:
            __ub_pinfos = __ub_unreal.TetherLevelLibrary.list_class_properties(__ub_cls_path)
...
def _wrap_for_attr_enrichment(user_code: str) -> str:
    ...
    """
    if os.environ.get("UNREAL_TETHER_NO_ATTR_ENRICH"):
        return user_code
...
def _preflight_or_skip(code: str,
    ...
    """
    try:
        from tether_preflight import lint  # local import keeps cold-start cheap
    except ImportError:
        return [], []
    try:
...
class TetherProtocolError(ConnectionError):
...
def _decode_response_frame(frame: bytes) -> dict:
    ...
    try:
        ...
        raise TetherProtocolError(f"response is not valid UTF-8: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise TetherProtocolError(f"response is not valid JSON: {exc}") from exc
    if not isinstance(response, dict):
        raise TetherProtocolError("response JSON root must be an object")
    return response
...
def _validate_editor_status_response(response: dict) -> None:
    ...
    if type(response.get("success")) is not bool:
        raise TetherProtocolError("editor status response success must be a boolean")
    if not response["success"]:
        return
    for field in ("output", "error", "ui_state"):
        if type(response.get(field)) is not str:
            raise TetherProtocolError(f"editor status response {field} must be a string")
    if type(response.get("ready")) is not bool:
        raise TetherProtocolError("editor status response ready must be a boolean")

    status = response.get("editor_status")
    if not isinstance(status, dict):
        raise TetherProtocolError("successful editor status response is missing editor_status object")
    if type(status.get("schema_version")) is not int or status["schema_version"] != 1:
        raise TetherProtocolError("unsupported editor_status schema_version; expected integer 1")
    ...
    if not status["ui_state"]:
        raise TetherProtocolError("editor_status.ui_state must not be empty")
    for field in ("slate_tick_sequence", "engine_tick_sequence", "attention_id"):
        value = _require_status_type(status, field, int)
        if value < 0:
            raise TetherProtocolError(f"editor_status.{field} must be non-negative")
    ...
    if not isinstance(modal, dict):
        raise TetherProtocolError("editor_status.active_modal must be an object")
    ...
    for field in ("button_count", "input_count", "checkbox_count"):
        ...
        if value < 0:
            raise TetherProtocolError(f"editor_status.active_modal.{field} must be non-negative")

    if response["ready"] != status["editor_ready"]:
        raise TetherProtocolError("editor status ready fields disagree")
    if response["ui_state"] != status["ui_state"]:
        raise TetherProtocolError("editor status ui_state fields disagree")
    if status["attention_required"] != modal["present"]:
        raise TetherProtocolError("editor status attention fields disagree")
...
def _attach_modal_diagnostic(response: dict, modal: "dict | None") -> dict:
    ...
    hint = (
        "Inspect/refresh with `tether.py modal-status`; act only after reading "
    ...
    try:
        ...
        if args.json:
            ...
            print(
                ...
                "Is Unreal Editor running with Tether plugin enabled?",
                file=sys.stderr,
            )
        return 1
    ...
    if args.json:
        ...
        if resp.get("success"):
            ...
            if ready is False:
                ...
                suffix = " (ready)"
            print(f"Connected to Tether at {host}:{port}{suffix}")
        else:
...
def cmd_wait_compile(args):
    ...
    Each poll is one fast tether.exec (GetMaterialShaderCompileStatus), the
    ...
    async completion callbacks while tether is holding GT itself. See
...
def cmd_wait_pose_index(args):
    ...
    Each poll calls TetherPoseSearchLibrary.is_index_ready, which
...
def cmd_suggest(args):
    """Look up tether equivalents for raw unreal.* fallback patterns."""
    try:
        from tether_preflight import load_redirects
    except ImportError:
        print("ERROR: tether_preflight unavailable", file=sys.stderr)
        return 2
    redirects = load_redirects()
    if not redirects:
        print("ERROR: tether_redirects.json not found", file=sys.stderr)
        return 2
    ...
    if not matches:
        print(f"No redirect matches '{args.pattern}'.\n"
              f"List everything with: tether.py suggest")
        return 1

    for haystacks, e in matches:
        primary = haystacks[0] if haystacks else "(unknown)"
        print(f"-- {primary} --")
        tether = e.get("tether_replacement", "")
        for line in tether.splitlines():
            print(f"  {line}")
        reason = e.get("reason", "")
        if reason:
            print(f"  why: {reason}")
...
def _add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="local keeps multicast at TTL 0 (default); lan opts into TTL 1 "
             "(or set UNREAL_TETHER_DISCOVERY_SCOPE).",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="tether",
        description="Tether — execute Python in Unreal Engine from the command line",
    )
    parser.add_argument(
        "--timeout",
        type=float,
    ...
    parser.add_argument(
        "--no-preflight",
        action="store_true",
        help="Skip the AST preflight that validates unreal.Tether*Library "
    ...
    pf_parser = subparsers.add_parser(
        "preflight",
        help="Lint a script for tether-call errors WITHOUT sending it to UE",
    )
    ...
    sg_parser = subparsers.add_parser(
        "suggest",
        help="Look up the tether equivalent for a raw unreal.* fallback pattern",
    )
    ...
    wc_parser.add_argument("material_path",
        help="Material or MI asset path, e.g. /Game/TetherTemplates/MI_Foo")
