...
def select(endpoints: List[Endpoint],
    ...
    if not endpoints:
        raise DiscoveryError(
            "no Tether editors found (multicast and local probes timed out). "
            "Check — in this order:\n"
            "  1. UE editor is running (and loaded past the splash screen).\n"
            "  2. The Tether plugin is installed in that project's "
...
def load_token(ep: Endpoint, explicit_token: Optional[str] = None) -> Optional[str]:
    ...
    Priority: explicit CLI token → env UNREAL_TETHER_TOKEN →
    <Project>/Saved/Tether/token.txt (the path the server writes it to).
    ...
    if explicit_token:
        ...
        if not verified:
            raise DiscoveryError(
                ...
                "Check <Project>/Saved/Tether/token.txt for the current value."
            )
        return verified

    env_token = os.environ.get("UNREAL_TETHER_TOKEN")
    if env_token:
    ...
    if ep.project_path:
        ...
        token_file = os.path.join(saved_dir, "Saved", "Tether", "token.txt")
    ...
    raise DiscoveryError(
        ...
        "Pass --token=<secret>, set UNREAL_TETHER_TOKEN, "
        "or ensure <Project>/Saved/Tether/token.txt is readable."
    )
...
def _cli():
    """Small command-line driver: `python tether_discovery.py` to list editors."""
    import argparse
    parser = argparse.ArgumentParser(
        description="List every Tether editor reachable via UDP discovery.")
