"""Shared discovery and invocation for Tether project synchronization."""
...
SOURCE_ROOT_MARKER = (
    CONTAINING_ROOT / "Saved" / "Tether" / "source-root.txt"
)
...
def resolve_sync_script(source_dir: str | None = None) -> pathlib.Path | None:
    ...
    # Supports invoking the helper directly from an Tether source checkout.
...
def run_project_sync(
    ...
    *,
    ...
    if sync_script is None:
        sys.stderr.write(
            "Could not locate Tether sync_project.bat. "
            "Pass --sync-source <Tether repo root>, or run "
            "sync_project.bat once so the target project's "
            "Saved/Tether/source-root.txt marker is created.\n"
        )
        return 3
    ...
    if process.returncode != 0:
        ...
        sys.stderr.write(
            f"Tether project sync failed (rc={process.returncode})\n"
        )
        return 3
    return 0
