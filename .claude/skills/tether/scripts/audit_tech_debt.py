...
Scans for:
  1. Blueprint `PrintString` / `PrintText` / `PrintWarning` call sites
     (via `TetherBlueprintLibrary.FindBlueprintDebugPrints`).
...
Exit codes:
    ...
    1  tether unreachable / exec failed and --skip-bp was not set
"""
...
# scripts → tether → skills → .claude → <repo-root>
...
BP_EXEC_SCRIPT = """
import unreal, json
sites = unreal.TetherBlueprintLibrary.find_blueprint_debug_prints({pkg!r}, {cap})
out = []
for s in sites:
    out.append({{
...
print('TETHER_AUDIT_JSON_START' + json.dumps(out) + 'TETHER_AUDIT_JSON_END')
"""
...
def scan_blueprints(package_path: str, max_results: int) -> list[dict] | None:
    ...
    # tether.py puts --json before the subcommand (argparse parses it at the
    # top-level parser, not the subparser).
    result = subprocess.run(
        [sys.executable, str(TETHER_PY), "--json", "exec", script],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        print(f"[audit] BP scan tether call failed:\n{result.stderr.strip()}", file=sys.stderr)
        return None
    try:
        response = json.loads(result.stdout)
    except json.JSONDecodeError:
        print(f"[audit] tether --json response wasn't JSON:\n{result.stdout}", file=sys.stderr)
        return None
    if not response.get("success"):
        err = response.get("error") or response.get("output") or response
        print(f"[audit] tether exec reported failure:\n{err}", file=sys.stderr)
        return None
    output = response.get("output", "")
    match = re.search(
        r"TETHER_AUDIT_JSON_START(.*?)TETHER_AUDIT_JSON_END",
        output,
        re.DOTALL,
    )
    if not match:
        print(
            f"[audit] tether output missing sentinel markers:\n{output}",
            file=sys.stderr,
        )
        return None
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Default scans the repo's Plugin/Tether/Source + all Blueprints "
            "under /Game via the running UE editor."
        ),
    )
    parser.add_argument("--skip-bp", action="store_true", help="Skip Blueprint scan (no tether call).")
