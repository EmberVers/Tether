#!/usr/bin/env python3
"""Full C++ rebuild + editor relaunch for the Tether plugin.
...
Flow:
  1. (if editor is up) ask it to quit_editor via the tether
  ...
  6. poll `tether.py ping` until ready (or --no-wait)
...
TETHER_PY  = SCRIPT_DIR / "tether.py"
...
def get_editor_info_for_uproject(uproject: pathlib.Path,
    ...
    """Query tether discovery; return (matching_pid, discovery_succeeded).
...
def quit_editor_gracefully(timeout_s: float,
    ...
    """Try tether.exec('quit_editor()'); wait for the target editor to exit.

    `project_name` (the .uproject stem) is forwarded as tether.py's `--project`
    ...
    cmd = [sys.executable, str(TETHER_PY)]
    if project_name:
...
def wait_for_tether(timeout_s: float, project_name: "str | None" = None) -> bool:
    deadline = time.time() + timeout_s
    last_reported = 0.0
    cmd = [sys.executable, str(TETHER_PY), "--json"]
    if project_name:
        cmd += ["--project", project_name]
    cmd += ["--timeout", "3", "ping"]
    while time.time() < deadline:
        ...
        if now - last_reported > 15:
            ...
            print(f"[rebuild] waiting for tether ... ({remaining}s remaining)")
            last_reported = now
        time.sleep(2.0)
    return False
...
def main() -> int:
    ...
    ap.add_argument("--sync-source",
                    help="Tether source-repo root. Defaults to the source "
    ...
    ap.add_argument("--no-wait", action="store_true",
                    help="Don't poll the tether after launching.")
    ap.add_argument("--wait-timeout", type=float, default=300,
                    help="Seconds to wait for tether to come up (default: 300).")
    ...
    if target_pid is not None:
        ...
        if args.allow_global_kill:
            ...
            sys.stderr.write(
                ...
                "Either: (a) check that Tether plugin is enabled in the "
    ...
    # 6: wait for tether — filter by project so we don't lock onto a
    # sibling editor's tether while ours is still loading.
    if not args.no_launch and not args.no_wait:
        if wait_for_tether(args.wait_timeout, project_name=uproject.stem):
            print("[rebuild] tether is ready.")
            return 0
        sys.stderr.write(
            f"Tether did not come up within {args.wait_timeout:.0f}s.\n"
        )
        return 7
