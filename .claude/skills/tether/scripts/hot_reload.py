#!/usr/bin/env python3
"""Hot-reload the Tether plugin via Live Coding (no editor restart).
...
running editor in place; the TCP tether, open assets, and PIE state all
survive.

Flow:
  1. ping tether (bail out if the editor isn't up)
  2. sync the plugin and matching skills (unless --no-sync)
  3. call TetherEditorLibrary.TriggerLiveCodingCompile via tether
  4. report Status (Success / NoChanges / Failure / ...)

Exit codes:
    0  Success or NoChanges
    1  tether ping / exec failed
...
def run_tether(argv: list[str], capture: bool = True) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, str(TETHER_PY), *argv],
...
def ping() -> bool:
    p = run_tether(["ping"])
    if p.returncode != 0:
        sys.stderr.write(p.stderr or "tether ping failed\n")
        return False
    return True
...
def trigger_live_coding(wait: bool, timeout: int) -> dict:
    ...
    script = (
        "import unreal, json\n"
        f"r = unreal.TetherEditorLibrary.trigger_live_coding_compile({wait_lit})\n"
        "log_path = unreal.TetherEditorLibrary.get_log_file_path()\n"
        "print(json.dumps({"
    ...
    p = run_tether(["--json", "--timeout", str(timeout), "exec", script])
    if p.returncode != 0:
        sys.stderr.write(p.stderr or "tether exec failed\n")
        return {}
    try:
        envelope = json.loads(p.stdout)
    except json.JSONDecodeError:
        sys.stderr.write(f"Bad tether envelope: {p.stdout!r}\n")
        return {}
...
def main() -> int:
    ...
    ap.add_argument("--timeout", type=int, default=300,
                    help="Tether exec timeout in seconds (default: 300).")
    args = ap.parse_args()
