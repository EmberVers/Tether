...
`unreal.TetherGameplayAbilityLibrary` — introspection for the GameplayAbilitySystem plugin (abilities, effects, attribute sets, ability system components, gameplay tags).
...
### get_gameplay_ability_blueprint_info(ability_blueprint_path) -> FTetherGameplayAbilityInfo

Read metadata from a `UGameplayAbility` Blueprint's CDO.

```python
info = unreal.TetherGameplayAbilityLibrary.get_gameplay_ability_blueprint_info(
...
### FTetherGameplayAbilityInfo fields
...
### get_gameplay_effect_blueprint_info(effect_blueprint_path) -> FTetherGameplayEffectInfo
...
```python
info = unreal.TetherGameplayAbilityLibrary.get_gameplay_effect_blueprint_info(
...
### FTetherGameplayEffectInfo fields
...
| `modifiers` | list[FTetherGEModifierInfo] | Modifier entries |
| `stacking_type` | str | `None` / `AggregateBySource` / `AggregateByTarget` |
| `stack_limit_count` | int | Stack cap |
| `components` | list[FTetherGEComponentInfo] | Attached `UGameplayEffectComponent`s |

### FTetherGEModifierInfo fields
...
### get_attribute_set_info(attribute_set_class_path) -> FTetherAttributeSetInfo
...
```python
info = unreal.TetherGameplayAbilityLibrary.get_attribute_set_info(
...
### FTetherAttributeSetInfo fields

| Field | Type | Description |
|-------|------|-------------|
| `name` | str | Class name |
| `parent_class_name` | str | Super class name |
| `attributes` | list[FTetherAttributeInfo] | Reflected attribute fields |

### FTetherAttributeInfo fields
...
```python
paths = unreal.TetherGameplayAbilityLibrary.list_abilities_by_tag('Ability.Combat', 50)
for p in paths:
    print(p)

ge_paths = unreal.TetherGameplayAbilityLibrary.list_gameplay_effects_by_tag('Effect.Buff', 50)
```
...
### get_actor_ability_system_info(actor_name) -> FTetherActorAbilitySystemInfo
...
```python
info = unreal.TetherGameplayAbilityLibrary.get_actor_ability_system_info('BP_Hero_C_1')
if info.found:
...
### FTetherActorAbilitySystemInfo fields

| Field | Type | Description |
|-------|------|-------------|
| `actor_name` | str | Resolved actor label |
| `found` | bool | `True` if an ASC was located |
| `granted_abilities` | list[FTetherGrantedAbilityInfo] | Ability specs currently granted |
...
### FTetherGrantedAbilityInfo fields
...
```python
sets = unreal.TetherGameplayAbilityLibrary.list_attribute_sets('MyGame', 50)
for s in sets:
    print(s)
```
...
### get_attribute_value(actor_name, attribute_name) -> FTetherAttributeValue
...
```python
v = unreal.TetherGameplayAbilityLibrary.get_attribute_value('BP_Hero_C_1', 'Health')
if v.found:
    print(f'{v.attribute_name}: {v.current_value} (base {v.base_value})')
```

### FTetherAttributeValue fields
...
### get_actor_active_effects(actor_name, max_results) -> list[FTetherActiveEffectInfo]
...
```python
effs = unreal.TetherGameplayAbilityLibrary.get_actor_active_effects('BP_Hero_C_1', 0)
for e in effs:
...
### FTetherActiveEffectInfo fields
...
```python
# Direct children
immediate = unreal.TetherGameplayAbilityLibrary.find_child_tags('Mover', False)
# Whole subtree
subtree = unreal.TetherGameplayAbilityLibrary.find_child_tags('Ability.Combat', True)
```
...
```python
# "A.B.C" -> ["A", "A.B"]
print(unreal.TetherGameplayAbilityLibrary.get_tag_parents('Mover.IsOnGround'))
# -> ['Mover']
```
...
```python
r = unreal.TetherGameplayAbilityLibrary.actor_has_gameplay_tag(
...
### get_ability_cooldown_info(actor_name, ability_blueprint_path) -> FTetherAbilityCooldownInfo
...
```python
info = unreal.TetherGameplayAbilityLibrary.get_ability_cooldown_info(
...
### FTetherAbilityCooldownInfo fields
...
### find_active_effects_by_tag(actor_name, tag_query, max_results) -> list[FTetherActiveEffectInfo]
...
Invalid `tag_query` / missing actor / missing ASC → empty list (silent). Returns the same `FTetherActiveEffectInfo` struct as `get_actor_active_effects`.

```python
dots = unreal.TetherGameplayAbilityLibrary.find_active_effects_by_tag(
...
```python
all_abilities = unreal.TetherGameplayAbilityLibrary.list_ability_blueprints('/Game/Abilities', 100)
...
```python
ges = unreal.TetherGameplayAbilityLibrary.list_gameplay_effect_blueprints('/Game/Effects', 100)
for p in ges:
    print(p)
```
...
```python
loaded = unreal.TetherGameplayAbilityLibrary.list_attribute_sets('', 200)
bp = unreal.TetherGameplayAbilityLibrary.list_attribute_set_blueprints('', 200)
...
```python
GA = unreal.TetherGameplayAbilityLibrary
...
### get_actor_attributes(actor_name) -> list[FTetherAttributeValue]
...
```python
for a in unreal.TetherGameplayAbilityLibrary.get_actor_attributes('BP_Hero_C_1'):
...
```python
tags = unreal.TetherGameplayAbilityLibrary.list_gameplay_tags('Ability.Combat', 100)
for t in tags:
    print(t)
```
...
### get_ability_tag_requirements(ability_blueprint_path) -> FTetherAbilityTagRequirements
...
```python
req = unreal.TetherGameplayAbilityLibrary.get_ability_tag_requirements(
...
### FTetherAbilityTagRequirements fields
...
### get_ability_triggers(ability_blueprint_path) -> list[FTetherAbilityTriggerInfo]
...
```python
for t in unreal.TetherGameplayAbilityLibrary.get_ability_triggers(
        '/Game/Abilities/GA_OnDamageReaction'):
    print(t.trigger_tag, t.trigger_source)
```

### FTetherAbilityTriggerInfo fields
...
```python
blocked = unreal.TetherGameplayAbilityLibrary.get_actor_blocked_ability_tags('BP_Hero_C_1')
for t in blocked:
    print('blocked:', t)
```
...
### get_actor_active_abilities(actor_name) -> list[FTetherActiveAbilityInfo]
...
```python
for a in unreal.TetherGameplayAbilityLibrary.get_actor_active_abilities('BP_Hero_C_1'):
    print(f'{a.ability_class_name} lvl={a.level} instances={a.active_count}')
```

### FTetherActiveAbilityInfo fields
...
```python
rv = unreal.TetherGameplayAbilityLibrary.send_gameplay_event_by_name(
    'BP_UnitCharacterBase_C_0', 'Event.Combat.Hit', 25.0)
print(rv)   # -1 = no ASC; 0 = fired but no ability triggered; >0 = ability count
```

Pairs naturally with `TetherReactiveLibrary.register_runtime_gameplay_event` — this is the firing side, reactive is the listening side.
...
name = unreal.TetherGameplayLibrary.spawn_actor_in_pie(
    '/Script/Engine.Actor', unreal.Vector(0,0,500), unreal.Rotator(0,0,0))
unreal.TetherGameplayAbilityLibrary.ensure_ability_system_component(name)

# Exercise the Pawn→Controller walker branch:
pawn = unreal.TetherGameplayLibrary.get_all_pawns()[0]
unreal.TetherGameplayAbilityLibrary.ensure_ability_system_component(pawn, 'Controller')
```

### ensure_tether_test_attribute_set(actor_name) -> bool

Register a `UTetherTestAttributeSet` (Health + Mana, each initialised to 100) on the actor's ASC. Resolves the ASC via the standard walker. Safe to call multiple times — no-op if already spawned.

Required before `register_runtime_attribute_changed` can match "Health" / "Mana" attributes on the actor.

```python
unreal.TetherGameplayAbilityLibrary.ensure_tether_test_attribute_set('BP_UnitCharacterBase_C_0')
```

### set_actor_attribute_value(actor_name, attribute_name, value) -> bool

Write a numeric attribute base value on the actor's ASC via `SetNumericAttributeBase`. Fires the attribute-value-change delegate that `FTetherAttributeChangedAdapter` listens to.

`attribute_name` accepts `"AttrSet.Field"` or bare `"Field"`.

```python
# After ensure_tether_test_attribute_set:
unreal.TetherGameplayAbilityLibrary.set_actor_attribute_value(
...
`TetherReactiveLibrary` is a separate library that registers **Python scripts to run when ASC / character / actor events fire**. If you want a script that reacts to `Event.Combat.Hit` without polling, read `tether-reactive.md`.

Key links between this library and the reactive framework:
...
```python
import unreal
L = unreal.TetherGameplayAbilityLibrary
...
Author the `ActivateAbility` / `ActivateAbilityFromEvent` / `EndAbility` / `CanActivateAbility` graphs on a `UGameplayAbility` Blueprint — spawn `UAbilityTask_*` async nodes and `K2Node_CallFunction` nodes pointing at GA methods. Pair these with the node-connection UFUNCTIONs in `tether-blueprint-api.md` (`connect_pins`, `disconnect_pins`) to wire a complete activation flow.
...
```python
tasks = unreal.TetherGameplayAbilityLibrary.list_ability_task_classes("Wait", 10)
...
```python
factories = unreal.TetherGameplayAbilityLibrary.list_ability_task_factories(
    "/Script/GameplayAbilities.AbilityTask_WaitGameplayEvent")
# ['WaitGameplayEvent']
```
...
```python
guid = unreal.TetherGameplayAbilityLibrary.add_ability_task_node(
...
# via unreal.TetherBlueprintLibrary.connect_pins.
...
**Pitfalls**
- The node spawner doesn't auto-connect `OwningAbility` pins — async tasks all have an `OwningAbility` input that defaults to the containing BP's `self`, but the connection only happens at compile time if left unconnected. For explicit wiring, grab a `Self` reference via `unreal.TetherBlueprintLibrary.add_self_node` and connect it.
...
# 3. Wrapped magnitude structs — tether C++ helpers (Python blocked)
...
# 5. GEComponents — add via tether, configure inner tags via tether
...
# the tether yet; fall back to tether-blueprint-api.md generic node-write
...
### find_gameplay_tag_references(tag_query, package_path, match_exact, max_results) -> FTetherTagReferenceReport
...
Returned `FTetherTagReferenceReport`:
...
| `references` | List of `FTetherTagReference`: `asset_path`, `asset_class`, `location` (e.g. `"GEComponents[0]->InheritableAssetTags.Added"`), `context` (`"CDO"` / `"Row: <name>"` / `"Graph: X, Node: Y"`), `matched_tag` (the actual tag found — useful when `match_exact=False`). |
...
- BP graph pin scan only catches *literal* defaults (`Make_LiteralGameplayTag` style). Tags computed at runtime (e.g. read from a variable, returned from a function) are invisible — you'd need execution-tracing for that, which the tether doesn't have.
