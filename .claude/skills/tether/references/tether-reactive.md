...
- You want **persistent behaviour** that outlives a single tether call.
...
Before your script runs, the tether injects a preamble that provides:
...
tether_state        # dict shared across ALL handlers — use for cross-handler coordination
state               # dict private to THIS handler (keyed by handler_id internally)

# Helpers
log(msg)            # -> '[reactive:rt_0042_b3a1|task_name] <msg>' in LogTether
...
```python
hid = unreal.TetherReactiveLibrary.register_runtime_gameplay_event(
    ...
    script="""
        tether_state.setdefault('hit_count', 0)
        tether_state['hit_count'] += 1
        log('took a hit, total=' + str(tether_state['hit_count']))
    """,
    script_path="",
    tags=["combat"],
    lifetime="Permanent",
...
> **ASC resolution.** The target actor doesn't need a direct ASC — the tether walks `Actor → Pawn.PlayerState → Pawn.Controller → PC.PlayerState`. Fires `SendGameplayEventByName` through the same walker.
...
hid = unreal.TetherReactiveLibrary.register_runtime_attribute_changed(
    ...
    target_actor_name="BP_UnitCharacterBase_C_0",
    attribute_name="Health",   # or "TetherTestAttributeSet.Health" for disambiguation
    script="""
        if ctx['old_value'] >= 25 and ctx['new_value'] < 25:
            log('CRITICAL: health dropped under 25')
            tether_state['warn_played'] = True
    """,
    script_path="",
...
```python
hid = unreal.TetherReactiveLibrary.register_runtime_actor_lifecycle(
    task_name="enemy_killed",
    description="Grant XP when Enemy_42 dies",
    target_actor_name="BP_Enemy_C_3",
    event_type="Destroyed",
    script="tether_state.setdefault('kills', 0); tether_state['kills'] += 1",
    script_path="", tags=[],
...
```python
hid = unreal.TetherReactiveLibrary.register_runtime_movement_mode_changed(
    ...
    script="""
        if ctx['prev_mode_name'] == 'Falling' and ctx['current_mode_name'] == 'Walking':
            log('landed!')
            tether_state['last_landing_time'] = unreal.SystemLibrary.get_game_time_in_seconds(ctx['character'])
    """,
...
```python
hid = unreal.TetherReactiveLibrary.register_runtime_timer(
    task_name="hp_regen",
    description="Every 0.5s: add 1 HP if below max",
    interval_seconds=0.5,
    script="""
        import unreal as u
        info = u.TetherGameplayAbilityLibrary.get_actor_attributes('BP_UnitCharacterBase_C_0')
...
```python
hid = unreal.TetherReactiveLibrary.register_runtime_input_action(
    task_name="jump_log",
...
```python
hid = unreal.TetherReactiveLibrary.register_editor_asset_event(
    ...
    script="""
        ...
        tether_state.setdefault('renames', []).append(
...
```python
hid = unreal.TetherReactiveLibrary.register_editor_pie_event(
...
hid = unreal.TetherReactiveLibrary.register_editor_bp_compiled(
...
hid_any = unreal.TetherReactiveLibrary.register_editor_bp_compiled(
    task_name="bp_compile_counter",
    description="Count total compiles across the session",
    blueprint_path_filter="",          # empty → global
    script="tether_state['n_compiles'] = tether_state.get('n_compiles', 0) + 1",
...
summaries = unreal.TetherReactiveLibrary.list_all_handlers(
...
detail = unreal.TetherReactiveLibrary.get_handler(handler_id)
print(detail.script)

# Runtime stats.
stats = unreal.TetherReactiveLibrary.get_handler_stats(handler_id)
...
unreal.TetherReactiveLibrary.pause(handler_id)
unreal.TetherReactiveLibrary.resume(handler_id)

# Unregister a single handler.
unreal.TetherReactiveLibrary.unregister(handler_id)

# Nuke everything.
n = unreal.TetherReactiveLibrary.clear_all("runtime")   # or "editor" or "all"

# Describe what ctx keys a given trigger injects.
ctx_spec = unreal.TetherReactiveLibrary.describe_trigger_context("MovementModeChanged")
...
pawn_name = unreal.TetherGameplayLibrary.get_all_pawns()[0]
...
hid = unreal.TetherReactiveLibrary.register_runtime_movement_mode_changed(
    ...
    script="""
        ...
        last = tether_state.get('last_land', -99.0)
        if now - last < 0.2:
            return
        tether_state['last_land'] = now
        tether_state.setdefault('landings', []).append(now)
...
import _tether_reactive_state as bs
...
Goal: within 0.8s of an Event.Combat.Hit, if the player presses Jump, trigger a finisher ability. Two handlers share state via `tether_state`.

```python
pawn_name = unreal.TetherGameplayLibrary.get_all_pawns()[0]

# 1) Hit opens a combo window.
hid_hit = unreal.TetherReactiveLibrary.register_runtime_gameplay_event(
    ...
    script="""
        import unreal as u
        tether_state['combo_until'] = u.SystemLibrary.get_game_time_in_seconds(ctx['event_target']) + 0.8
...
hid_jump = unreal.TetherReactiveLibrary.register_runtime_input_action(
    ...
    script="""
        ...
        if now < tether_state.get('combo_until', 0):
            u.TetherGameplayAbilityLibrary.send_gameplay_event_by_name(
                handler_task_name_target := '""" + pawn_name + """',
                'Event.Combat.Finisher', 1.0)
            log('FINISHER triggered')
            tether_state['combo_until'] = 0
    """,
    script_path="", tags=["combat"],
    lifetime="Permanent", error_policy="LogContinue", throttle_ms=0,
)
...
Note: the two handlers are independent registrations, but share `tether_state['combo_until']` implicitly — that's what the shared-state design exists for.

---

## Persistence (P6.B3)

Handlers **survive editor restarts automatically.** The subsystem auto-saves the registry to `<ProjectSaved>/Tether/reactive-handlers.json` ~100 ms after any Register/Unregister/Clear, and auto-loads on editor startup. HandlerIds are preserved so agents can stash an id in their context and `unregister(id)` / `get_handler_stats(id)` after a restart.
...
unreal.TetherReactiveLibrary.save_all_handlers()

# Force reload from disk (e.g. after hand-editing the JSON):
n = unreal.TetherReactiveLibrary.load_all_handlers()

# Where does the JSON live?
path = unreal.TetherReactiveLibrary.get_persistence_path()
...
- **`tether_state` and `state` are Python dicts in the UE process** — the Python module survives tether reconnects but NOT editor restart. If you need session-survivable data outside the handler registry, write it to a file yourself.
...
- `unreal.TetherGameplayAbilityLibrary.send_gameplay_event_by_name(actor_name, tag, magnitude)` — fire a GameplayEvent on an actor's ASC from Python. Uses the same walker as the reactive framework.
- `unreal.TetherGameplayAbilityLibrary.ensure_ability_system_component(actor_name, location="Actor"|"Controller"|"PlayerState")` — test scaffolding: attach a fresh ASC to a bare actor.
- `unreal.TetherGameplayAbilityLibrary.ensure_tether_test_attribute_set(actor_name)` — test scaffolding: attach `UTetherTestAttributeSet` (Health + Mana, each initialised to 100) to the actor's ASC.
- `unreal.TetherGameplayAbilityLibrary.set_actor_attribute_value(actor_name, attr_name, value)` — write a numeric attribute base value, triggering the change delegate.
