...
`unreal.TetherPerfLibrary` — AAA-grade performance instrumentation. Returns structured UPROPERTY data instead of parsing `stat unit` text output. Eight dimensions:

| Dimension | Functions |
|---|---|
| Point-in-time | `get_frame_timing` · `get_render_counters` · `get_memory_stats` · `get_u_object_stats` · `get_perf_snapshot` |
| Memory & asset breakdown | (covered in [tether-perf-api § Memory dimension](#) — see source for the full set: `get_texture_memory_breakdown` / `get_mesh_memory_breakdown` / `get_audio_memory_breakdown` / `get_uobject_memory_breakdown` / `get_world_actor_breakdown` / `get_asset_size_top_n`) |
...
- `TraceServices::IAnalysisService::Analyze` (synchronous, blocks the tether exec) → all `parse_*_trace_to_summary` parsers
...
## get_frame_timing() -> FTetherFrameTiming
...
```python
import unreal
t = unreal.TetherPerfLibrary.get_frame_timing()
...
## get_render_counters() -> FTetherRenderCounters
...
**Example**
```python
c = unreal.TetherPerfLibrary.get_render_counters()
...
## get_memory_stats() -> FTetherMemoryStats
...
| `top_classes` | array of `FTetherUObjectClassCount` | `{class_name, count}`, descending by `count`. |
...
**Example**
```python
u = unreal.TetherPerfLibrary.get_u_object_stats(10)
...
## get_perf_snapshot(include_uobject_stats=False, uobject_top_n=20) -> FTetherPerfSnapshot

One-call aggregate for regression tests / CI-style sampling.

| Field | Type | Notes |
|---|---|---|
| `timing` | `FTetherFrameTiming` | See `get_frame_timing`. |
| `render` | `FTetherRenderCounters` | See `get_render_counters`. |
| `memory` | `FTetherMemoryStats` | See `get_memory_stats`. |
| `u_objects` | `FTetherUObjectStats` | Zero-filled when `include_uobject_stats=False` (default). |
...
```python
import unreal, json
s = unreal.TetherPerfLibrary.get_perf_snapshot(False)
...
```python
s = unreal.TetherPerfLibrary.get_perf_snapshot(True, 15)
...
## parse_cook_trace_to_summary(utrace_path, top_n=50) -> FTetherPerfCookSummary
...
| `packages` | array of `FTetherPerfCookRow` | Top-N by `total_cook_time_ms` desc. |
...
## parse_net_trace_to_summary(utrace_path) -> FTetherPerfNetSummary
...
| `game_instances` | array of `FTetherPerfNetGameInstance` | One row per game instance (server / client / dedicated). |
| `success` / `error` | bool / str | |

### `FTetherPerfNetGameInstance`
...
| `connections` | array of `FTetherPerfNetConnection` | One row per connection on the instance. |

### `FTetherPerfNetConnection`
...
## parse_alloc_trace_to_summary(utrace_path) -> FTetherPerfAllocSummary
...
| `tags` | array of `FTetherPerfAllocTag` | Full LLM tag inventory (`{id, parent_id, name, full_path}`; root tags have `parent_id=-1`). |
...
## analyze_all_materials(top_n=30) -> FTetherAllMaterialsAnalysis
...
| `rows` | array of `FTetherMaterialPerfRow` | Top-N by `complexity_score` desc. |

### `FTetherMaterialPerfRow`
...
## get_per_pass_gpu_timings() -> FTetherGpuPassTimings
...
| `passes` | array of `FTetherGpuPassTiming` | Sorted by `average_ms` desc. |
| `diagnostic` | str | Why `available=false` when applicable. |

### `FTetherGpuPassTiming`
...
## get_render_target_memory(top_n=30) -> FTetherRenderTargetMemory
...
| `entries` | array of `FTetherRenderTargetEntry` | Top-N by `bytes` desc. |

### `FTetherRenderTargetEntry`
...
**Scope** — surfaces user-created + editor RTs only (`UTextureRenderTarget*` UObjects). **Does NOT cover** engine-internal RTs: GBuffer, shadow atlas, Lumen surface cache, virtual-texture pool, lighting cache. Those live inside the renderer module's private `FScene` state and require renderer-module access (out of scope for the tether plugin). Use `stat scenerendertargets` or Insights for engine-internal RT memory.

---

## get_texture_streaming_residency(top_n=30) -> FTetherTextureStreamingState
...
| `rows` | array of `FTetherTextureStreamingRow` | Top-N by `resident_bytes` desc. |

### `FTetherTextureStreamingRow`
...
## end_auto_hitch_capture() -> array of FTetherAutoHitchEntry
...
## get_auto_hitch_state() -> FTetherAutoHitchState
...
```python
import unreal
unreal.TetherPerfLibrary.begin_auto_hitch_capture(50.0, 100)
# ... PIE + camera fly-through for some duration ...
hitches = unreal.TetherPerfLibrary.end_auto_hitch_capture()
...
## begin_insights_for_trace(utrace_path) -> FTetherInsightsLaunchResult
...
## compare_perf_snapshots(before, after, regression_threshold=0.10) -> FTetherPerfSnapshotDelta

**(M8-2)** Diff two `FTetherPerfSnapshot` instances. Returns per-field deltas (`After - Before`) plus a list of human-readable regression strings for any metric that changed by ≥ `regression_threshold` (default 10%).
...
```python
import unreal
before = unreal.TetherPerfLibrary.get_perf_snapshot(True)
# ... do edit / re-cook / etc ...
after  = unreal.TetherPerfLibrary.get_perf_snapshot(True)
delta  = unreal.TetherPerfLibrary.compare_perf_snapshots(before, after, 0.10)
...
ps = unreal.TetherPerfLibrary.get_frame_time_percentiles([50, 90, 95, 99])
...
## parse_trace_to_summary(utrace_path, top_n=20, top_n_per_thread=10, top_n_counters=100) -> FTetherPerfTraceSummary
...
| `hot_scopes` | array of `FTetherPerfHotScope` | Top-N CPU timers across **all** CPU thread timelines, ranked by total inclusive time desc. |
| `gpu_hot_scopes` | array of `FTetherPerfHotScope` | **(M5-1)** Top-N GPU timers across every GPU queue (incl. legacy GPU1/GPU2 timelines for old traces). Empty when the trace did not include the `gpu` channel. |
| `per_thread_hot_scopes` | array of `FTetherPerThreadHotScopes` | **(M5-2)** One row per CPU thread (`GameThread`, `RenderThread N`, `RHIThread`, every `WorkerThread`, `FAssetDataGatherer`, …). Sorted by `total_cpu_ms` desc. |
| `load_time_breakdown` | array of `FTetherPerfLoadTimeRow` | **(M5-5)** Top-N packages by total load time (main thread + async loading thread). Empty when the trace lacks the `loadtime` channel **or** was captured after engine init when most packages were already cached. For useful cold-load attribution, capture from engine startup (`-trace=loadtime,frame,cpu` on the editor command line). |
| `counters` | array of `FTetherPerfCounter` | **(M5-3)** Trace counters with min/max/avg/last/sum aggregates over the full session interval. Ranked by `sample_count` desc — busiest counters surface first. Capped by `top_n_counters`. Counters with 0 samples are dropped. Empty when the trace lacks the `counter` channel. |
...
| `top_scopes` | array of `FTetherPerfHotScope` | Top-N scopes on this thread (capped at `top_n_per_thread`). |
...
Dominated by `Analyze()` — typically 1-10s per 100 MB of trace. Synchronous, blocks the tether exec; for large traces, expect tens of seconds (see `feedback_tether_exec_holds_gamethread`).
...
```python
import unreal
s = unreal.TetherPerfLibrary.parse_trace_to_summary(
...
Run two tether execs — the first starts a viewport camera flight / PIE / etc., the second samples. Tether `exec` holds the GameThread, so *don't* loop sleep-and-sample inside a single exec (see `feedback_tether_exec_holds_gamethread` in memory).
