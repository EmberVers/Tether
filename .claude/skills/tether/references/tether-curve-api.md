# Tether Curve Library API

Module: `unreal.TetherCurveLibrary`
...
1. **Batch evaluating** a channel at N points — one tether round-trip instead of N.
...
### get_curve_info(curve_path) -> FTetherCurveInfo

Identify + per-channel key counts + infinity extrap. Cheap.

```python
info = unreal.TetherCurveLibrary.get_curve_info('/Game/Curves/C_Damage')
print(info.class_name, list(info.channel_names), list(info.num_keys_per_channel))
```

### get_curve_keys(curve_path, channel_index) -> list[FTetherRichCurveKey]
...
def mk(t, v, interp="Cubic", tan="Auto"):
    k = unreal.TetherRichCurveKey(); k.time = t; k.value = v
    k.interp_mode = interp; k.tangent_mode = tan; k.tangent_weight_mode = "None"
    return k

keys = [mk(0, 0), mk(1, 10), mk(2, 5)]
unreal.TetherCurveLibrary.set_curve_keys('/Game/Curves/C_Damage', 0, keys)
```
...
import math
unreal.TetherCurveLibrary.set_curve_key_tangents(
...
Evaluate one channel at N input times; one tether round-trip.

```python
ts = [i * 0.1 for i in range(101)]    # 0.0 .. 10.0 step 0.1
ys = unreal.TetherCurveLibrary.evaluate_curve('/Game/Curves/C_Damage', 0, ts)
```
...
### get_curve_table_info(curve_table_path) -> FTetherCurveTableInfo
...
### get_curve_table_row_keys(curve_table_path, row_name) -> list[FTetherRichCurveKey]
...
- `RichCurves` table: all tether key fields are applied.
...
scaled = []
for k in row:
    new_k = unreal.TetherRichCurveKey()
    new_k.time = k.time
