...
`unreal.TetherGameplayLibrary` — agent sensors + navigation for PIE automation.
...
```python
import unreal
obs = unreal.TetherGameplayLibrary.get_agent_observation(
...
**World selection** — PIE world if PIE is active, otherwise the editor world. Both must have a built NavMesh (see `tether-navigation-api.md` or call `execute_console_command('RebuildNavigation')` first).
...
```python
import unreal
ok, waypoints, length = unreal.TetherGameplayLibrary.find_nav_path(
...
```python
origin = unreal.TetherGameplayLibrary.get_camera_hit_location(10000.0)
if origin is not None:
    target = unreal.TetherGameplayLibrary.project_point_to_navmesh(
...
obs = unreal.TetherGameplayLibrary.get_agent_observation()
if obs.valid:
    wander = unreal.TetherGameplayLibrary.get_random_reachable_point_in_radius(
...
- All four queries return None outside a navmesh-enabled level, even if
  ...
  from `tether-editor-api` and retry.
...
```python
origin, direction = unreal.TetherGameplayLibrary.deproject_screen_to_world(0.5, 0.5)
```
...
```python
ui_pos = unreal.TetherGameplayLibrary.project_world_to_screen(
...
```python
name = unreal.TetherGameplayLibrary.get_actor_at_screen_position(0.5, 0.5)
```
...
```python
unreal.TetherGameplayLibrary.set_camera_fov(30.0)   # tight zoom
# ... inspect scene ...
unreal.TetherGameplayLibrary.unlock_camera_fov()
```
...
`tether-level-api` spawn/destroy pair, which target the editor world.
...
- PIE actors created via `spawn_actor_in_pie` vanish when PIE stops —
  ...
  `tether-level-api.spawn_actor` instead.
...
`tether-editor-api.pause_pie` — pause hard-stops the play session;
...
unreal.TetherGameplayLibrary.set_global_time_dilation(0.25)
unreal.TetherGameplayLibrary.set_actor_time_dilation('Boss', 4.0)
...
```python
hits = unreal.TetherGameplayLibrary.apply_radial_damage(
...
```python
for name in unreal.TetherGameplayLibrary.get_all_pawns():
    ctrl = unreal.TetherGameplayLibrary.get_actor_controller(name)
    print(f'{name}: {ctrl}')
```
...
```python
unreal.TetherGameplayLibrary.add_on_screen_debug_message(
...
```python
if unreal.TetherGameplayLibrary.get_distance_to_pawn(waypoint) < 100:
...
`tether-level-api.set_actor_simulate_physics`. Non-PIE calls and
...
unreal.TetherGameplayLibrary.add_impulse_to_pie_actor(
...
unreal.TetherGameplayLibrary.respawn_player_pawn()
```
...
from `tether-editor-api.pause_pie`, which targets the editor's
...
path, *_ = unreal.TetherGameplayLibrary.find_nav_path(
    obs.pawn_location, goal)
for i in range(len(path) - 1):
    unreal.TetherGameplayLibrary.draw_debug_line(path[i], path[i+1], 2.0, 5.0)
```
...
```python
ty = unreal.TetherGameplayLibrary.get_input_action_value_type(
...
```python
names, thresholds = unreal.TetherGameplayLibrary.get_input_action_triggers(
...
```python
L = unreal.TetherGameplayLibrary
...
unreal.TetherGameplayLibrary.apply_movement_input(forward, 1.0)
```
...
or `IA_Look` needs a value every UE frame; the Python tether can't
...
Each `tether.exec` runs on the GameThread, so a Python `time.sleep`
...
unreal.TetherGameplayLibrary.set_sticky_input(IA, unreal.Vector(0, 1, 0))
```
```bash
...
# exec #2 — release
import unreal
unreal.TetherGameplayLibrary.clear_sticky_input('/Game/Input/IA_Move.IA_Move')
```
...
`tether.exec` runs on the GameThread, so `time.sleep` halts UE itself —
...
(`TetherReactiveLibrary.register_runtime_timer`, see
`tether-reactive.md`), which fires a Python callback between ticks at
...
Run with:  tether.py exec-file chase_target.py
...
L = unreal.TetherGameplayLibrary
R = unreal.TetherReactiveLibrary
...
# a `log()` helper (see tether-reactive.md). We bake constants into the
# script source via f-string substitution so the closure is self-contained.
chase_script = f'''
import math
import unreal as u
G = u.TetherGameplayLibrary
R = u.TetherReactiveLibrary
...
```python
import unreal
if not unreal.TetherGameplayLibrary.is_in_pie():
...
```python
rot = unreal.TetherGameplayLibrary.get_control_rotation()
if rot is not None:
...
```python
count, paths, values = unreal.TetherGameplayLibrary.get_sticky_inputs()
...
```python
target = unreal.TetherGameplayLibrary.get_camera_hit_actor(5000.0)
if target:
...
```python
loc = unreal.TetherGameplayLibrary.get_camera_hit_location(10000.0)
if loc is not None:
...
```python
h = unreal.TetherGameplayLibrary.get_pawn_ground_height()
if h < 0:
...
```python
import unreal
ok = unreal.TetherGameplayLibrary.teleport_pawn(
...
```python
caps = unreal.TetherGameplayLibrary.get_pawn_capabilities()
if caps:
...
tether, both the down and up events are processed in the same frame.
...
`exec` blocks the GameThread. For held keys, use two separate tether
calls with a bash-side sleep in between:

```bash
tether.py exec "unreal.TetherGameplayLibrary.press_key('Gamepad_FaceButton_Bottom', True)"
sleep 0.2
tether.py exec "unreal.TetherGameplayLibrary.press_key('Gamepad_FaceButton_Bottom', False)"
```
...
process (NOT inside tether exec). This is useful when the tether is
unreachable or for non-UE windows.

```python
# Run from bash, NOT inside tether exec
python -c "
import ctypes, time
VK_E = 0x45
...
Prefer `press_key` over OS-level simulation whenever the tether is
