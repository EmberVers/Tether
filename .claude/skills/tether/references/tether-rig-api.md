# Tether Control Rig / IK Rig / IK Retargeter API

Module: `unreal.TetherRigLibrary`

Preferred kwargs-only wrapper: `from tether import Rig`
...
Use a single tether heredoc for each coherent author/validate unit, but keep PIE
or other asynchronous editor transitions in separate tether calls.
...
`input_controls` is an array of `unreal.TetherRigNamedTransform` values with
