# Tether UMG Library API

Module: `unreal.TetherUMGLibrary`

Audited kwargs-only wrapper: `from tether import UMG`
...
- Widget Blueprint, widget tree, layout/style, animation, and live-widget APIs
  are available on every supported Tether engine version (UE 5.3+).
...
- UI material graph creation belongs to `TetherMaterialLibrary`. UMG
...
9. Start PIE in a separate tether call, spawn the screen, and validate live
   state and semantics.
...
each create/save in its own tether exec. Tree/property/animation mutations may
...
### list_widget_classes(query, include_abstract, max_results) -> list[FTetherWidgetClassInfo]

Discover loaded `UWidget` subclasses accepted by `add_widget`. Filter before
using a large result.

```python
from tether import UMG
...
### create_widget_blueprint(asset_path, parent_class_path) -> FTetherWidgetOperationResult
...
`FTetherWidgetOperationResult` fields: `success`, `path`, `name`, `error`.

### add_widget(widget_blueprint_path, widget_class_path, widget_name, parent_name, insert_index) -> FTetherWidgetOperationResult
...
### get_widget_tree(widget_blueprint_path) -> list[FTetherWidgetInfo]

Return the hierarchy as a flat, parent-linked list.

`FTetherWidgetInfo` fields:
...
### search_widgets(widget_blueprint_path, query) -> list[FTetherWidgetInfo]

Case-insensitive name/class substring search.

### get_widget_properties(widget_blueprint_path, widget_name) -> list[FTetherWidgetPropertyValue]

Return non-default reflected values on a widget template.

### get_widget_slot_properties(widget_blueprint_path, widget_name) -> list[FTetherWidgetPropertyValue]

Return non-default values from the widget's parent slot.

`FTetherWidgetPropertyValue` fields: `name`, `type`, `value`. Values are UE
...
# apply_material_graph_ops; see tether-material-api.md. Compile and save in a
# separate tether exec, then assign it with set_widget_brush.
```
...
### compile_and_validate_widget_blueprint(widget_blueprint_path, save, check_accessibility) -> FTetherWidgetValidationReport
...
`FTetherWidgetValidationReport` fields:
...
`FTetherWidgetValidationIssue` fields: `severity`, `code`, `widget_name`,
`message`.
...
for time, value in ((0.0, 0.0), (0.5, 1.0)):
    key = unreal.TetherWidgetFloatKey()
    key.time = time
    key.value = value
    keys.append(key)
...
- `FTetherWidgetFloatKey`: `time`, `value`.
- `FTetherWidgetColorKey`: `time`, `value` (`FLinearColor`).
- `FTetherWidgetTransformKey`: `time`, `translation`, `scale`, `shear`,
  `angle`.

### get_widget_animations(widget_blueprint_path) -> list[FTetherWidgetAnimationInfo]

`FTetherWidgetAnimationInfo`: `name`, `duration`, `display_rate`, `tracks`.

`FTetherWidgetAnimTrack`: `widget_name`, `track_type`, `display_name`,
...
### create_mvvm_view_model_blueprint(asset_path, parent_class_path) -> FTetherWidgetOperationResult
...
```python
from tether import Blueprint, Editor, UMG
...
### get_mvvm_view_models(widget_blueprint_path) -> list[FTetherMVVMViewModelInfo]

Fields: `id`, `name`, `class_path`, `creation_type`, `optional`,
`create_getter`, `create_setter`.

### get_mvvm_bindings(widget_blueprint_path) -> list[FTetherMVVMBindingInfo]
...
### get_widget_bindings(widget_blueprint_path) -> list[FTetherWidgetBindingInfo]
...
### get_widget_events(widget_blueprint_path) -> list[FTetherWidgetEventInfo]
...
Start PIE in its own tether call and wait until `Editor.is_in_pie()` is true.
...
### get_live_widget_tree(instance_handle) -> list[FTetherLiveWidgetInfo]
