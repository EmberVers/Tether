# Tether Editor Library API

Module: `unreal.TetherEditorLibrary`
...
## Tether protocol commands (non-`exec`)

These are **server-level** commands, not Library functions — they're handled directly by `FTetherServer::HandleClient` and bypass the FTSTicker exec queue. Use them when an `exec` is hung and you need to know *why* without queueing behind the stuck script.

Each is a separate TCP connection (workers are per-connection, capped at 16). Prefer the `tether.py` subcommand. A hand-crafted request must use protocol v2: `{"command":"exact_<name>","expected":{"protocol_version":2,"instance_id":"...","pid":...,"project_path":"..."},"request":{...}}`. Copy every identity value, including the case and separators of `project_path`, verbatim from one discovery response or Server startup line. The production dispatcher rejects malformed identity/request objects and legacy or unknown commands before any command body or GameThread dispatch.

| Exact command | CLI | Purpose |
|---|---|---|
| `exact_ping` | `tether.py ping` | TCP-only liveness — proves the server process and accept loop are alive. Returns `pong` plus `ready: bool` (false during the editor's startup window before main frame creation). |
| `exact_editor_status` | `tether.py status` | Read-only cached health — reports main-frame readiness, Engine/Slate tick ages and staleness, plus a modal-attention summary. Discovery must advertise this optional capability; otherwise `status` fails before TCP while other base commands remain usable. A successful response must pass schema-v1/type validation. It neither enters the exec queue nor dispatches fresh GameThread work; the TCP worker never touches Slate. See `docs/editor-health-status.md`. |
| `exact_gamethread_ping` with `request.timeout` | `tether.py gamethread-ping [--probe-timeout S]` | GameThread liveness — dispatches a no-op `AsyncTask(GameThread)` and waits up to `timeout` seconds (default 2s, max 10s). Response includes `latency_ms`. |
| `exact_debug_resume` | `tether.py resume` | Recover from a stuck Blueprint breakpoint. Calls `FKismetDebugUtilities::RequestAbortingExecution` + `FSlateApplication::LeaveDebuggingMode` via `AsyncTask(GameThread)`. Fire-and-forget. |

### `exact_editor_status` versus fresh probes

Use `tether.py status` (`exact_editor_status` on the wire) first when the editor may be blocked. Its Engine timestamp is
...
| `unresponsive` | == probe timeout | GT cannot pump TaskGraph at all | Modal dialog open, deadlock, pure-Python tight loop holding the GIL inside `IPythonScriptPlugin::ExecPythonCommandEx`, or Slate nested debug loop (try `tether.py resume` for the last case). |
...
### get_editor_state() -> FTetherEditorState

Snapshot of the current editor session.

```python
s = unreal.TetherEditorLibrary.get_editor_state()
...
### get_opened_assets() -> list[FTetherOpenedAsset]

```python
for a in unreal.TetherEditorLibrary.get_opened_assets():
    dirty = '*' if a.is_dirty else ''
    print(f'[{a.class_name}] {a.path}{dirty}')
```

### FTetherOpenedAsset fields
...
```python
unreal.TetherEditorLibrary.set_content_browser_path('/Game/Maps')
```
...
### get_editor_viewport_camera() -> FTetherViewportCamera

Current perspective viewport camera.

### FTetherViewportCamera fields
...
```python
unreal.TetherEditorLibrary.take_high_res_screenshot(2.0)  # 2x native
```

### capture_active_viewport(out_file_path, include_base64) -> FTetherScreenshotResult
...
FTetherScreenshotResult fields (UE Python strips the `b` prefix from bool fields):
...
```python
r = unreal.TetherEditorLibrary.capture_active_viewport(
...
### capture_active_viewport_as_displayed(out_file_path, include_base64) -> FTetherScreenshotResult
...
```python
r = unreal.TetherEditorLibrary.capture_active_viewport_as_displayed(
...
### capture_viewport_channel(channel, out_file_path, width, height, max_depth_clamp, include_base64) -> FTetherChannelCaptureResult
...
r = unreal.TetherEditorLibrary.capture_viewport_channel(
...
**Gotcha — SceneCapture readback is top-down (verified UE 5.7, 2026-04-20).** Unlike the `FLevelEditorViewportClient::Viewport::ReadPixels` path in `TetherLevelLibrary::CaptureSceneToPng`, the render-target resource ReadPixels / ReadLinearColorPixels here returns rows top-down already — no vertical flip needed. If you adapt this pattern, do NOT add an extra flip or the output lands upside-down.
...
### capture_channel_from_pose(channel, location, rotation, fov, width, height, max_depth_clamp, out_file_path, include_base64) -> FTetherChannelCaptureResult

Same GBuffer channel capture as `capture_viewport_channel`, but takes an explicit camera pose instead of reading the active viewport. **This is the PIE-compatible path** — pair with `TetherGameplayLibrary` to grab the player's camera and capture Depth / Normal / BaseColor from the player's perspective at runtime.
...
loc, rot = unreal.TetherGameplayLibrary.get_camera_view_point()
fov      = unreal.TetherGameplayLibrary.get_camera_fov()

# 2. capture Depth from that pose with sky clipped to 100m
r = unreal.TetherEditorLibrary.capture_channel_from_pose(
...
### capture_viewport_hit_proxy_map(out_file_path, include_base64) -> FTetherScreenshotResult
...
```python
r = unreal.TetherEditorLibrary.capture_viewport_hit_proxy_map(
...
```python
actor = unreal.TetherEditorLibrary.get_actor_under_viewport_pixel(1236, 565)
...
```python
unreal.TetherEditorLibrary.set_viewport_view_mode('Unlit')
```
...
```python
unreal.TetherEditorLibrary.set_viewport_show_flag('Grid', False)
```
...
```python
unreal.TetherEditorLibrary.set_viewport_type('Top')
```
...
```python
unreal.TetherEditorLibrary.show_editor_notification(
...
project (e.g. `['Tether', 'EnhancedInput', 'GameplayAbilities', ...]`).
...
```python
if not unreal.TetherEditorLibrary.is_plugin_enabled('PythonScriptPlugin'):
    raise RuntimeError('PythonScriptPlugin disabled — tether is broken')
```
...
Emit a message to `GLog` under the dedicated `LogTetherPy`
...
```python
unreal.TetherEditorLibrary.write_log_message('import finished', 'Log')
unreal.TetherEditorLibrary.write_log_message('asset missing', 'Warning')
```
...
```python
unreal.TetherLevelLibrary.destroy_actors(many_actor_names)
unreal.TetherEditorLibrary.trigger_garbage_collection(False)
print(unreal.TetherEditorLibrary.get_memory_usage_mb())
```
...
```python
if unreal.TetherEditorLibrary.is_compiling():
    unreal.TetherEditorLibrary.flush_compilation()
...
unreal.TetherEditorLibrary.clear_log_buffer()
run_big_thing()
for l in unreal.TetherEditorLibrary.get_recent_log_lines(0, 'Warning'):
    print(l)
```
...
```python
p = unreal.TetherEditorLibrary.get_module_binary_path('Tether')
# p → ".../Plugins/Tether/Binaries/Win64/UnrealEditor-Tether.dll"
```
...
prev_mode = unreal.TetherEditorLibrary.get_widget_mode()
unreal.TetherEditorLibrary.set_widget_mode('Rotate')
unreal.TetherEditorLibrary.set_coord_system('Local')
# ... user interacts ...
unreal.TetherEditorLibrary.set_widget_mode(prev_mode)
```
...
prev = unreal.TetherEditorLibrary.is_auto_save_enabled()
unreal.TetherEditorLibrary.set_auto_save_enabled(False)
run_capture()
unreal.TetherEditorLibrary.set_auto_save_enabled(prev)
```
...
import datetime
p = '/Game/Maps/MyLevel'
mtime = unreal.TetherEditorLibrary.get_asset_last_modified_time(p)
if mtime:
...
Applies to every `TetherEditorImpl::LoadAssetFromPath` caller (`open_asset`, `save_asset`, `reload_asset`, `sync_content_browser_to_assets`, etc.).
...
```python
unreal.TetherEditorLibrary.load_level('/Game/Maps/TestLevel', True)
```
...
```python
unreal.TetherEditorLibrary.create_new_level(True)
```
...
```python
unreal.TetherEditorLibrary.start_simulate()
# ... observe ...
unreal.TetherEditorLibrary.stop_pie()   # same StopPIE works
```
...
```python
t0 = unreal.TetherEditorLibrary.get_pie_world_time()
# ... run some gameplay ...
elapsed = unreal.TetherEditorLibrary.get_pie_world_time() - t0
```
...
```python
out = unreal.TetherEditorLibrary.execute_console_command('stat fps')
print(out)
```
...
```python
out = unreal.TetherEditorLibrary.execute_pie_console_command(
...
```python
n = unreal.TetherEditorLibrary.fixup_redirectors(['/Game'])
```
...
```python
dirty = unreal.TetherEditorLibrary.get_dirty_package_names()
print('\n'.join(dirty))
```
...
```python
if unreal.TetherEditorLibrary.is_asset_editor_open('/Game/BP/BP_Hero'):
    unreal.TetherEditorLibrary.close_all_asset_editors()
```
...
```python
if not unreal.TetherEditorLibrary.is_asset_loaded('/Game/BP/BP_Hero.BP_Hero'):
...
```python
unreal.TetherEditorLibrary.close_asset_editor('/Game/BP/BP_Hero')
```
...
```python
saved = unreal.TetherEditorLibrary.save_assets([
    '/Game/Data/DT_Weapons',
    '/Game/BP/BP_Hero',
])
```

### compile_blueprints(blueprint_paths) -> list[FTetherCompileResult]

Compile the listed Blueprints. Returns per-BP success + error summary.

```python
results = unreal.TetherEditorLibrary.compile_blueprints(['/Game/BP/BP_Hero', '/Game/BP/BP_Enemy'])
for r in results:
    ok = 'OK' if r.success else 'FAIL'
    print(f'[{ok}] {r.path} {r.error_message}')
```

### FTetherCompileResult fields
...
Patch recompiled cpp changes into the running editor without restart. The TCP tether, open assets, PIE state, and viewport camera all survive. **Windows only** — `trigger_live_coding_compile` returns `Status="Unavailable"` on other platforms.
...
1. Edit cpp in `Plugin/Tether/Source/`
2. Run `sync_plugin.bat` to copy into the project's `Plugins/` dir
3. Call `trigger_live_coding_compile(True)` via the tether
...
### trigger_live_coding_compile(wait_for_completion) -> FTetherLiveCodingResult
...
FTetherLiveCodingResult fields (UE Python strips the `b` prefix from bool fields):
...
```python
r = unreal.TetherEditorLibrary.trigger_live_coding_compile(True)
...
Every TCP request to the tether is captured in a session-local ring buffer (default capacity 500). Use this to answer "what's the tether being asked to do?", "which `exec` calls take the longest?", or "did that recent failure hit the server at all?" without grepping `Saved/Logs/<Project>.log`.

Captured fields per entry:

| Field | Type | Notes |
|---|---|---|
| `request_id` | str | UUID4 sent by `tether.py` (or `<missing>` for raw clients). |
...
### clear_tether_call_log() -> int32

Drops every buffered record. Returns how many were dropped. Use before a benchmark to get a clean window.

### get_tether_call_log_capacity() / set_tether_call_log_capacity(capacity) -> int32

Capacity is clamped to `[10, 5000]`. Shrinking discards the oldest entries.

**Pitfalls**
- The current `exec` (the one running this code) is **not** in the log yet — it's appended after the response sends. To see your own call, do `get_tether_call_log` from a separate `tether.py exec` invocation.
- `total_duration_ms` is server-side only and excludes network RTT. For wall-clock-from-the-client, time `tether.py exec` itself.
- The ring is in-memory and process-local — restart loses everything. Persist to disk via `tether.py exec` + `json.dumps` if you need cross-session data.

---

## Signature registry dump

### dump_tether_signature_registry() -> str (JSON)

Returns a single condensed-JSON string describing every BlueprintCallable UFUNCTION on every `UTether*Library` class. Intended as a one-call alternative to paging through the `tether-*-api.md` docs when an agent needs the complete API surface (parameters, defaults, return types).

```python
import unreal, json
data = json.loads(unreal.TetherEditorLibrary.dump_tether_signature_registry())
...
{ "generated_utc": "...", "engine_version": "...",
  "libraries": [
    { "class_name": "TetherEditorLibrary",
      "python_name": "tether_editor_library",
      "functions": [
        { "name": "GetTetherCallLog",
          "python_name": "get_tether_call_log",
          "category": "Tether|Editor",
          "tooltip": "...",
          "params": [
...
**What's covered.** Every BlueprintCallable UFUNCTION whose owning class lives in the `/Script/Tether` package — i.e. all `UTether*Library` classes. Current count: 1390 reflected UFUNCTIONs across 26 libraries (the Python-callable manifest currently exposes 1388 methods).
...
Stand-alone Python notifier — POSTs a payload to a webhook URL with auto-detected Slack / Discord / generic format. Lives at `.claude/skills/tether/scripts/notify.py`. No tether call needed — runs from the host shell so it works whether the editor is up or not.
...
**CLI options**
- `--url <url>` — required, or set `TETHER_WEBHOOK_URL` env var.
...
- `--source` — footer / attribution. Default `Tether@<hostname>`.
