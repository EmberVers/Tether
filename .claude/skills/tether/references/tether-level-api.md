...
### get_streaming_levels() -> list[FTetherStreamingLevel]

List streaming sublevels with load/visible flags.

### FTetherStreamingLevel fields
...
```python
unreal.TetherLevelLibrary.set_kill_z(-500.0)   # tighter pit
unreal.TetherLevelLibrary.set_world_gravity(-200.0, True)  # low-g scenario
```
...
```python
clearance = unreal.TetherLevelLibrary.get_actor_ground_clearance('Cube')
if clearance > 200:
    unreal.TetherLevelLibrary.snap_actor_to_floor('Cube')
```
...
```python
for a in unreal.TetherLevelLibrary.get_actor_names('', '', ''):
    d = unreal.TetherLevelLibrary.get_attachment_depth(a)
    if d > 3:
...
for a in unreal.TetherLevelLibrary.get_actor_names('StaticMeshActor', '', ''):
    t = unreal.TetherLevelLibrary.get_actor_triangle_count(a)
    if t > 100_000:
...
unreal.TetherLevelLibrary.add_actor_tag('Cube', unreal.Name('probe'))
n = unreal.TetherLevelLibrary.count_actors_by_tag(unreal.Name('probe'))
# ...do work...
unreal.TetherLevelLibrary.remove_tag_from_all_actors(unreal.Name('probe'))
```
...
```python
hier = unreal.TetherLevelLibrary.get_actor_class_hierarchy('Cube')
...
src = unreal.TetherLevelLibrary.duplicate_actors(['Cube'])
unreal.TetherLevelLibrary.mirror_actors(src, 'X')
```
...
unreal.TetherLevelLibrary.move_actors_to_folder(probes, 'Probes')
# ...work...
unreal.TetherLevelLibrary.dissolve_folder('Probes')
```
...
unreal.TetherLevelLibrary.select_actors_in_sphere(
    unreal.Vector(0, 0, 0), 2000.0)
selection = unreal.TetherLevelLibrary.get_selected_actors()
unreal.TetherLevelLibrary.move_actors_to_folder(selection, 'NearOrigin')
```
...
### get_selection_bounds() -> FTetherActorBounds
...
```python
if len(unreal.TetherLevelLibrary.get_selection_class_set()) > 1:
...
cam_loc, cam_rot = unreal.TetherGameplayLibrary.get_camera_view_point()
fwd = cam_rot.rotate_vector(unreal.Vector(1, 0, 0))
if unreal.TetherLevelLibrary.is_actor_in_cone(
...
```python
names = unreal.TetherLevelLibrary.get_actor_names('StaticMeshActor', '', 'Wall')
```

### list_actors(class_filter, tag_filter, name_filter, selected_only, max_results) -> list[FTetherActorBrief]
...
```python
briefs = unreal.TetherLevelLibrary.list_actors('', '', '', False, 50)
for a in briefs:
    print(f'[{a.class_name}] {a.label} @ {a.location}')
```

### FTetherActorBrief fields
...
### find_actors_in_radius(location, radius, class_filter) -> list[FTetherActorRadiusHit]

Actors within `radius` (cm) of `location`, distance-sorted.

```python
hits = unreal.TetherLevelLibrary.find_actors_in_radius(unreal.Vector(0,0,0), 500.0, '')
for h in hits:
    print(f'{h.name} [{h.class_name}] d={h.distance:.1f}')
```

### FTetherActorRadiusHit fields
...
### get_actor_info(actor_name) -> FTetherActorInfo

Look up by FName or label. On duplicate labels, first match wins.

```python
info = unreal.TetherLevelLibrary.get_actor_info('BP_Hero_1')
...
### FTetherLevelComponentInfo fields
...
| `relative_transform` | FTetherTransform | Relative transform |

### get_actor_transform(actor_name) -> FTetherTransform

### get_actor_components(actor_name) -> list[FTetherLevelComponentInfo]
...
```python
label = unreal.TetherLevelLibrary.spawn_actor(
...
### get_actor_bounds(actor_name) -> FTetherActorBounds
...
```python
b = unreal.TetherLevelLibrary.get_actor_bounds('BP_Cube_1')
print(b.origin, b.box_extent, b.sphere_radius)
```

### FTetherActorBounds fields
...
```python
names = unreal.TetherLevelLibrary.get_actors_in_box(
...
```python
unreal.TetherLevelLibrary.add_actor_tag('BP_Cube_1', 'interactable')
```
...
```python
for line in unreal.TetherLevelLibrary.get_actor_class_histogram()[:10]:
    print(line)
```
...
```python
unreal.TetherLevelLibrary.get_actor_materials('BP_Cube_1')
...
```python
unreal.TetherLevelLibrary.get_actor_folder('BP_Hero_1')  # "Heroes/Main"
```
...
```python
unreal.TetherLevelLibrary.set_actor_folder('BP_Cube_1', 'Props/Crates')
```
...
for label in unreal.TetherLevelLibrary.get_actors_in_folder('Old/Legacy', True):
    unreal.TetherLevelLibrary.set_actor_folder(label, 'Archive')
```
...
```python
hit = unreal.TetherLevelLibrary.line_trace_first_actor(
...
```python
pierced = unreal.TetherLevelLibrary.multi_line_trace_actors(
...
```python
hit = unreal.TetherLevelLibrary.sphere_trace_first_actor(
...
```python
hit = unreal.TetherLevelLibrary.box_trace_first_actor(
...
```python
nearby = unreal.TetherLevelLibrary.overlap_sphere_actors(
...
```python
unreal.TetherLevelLibrary.snap_actor_to_floor('Cube', 5000.0)
```
...
```python
cubes = unreal.TetherLevelLibrary.get_actor_names('StaticMeshActor', '', 'Cube')
unreal.TetherLevelLibrary.snap_actors_to_grid(cubes, 100.0)
```
...
```python
unreal.TetherLevelLibrary.offset_actors(cubes, unreal.Vector(0, 0, 200))
```

### get_level_bounds() -> FTetherActorBounds
...
```python
b = unreal.TetherLevelLibrary.get_level_bounds()
...
```python
cubes = unreal.TetherLevelLibrary.get_actor_names('StaticMeshActor', '', 'Cube')
unreal.TetherLevelLibrary.isolate_actors(cubes[:1])
```
...
```python
unreal.TetherLevelLibrary.set_actor_mesh('Cube', '/Engine/BasicShapes/Sphere')
```
...
```python
unreal.TetherLevelLibrary.set_actor_material(
...
```python
unreal.TetherLevelLibrary.set_actor_simulate_physics('Cube', True)
...
```python
comp = unreal.TetherLevelLibrary.add_component_of_class(
...
```python
for entry in unreal.TetherLevelLibrary.get_actor_sockets('BP_Weapon_1'):
    print(entry)  # e.g. "Mesh:Muzzle"
```

### get_socket_world_transform(actor_name, component_name, socket_name) -> FTetherTransform
...
```python
t = unreal.TetherLevelLibrary.get_socket_world_transform(
    'BP_Character_1', 'Mesh', 'hand_r')
print(t.location, t.rotation)
```

### get_component_world_transform(actor_name, component_name) -> FTetherTransform
...
```python
unreal.TetherLevelLibrary.set_component_visibility(
...
```python
unreal.TetherLevelLibrary.set_component_mobility(
...
"call `line_trace_hit_info` N times from Python" pattern — one tether
...
them. Lives in a process-global singleton (survives across tether exec
