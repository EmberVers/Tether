# tether-navigation-api

`unreal.TetherNavigationLibrary` — navigation system introspection and export.
...
**Output footprint** — small (two values: bool + one-line string). The OBJ itself goes to disk, not to the tether response.

**Example**
```python
import unreal
ok, msg = unreal.TetherNavigationLibrary.export_nav_mesh_to_obj('')
print(msg)
