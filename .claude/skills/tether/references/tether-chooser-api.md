# Tether Chooser Library API

Module: `unreal.TetherChooserLibrary`
...
> **Where this fits:** ChooserTables are the dispatch layer between AnimBP variables and PoseSearchDatabase / Montage / asset selection. To inspect a PSD that a chooser returns, see `tether-pose-search-api.md`. To drive choosers from an AnimBP at runtime, the AnimBP wires a `Chooser Player` node — see `tether-anim-api.md`.
...
### `get_chooser_info(chooser_table_path) → FTetherCHTInfo`

```python
info = unreal.TetherChooserLibrary.get_chooser_info(
...
### `list_chooser_columns(chooser_table_path) → TArray<FTetherCHTColumn>`
...
### `list_chooser_rows(chooser_table_path) → TArray<FTetherCHTRow>`
...
### `get_chooser_row_result(chooser_table_path, row_index) → FTetherCHTRowResult`
...
### `list_possible_results(chooser_table_path) → TArray<FTetherCHTRowResult>`
...
```python
unreal.TetherChooserLibrary.set_chooser_fallback_asset(chooser_table_path='...', asset_path='/Game/.../FallbackAsset')
unreal.TetherChooserLibrary.clear_chooser_fallback(chooser_table_path='...')
```
...
```python
col = unreal.TetherChooserLibrary.add_chooser_column_enum(
    chooser_table_path='...',
...
### `evaluate_chooser_with_context_object(chooser_table_path, context_object_path) → FTetherCHTEvaluation`

```python
out = unreal.TetherChooserLibrary.evaluate_chooser_with_context_object(
    chooser_table_path='/Game/.../CHT_PoseSearchDatabases',
...
> **`matched_row=-1` doesn't mean failure.** It means "I didn't record which row" — happens by default. To get authoritative row tracking you'd need to flip `bEnableDebugTesting` on the chooser asset (no UFUNCTION yet, requires editor inspector or a follow-up tether call).
...
`Asset.get_package_referencers(package_name='/Game/.../X')` from `tether-asset-api.md` works fine — chooser column row values store soft refs that the asset registry sees. Filter the result by `class_path == "/Script/Chooser.ChooserTable"`.
