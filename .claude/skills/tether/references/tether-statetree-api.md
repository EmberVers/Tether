# Tether StateTree Library API

Module: `unreal.TetherStateTreeLibrary`
Preferred wrapper: `from tether import StateTree`
...
Use one tether heredoc for the whole loop. After every structural edit, re-list
...
- `control_state_tree_component(component_path, action, reason='Tether')`
  where action is `Start`, `Restart`, `Stop`, `Pause`, or `Resume`.
- `send_state_tree_component_event(component_path, event_tag,
  origin='Tether')`; the GameplayTag must already be registered.
