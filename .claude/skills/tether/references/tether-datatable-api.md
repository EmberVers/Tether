# Tether DataTable Library API

Module: `unreal.TetherDataTableLibrary`
...
### get_data_table_summary(data_table_path) -> FTetherDataTableInfo

Schema only: name, row struct, num rows, column names. No row data.

```python
s = unreal.TetherDataTableLibrary.get_data_table_summary('/Game/Data/DT_Weapons')
...
### get_data_table_row(data_table_path, row_name) -> FTetherDataTableRow
...
```python
hits = unreal.TetherDataTableLibrary.search_data_table_rows(
...
### get_data_table_rows(data_table_path) -> FTetherDataTableInfo

Dumps the entire table including all nested struct text. Avoid unless needed.

```python
info = unreal.TetherDataTableLibrary.get_data_table_rows('/Game/Data/DT_Weapons')
for row in info.rows:
    print(f'  {row.row_name}: {list(row.fields)}')
```

### get_data_table_rows_filtered(data_table_path, row_filter, column_filter) -> FTetherDataTableInfo
...
```python
view = unreal.TetherDataTableLibrary.get_data_table_rows_filtered(
    '/Game/Data/DT_Weapons', ['Sword_01','Sword_02'], ['Damage','Range']
)
```

### FTetherDataTableInfo fields
...
| `rows` | list[FTetherDataTableRow] | Row data (empty for summary) |

### FTetherDataTableRow fields
...
```python
tables = unreal.TetherDataTableLibrary.get_data_tables_using_struct('WeaponData')
```
...
```python
unreal.TetherDataTableLibrary.set_data_table_row_field(
...
```python
unreal.TetherDataTableLibrary.add_data_table_row(
...
```python
if unreal.TetherDataTableLibrary.does_data_table_row_exist('/Game/Data/DT_Weapons', 'Sword_01'):
    ...
```
...
```python
unreal.TetherDataTableLibrary.set_data_table_row_fields(
...
### get_data_table_column_types(data_table_path) -> list[FTetherDataTableColumn]

Column names + property type names for the row struct. Cheap — no row data.

```python
cols = unreal.TetherDataTableLibrary.get_data_table_column_types('/Game/Data/DT_Weapons')
for c in cols:
...
FTetherDataTableColumn fields: `name`, `type_name` (e.g. `FloatProperty`, `StructProperty`), `inner_type_name` (for struct/object/enum/array properties; empty otherwise).

### get_data_table_row_as_map(data_table_path, row_name) -> dict[str, str]

Single row as a `{FieldName: exported_text}` dict. More ergonomic than the flat `FTetherDataTableRow.fields` list. Empty dict if the row is missing.

```python
row = unreal.TetherDataTableLibrary.get_data_table_row_as_map('/Game/Data/DT_Weapons', 'Sword_01')
...
```python
import json
s = unreal.TetherDataTableLibrary.get_data_table_row_as_json_string('/Game/Data/DT_Weapons', 'Sword_01')
row = json.loads(s)
```
...
```python
n = unreal.TetherDataTableLibrary.clear_data_table('/Game/Data/DT_Scratch')
```
...
n = unreal.TetherDataTableLibrary.copy_data_table_rows(
...
```python
p = unreal.TetherDataTableLibrary.get_data_table_row_struct_path('/Game/Data/DT_Weapons')
```
...
```python
hits = unreal.TetherDataTableLibrary.find_data_table_rows_by_field_value(
...
```python
import json
raw = unreal.TetherDataTableLibrary.get_data_table_rows_as_json_array(
    '/Game/Data/DT_Weapons', ['Sword_01','Sword_02'], ['Damage','Range']
)
rows = json.loads(raw)
...
```python
changed = unreal.TetherDataTableLibrary.diff_data_table_rows(
    '/Game/Data/DT_Weapons',        'Sword_01',
...
```python
defaults = unreal.TetherDataTableLibrary.get_data_table_row_defaults('/Game/Data/DT_Weapons')
