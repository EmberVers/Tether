# Tether PoseSearch Library API

Module: `unreal.TetherPoseSearchLibrary`
...
> **What this library does NOT cover:** the Chooser tables that *select* a PSD at runtime live in `tether-chooser-api.md`. The MotionMatching anim node and its Database pin live in `tether-anim-api.md` (use the existing AnimGraph write ops to wire it).

> **Index rebuilds are async.** Every PSD write op (add/remove/edit) automatically schedules a fresh DDC rebuild via `FAsyncPoseSearchDatabasesManagement::RequestAsyncBuildIndex(NewRequest)`. The CLI `tether.py wait-pose-index <psd_path>` polls until the rebuild lands. Don't `wait` inside an `exec` — the GameThread is held; rebuild can't progress.
...
### `get_schema_info(schema_path) → FTetherPSSInfo`

```python
info = unreal.TetherPoseSearchLibrary.get_schema_info(
...
### `list_schema_channels(schema_path) → TArray<FTetherPSSChannel>`
...
### `set_crashing_legs_channel(...) → FTetherPSSCrashingLegsChannelResult`
...
```python
result = unreal.TetherPoseSearchLibrary.set_crashing_legs_channel(
...
### `get_database_info(database_path) → FTetherPSDInfo`

```python
psd = unreal.TetherPoseSearchLibrary.get_database_info(
...
### `list_database_animations(database_path) → TArray<FTetherPSDAnimEntry>`
...
> Every write below dirties the package + schedules an async re-index. Save with `unreal.EditorAssetLibrary.save_loaded_asset(psd)` when you're ready; the tether does not auto-save.

### `add_animation_to_database(...) → FTetherPSDAddResult`

```python
r = unreal.TetherPoseSearchLibrary.add_animation_to_database(
...
Accepts `AnimSequence`, `BlendSpace`, `AnimComposite`, `AnimMontage`. **Failure is reported via `result.error`, not via UE_LOG** (UE_LOG is invisible to tether clients).
...
### `add_blend_space_to_database(...) → FTetherPSDAddResult`

Same shape, plus BlendSpace knobs:

```python
r = unreal.TetherPoseSearchLibrary.add_blend_space_to_database(
    database_path='...',
...
```python
unreal.TetherPoseSearchLibrary.set_database_animation_enabled(
    database_path='...', index=3, enabled=False)
unreal.TetherPoseSearchLibrary.set_database_animation_sampling_range(
    database_path='...', index=3, sampling_range_min=0.5, sampling_range_max=2.5)
unreal.TetherPoseSearchLibrary.set_database_animation_mirror_option(
    database_path='...', index=3, mirror_option='UnmirroredAndMirrored')
unreal.TetherPoseSearchLibrary.set_database_blend_space_sampling(
...
```bash
python tether.py wait-pose-index '/Game/.../PSD_Foo' --wait-timeout 300 --poll-interval 1.0
```
...
- **Channel sub-structure beyond top-level isn't surfaced** by `list_schema_channels` — only `sub_channel_count` is. To see what's inside a Group, you currently need `unreal.load_object(...).get_editor_property('channels')[i].get_editor_property('sub_channels')`. (Could be lifted into a future tether call.)
...
The C++ `UPoseSearchSchema::AddChannel(UPoseSearchFeatureChannel*)` and `AddSkeleton(...)` are public — straightforward to wrap into tether functions when needed. **Open an issue / ask before reaching for raw reflection** — usually the right answer is "extend this library."
