# Tether Niagara / VFX API

Module: `unreal.TetherNiagaraLibrary`

Preferred kwargs-only wrapper: `from tether import Niagara`
...
3. Create from a suitable template or a `TetherNiagaraSystemRecipe`. Re-list
   Emitters, modules, inputs, renderers, and User parameters after structural
   edits; stable IDs are returned by Tether and must not be guessed.
...
`TetherNiagaraSystemRecipe` contains optional `template_path`, warmup, fixed
bounds, `effect_type_path`, `user_parameters`, and `emitters`. Each
`TetherNiagaraEmitterSpec` contains template path, enabled/local-space/CPU-GPU
settings, deterministic seed, bounds, modules, and renderers. Module inputs use
`TetherNiagaraInputValue.mode`:
...
| `link_niagara_module_input(...)` | Replaces the override with a linked parameter; readback returns the parameter name, for example `User.TetherColor`. |
...
`NiagaraInt32`, `Vector3f`, and `LinearColor`; the tether treats the aliases as
...
`TetherNiagaraCompileMessage` includes severity, Emitter ID, script usage,
...
| `create_explosion_effect(...)` | Duplicates the multi-layer SimpleExplosion System, optionally adds a real `ShockwaveRibbon` Emitter and Light renderer, and enables fixed bounds. Exposes core color, debris count, duration, mesh scale, and derived `TetherShockwaveRadius = 250 * scale`. |
| `create_dissolve_effect(...)` | Builds a looping BlowingParticles breakup layer with color, spawn rate (`TetherCount` is a float rate), lifetime, sphere radius, and wind direction links. `DissolveAmount` is exposed for synchronization with a dissolve-capable source material. |
...
| Function | Notes |
|---|---|
| `spawn_niagara_preview(system_path, transform, auto_activate, reset_on_change)` | Spawns a transient editor-world component and returns a session-local handle. If the System is not ready after cold load, the tether compiles and waits before spawning. |
...
| Preset exposes parameters but they do not affect the effect | Verify every intended module input is `Linked` to the User parameter. Engine type names such as `NiagaraFloat` are aliases of tether `Float`, but the selected input still has to be semantically active. |
