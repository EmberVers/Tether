# Tether Blueprint Library API

Module: `unreal.TetherBlueprintLibrary`
...
1. **说明优势与劣势**：告诉用户你可以用 tether 原语 spawn 节点 / 连线 / 建变量 / 建函数 / auto-layout / lint / compile，但**成品的美感与可维护性不如人工或 C++ 实现**（列出上面具体几点）。
2. **给出替代方案**：提议改用 C++（新 `UCLASS` / `UFUNCTION(BlueprintCallable)`），蓝图里只做薄封装调用；或者改用现有的 tether 写能力（材质 / 动画 / UMG / DataTable / Level 等都有结构化 API，这些领域 agent 可以做得很好）。
...
### get_blueprint_parent_class(blueprint_path) -> (bool, FTetherClassInfo)

Get the direct parent class of a Blueprint.

```python
success, parent = unreal.TetherBlueprintLibrary.get_blueprint_parent_class(
    '/Game/BP/MyBP'
)
print(parent.class_name, parent.class_path, parent.is_native)
```

### get_blueprint_class_hierarchy(blueprint_path) -> list[FTetherClassInfo]

Full inheritance chain. Index 0 = BP itself, last = UObject.

```python
chain = unreal.TetherBlueprintLibrary.get_blueprint_class_hierarchy('/Game/BP/MyBP')
for cls in chain:
    tag = '[Native]' if cls.is_native else '[BP]'
    print(f'{tag} {cls.class_name} — {cls.class_path}')
```

### FTetherClassInfo fields
...
### get_blueprint_variables(blueprint_path, include_inherited=False) -> list[FTetherVariableInfo]

List all variables defined in a Blueprint.

```python
vars = unreal.TetherBlueprintLibrary.get_blueprint_variables('/Game/BP/MyBP')
for v in vars:
    print(f'{v.name}: {v.type} = {v.default_value}  (editable={v.instance_editable})')

# Include inherited variables from parent classes
all_vars = unreal.TetherBlueprintLibrary.get_blueprint_variables('/Game/BP/MyBP', True)
```

### FTetherVariableInfo fields
...
### get_blueprint_functions(blueprint_path, include_inherited=False) -> list[FTetherFunctionInfo]

List all functions and events.

> ⚠️ **Token cost: MEDIUM–HIGH.** Returns full `FTetherFunctionInfo` per function (params, description, category, access). A Character BP with 30+ functions × multi-param signatures × tooltips easily runs several KB. **Prefer `get_blueprint_overview`** (returns compact `FTetherFunctionSummary` — name + kind + signature) when you only need a catalog. `include_inherited=True` multiplies the output by the inheritance chain depth.

```python
funcs = unreal.TetherBlueprintLibrary.get_blueprint_functions('/Game/BP/MyBP')
for f in funcs:
...
| `params` | list[FTetherFunctionParam] | Input/output parameters |

### FTetherFunctionParam fields
...
### get_blueprint_components(blueprint_path) -> list[FTetherComponentInfo]

Get the component tree (own + inherited from parent CDO).

```python
comps = unreal.TetherBlueprintLibrary.get_blueprint_components('/Game/BP/MyActor')
for c in comps:
...
### get_blueprint_interfaces(blueprint_path) -> list[FTetherInterfaceInfo]

List all implemented interfaces and their functions.

```python
ifaces = unreal.TetherBlueprintLibrary.get_blueprint_interfaces('/Game/BP/MyBP')
for i in ifaces:
    bp_tag = '[BP]' if i.is_blueprint_implemented else '[Native]'
    print(f'{bp_tag} {i.interface_name}: {", ".join(i.functions)}')
```

### FTetherInterfaceInfo fields
...
### get_blueprint_overview(blueprint_path) -> (bool, FTetherBlueprintOverview)
...
```python
ov = unreal.TetherBlueprintLibrary.get_blueprint_overview('/Game/BP/MyBP')
...
### FTetherBlueprintOverview fields

| Field | Type | Description |
|-------|------|-------------|
| `blueprint_name` | str | Blueprint asset name |
| `blueprint_type` | str | First native ancestor: "Actor", "Character", "AnimInstance", etc. |
| `parent_class` | FTetherClassInfo | Direct parent class info |
| `variables` | list[FTetherVariableSummary] | Compact variable list (name + type only) |
| `functions` | list[FTetherFunctionSummary] | Compact function list (name + kind + signature) |
| `components` | list[FTetherComponentSummary] | Compact component list (name + class + parent) |
...
### FTetherComponentSummary fields
...
### get_event_dispatchers(blueprint_path) -> list[FTetherEventDispatcherInfo]

Get all event dispatchers with their parameter signatures.

```python
dispatchers = unreal.TetherBlueprintLibrary.get_event_dispatchers('/Game/BP/MyBP')
for d in dispatchers:
    params = ', '.join(f'{p.name}: {p.type}' for p in d.params)
    print(f'{d.name}({params})')
```

### FTetherEventDispatcherInfo fields

| Field | Type | Description |
|-------|------|-------------|
| `name` | str | Dispatcher name |
| `params` | list[FTetherFunctionParam] | Delegate parameters |

---

## Graph Listing

### get_graph_names(blueprint_path) -> list[FTetherGraphInfo]

Lightweight list of all graphs. Use before diving into node details.

```python
graphs = unreal.TetherBlueprintLibrary.get_graph_names('/Game/BP/MyBP')
for g in graphs:
    print(f'[{g.graph_type}] {g.name}')
```

### FTetherGraphInfo fields
...
### get_function_call_graph(blueprint_path, function_name) -> list[FTetherCallEdge]
...
```python
edges = unreal.TetherBlueprintLibrary.get_function_call_graph('/Game/BP/MyBP', 'TakeDamage')
for e in edges:
    print(f'  -> [{e.target_kind}] {e.target_class}::{e.target_name}')
```

### FTetherCallEdge fields
...
### get_function_nodes(blueprint_path, function_name, node_type_filter) -> list[FTetherNodeInfo]
...
```python
nodes = unreal.TetherBlueprintLibrary.get_function_nodes('/Game/BP/MyBP', '', 'FunctionCall')
for n in nodes:
    print(f'[{n.node_type}] {n.title}  target={n.target_class} var={n.variable_name}')
```

### FTetherNodeInfo fields
...
### get_function_execution_flow(blueprint_path, function_name) -> list[FTetherExecStep]
...
```python
steps = unreal.TetherBlueprintLibrary.get_function_execution_flow('/Game/BP/MyBP', '')
for s in steps:
...
| `exec_outputs` | list[FTetherExecConnection] | Outgoing exec connections |

### FTetherExecConnection fields
...
### get_node_pin_connections(blueprint_path, function_name) -> list[FTetherPinConnection]
...
```python
connections = unreal.TetherBlueprintLibrary.get_node_pin_connections('/Game/BP/MyBP', '')
...
### get_component_property_values(blueprint_path, component_name) -> list[FTetherPropertyValue]
...
```python
props = unreal.TetherBlueprintLibrary.get_component_property_values(
...
### search_blueprint_nodes(blueprint_path, query, graph_name="") -> list[FTetherNodeSearchResult]
...
```python
results = unreal.TetherBlueprintLibrary.search_blueprint_nodes(
...
### FTetherNodeSearchResult fields
...
### get_timeline_info(blueprint_path) -> list[FTetherTimelineInfo]

Get all Timeline components in a Blueprint with their tracks and settings.

```python
timelines = unreal.TetherBlueprintLibrary.get_timeline_info('/Game/BP/MyActor')
for tl in timelines:
...
| `tracks` | list[FTetherTimelineTrack] | Tracks in timeline |

### FTetherTimelineTrack fields
...
```python
ok = unreal.TetherBlueprintLibrary.set_blueprint_variable_default(
...
```python
ok = unreal.TetherBlueprintLibrary.set_component_property(
...
```python
ok = unreal.TetherBlueprintLibrary.add_blueprint_variable(
...
```python
unreal.TetherBlueprintLibrary.remove_blueprint_variable('/Game/BP/MyBP', 'Score')
```
...
```python
unreal.TetherBlueprintLibrary.rename_blueprint_variable('/Game/BP/MyBP', 'Health', 'HP')
```
...
### get_function_local_variables(blueprint_path, function_name) -> list[FTetherVariableInfo]

List local variables on a function. Returns the same `FTetherVariableInfo` fields as `get_blueprint_variables` — name, type, category, default_value, description, instance_editable, blueprint_read_only. `replication_condition` is always `"None"` (local vars can't be replicated).

```python
L = unreal.TetherBlueprintLibrary
...
```python
unreal.TetherBlueprintLibrary.add_blueprint_interface(
    '/Game/BP/MyBP',
...
```python
unreal.TetherBlueprintLibrary.remove_blueprint_interface('/Game/BP/MyBP', 'BPI_Interactable_C')
```
...
```python
unreal.TetherBlueprintLibrary.add_blueprint_component(
    '/Game/BP/MyBP',
...
```python
guid = unreal.TetherBlueprintLibrary.add_call_function_node(
...
lib.set_pin_default_value(bp, g, print_g, 'InString',  'Hello from tether')
...
Remove a user function. Recompiles. The tether automatically closes any
...
### get_compile_errors(blueprint_path) -> list[FTetherCompileMessage]

Compile the BP and return all compiler messages (errors + warnings + notes + info). `FTetherCompileMessage` fields:
...
```python
lib.set_blueprint_metadata(bp, 'My Hero', 'Player pawn', 'Tether|Demo', '')
```
...
> in a way that tether `exec` can't recover from — the break parks the
...
target function, and the tether can't recover from that freeze via
...
### get_breakpoints(blueprint_path) -> list[FTetherBreakpointInfo]
...
does **not** pump the FTSTicker-based Python exec queue the tether
...
**Recovery path — `python tether.py resume`.** The tether TCP server
...
> Never rely on it to recover from a pause; use `tether.py resume`
> instead.
...
3. Keep a second terminal open during exploratory sessions so
   `tether.py resume` is one keystroke away.
...
The returned node has no tracks yet. The current tether has no track-edit API; populate tracks via the Timeline editor in UE, or use the Python `unreal.TimelineTemplate` reflection if you need tracks set programmatically.
...
### get_blueprint_summary(blueprint_path) -> FTetherBlueprintSummary
...
```python
s = unreal.TetherBlueprintLibrary.get_blueprint_summary(
...
### get_function_summary(blueprint_path, function_name) -> FTetherFunctionSemantics
...
```python
s = unreal.TetherBlueprintLibrary.get_function_summary(
    '/Game/Blueprints/SandboxCharacter_CMC.SandboxCharacter_CMC',
    'GetDesiredGait')
...
### find_variable_references(blueprint_path, variable_name) -> list[FTetherReference]
### find_function_call_sites(blueprint_path, function_name) -> list[FTetherReference]
### find_event_handler_sites(blueprint_path, event_name) -> list[FTetherReference]
...
### find_function_call_sites_global(function_name, owning_class_filter, package_path, max_results) -> list[FTetherGlobalReference]
...
**Returns** — list of `FTetherGlobalReference` with:
...
| `node_guid` | 32-hex digits (same form as other tether APIs) |
...
### find_blueprint_debug_prints(package_path, max_results) -> list[FTetherDebugPrintSite]
...
**Returns** — list of `FTetherDebugPrintSite`:
...
### get_node_pins(blueprint_path, graph_name, node_guid) -> list[FTetherPinInfo]
...
### describe_node(blueprint_path, graph_name, node_guid) -> FTetherNodeDescription
...
defaults, and **link targets** — all in a single tether round-trip.
...
```python
desc = unreal.TetherBlueprintLibrary.describe_node(bp, fn, guid)
...
| `pins` | list of `FTetherPinInfo` with `linked_to` populated |
...
### get_function_signature(class_path, function_name) -> FTetherFunctionSignature
...
```python
sig = unreal.TetherBlueprintLibrary.get_function_signature(
...
#### FTetherFunctionSignature fields
...
| `parameters` | list of `FTetherFunctionParam` (see below) |

#### FTetherFunctionParam fields (extended)
...
- **AActor-derived BP** — spawned in the editor world with `RF_Transient`
  ...
  after the call. The spawn happens on the GameThread (tether's
  standard dispatch).
...
### get_editor_focus_state() -> FTetherEditorFocusState
...
### rename_member_variable_global(defining_blueprint_path, old_name, new_name, package_path) -> FTetherRenameReport
### rename_function_global(defining_blueprint_path, old_name, new_name, package_path) -> FTetherRenameReport
...
tether's cross-BP scan is the belt-and-suspenders layer for BPs that
...
The `FTetherRenameReport` fields: `success` (bool), `updated_node_count`
...
### diff_graph_snapshots(before_json, after_json) -> FTetherGraphDiff
...
**Fields on `FTetherGraphDiff`**: `added_nodes`, `removed_nodes`
...
### replace_node_preserving_connections(blueprint_path, graph_name, old_node_guid, new_node_class_path) -> FTetherReplaceNodeReport
...
### apply_graph_ops(blueprint_path, ops_json) -> list[FTetherGraphOpResult]

Execute a sequence of graph mutations in a single tether round-trip
...
### find_cdo_variable_overrides(defining_blueprint_path, variable_name, package_path) -> list[FTetherCdoOverride]
...
### get_pie_node_coverage(blueprint_path) -> list[FTetherNodeCoverageEntry]
...
**`FTetherNodeCoverageEntry` fields**: `node_guid`, `node_title`,
...
### get_last_breakpoint_hit(blueprint_path) -> FTetherBreakpointHit

Return the most recent breakpoint-hit snapshot for this BP.
Tether subscribes to `FBlueprintCoreDelegates::OnScriptException`
...
**Recovery when a break freezes the tether.** When a breakpoint
...
also time out. Use `python tether.py resume` from a separate
...
### get_node_layout(blueprint_path, graph_name, node_guid) -> FTetherNodeLayout
...
### get_node_pin_layouts(blueprint_path, graph_name, node_guid) -> list[FTetherPinLayout]
...
Must be called in one tether exec ON ITS OWN (Slate can't tick while
...
unreal.TetherEditorLibrary.open_asset(bp)
```
```python
...
### get_rendered_node_info(blueprint_path, graph_name) -> list[FTetherRenderedNode]
...
| `pins` | Array of `FTetherRenderedPin` (see below) |
| `is_live` | True if the widget was found and measured |

#### FTetherRenderedPin fields
...
### predict_node_size(kind, param_a, param_b, param_int) -> FTetherNodeSizeEstimate
...
formula matches `FTetherNodeLayout.estimated_*` exactly — prediction and
...
#### FTetherNodeSizeEstimate fields
...
### auto_layout_graph(blueprint_path, graph_name, strategy, anchor_node_guid, h_spacing, v_spacing) -> FTetherLayoutResult
...
Getting a pixel-accurate pin-aligned layout requires three tether
...
which can't happen while the tether exec is still holding the
...
### lint_blueprint(blueprint_path, severity_filter, oversized_fn_threshold, long_exec_chain_threshold, large_graph_threshold) -> list[FTetherLintIssue]
...
**FTetherLintIssue fields**: `severity`, `code`, `message`, `graph_name`, `node_guid`, `variable_name`, `function_name` (location fields are `""` when not meaningful).
...
```python
lib = unreal.TetherBlueprintLibrary
bp = '/Game/BP/BP_Hero'
g  = 'EventGraph'
...
unreal.TetherEditorLibrary.compile_blueprints([bp])
```
