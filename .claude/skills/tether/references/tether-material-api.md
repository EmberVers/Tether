# Tether Material Library API

Module: `unreal.TetherMaterialLibrary`
...
## refresh_texture_resource(texture_path, force_derived_data_rebuild=False) -> FTetherTextureRefreshResult
...
```python
result = unreal.TetherMaterialLibrary.refresh_texture_resource(
...
### FTetherTextureRefreshResult fields
...
## get_material_instance_parameters(material_path) -> FTetherMaterialInstanceInfo
...
```python
info = unreal.TetherMaterialLibrary.get_material_instance_parameters(
...
### FTetherMaterialInstanceInfo fields
...
| `parameters` | list[FTetherMaterialParam] | Parameter overrides on the MI |

### FTetherMaterialParam fields
...
## get_material_instance_layer_stack(material_instance_path) -> FTetherMaterialLayerStack
...
```python
stack = unreal.TetherMaterialLibrary.get_material_instance_layer_stack(
...
### FTetherMaterialLayerEntry fields
...
| `restrict_to_layer_relatives` | bool | The engine `RestrictToLayerRelatives[index]` flag, preserved without adding tether-specific restriction semantics |
...
```python
result = unreal.TetherMaterialLibrary.set_material_instance_layer_stack(
    '/Game/Materials/MI_Surface',
    stack.layers)
if not result.success:
    raise RuntimeError(result.error)
...
### FTetherMaterialLayerStackOpResult fields
...
## copy_material_instance_layer_stack(source_material_instance_path, destination_material_instance_path) -> FTetherMaterialLayerStackOpResult
...
## get_material_info(material_path) -> FTetherMaterialInfo
...
```python
info = unreal.TetherMaterialLibrary.get_material_info(
...
| `scalar_parameters` | list[FTetherMaterialParamDefault] | All scalar params declared on the master, with default values |
| `vector_parameters` | list[FTetherMaterialParamDefault] | All vector params, default as `"(R=,G=,B=,A=)"` |
| `texture_parameters` | list[FTetherMaterialParamDefault] | All texture params, default as asset path |
| `static_switch_parameters` | list[FTetherMaterialParamDefault] | All static-switch params, default as `"True"` / `"False"` |
...
### FTetherMaterialParamDefault fields
...
## list_material_functions(path_prefix, max_results) -> list[FTetherMaterialFunctionSummary]
...
mfs = unreal.TetherMaterialLibrary.list_material_functions("/Game", 50)
# All Engine MFs, no cap
mfs = unreal.TetherMaterialLibrary.list_material_functions("/Engine", 0)
# Whole project, cap 20
mfs = unreal.TetherMaterialLibrary.list_material_functions("", 20)
for s in mfs:
...
### FTetherMaterialFunctionSummary fields
...
## get_material_function(function_path) -> FTetherMaterialFunctionInfo
...
```python
info = unreal.TetherMaterialLibrary.get_material_function(
...
### FTetherMaterialFunctionInfo fields
...
| `inputs` | list[FTetherMaterialFunctionPort] | Sorted by `sort_priority` ascending |
| `outputs` | list[FTetherMaterialFunctionPort] | Sorted by `sort_priority` ascending |
| `num_expressions` | int | Total `UMaterialExpression` count in the MF graph |

### FTetherMaterialFunctionPort fields
...
## list_material_instance_chain(material_path) -> FTetherMaterialInstanceChain
...
```python
chain = unreal.TetherMaterialLibrary.list_material_instance_chain(
    '/Engine/EngineMaterials/Widget3DPassThrough_Opaque')
...
### FTetherMaterialInstanceChain fields

| Field | Type | Description |
|-------|------|-------------|
| `found` | bool | `False` if the path could not be loaded |
| `path` | str | Full path of the leaf asset that was passed in |
| `layers` | list[FTetherMaterialInstanceLayer] | Ordered leaf → base. Element 0 is the input asset; last element is the ultimate `UMaterial` |

### FTetherMaterialInstanceLayer fields

| Field | Type | Description |
|-------|------|-------------|
| `name` / `path` | str | This layer's asset |
| `is_base_material` | bool | `True` for the final `UMaterial` leaf of the chain |
| `override_parameters` | list[FTetherMaterialParam] | Parameters *explicitly set on this layer*. Always empty for the base `UMaterial` (defaults for master parameters are reported by `get_material_info`, not here). Covers all 6 override categories: Scalar / Vector / DoubleVector / Texture / RuntimeVirtualTexture / StaticSwitch. |
...
## get_material_parameter_collection(collection_path) -> FTetherMaterialParameterCollectionInfo

**M1-9.** Read scalar + vector parameters from a `UMaterialParameterCollection` with their default values and stable GUIDs.

```python
info = unreal.TetherMaterialLibrary.get_material_parameter_collection(
...
### FTetherMaterialParameterCollectionInfo fields

| Field | Type | Description |
|-------|------|-------------|
| `found` | bool | `False` if the path could not be loaded |
| `name` / `path` | str | Asset name / full object path |
| `scalar_parameters` | list[FTetherMPCScalarParam] | All scalar params |
| `vector_parameters` | list[FTetherMPCVectorParam] | All vector params |

### FTetherMPCScalarParam fields
...
## get_material_graph(material_path) -> FTetherMaterialGraph
...
```python
g = unreal.TetherMaterialLibrary.get_material_graph('/Game/Materials/M_MyMaster')
if not g.found:
...
| `nodes` | list[FTetherMaterialGraphNode] | All expression nodes, in declaration order |
| `connections` | list[FTetherMaterialGraphConnection] | Physical expression wires plus explicit named-reroute and composite-gateway dependency edges; inspect `edge_kind` |
| `output_connections` | list[FTetherMaterialGraphConnection] | For `UMaterial` only: wires into `BaseColor` / `Metallic` / etc. `dst_guid` is the invalid (all-zero) GUID; `dst_property_name` is the enum name without the `MP_` prefix; `edge_kind="material_output"` |

### FTetherMaterialGraphNode fields
...
### FTetherMaterialGraphConnection fields
...
## get_material_stats(material_path, feature_level, quality) -> FTetherMaterialStats
...
```python
s = unreal.TetherMaterialLibrary.get_material_stats(
...
| `shaders` | list[FTetherMaterialShaderStat] | One entry per representative shader variant (de-duplicated). Empty for MI whose parent's shader map isn't materialized under this feature level, and for materials still compiling |
...
### FTetherMaterialShaderStat fields
...
- **Budget check pattern** — for per-template budget enforcement:
  ```python
  stats = unreal.TetherMaterialLibrary.get_material_stats(path, 'SM5', 'High')
...
```python
errors = unreal.TetherMaterialLibrary.get_material_compile_errors(
...
- On load failure — a single synthetic entry starting with `"Tether: could not load material"`
...
ok = unreal.TetherMaterialLibrary.preview_material(
...
```python
ok = unreal.TetherMaterialLibrary.preview_material_complexity(
    "/Game/Materials/M_Armor",
...
## create_material(path, domain, shading_model, blend_mode, two_sided, use_material_attributes) -> FTetherCreateAssetResult

**M2-1.** Create a new `UMaterial` asset.

```python
r = unreal.TetherMaterialLibrary.create_material(
...
### FTetherCreateAssetResult fields
| Field | Type | Description |
|---|---|---|
...
## create_material_instance(parent_path, instance_path) -> FTetherCreateAssetResult

**M2-2.** Create a `UMaterialInstanceConstant` with the given parent (itself a master or another MI).

```python
unreal.TetherMaterialLibrary.create_material_instance(
    "/Game/Materials/M_NewMaster",
    "/Game/Materials/MI_Variant_Red")
```

Same `FTetherCreateAssetResult` contract as `create_material`.

---

## add_material_expression(material_path, expression_class, x, y) -> FTetherAddExpressionResult
...
n = L.set_material_expression_properties(mat, scalar.guid, [
    unreal.TetherExpressionPropSet(name="ParameterName", value="Roughness"),
    unreal.TetherExpressionPropSet(name="DefaultValue", value="0.35"),
    unreal.TetherExpressionPropSet(name="Group", value="PBR"),
    unreal.TetherExpressionPropSet(name="SortPriority", value="1"),
])
assert n == 4
```

### FTetherExpressionPropSet fields
| Field | Type | Description |
|---|---|---|
...
## create_material_function(path, description, expose_to_library, library_category) -> FTetherCreateAssetResult
...
r = L.create_material_function(
    ...
    library_category="Tether/Normals")
```
...
## validate_material_graph_ops(material_path, ops) -> FTetherMaterialGraphOpResult
...
## apply_material_graph_ops(material_path, ops, compile) -> FTetherMaterialGraphOpResult
...
```python
def op(kind, **kw):
    o = unreal.TetherMaterialGraphOp()
    o.op = kind
...
### FTetherMaterialGraphOpResult fields
| Field | Type | Description |
|---|---|---|
...
The tether ships a shared HLSL snippet library at
`Plugin/Tether/Shaders/Private/TetherSnippets.ush`, registered under the virtual
path `/Plugin/Tether/` at plugin startup via `AddShaderSourceDirectoryMapping`.

From a `UMaterialExpressionCustom` node, `#include` the snippets via
`/Plugin/Tether/TetherSnippets.ush` (the `IncludePaths` array) and call any
...
## add_custom_expression(material_path, x, y, input_names, output_type, code, include_paths, description) -> FTetherAddExpressionResult
...
r = L.add_custom_expression(
    ...
    output_type="Float3",
    code="return TetherBlendAngleCorrectedNormals(BaseN, DetailN, Strength);",
    include_paths=["/Plugin/Tether/TetherSnippets.ush"],
...
| `code` | str | HLSL body — typically a single `return TetherFoo(In0, In1, ...);` call into a snippet, or an inline expression |
| `include_paths` | list[str] | Virtual paths to `.ush` files to `#include`. Use `"/Plugin/Tether/TetherSnippets.ush"` for the shared library |
| `description` | str | Shown in the node title / docstring |

Returns the standard `FTetherAddExpressionResult` (same as `add_material_expression`).

---

## list_shared_snippets() -> list[FTetherShaderSnippet]

**M2.5-4.** Enumerate snippets declared in `TetherSnippets.ush`. Fast — reads the file
and parses `//@snippet` headers without loading snippet bodies.

```python
for s in unreal.TetherMaterialLibrary.list_shared_snippets():
    print(f"{s.name:<40}  {s.instruction_estimate:<5}  {s.description}")
    print(f"    {s.signature}")
```

### FTetherShaderSnippet fields

| Field | Type | Description |
|---|---|---|
| `name` | str | e.g. `"TetherACESTonemap"` |
...
## get_shared_snippet(name) -> FTetherShaderSnippet
...
## First-wave snippets (TetherSnippets.ush)

| Name | Signature | Instr. |
|---|---|---|
| `TetherLuminance` | `float TetherLuminance(float3 Color)` | ~3 |
| `TetherUnpackORM` | `float3 TetherUnpackORM(float3 Packed)` | ~0 |
| `TetherPackORM` | `float3 TetherPackORM(float O, float R, float M)` | ~0 |
| `TetherACESTonemap` | `float3 TetherACESTonemap(float3 Color)` | ~10 |
| `TetherBlendAngleCorrectedNormals` | `float3 TetherBlendAngleCorrectedNormals(float3 BaseN, float3 DetailN, float Strength)` | ~14 |
| `TetherDepthFade` | `float TetherDepthFade(float PixelDepth, float SceneDepth, float FadeDistance)` | ~4 |
| `TetherDitherLODTransition` | `float TetherDitherLODTransition(float Opacity, float2 PixelPos)` | ~10 |
| `TetherHash21` / `TetherHash31` | `float TetherHash21(float2 P)` / `TetherHash31(float3 P)` | ~6 / ~8 |
| `TetherValueNoise3D` | `float TetherValueNoise3D(float3 Pos)` | ~60 |
| `TetherVoronoi2D` | `float TetherVoronoi2D(float2 UV, float Cells)` | ~70 |

All snippets use `Tether*` prefix to avoid name collisions with engine or user code
inlined into generated material shaders. To add a new snippet, edit
`Plugin/Tether/Shaders/Private/TetherSnippets.ush` following the header-tag
...
**Asset-modal deadlock gotcha.** UE shows a "completing asset references" progress modal on some `IAssetTools::CreateAsset` / `save_loaded_asset` calls, and because tether exec serializes through the GameThread, a modal from one op will deadlock subsequent queued ops. Rule: **one asset-write op per tether exec** — create, save, and multi-render operations each go in their own `tether.exec-file` call.

---

## set_mi_params(material_instance_path, params) -> FTetherMIParamResult
...
| `outcomes` | list[FTetherMIParamOutcome] | One result per input, in input order |

### FTetherMIParamOutcome fields
...
```python
ok = unreal.TetherMaterialLibrary.set_mi_and_preview(
    mi_path,
    [ps("Roughness", "Scalar", "0.35")],
    "shaderball", "hdri", 768, 30, 15, 0,
    "G:/Claude/Tether/temp/previews/tweak.png")
```
...
```python
paths = unreal.TetherMaterialLibrary.sweep_mi_params(
    mi_path, "Roughness",
...
## set_material_parameter_collection(collection_path, params) -> FTetherMIParamResult

**M6-4.** Write Scalar / Vector default values on a `UMaterialParameterCollection`. Same `FTetherMIParamSet` payload as `set_mi_params`, but `type` must be `"Scalar"` or `"Vector"` (the two MPC value kinds).
...
## create_post_process_material(path, blendable_location, output_alpha) -> FTetherCreateAssetResult
...
r = L.create_post_process_material(
    "/Game/TetherTemplates/PP_Sketch",
...
Returns `FTetherCreateAssetResult { success, path, error }`. The created material has an empty graph — use `add_material_expression` / `apply_material_graph_ops` (M2) to wire it.

---

## get_post_process_state() -> list[FTetherPostProcessVolumeInfo]
...
### FTetherPostProcessVolumeInfo fields
...
| `blendables` | list[FTetherPostProcessBlendable] | `{material_path, weight}` entries |

---

## apply_post_process_material(volume_actor, material_path, weight) -> bool

**M4-7.** Append (or update) a PostProcess material on a PPV's `WeightedBlendables`. If `volume_actor` is empty, targets the first unbound PPV — creating a new `"PPV_Tether"` unbound volume if none exists yet.

```python
# Create + apply + verify in three calls.
L.create_post_process_material("/Game/TetherTemplates/PP_Sketch", "AfterTonemapping", False)
L.apply_post_process_material("", "/Game/TetherTemplates/PP_Sketch", 1.0)    # "" = use/create unbound PPV
...
## analyze_material(material_path, instruction_budget, sampler_budget) -> FTetherMaterialAnalysis
...
| `M5-11` | Custom HLSL node body is trivially short (<64 chars / ≤1 newline) AND doesn't include a `.ush` snippet — worth inlining as plain graph nodes so UE's compiler can apply constant folding, CSE, and DCE. Snippet trampolines (`return TetherXxx(...);` with `IncludeFilePaths`) are exempt. |
...
```python
L = unreal.TetherMaterialLibrary
r = L.analyze_material("/Game/TetherTemplates/M_Character_Armor",
...
### FTetherMaterialAnalysis fields
...
## auto_fix_material(material_path, fixes, save_after) -> FTetherMaterialAutoFixResult
...
r = L.auto_fix_material(
    "/Game/TetherTemplates/M_Character_Armor",
...
### FTetherMaterialAutoFixResult fields
...
**M3-2** — ``M_Character_Armor`` master. PBR core + optional Detail-Normal blend (RNM via `TetherBlendAngleCorrectedNormals`), Wear mask, Wetness, Emissive, Anisotropy. Every feature is gated by a `StaticSwitchParameter` — default MI has all switches off and compiles to ~80 instructions.

```python
from material_templates import character_armor

r = character_armor.build(
    master_path="/Game/TetherTemplates/M_Character_Armor",
    mi_path="/Game/TetherTemplates/MI_Character_Armor_Test",
...
|---|---|---|
| `master_path` | `/Game/TetherTemplates/M_Character_Armor` | Master asset path |
| `mi_path` | `/Game/TetherTemplates/MI_Character_Armor_Test` | Test MI (pass `None` to skip) |
...
r = environment_prop.build(
    master_path="/Game/TetherTemplates/M_Environment_Prop",
    mi_path="/Game/TetherTemplates/MI_Environment_Prop_Test",
...
| helper | purpose |
|---|---|
| `OpList` | Accumulates `FTetherMaterialGraphOp` entries with symbolic names → resolved to `$N` refs at flush time. Supports `add` / `comment` / `reroute` / `setp` / `connect` / `connect_out` / `add_literal` (for guids produced by calls outside the batch, e.g. Custom nodes). |
...
1. New module under `Plugin/Tether/Content/Python/material_templates/<name>.py` (or inside a subpackage).
...
4. If the template needs HLSL, prefer a shared snippet in `TetherSnippets.ush` + `add_custom_expression` (always separate from `apply_material_graph_ops` — register the returned guid via `ops.add_literal(str(guid), "name")` before referencing it in connects).
