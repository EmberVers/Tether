# Tether Asset Library API

Module: `unreal.TetherAssetLibrary`
...
>
> ```python
> paths, _ = unreal.TetherAssetLibrary.search_assets_in_all_content('Hero', 20)
...
```python
paths, tokens = unreal.TetherAssetLibrary.search_assets_in_all_content('Interaction', 20)
for p in paths:
...
```python
paths, tokens = unreal.TetherAssetLibrary.search_assets(
    'MyAsset',                                          # query
    unreal.TetherAssetSearchScope.ALL_ASSETS,           # scope
...
```python
paths, _ = unreal.TetherAssetLibrary.search_assets_under_path('/Game/Characters', 'hero', 20)
```

### ETetherAssetSearchScope
...
excluded = set()
derived = unreal.TetherAssetLibrary.get_derived_classes(base, excluded)
for cls in derived:
    print(cls.get_name())
```
...
```python
derived = unreal.TetherAssetLibrary.get_derived_classes_by_blueprint_path(
...
```python
deps, refs = unreal.TetherAssetLibrary.get_asset_references('/Game/BP/MyActor')
...
### get_searchable_names_used_by_asset(asset_path, struct_type_filter, max_results) -> list[TetherSearchableNameRef]
...
> **NOTE**: this only returns *used* values. For all *registered* GameplayTags (including those defined in config but never referenced), use `unreal.TetherGameplayTagLibrary.list_all_registered_tags(...)` — see `tether-gameplaytag-api.md`.

`TetherSearchableNameRef` fields:
...
### create_data_asset(asset_path, data_asset_class_path, save=True) -> FTetherDataAssetCreateResult
...
```python
assets = unreal.TetherAssetLibrary.get_data_assets_by_base_class(unreal.MyDataAsset)
for a in assets:
...
```python
paths = unreal.TetherAssetLibrary.list_assets_under_path('/Game/Characters', True)
```
...
```python
subs = unreal.TetherAssetLibrary.get_sub_folder_paths('/Game')
```
...
```python
unreal.TetherAssetLibrary.does_asset_exist('/Game/BP/BP_MyActor')  # True/False
```

### get_asset_info(asset_path) -> FTetherAssetInfo

Read registry-backed metadata: class path, redirector flag, disk size, and every tag key/value pair.

```python
info = unreal.TetherAssetLibrary.get_asset_info('/Game/BP/BP_MyActor')
if info.found:
...
| `tags` | list[FTetherAssetTag] | All AssetRegistry tag key/value pairs |

#### FTetherAssetTag fields
...
tex = unreal.TetherAssetLibrary.get_assets_by_class('/Script/Engine.Texture2D', False)

# All actors derived from a specific Blueprint
derived = unreal.TetherAssetLibrary.get_assets_by_class(
...
same_parent = unreal.TetherAssetLibrary.get_assets_by_tag_value(
    'ParentClass',
...
```python
target = unreal.TetherAssetLibrary.resolve_redirector('/Paper2D/DummySpriteTexture')
if target:
    print('Redirects to:', target)
```

For batch redirector cleanup, prefer `find_redirectors_under_path(folder, recursive)` (below) plus `unreal.TetherEditorLibrary.fixup_redirectors(...)`.
...
```python
unreal.TetherAssetLibrary.get_asset_class_path('/Game/BP/BP_MyActor')
# '/Script/Engine.Blueprint'
```
...
```python
parent = unreal.TetherAssetLibrary.get_asset_tag_value(
...
tex_and_mat = unreal.TetherAssetLibrary.get_assets_of_classes(
...
Find every `UObjectRedirector` under a folder. Pair with `unreal.TetherEditorLibrary.fixup_redirectors(...)` for batch cleanup.
...
```python
deps = unreal.TetherAssetLibrary.get_package_dependencies(
...
```python
refs = unreal.TetherAssetLibrary.get_package_referencers(
...
```python
roots = unreal.TetherAssetLibrary.get_content_roots()
...
### get_static_mesh_info(asset_path) -> FTetherStaticMeshInfo
...
### get_skeletal_mesh_info(asset_path) -> FTetherSkeletalMeshInfo
...
#### get_mesh_material_slots(mesh_asset_path) -> list[FTetherMeshMaterialSlot]
...
assignments = [
    unreal.TetherMeshMaterialAssignment(
        material_index=0,
...
`FTetherMeshMaterialEditResult` fields:
...
### get_texture_info(asset_path) -> FTetherTextureInfo
...
### get_sound_info(asset_path) -> FTetherSoundInfo
