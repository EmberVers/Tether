...
`unreal.TetherPCGLibrary` — Lane 3 of the procedural-content roadmap.
**Read-only graph access + override edit + Generate/Cleanup trigger** for
existing PCG content. **No graph editing** — tether does not duplicate the
...
- ⚠ **`wait_for_pcg_generate` cannot be used to actually wait.** Its 50ms poll
  ...
  tether execs** from Python — the GT drains between calls. See "wait
...
- `set_pcg_component_override` uses `Property->ImportText`. Empty inputs are
  ...
  so the tether enforces non-empty + snapshot-restores the prior value when
  the parser refuses to consume any chars).

## What's shipped (8 ops)

- `list_pcg_graph_assets(filter, max=200) -> list[str]` (L3-1)
- `list_pcg_components_in_level(level_filter, max=200) -> list[FTetherPCGComponentEntry]` (L3-2)
- `get_pcg_component_state(actor_label, component_name) -> FTetherPCGComponentState` (L3-3)
- `get_pcg_component_overrides(actor_label, component_name) -> list[FTetherPCGOverrideEntry]` (L3-4)
...
- `wait_for_pcg_generate(actor_label, component_name, timeout_sec=60.0) -> FTetherPCGWaitResult` (L3-7)
- `cleanup_pcg_component(actor_label, component_name, b_remove_components=False) -> bool` (L3-8)

## USTRUCTs

### FTetherPCGComponentEntry
| Field | Type | Source |
|---|---|---|
...
- `unreal.PCGVolume` actors auto-attach a component named **`'PCG Component'`**
  ...
  component). Pass that string to op calls; the tether resolves both
...
- `b_remove_components=False` (default): "soft cleanup" — purges generated
  ...
  retains the cached value. This is PCG's documented behavior, not a tether
  bug.
...
- On parse failure (no characters consumed by ImportText), the tether
...
actual waits, poll from Python across multiple tether execs:

```python
import time, unreal as u
P = u.TetherPCGLibrary
...
(Each `P.get_pcg_component_state` call is its own tether exec from your
...
- **`b_dirty` is always false in the current build.** `bDirtyGenerated`
  ...
  use case.
- **Big PCG generations block the tether for as long as
