# Tether Animation Library API

Module: `unreal.TetherAnimLibrary`
...
> `TetherReactiveLibrary.register_runtime_anim_notify` — see
> `tether-reactive.md` for the full registration shape. Pair
...
> **Visual pose inspection lives in `tether-level-api.md`, not here.**
...
> `tether-level-api.md` for full signatures.
>
...
The AnimGraph write ops in this file + the graph-write ops in `tether-blueprint-api.md` are the authoring surface. State machines, states, conduits, transitions, rule logic, and the outer blend graph can all be authored from a single `exec-file` script. Reads (`list_anim_graphs`, `find_anim_graph_node_by_class`, `get_anim_graph_info`) close the gaps when write ops need node GUIDs you haven't stored in a variable.

### Workflow (in order — each step depends on the previous)

**1. Plan on paper first.** Before any tether call, write down in text:
...
**2. Create the ABP asset (raw `unreal` — there's no tether wrapper for this):**
```python
...
import unreal
ABP = '...'  # your ABP path
alib = unreal.TetherAnimLibrary
blib = unreal.TetherBlueprintLibrary
elib = unreal.TetherEditorLibrary
...
| Python `AttributeError: 'FTetherX' object has no attribute 'b_...'` | UE Python strips `b` prefix from USTRUCT bool fields | `.succeeded` not `.b_succeeded`; `.is_default` not `.b_is_default` |

---

## State Machine Info

### get_anim_graph_info(anim_blueprint_path) -> list[FTetherStateMachineInfo]

Get all state machines in an Animation Blueprint with states and transitions.

```python
machines = unreal.TetherAnimLibrary.get_anim_graph_info('/Game/Anim/ABP_Character')
for sm in machines:
    print(f'SM: {sm.name}')
    for s in sm.states:
...
### get_anim_graph_nodes(anim_blueprint_path) -> list[FTetherAnimGraphNodeInfo]
...
```python
nodes = unreal.TetherAnimLibrary.get_anim_graph_nodes('/Game/Anim/ABP_Character')
for n in nodes:
...
### FTetherAnimGraphNodeInfo fields
...
| `connections` | list[FTetherAnimNodeConnection] | Output connections |

---

## Linked Anim Layers

### get_anim_linked_layers(anim_blueprint_path) -> list[FTetherAnimLayerInfo]

Get linked anim layer bindings — the UE5 modular animation architecture.

```python
layers = unreal.TetherAnimLibrary.get_anim_linked_layers('/Game/Anim/ABP_Character')
for l in layers:
    print(f'{l.interface_name} - {l.layer_name}')
```

### FTetherAnimLayerInfo fields
...
### get_anim_slots(anim_blueprint_path) -> list[FTetherAnimSlotInfo]

Get all Slot nodes and their slot names (montage playback paths).

```python
slots = unreal.TetherAnimLibrary.get_anim_slots('/Game/Anim/ABP_Character')
for s in slots:
    print(f'Slot: {s.slot_name} in {s.graph_name}')
```

### FTetherAnimSlotInfo fields
...
```python
details = unreal.TetherAnimLibrary.get_anim_node_details('/Game/Anim/ABP_Char', 'MotionMatching')
for d in details:
    print(f'  {d}')
```
...
```python
curves = unreal.TetherAnimLibrary.get_anim_curves('/Game/Anim/ABP_Character')
for c in curves:
    print(c)
```
...
### get_anim_sequence_info(sequence_path) -> FTetherAnimSequenceInfo
...
> `register_runtime_anim_notify(notify_name=...)` from `tether-reactive.md`
> to run Python when a notify plays during PIE.

```python
info = unreal.TetherAnimLibrary.get_anim_sequence_info('/Game/Anim/Run_Fwd')
...
### FTetherAnimSequenceInfo fields
...
| `notifies` | list[FTetherAnimNotifyInfo] | Animation notifies |
| `curves` | list[FTetherAnimCurveInfo] | Animation curves |

---

## Montage Info

### get_montage_info(montage_path) -> FTetherMontageInfo
...
> `register_runtime_anim_notify` in `tether-reactive.md` to react to
> frame-perfect events (impact, SFX cue, combo window).

```python
info = unreal.TetherAnimLibrary.get_montage_info('/Game/Anim/AM_Attack')
...
| `sections` | list[FTetherMontageSectionInfo] | Composite sections |
| `notifies` | list[FTetherAnimNotifyInfo] | Animation notifies |
...
### get_blend_space_info(blend_space_path) -> FTetherBlendSpaceInfo

Get blend space axes and sample points.

```python
info = unreal.TetherAnimLibrary.get_blend_space_info('/Game/Anim/BS_Locomotion')
for ax in info.axes:
...
| `skeleton_name` | str | Skeleton name |
| `num_axes` | int | 1 or 2 |
| `axes` | list[FTetherBlendSpaceAxis] | Axis definitions |
| `samples` | list[FTetherBlendSpaceSample] | Sample points |

---

## Skeleton Bone Tree

### get_skeleton_bone_tree(skeleton_path) -> list[FTetherBoneInfo]
...
```python
bones = unreal.TetherAnimLibrary.get_skeleton_bone_tree('/Game/Skel/SK_Character_Skeleton')
for b in bones:
    indent = '  ' * (b.bone_index // 5)  # rough depth
    print(f'{b.bone_name} (idx={b.bone_index}, parent={b.parent_name})')
```

### FTetherBoneInfo fields
...
### get_skeleton_sockets(skeleton_path) -> list[FTetherSocketInfo]

Get all sockets defined on a skeleton (attach points for weapons, FX, etc.).

```python
sockets = unreal.TetherAnimLibrary.get_skeleton_sockets('/Game/Skel/SK_Character_Skeleton')
for s in sockets:
    print(f'{s.socket_name} on {s.parent_bone_name} @ {s.relative_location}')
```

### FTetherSocketInfo fields
...
### get_skeleton_virtual_bones(skeleton_path) -> list[FTetherVirtualBoneInfo]
...
```python
vbs = unreal.TetherAnimLibrary.get_skeleton_virtual_bones('/Game/Skel/SK_Hero_Skeleton')
for v in vbs:
    print(f'{v.virtual_bone_name}: {v.source_bone_name} -> {v.target_bone_name}')
```

### FTetherVirtualBoneInfo fields
...
### get_montage_slot_segments(montage_path) -> list[FTetherMontageSlotSegment]
...
```python
segs = unreal.TetherAnimLibrary.get_montage_slot_segments('/Game/Anim/AM_Combo')
for s in segs:
...
### FTetherMontageSlotSegment fields
...
```python
ok = unreal.TetherAnimLibrary.add_anim_notify(
...
```python
n = unreal.TetherAnimLibrary.remove_anim_notifies_by_name('/Game/Anim/Attack', 'HitFrame')
```
...
```python
ok = unreal.TetherAnimLibrary.add_anim_notify_state(
    '/Game/Anim/DoorOpen',
...
```python
removed = unreal.TetherAnimLibrary.remove_anim_notify_states_by_class(
    '/Game/Anim/DoorOpen',
    '/Script/MyGame.ANS_InterruptWindow')
```

### get_motion_warping_notifies(animation_path) -> list[FTetherMotionWarpingNotifyInfo]
...
```python
ok = unreal.TetherAnimLibrary.set_motion_warping_notify(
    '/Game/Anim/DoorOpen', 'DoorInteract', 0.0001, 1.85)
windows = unreal.TetherAnimLibrary.get_motion_warping_notifies(
    '/Game/Anim/DoorOpen')
unreal.EditorAssetLibrary.save_asset('/Game/Anim/DoorOpen')
```
...
```python
unreal.TetherAnimLibrary.add_montage_section('/Game/Anim/AM_Combo', 'Swing2', 0.45)
```
...
```python
unreal.TetherAnimLibrary.remove_montage_section('/Game/Anim/AM_Combo', 'Swing2')
```
...
unreal.TetherAnimLibrary.set_montage_blend_times('/Game/Anim/AM_Attack', 0.1, -1.0, -1.0, True)
```
...
```python
ok = unreal.TetherAnimLibrary.add_skeleton_socket(
...
```python
montages = unreal.TetherAnimLibrary.list_assets_for_skeleton(
...
```python
ok = unreal.TetherAnimLibrary.remove_skeleton_socket(
...
### get_anim_sync_markers(sequence_path) -> list[FTetherAnimSyncMarker]
...
```python
marks = unreal.TetherAnimLibrary.get_anim_sync_markers('/Game/Anim/Run_Fwd')
for m in marks:
    print(f'{m.marker_name} @ {m.time:.3f}s (track {m.track_index})')
```

### FTetherAnimSyncMarker fields
...
### get_skeleton_blend_profiles(skeleton_path) -> list[FTetherBlendProfileInfo]
...
```python
profiles = unreal.TetherAnimLibrary.get_skeleton_blend_profiles(
    '/Game/Skel/SK_Hero_Skeleton')
for p in profiles:
    print(f'{p.name}  mode={p.mode}  entries={p.num_entries}')
```

### FTetherBlendProfileInfo fields
...
### get_blend_profile_entries(skeleton_path, profile_name) -> list[FTetherBlendProfileEntry]
...
```python
entries = unreal.TetherAnimLibrary.get_blend_profile_entries(
    '/Game/Skel/SK_Hero_Skeleton', 'UpperBody')
for e in entries:
    print(f'{e.bone_name}  scale={e.blend_scale}')
```

### FTetherBlendProfileEntry fields
...
```python
unreal.TetherAnimLibrary.set_montage_section_start_time(
...
```python
ok = unreal.TetherAnimLibrary.add_anim_sync_marker(
...
```python
n = unreal.TetherAnimLibrary.remove_anim_sync_markers_by_name(
...
```python
ok = unreal.TetherAnimLibrary.set_skeleton_socket_transform(
    '/Game/Skel/SK_Hero_Skeleton',
    'weapon_r_socket',
...
```python
ok = unreal.TetherAnimLibrary.rename_skeleton_socket(
...
### get_anim_blueprint_info(anim_blueprint_path) -> FTetherAnimBlueprintInfo
...
```python
info = unreal.TetherAnimLibrary.get_anim_blueprint_info('/Game/Anim/ABP_Character')
...
### FTetherAnimBlueprintInfo fields
...
- does **not** force-compile — call `unreal.TetherEditorLibrary.recompile_blueprint(path)` once after a batch of edits and `unreal.EditorAssetLibrary.save_loaded_asset(...)` to persist.
...
### list_anim_graphs(anim_blueprint_path) → list[FTetherAnimGraphSummary]
...
```python
for g in unreal.TetherAnimLibrary.list_anim_graphs(ABP):
...
```python
for row in unreal.TetherAnimLibrary.list_anim_graph_nodes(ABP, 'Idle'):
    guid, cls, title = row.split('\t', 2)
    print(cls, title)
```

Prefer this over `TetherBlueprintLibrary.get_function_nodes` when addressing state-machine interiors: the BP-side helper only walks top-level `FunctionGraphs`.
...
Mirrors the BP `auto_layout_graph` shape but tuned for anim graphs (pose-flow layering) and state machines (grid). Returns `FTetherAnimLayoutResult { succeeded, nodes_positioned, layer_count, bounds_width, bounds_height, warnings }`.
...
```python
unreal.TetherEditorLibrary.recompile_blueprint(ABP)
unreal.EditorAssetLibrary.save_loaded_asset(unreal.load_asset(ABP))
```

### Gotchas

- **Python bool field names.** `FTetherAnimLayoutResult::bSucceeded` → `.succeeded` in Python (UE Python strips the `b` prefix from USTRUCT bool fields). Same for `FTetherAnimState::bIsDefault` → `.is_default`, `bIsConduit` → `.is_conduit`, and `FTetherAnimTransition::bBidirectional` → `.bidirectional`.
...
- **Rule authoring uses the Blueprint library.** `TetherBlueprintLibrary.find_graph_by_name` was extended to walk into state-machine interiors + state / conduit / transition `BoundGraph`s + nested `SubGraphs`, so `add_variable_node` / `add_call_function_node` / `connect_graph_pins` / `set_pin_default_value` all work against `graph_name="Rule_Idle_to_Walk"` directly.
